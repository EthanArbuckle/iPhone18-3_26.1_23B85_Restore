...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    [v3 setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithLongLong:", self->_expirationDate), @"expiration_date"}];
  }
...
LABEL_11:
    ...
    if (*&self->_has)
    {
      if ((*(a3 + 24) & 1) == 0 || self->_expirationDate != *(a3 + 1))
      {
        goto LABEL_11;
      }
