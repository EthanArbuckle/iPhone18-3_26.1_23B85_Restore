...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    [v3 setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithInt:", self->_countInCluster), @"countInCluster"}];
    has = self->_has;
  }
...
LABEL_19:
    ...
    if ((has & 2) != 0)
    {
      if ((*(a3 + 32) & 2) == 0 || self->_countInCluster != *(a3 + 4))
      {
        goto LABEL_19;
      }
    }
    ...
    if ((has & 4) != 0)
    {
      if ((*(a3 + 32) & 4) == 0 || self->_deletionCount != *(a3 + 5))
      {
        goto LABEL_19;
      }
