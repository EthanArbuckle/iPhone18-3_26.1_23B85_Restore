...
{
  v3.receiver = self;
  v3.super_class = AWDWiFiMetricsManagerEvent;
  return [MEMORY[0x29EDBA0F8] stringWithFormat:@"%@ %@", -[AWDWiFiMetricsManagerEvent description](&v3, sel_description), -[AWDWiFiMetricsManagerEvent dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
LABEL_16:
    ...
    if ((*&self->_has & 4) != 0)
    {
      if ((*(equal + 24) & 4) == 0 || self->_status != *(equal + 5))
      {
        goto LABEL_16;
      }
    }
    ...
    if ((*&self->_has & 2) != 0)
    {
      if ((*(equal + 24) & 2) == 0 || self->_reason != *(equal + 4))
      {
        goto LABEL_16;
      }
