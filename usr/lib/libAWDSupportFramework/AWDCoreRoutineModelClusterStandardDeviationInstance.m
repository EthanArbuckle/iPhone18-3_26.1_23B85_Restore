...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    [v3 setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithInt:", self->_entryStandardDeviation), @"entryStandardDeviation"}];
    has = self->_has;
  }

  if ((has & 2) != 0)
  {
    [v3 setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithInt:", self->_exitStandardDeviation), @"exitStandardDeviation"}];
  }
...
LABEL_5:
    *(a3 + 3) = self->_exitStandardDeviation;
    *(a3 + 16) |= 2u;
    return;
  }

  *(a3 + 2) = self->_entryStandardDeviation;
  *(a3 + 16) |= 1u;
  if ((*&self->_has & 2) != 0)
  {
    goto LABEL_5;
...
- (BOOL)isEqual:(id)a3
{
  ...
  if (v5)
  {
    if (*&self->_has)
    {
      if ((*(a3 + 16) & 1) == 0 || self->_entryStandardDeviation != *(a3 + 2))
      {
        goto LABEL_11;
      }
    }
...
LABEL_11:
    ...
    if ((*&self->_has & 2) != 0)
    {
      if ((*(a3 + 16) & 2) == 0 || self->_exitStandardDeviation != *(a3 + 3))
      {
        goto LABEL_11;
      }
...
LABEL_5:
    self->_exitStandardDeviation = *(a3 + 3);
    *&self->_has |= 2u;
    return;
  }

  self->_entryStandardDeviation = *(a3 + 2);
  *&self->_has |= 1u;
  if ((*(a3 + 16) & 2) != 0)
  {
    goto LABEL_5;
