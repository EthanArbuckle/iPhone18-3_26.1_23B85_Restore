...
LABEL_7:
  ...
  {
LABEL_8:
    [v3 setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithUnsignedInt:", self->_publicationDate), @"publicationDate"}];
  }
...
LABEL_24:
    ...
    if ((has & 4) != 0)
    {
      if ((*(a3 + 40) & 4) == 0 || self->_delayMs != *(a3 + 5))
      {
        goto LABEL_24;
      }
    }
    ...
    if ((has & 2) != 0)
    {
      if ((*(a3 + 40) & 2) == 0 || self->_connectionType != *(a3 + 4))
      {
        goto LABEL_24;
      }
    }
    ...
    if ((has & 8) != 0)
    {
      if ((*(a3 + 40) & 8) == 0 || self->_publicationDate != *(a3 + 6))
      {
        goto LABEL_24;
      }
