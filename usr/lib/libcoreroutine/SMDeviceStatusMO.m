...
uint64_t __87__SMDeviceStatusMO_managedObjectWithDeviceStatus_managedObject_inManagedObjectContext___block_invoke(uint64_t a1)
{
  ...
  if (!v2)
  {
    v3 = [[SMDeviceStatusMO alloc] initWithContext:*(a1 + 40)];
  }
  ...
  [*(*(*(a1 + 56) + 8) + 40) setBatteryRemaining:{objc_msgSend(*(a1 + 48), "batteryRemaining")}];
  [*(*(*(a1 + 56) + 8) + 40) setCellularStrength:{objc_msgSend(*(a1 + 48), "cellularStrength")}];
  v5 = [*(a1 + 48) date];
  [*(*(*(a1 + 56) + 8) + 40) setDate:v5];

  v6 = [*(a1 + 48) wifiStrength];
  v7 = *(*(*(a1 + 56) + 8) + 40);
