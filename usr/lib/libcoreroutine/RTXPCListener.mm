...
- (RTXPCListener)initWithMachServiceName:(id)name;
- (void)_setup;
...
- (void)logClients
{
  ...
  if (os_log_type_enabled(MEMORY[0x277D86220], OS_LOG_TYPE_DEBUG))
  {
    ...
    [connectedClients2 enumerateObjectsUsingBlock:&__block_literal_global_8];
  }
  ...
  if (os_log_type_enabled(MEMORY[0x277D86220], OS_LOG_TYPE_DEBUG))
  {
    ...
    [disconnectedClients2 enumerateObjectsUsingBlock:&__block_literal_global_6];
  }
}
...
- (RTXPCListener)initWithMachServiceName:(id)name
{
  ...
  if (nameCopy)
  {
    ...
    v5 = [(RTXPCListener *)&v21 initWithMachServiceName:nameCopy];
    if (v5)
    {
      v6 = [nameCopy copy];
      ...
      {
        uTF8String = [(RTXPCListener *)v8 UTF8String];
      }
...
- (void)_setup
{
  ...
  [(RTXPCListener *)self _setQueue:queue];

  [(RTXPCListener *)self setDelegate:self];
  [(RTXPCListener *)self resume];
  DarwinNotifyCenter = CFNotificationCenterGetDarwinNotifyCenter();
  machServiceName = [(RTXPCListener *)self machServiceName];
  CFNotificationCenterPostNotification(DarwinNotifyCenter, machServiceName, 0, 0, 1u);
}
