...
@implementation AntMappingClient
...
- (void)extractPolicyEntries:(id)entries
{
  ...
  if (entriesCopy)
  {
    +[WCM_Logging logLevel:message:](WCM_Logging, "logLevel:message:", 3, @"Extracted %lu policies for Antenna Mapping", [entriesCopy count]);
    v18 = 0u;
    v19 = 0u;
    v16 = 0u;
    v17 = 0u;
    ...
    if (v7)
    {
      ...
      do
      {
        for (i = 0; i != v8; i = i + 1)
        {
          ...
          v11 = *(*(&v16 + 1) + 8 * i);
          v12 = objc_alloc_init(AntMappingPolicy);
          [(AntMappingPolicy *)v12 extractPolicy:v11];
  ...
  {
    [WCM_Logging logLevel:3 message:@"No policies found for client Antenna Mapping"];
  }
}
...
{
  v3.receiver = self;
  v3.super_class = AntMappingClient;
...
- (id)constructXpcMessage
{
  ...
  if (v4)
  {
    ...
    if (v5)
    {
      constructXpcMessagePolicyEntries = [(AntMappingClient *)self constructXpcMessagePolicyEntries];
      xpc_dictionary_set_value(v5, "kWCMCellularSetWCI2TxAntMap_InformationSet", constructXpcMessagePolicyEntries);
    }
  }
