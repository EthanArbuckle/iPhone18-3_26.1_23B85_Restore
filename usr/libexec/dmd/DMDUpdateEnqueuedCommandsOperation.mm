...
{
  v2.receiver = self;
  v2.super_class = DMDUpdateEnqueuedCommandsOperation;
  [(DMDUpdateEnqueuedCommandsOperation *)&v2 waitUntilFinished];
}
...
- (void)runWithRequest:(id)request
{
  ...
  if (configurationEngine)
  {
    ...
    v7[3] = &unk_1000CEE68;
    v7[4] = self;
    [configurationEngine2 handleUpdateCommandsRequest:requestCopy completionHandler:v7];
  }

  else
  {
    configurationEngine2 = DMFErrorWithCodeAndUserInfo();
    [(DMDUpdateEnqueuedCommandsOperation *)self endOperationWithError:configurationEngine2];
  }
}
...
+ (BOOL)validateRequest:(id)request error:(id *)error
{
  requestCopy = request;
  v10.receiver = self;
  v10.super_class = &OBJC_METACLASS___DMDUpdateEnqueuedCommandsOperation;
