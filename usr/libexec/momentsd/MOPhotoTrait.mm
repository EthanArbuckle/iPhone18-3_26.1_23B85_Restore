...
- (MOPhotoTrait)initWithPhotoTraitMO:(id)o;
...
- (id)description
{
  ...
  v10 = [NSString stringWithFormat:@"<MOPhotoTrait localIdentifier, %@, name, %@, labelType, %@, holidayIdentifier, %@, meaningIdentifier, %@, relevantAssetUUIDs, %@, associatedPersonLocalIdentifiers, %@, >", identifier, name, labelType, holidayIdentifier, meaningIdentifier, relevantAssetUUIDs, associatedPersonLocalIdentifiers];
...
- (MOPhotoTrait)initWithCoder:(id)coder
{
  ...
  v10 = [coderCopy decodeObjectOfClasses:v9 forKey:@"relevantAssetUUIDs"];

  v11 = objc_opt_class();
  v12 = [NSSet setWithObjects:v11, objc_opt_class(), 0];
  v13 = [coderCopy decodeObjectOfClasses:v12 forKey:@"associatedPersonLocalIdentifiers"];
...
- (MOPhotoTrait)initWithPhotoTraitMO:(id)o
{
  if (o)
  {
    ...
    name = [oCopy name];
    labelType = [oCopy labelType];
    holidayIdentifier = [oCopy holidayIdentifier];
    meaningIdentifier = [oCopy meaningIdentifier];
    relevantAssetUUIDs = [oCopy relevantAssetUUIDs];
    associatedPersonLocalIdentifiers = [oCopy associatedPersonLocalIdentifiers];

    self = [(MOPhotoTrait *)self initWithIdentifier:identifier name:name labelType:labelType holidayIdentifier:holidayIdentifier meaningIdentifier:meaningIdentifier relevantAssetUUIDs:relevantAssetUUIDs associatedPersonLocalIdentifiers:associatedPersonLocalIdentifiers];
    selfCopy = self;
  }
