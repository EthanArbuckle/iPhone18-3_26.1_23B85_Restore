...
- (BOOL)isUnifiedAccessForHome:(id)a3;
- (BOOL)suspendWithInfo:(id)a3;
- (BOOL)willStartSession;
- (_NFUnifiedAccessSession)initWithRemoteObject:(id)a3 driverWrapper:(id)a4 workQueue:(id)a5;
...
- (_NFUnifiedAccessSession)initWithRemoteObject:(id)a3 driverWrapper:(id)a4 workQueue:(id)a5
{
  ...
  v22 = [(_NFContactlessPaymentSession *)&v25 initWithRemoteObject:v12 workQueue:v11 allowsBackgroundMode:0 hostCardEmulationSupport:1];

  if (v22)
  {
    objc_storeStrong((v22 + 388), a4);
  }
...
- (BOOL)willStartSession
{
  ...
  v7 = [(NFStepUpController *)v3 initWithQueue:v4 driverWrapper:v5 expressModeManager:v6];
...
- (void)startSTSNotificationListenerEndpoint:(id)a3
{
  ...
  {
    v20 = [*(&self->_driverWrapper + 4) connectToNotificationListener:v5];
  }
}
...
- (BOOL)suspendWithInfo:(id)a3
{
  ...
  v4 = [(_NFContactlessPaymentSession *)&v6 suspendWithInfo:a3];
...
- (void)didStartSession:(id)a3
{
  ...
  if (v5)
  {
    ...
    if (v15)
    {
      ...
      v16(3, "%c[%{public}s %{public}s]:%i error: %{public}@", v19, v37, v40, 181, v5);
    }
    ...
    v25 = [(_NFXPCSession *)self remoteObject];
    [v25 didStartSession:v5];
  }

  else
  {
    v25 = [*(&self->_driverWrapper + 4) startISO18013WithConnectionHandoverConfiguration];
    if (v25)
    {
      dispatch_get_specific(kNFLOG_DISPATCH_SPECIFIC_KEY);
      v26 = NFLogGetLogger();
      if (v26)
      {
        ...
        v27(3, "%c[%{public}s %{public}s]:%i Reader start error: %{public}@", v30, v38, v41, 187, v25);
      }
...
{
  v4 = a3;
  v5 = [(_NFXPCSession *)self expressModeManager];
  v6 = sub_100032938(v5, v4);

  if ([v4 NF_isPrefixOfHexEncodedEqualToBytes:&unk_100297BFA length:7])
  {
    v7 = 1;
  }

  else if ([v4 NF_isPrefixOfHexEncodedEqualToBytes:&unk_100297C01 length:9])
  {
