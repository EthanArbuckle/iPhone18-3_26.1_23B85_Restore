...
- (ocv_drift_model)initWithConfiguration:(id)configuration error:(id *)error;
- (ocv_drift_model)initWithContentsOfURL:(id)l configuration:(id)configuration error:(id *)error;
- (ocv_drift_model)initWithContentsOfURL:(id)l error:(id *)error;
- (ocv_drift_model)initWithMLModel:(id)model;
...
- (ocv_drift_model)initWithMLModel:(id)model
{
  ...
  {
    v10.receiver = self;
    v10.super_class = ocv_drift_model;
...
- (ocv_drift_model)init
{
  ...
  v4 = [(ocv_drift_model *)self initWithContentsOfURL:uRLOfModelInThisBundle error:0];
...
- (ocv_drift_model)initWithConfiguration:(id)configuration error:(id *)error
{
  ...
  uRLOfModelInThisBundle = [objc_opt_class() URLOfModelInThisBundle];
  v8 = [(ocv_drift_model *)self initWithContentsOfURL:uRLOfModelInThisBundle configuration:configurationCopy error:error];
...
- (id)predictionFromFeatures:(id)features error:(id *)error
{
  ...
  v8 = [(ocv_drift_model *)self predictionFromFeatures:featuresCopy options:v7 error:error];
...
- (id)predictionFromFeatures:(id)features options:(id)options error:(id *)error
{
  optionsCopy = options;
  featuresCopy = features;
  model = [(ocv_drift_model *)self model];
  ...
  if (v11)
  {
    ...
    v15 = [(ocv_drift_modelOutput *)v12 initWithPrediction:multiArrayValue];
  }
...
{
  inputCopy = input;
  v7 = [[ocv_drift_modelInput alloc] initWithInput:inputCopy];

  v8 = [(ocv_drift_model *)self predictionFromFeatures:v7 error:error];
  ...
  if (v12)
  {
    ...
    if ([v12 count] >= 1)
    {
      ...
      do
      {
        ...
        v19 = [(ocv_drift_modelOutput *)v16 initWithPrediction:multiArrayValue];
