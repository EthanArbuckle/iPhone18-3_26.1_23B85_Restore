...
+ (id)dedicatedCameraContentForDeviceType:(int64_t)type;
+ (id)notificationContentForSystemService:(int64_t)service;
+ (id)restrictedAccessContentForRestrictionType:(int64_t)type;
- (NSArray)bodyArguments;
- (NSArray)titleArguments;
- (void)addBodyArgument:(id)argument;
- (void)addTitleArgument:(id)argument;
@end
...
+ (CDUserNotificationContent)appSignInContent
{
  ...
  [(CDUserNotificationContent *)v2 setBodyKey:uppercaseString];
...
+ (id)notificationContentForSystemService:(int64_t)service
{
  ...
  {
    case 4:
      _recognizeMyVoiceContent = [self _recognizeMyVoiceContent];
      break;
    case 3:
      _recognizeMyVoiceContent = [self _photosContent];
      break;
    case 2:
      _recognizeMyVoiceContent = [self _sharedWithYouContent];
      break;
    default:
      _recognizeMyVoiceContent = 0;
      break;
  }

  return _recognizeMyVoiceContent;
}
...
+ (id)restrictedAccessContentForRestrictionType:(int64_t)type
{
  ...
  if ((type - 1) <= 5)
  {
    [(CDUserNotificationContent *)v4 setBodyKey:off_10008A8E0[type - 1]];
  }
...
+ (id)dedicatedCameraContentForDeviceType:(int64_t)type
{
  ...
  [(CDUserNotificationContent *)v3 setBodyKey:uppercaseString];
