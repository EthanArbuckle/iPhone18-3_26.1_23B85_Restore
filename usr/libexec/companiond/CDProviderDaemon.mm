...
- (void)_activated;
- (void)_foundDeviceWithIdentifier:(id)identifier discoveryType:(int64_t)type;
- (void)_invalidated;
- (void)_startBluetoothListener;
- (void)_startDeviceDiscovery;
- (void)_startIDSService;
- (void)_startPhotoSetupSessionWithBluetoothDevice:(id)device;
...
- (void)_foundDeviceWithIdentifier:(id)identifier discoveryType:(int64_t)type
{
  ...
  {
    v7 = [[CDProviderSession alloc] initWithDeviceIdentifier:identifierCopy discoveryType:type];
    currentSession = self->_currentSession;
...
- (void)_startPhotoSetupSessionWithBluetoothDevice:(id)device
{
  ...
  if (([(CUSystemMonitor *)self->_systemMonitor meDeviceIsMe]& 1) != 0 || [(CDProviderDaemon *)self _isVirtualMachine])
  {
    ...
    {
      v5 = [[CDPhotoSetupSession alloc] initWithBluetoothDevice:deviceCopy];
      photoSetupSession = self->_photoSetupSession;
...
- (id)_authenticationSessionWithClient:(id)client request:(id)request
{
  ...
  {
    v9 = [[CDProviderSession alloc] initWithLocalAuthenticationRequest:requestCopy];
  }
...
- (id)descriptionWithMultilinePrefix:(id)prefix
{
  v3 = [(CDProviderDaemon *)self descriptionBuilderWithMultilinePrefix:prefix];
  build = [v3 build];
...
- (id)succinctDescription
{
  succinctDescriptionBuilder = [(CDProviderDaemon *)self succinctDescriptionBuilder];
...
- (void)serviceConnection:(id)connection serviceClient:(id)client startAuthenticationSessionWithRequest:(id)request completionHandler:(id)handler
{
  ...
  {
    v16 = [(CDProviderDaemon *)self _authenticationSessionWithClient:clientCopy request:requestCopy];
    currentSession = self->_currentSession;
