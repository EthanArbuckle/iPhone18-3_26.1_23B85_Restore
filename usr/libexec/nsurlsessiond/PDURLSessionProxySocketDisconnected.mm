...
- (void)mergeFrom:(id)from
{
  ...
  else
  {
    ...
    [(PDURLSessionProxySocketDisconnected *)self setTask:?];
  }
...
LABEL_7:
  if (fromCopy[1])
  {
    [(PDURLSessionProxySocketDisconnected *)self setSocketUUID:?];
    fromCopy = v7;
  }
}
...
LABEL_15:
      ...
      if ((v13 >> 3) == 1)
      {
        ...
        v18[0] = 0;
        v18[1] = 0;
        if (!PBReaderPlaceMark() || !sub_10006CE20(socketUUID, from))
        {
...
{
  v7.receiver = self;
  v7.super_class = PDURLSessionProxySocketDisconnected;
  v3 = [(PDURLSessionProxySocketDisconnected *)&v7 description];
  dictionaryRepresentation = [(PDURLSessionProxySocketDisconnected *)self dictionaryRepresentation];
