...
- (BOOL)isInitialized;
- (TransparencyGPBInt32ObjectDictionary)initWithDictionary:(id)dictionary;
- (TransparencyGPBInt32ObjectDictionary)initWithObjects:(const void *)objects forKeys:(const int *)keys count:(unint64_t)count;
...
{
  v12.receiver = self;
  v12.super_class = TransparencyGPBInt32ObjectDictionary;
...
{
  v3.receiver = self;
  v3.super_class = TransparencyGPBInt32ObjectDictionary;
  [(TransparencyGPBInt32ObjectDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)zone
{
  v4 = [TransparencyGPBInt32ObjectDictionary allocWithZone:zone];
...
- (void)writeToCodedOutputStream:(id)stream asField:(id)field
{
  ...
  if (nextObject)
  {
    ...
    do
    {
      ...
      [stream writeInt32NoTag:{sub_1000C416C(v14, v7) + v16}];
      sub_1000C5700(stream, intValue, 1, mapKeyDataType);
      sub_1000C42A0(stream, v14, v7);
      nextObject2 = [keyEnumerator nextObject];
    }
