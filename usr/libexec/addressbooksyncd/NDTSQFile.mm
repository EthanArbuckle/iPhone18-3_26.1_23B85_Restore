@interface NDTSQFile
- (NDTSQFile)initWithPath:(id)path;
- (NDTSQFile)initWithPath:(id)path queue:(id)queue;
...
{
  pathCopy = path;
  pathCopy = [NSString stringWithFormat:@"NDTSQFile.%@", pathCopy];
  uTF8String = [pathCopy UTF8String];
  v7 = dispatch_queue_attr_make_with_autorelease_frequency(0, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM);
  v8 = dispatch_queue_create(uTF8String, v7);

  v9 = [(NDTSQFile *)self initWithPath:pathCopy queue:v8];
  return v9;
}
...
- (NDTSQFile)initWithPath:(id)path queue:(id)queue
{
  ...
  if (v9)
  {
    objc_storeStrong(&v9->_pathTemplate, path);
...
{
  path = [(NDTSQFile *)self path];
  v4 = sqlite3_open_v2([path UTF8String], &self->_db, 4194310, 0);
...
- (id)_makeSqlErrorReasonString:(id)string
{
  ...
  v6 = [NSString stringWithFormat:@"%@: %s", stringCopy, sqlite3_errstr(v5)];
