...
- (BOOL)recordUsage:(int)usage;
- (_UWBUsageRecord)initWithDayNumber:(int)number;
- (_UWBUsageRecord)initWithDictionary:(id)dictionary;
- (id)convertToDictionary;
- (id)description;
- (void)aggregateUsageWith:(id)with;
@end
...
- (_UWBUsageRecord)initWithDictionary:(id)dictionary
{
  ...
  if (dictionaryCopy && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0))
  {
    ...
    if (v5)
    {
      v6 = [dictionaryCopy objectForKeyedSubscript:@"DayNumber"];
      v5->_dayNumber = [v6 intValue];

      v7 = [dictionaryCopy objectForKeyedSubscript:@"AnyUsage"];
      v5->_anyUsage = [v7 BOOLValue];

      v8 = [dictionaryCopy objectForKeyedSubscript:@"AnyPrecisionFinding"];
...
- (BOOL)recordUsage:(int)usage
{
  ...
  v3 = self->_usageByType[usage];
  if (!v3)
  {
    self->_usageByType[usage] = 1;
  }
