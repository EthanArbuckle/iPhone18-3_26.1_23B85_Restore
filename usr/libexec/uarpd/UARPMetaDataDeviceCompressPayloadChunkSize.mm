@interface UARPMetaDataDeviceCompressPayloadChunkSize
- (UARPMetaDataDeviceCompressPayloadChunkSize)init;
- (UARPMetaDataDeviceCompressPayloadChunkSize)initWithLength:(unint64_t)length value:(void *)value;
- (UARPMetaDataDeviceCompressPayloadChunkSize)initWithPropertyListValue:(id)value relativeURL:(id)l;
- (id)description;
- (id)tlvValue;
@end

@implementation UARPMetaDataDeviceCompressPayloadChunkSize

- (UARPMetaDataDeviceCompressPayloadChunkSize)init
{
  v6.receiver = self;
  v6.super_class = UARPMetaDataDeviceCompressPayloadChunkSize;
  v2 = [(UARPMetaData *)&v6 init];
  v3 = v2;
  if (v2)
  {
    v2->super._tlvType = -858619637;
    v2->super._tlvLength = 2;
    tlvName = v2->super._tlvName;
    v2->super._tlvName = @"Payload Compression ChunkSize";
  }

  return v3;
}

- (UARPMetaDataDeviceCompressPayloadChunkSize)initWithPropertyListValue:(id)value relativeURL:(id)l
{
  valueCopy = value;
  v6 = [(UARPMetaDataDeviceCompressPayloadChunkSize *)self init];
  if (v6)
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      unsignedShortValue = [valueCopy unsignedShortValue];
    }

    else
    {
      objc_opt_class();
      if ((objc_opt_isKindOfClass() & 1) == 0)
      {
        v8 = 0;
        goto LABEL_8;
      }

      unsignedShortValue = [valueCopy integerValue];
    }

    v6->_chunkSize = unsignedShortValue;
  }

  v8 = v6;
LABEL_8:

  return v8;
}

- (UARPMetaDataDeviceCompressPayloadChunkSize)initWithLength:(unint64_t)length value:(void *)value
{
  v6 = [(UARPMetaDataDeviceCompressPayloadChunkSize *)self init];
  v7 = v6;
  if (!v6)
  {
    goto LABEL_4;
  }

  v11.receiver = v6;
  v11.super_class = UARPMetaDataDeviceCompressPayloadChunkSize;
  v8 = [(UARPMetaData *)&v11 numberWithLength:length value:value];
  v9 = v8;
  if (v8)
  {
    v7->_chunkSize = [(UARPMetaDataDeviceCompressPayloadChunkSize *)v8 unsignedShortValue];

LABEL_4:
    v9 = v7;
  }

  return v9;
}

- (id)tlvValue
{
  v4.receiver = self;
  v4.super_class = UARPMetaDataDeviceCompressPayloadChunkSize;
  v2 = [(UARPMetaData *)&v4 tlvValueWithUInt16:[(UARPMetaDataDeviceCompressPayloadChunkSize *)self chunkSize]];

  return v2;
}

- (id)description
{
  tlvName = [(UARPMetaData *)self tlvName];
  v4 = [NSString stringWithFormat:@"<%@: %u>", tlvName, [(UARPMetaDataDeviceCompressPayloadChunkSize *)self chunkSize]];

  return v4;
}

@end