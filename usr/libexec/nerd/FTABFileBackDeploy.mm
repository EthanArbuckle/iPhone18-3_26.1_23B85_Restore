...
- (FTABFileBackDeploy)init
{
  ...
  if (v2)
  {
    ...
    v8 = [NSData dataWithBytes:&kFTABMagicLowercase length:8];
    magic = v2->_magic;
    v2->_magic = v8;
  }
...
- (BOOL)parseFileData
{
  if (self->_fileLength > 0x2F)
  {
    ...
    if (*(filePointer + 4) == 0x42415446534F4B52 || *v5 == 0x62617466736F6B72)
    {
      ...
      while ([(FTABFileBackDeploy *)self fileValidForOffset:v16 length:16])
      {
        ...
        if (!v19)
        {
          ...
          if (os_log_type_enabled(v2, OS_LOG_TYPE_ERROR))
          {
            [(FTABFileBackDeploy *)v15 parseFileData];
          }
    ...
    else
    {
      ...
      if (os_log_type_enabled(v2, OS_LOG_TYPE_ERROR))
      {
        [(FTABFileBackDeploy *)v5 parseFileData];
      }
    }
  }
...
- (id)createFileHandleForWritingToURL:(id)l
{
  ...
  if (v6)
  {
    ...
    if ((v8 & 1) == 0)
    {
      ...
      {
        [FTABFileBackDeploy createFileHandleForWritingToURL:lCopy];
      }
    ...
    while (2)
    {
      for (i = 0; i != v9; i = i + 1)
      {
        ...
        if (![(FTABFileBackDeploy *)self writeBytes:&v28 length:16])
        {
          ...
          {
            [(FTABFileBackDeploy *)v13 writeToDestination];
          }
    ...
    while (2)
    {
      for (j = 0; j != v17; j = j + 1)
      {
        ...
        if (!-[FTABFileBackDeploy writeBytes:length:](self, "writeBytes:length:", [v20 dataPointer], objc_msgSend(v20, "dataLength")))
        {
          ...
          if (os_log_type_enabled(v22, OS_LOG_TYPE_ERROR))
          {
            [(FTABFileBackDeploy *)v20 writeToDestination];
          }
...
- (BOOL)writeToURL:(id)l
{
  ...
  else
  {
    ...
    {
      [FTABFileBackDeploy writeToURL:lCopy];
    }
