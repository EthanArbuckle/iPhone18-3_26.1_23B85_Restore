...
- (id)_getSFRManifestHashForPayload:(id)a3
{
  ...
  v16 = 0u;
  *md = 0u;
  if (!v5 || Img4DecodeParseLengthFromBuffer([v5 bytes], -[IODualSPIWriter _maxImageSize](self, "_maxImageSize"), &v13) || (v7 = objc_msgSend(v5, "bytes"), Img4DecodeInit(v7, v13, v17), v8))
  {
    v9 = 0;
  }
  ...
  {
    Img4DecodeGetManifest(v17, &data, &len);
    CC_SHA384(data, len, md);
    v9 = [NSData dataWithBytes:md length:48];
  }
...
- (int)writeNewerGenerationFirmware:(id)a3 withError:(id *)a4
{
  ...
  do
  {
    ...
    if ([v6 count] <= 1)
    {
      ...
      {
        v112 = [v6 count];
        *a4 = MSUBootFirmwareError(-536870167, 0, @"writeNewerGenerationFirmware: couldn't find enough headers (only found %d of %d)", v113, v114, v115, v116, v117, v112);
      }
...
LABEL_27:
    ...
    if ([(IODualSPIWriter *)self _usesExtendediBootRegion]&& [(IODualSPIWriter *)self _crossingRegionChangeBoundary])
    {
      ...
      *(&self->_regBuffer + 4) = &kJadePORExtendedRegions;
    ...
    v75 = [(IODualSPIWriter *)self _writeFirmware:a3 toHeader:v8 withError:&v122];
    v76 = [v8 headerGeneration];
    [v8 startLocation];
    if (v75)
    {
      iBU_LOG_real(@"Failed to write newer-generation firmware images to update header with generation 0x%x at offset 0x%x. Invalidating this header, then trying again with a different header.", "[IODualSPIWriter writeNewerGenerationFirmware:withError:]", v77, v78, v79, v80, v81, v82, v76);
    }

    else
    {
      iBU_LOG_real(@"Successfully wrote newer-generation firmware images to update header with generation 0x%x at offset 0x%x.", "[IODualSPIWriter writeNewerGenerationFirmware:withError:]", v77, v78, v79, v80, v81, v82, v76);
      if (![(IOServiceWriter *)self shouldCommit])
      {
        goto LABEL_53;
      }

      v89 = [v8 headerGeneration];
      iBU_LOG_real(@"Committing updated header with newer-generation 0x%x", "[IODualSPIWriter writeNewerGenerationFirmware:withError:]", v90, v91, v92, v93, v94, v95, v89);
      v96 = [(IODualSPIWriter *)self _commitHeader:v8 withError:&v122];
      v97 = [v8 headerGeneration];
      [v8 startLocation];
      if (!v96)
      {
        iBU_LOG_real(@"Successfully committed header with newer-generation 0x%x at offset 0x%x.", "[IODualSPIWriter writeNewerGenerationFirmware:withError:]", v98, v99, v100, v101, v102, v103, v97);
LABEL_53:
...
LABEL_54:
      ...
      iBU_LOG_real(@"Failed to commit header with newer-generation 0x%x at offset 0x%x. Invalidating this header, then trying again with a different header.", "[IODualSPIWriter writeNewerGenerationFirmware:withError:]", v98, v99, v100, v101, v102, v103, v97);
    }
