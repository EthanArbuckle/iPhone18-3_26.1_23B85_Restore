...
- (SiriGKImageLinkView)initWithImageLinkedAnswer:(id)answer safariBackURL:(id)l isVideoResult:(BOOL)result usingPersistentStore:(id)store;
- (id)_applyBlurredPlayIconToImage:(id)image;
- (id)imageResource;
- (void)_configureWithImage:(id)image asVideoResult:(BOOL)result;
- (void)layoutSubviews;
@end
...
- (SiriGKImageLinkView)initWithImageLinkedAnswer:(id)answer safariBackURL:(id)l isVideoResult:(BOOL)result usingPersistentStore:(id)store
{
  ...
  if (v14)
  {
    objc_storeStrong(&v14->_imageLinkedAnswer, answer);
    v69 = resultCopy;
    v15->_isVideoResult = resultCopy;
    ...
    if (punchOut)
    {
      ...
      v15->_punchoutURL = punchOutUri;

      if (lCopy)
      {
        punchOutUri2 = [v17 punchOutUri];
        v21 = SiriGK_safariPunchoutURL(punchOutUri2, lCopy);
    ...
    if (resultCopy)
    {
      ...
      layer = [(UIImageView *)v15->_playIconImageView layer];
    ...
    thumbnails = [answerCopy thumbnails];
    v50 = [thumbnails count];

    if (v50)
    {
      thumbnails2 = [answerCopy thumbnails];
      firstObject = [thumbnails2 firstObject];
      resourceUrl = [firstObject resourceUrl];
    }

    else
    {
      thumbnails2 = [answerCopy imageResource];
      resourceUrl = [thumbnails2 resourceUrl];
    }

    absoluteString = [resourceUrl absoluteString];
    v55 = [storeCopy imageForKey:absoluteString];
    if (v55)
    {
    ...
    else
    {
      ...
      v60 = v59;
      v62 = v61;
      imageResource = [answerCopy imageResource];
      userAgent = [imageResource userAgent];
      ...
      v73 = v15;
      v74 = v69;
      v65 = [v58 imageTaskWithHTTPGetRequest:resourceUrl client:v73 fitToSize:0 fillColor:userAgent userAgent:v70 completionHandler:{v60, v62}];
...
- (void)_configureWithImage:(id)image asVideoResult:(BOOL)result
{
  ...
  if (resultCopy)
  {
    ...
    v7 = [(SiriGKImageLinkView *)self _applyBlurredPlayIconToImage:imageCopy];
...
- (id)_applyBlurredPlayIconToImage:(id)image
{
  ...
  v20 = [imageCopy _applyBackdropViewSettings:v19];
...
- (id)imageResource
{
  ...
  {
    imageResource = [(SAGKImageLinkedAnswer *)self->_imageLinkedAnswer imageResource];
    v2 = [imageResource copy];
  }
...
- (void)layoutSubviews
{
  ...
  subviews = [(UIView *)self->_placeholderView subviews];
