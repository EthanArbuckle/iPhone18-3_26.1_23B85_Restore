@interface NSDateFormatter
+ (id)_ma_sharedDateFormatter;
+ (id)_maps_abbreviatedDateStringWithoutYearForDate:(id)date;
+ (id)_maps_abbreviatedWeekdayForDate:(id)date;
+ (id)_maps_dateStringWithoutYearForDate:(id)date;
+ (id)_maps_numericMonthDayString:(id)string;
+ (id)_maps_timeForDate:(id)date;
+ (id)_maps_timeForDate:(id)date timeZone:(id)zone;
+ (id)_maps_weekdayForDate:(id)date;
@end

@implementation NSDateFormatter

+ (id)_ma_sharedDateFormatter
{
  if (qword_51350 != -1)
  {
    sub_11C60();
  }

  v2 = qword_51348;
  v3 = +[NSLocale _ma_locale];
  [v2 setLocale:v3];

  v4 = qword_51348;

  return v4;
}

+ (id)_maps_timeForDate:(id)date
{
  v3 = qword_51390;
  dateCopy = date;
  if (v3 != -1)
  {
    sub_11CB0();
  }

  v5 = [qword_51388 stringFromDate:dateCopy];

  return v5;
}

+ (id)_maps_timeForDate:(id)date timeZone:(id)zone
{
  v5 = qword_513A0;
  zoneCopy = zone;
  dateCopy = date;
  if (v5 != -1)
  {
    sub_11CC4();
  }

  [qword_51398 setTimeZone:zoneCopy];

  v8 = [qword_51398 stringFromDate:dateCopy];

  return v8;
}

+ (id)_maps_abbreviatedWeekdayForDate:(id)date
{
  v3 = qword_513B0;
  dateCopy = date;
  if (v3 != -1)
  {
    sub_11CD8();
  }

  [qword_513A8 setLocalizedDateFormatFromTemplate:@"E"];
  v5 = [qword_513A8 stringFromDate:dateCopy];

  return v5;
}

+ (id)_maps_weekdayForDate:(id)date
{
  v3 = qword_513C0;
  dateCopy = date;
  if (v3 != -1)
  {
    sub_11CEC();
  }

  [qword_513B8 setLocalizedDateFormatFromTemplate:@"EEEE"];
  v5 = [qword_513B8 stringFromDate:dateCopy];

  return v5;
}

+ (id)_maps_dateStringWithoutYearForDate:(id)date
{
  v3 = qword_513D0;
  dateCopy = date;
  if (v3 != -1)
  {
    sub_11D00();
  }

  [qword_513C8 setLocalizedDateFormatFromTemplate:@"MMMMd"];
  v5 = [qword_513C8 stringFromDate:dateCopy];

  return v5;
}

+ (id)_maps_abbreviatedDateStringWithoutYearForDate:(id)date
{
  v3 = qword_513E0;
  dateCopy = date;
  if (v3 != -1)
  {
    sub_11D14();
  }

  [qword_513D8 setLocalizedDateFormatFromTemplate:@"MMMd"];
  v5 = [qword_513D8 stringFromDate:dateCopy];

  return v5;
}

+ (id)_maps_numericMonthDayString:(id)string
{
  v3 = qword_513F0;
  stringCopy = string;
  if (v3 != -1)
  {
    sub_11D28();
  }

  [qword_513E8 setLocalizedDateFormatFromTemplate:@"Md"];
  v5 = [qword_513E8 stringFromDate:stringCopy];

  return v5;
}

@end