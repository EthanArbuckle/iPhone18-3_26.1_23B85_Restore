...
- (PFBilateralGridHash)initWithWidth:(int)width height:(int)height maxHashTableSize:(unint64_t)size;
...
{
  v12.receiver = self;
  v12.super_class = PFBilateralGridHash;
...
- (void)dealloc
{
  ...
  sub_169E4(self->_hash_map);
  v3.receiver = self;
  v3.super_class = PFBilateralGridHash;
...
- (int)_createWithSurface:(__IOSurface *)surface region:(CGRect)region cropRect:(CGRect)rect sigma_s:(int)sigma_s sigma_r_luma:(int)sigma_r_luma sigma_r_chroma:(int)sigma_r_chroma
{
  ...
  if ([(PFBilateralGridHash *)self _computeBilateralSpaceYCC444:surface region:v10 cropRect:v9 sigma_s:v8 sigma_r_luma:v18 sigma_r_chroma:v17, v16, v15, x, y, width, height])
  {
...
- (int)createWithSurface:(__IOSurface *)surface region:(CGRect)region cropRect:(CGRect)rect sigma_s:(int)sigma_s sigma_r_luma:(int)sigma_r_luma sigma_r_chroma:(int)sigma_r_chroma
{
  ...
  v16 = region.origin.y;
  v17 = region.origin.x;
  result = [PFBilateralGridHash _createWithSurface:"_createWithSurface:region:cropRect:sigma_s:sigma_r_luma:sigma_r_chroma:" region:surface cropRect:*&sigma_s sigma_s:*&sigma_r_luma sigma_r_luma:*&sigma_r_chroma sigma_r_chroma:?];
  if (result)
  {

    return [(PFBilateralGridHash *)self _createWithSurface:surface region:v9 cropRect:v8 sigma_s:255 sigma_r_luma:v17 sigma_r_chroma:v16, v15, v14, x, y, width, height];
  }
...
- (void)splat:(__CVBuffer *)splat pout:(float *)pout
{
  countVertices = [(PFBilateralGridHash *)self countVertices];
  width = [(PFBilateralGridHash *)self width];
  height = [(PFBilateralGridHash *)self height];
...
- (void)blur:(const float *)blur pout:(float *)pout
{
  countDims = [(PFBilateralGridHash *)self countDims];
  countVertices = [(PFBilateralGridHash *)self countVertices];
  if (countVertices >= 1)
  {
    v9 = countVertices;
...
LABEL_46:
        ...
        while (1)
        {
          ...
          v48 = [(PFBilateralGridHash *)self _addHashKeyAtX:v52 Y:v49 key:v50(v63) isKeyExist:0];
          if (v48)
          {
            break;
          }
...
LABEL_52:
        ...
        while (1)
        {
          v48 = [(PFBilateralGridHash *)self _addHashKeyAtX:v55 Y:v53 key:(v55 >> v30) | (((v53 >> v30) | (v54[1] >> v33 << 8) | (*v54 >> v28 << 16) | (*(v54 - 1) >> v28 << 24)) << 8) isKeyExist:0];
          if (v48)
          {
            break;
          }
...
LABEL_35:
      ...
      while (1)
      {
        v47 = *v46;
        v46 += 4;
        v48 = [(PFBilateralGridHash *)self _addHashKeyAtX:v45 Y:v43 key:(v45 >> v30) | ((v43 >> v30) << 8) | (v47 >> v33 << 16) isKeyExist:0];
        if (v48)
        {
          break;
        }
    ...
    do
    {
      ...
      else
      {
        ...
        do
        {
          ...
          {
            v14 = v13;
            v15 = sub_28200(var0, v9, dword_54AC0[v12]);
            if (v15 != -1)
            {
              v20 = 0;
              if ([(PFBilateralGridHash *)self _hashMapFindKey:v15 to:&v20])
              {
                v16 = v20;
...
{
  v18 = 0;
  if ([(PFBilateralGridHash *)self _hashMapFindKey:key to:&v18])
  {
    v11 = v18;
    hash_table = self->_hash_table;
...
LABEL_6:
  ...
  if (self->_max_hash_table_size > hash_table_size)
  {
    ...
    v16->var0.var0 = key;
    *&v16->var1 = xmmword_54AB0;
    self->_hash_matrix_data[x + self->_width * y] = hash_table_size;
    [(PFBilateralGridHash *)self _hashMapAddKey:key andValue:?];
    v13 = 0;
    if (exist)
    {
      goto LABEL_6;
