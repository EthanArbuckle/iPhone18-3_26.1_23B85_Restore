...
- (FRCPyramid)initWithDevice:(id)device commmandQueue:(id)queue;
...
- (FRCPyramid)initWithDevice:(id)device commmandQueue:(id)queue
{
  ...
  v4 = [(VEMetalBase *)&v17 initWithDevice:device commmandQueue:queue];
  if (v4)
  {
    ...
    contents = [(MTLBuffer *)v4->_gaussianCoefficientBuffer contents];
    *(contents + 48) = 10386;
    *contents = GaussianFilterCoeffients;
    *(contents + 16) = unk_2487C39B8;
    *(contents + 32) = xmmword_2487C39C8;
    ...
    contents2 = [(MTLBuffer *)v4->_gaussian3x3CoefficientBuffer contents];
    *(contents2 + 16) = 12027;
    *contents2 = Gaussian3x3FilterCoeffients_0;
  }
...
- (void)createResiduePyramidFromBuffer:(__CVBuffer *)buffer toBuffer:(__CVBuffer *)toBuffer levels:(unint64_t)levels
{
  ...
  [v17 enqueue];
  [(FRCPyramid *)self encodeResiduePyramidToCommandBuffer:v17 fromTexture:v8 toTexture:&v21 levels:levels];
  [v17 commit];
...
- (void)encodeResiduePyramidToCommandBuffer:(id)buffer fromTexture:(id)texture toTexture:(const void *)toTexture levels:(unint64_t)levels
{
  ...
  {
    case 4uLL:
      [(FRCPyramid *)self encodeSubsampleTextureWith5x5GaussianToCommandBuffer:bufferCopy source:textureCopy destination:toTexture[4] clampToEdge:1];
      [(FRCPyramid *)self encodeSubsampleTextureWith5x5GaussianToCommandBuffer:bufferCopy source:toTexture[4] destination:toTexture[2] clampToEdge:0];
      [(FRCPyramid *)self encodeSubsampleTextureWith5x5GaussianToCommandBuffer:bufferCopy source:toTexture[2] destination:toTexture[3] clampToEdge:0];
      [(FRCPyramid *)self encodeResidueToCommandBuffer:bufferCopy base:v11 low:toTexture[4] destination:*toTexture];
      [(FRCPyramid *)self encodeResidueToCommandBuffer:bufferCopy base:toTexture[4] low:toTexture[2] destination:toTexture[1]];
      [(FRCPyramid *)self encodeResidueInplaceToCommandBuffer:bufferCopy base:toTexture[2] low:toTexture[3]];
      break;
    case 3uLL:
      v12 = textureCopy;
      height = [v12 height];
      if (height != [*toTexture height])
      {
        ...
        [(FRCPyramid *)self encodeAddBottomPaddingToCommandBuffer:bufferCopy source:v12 destination:v14];
        v12 = v14;
      }

      [(FRCPyramid *)self encodeSubsampleTextureWith5x5GaussianToCommandBuffer:bufferCopy source:v12 destination:toTexture[3] clampToEdge:0];
      [(FRCPyramid *)self encodeSubsampleTextureWith5x5GaussianToCommandBuffer:bufferCopy source:toTexture[3] destination:toTexture[2] clampToEdge:0];
      [(FRCPyramid *)self encodeResidueToCommandBuffer:bufferCopy base:v12 low:toTexture[3] destination:*toTexture];
      [(FRCPyramid *)self encodeResidueToCommandBuffer:bufferCopy base:toTexture[3] low:toTexture[2] destination:toTexture[1]];

      break;
    case 2uLL:
      [(FRCPyramid *)self encode3x3GaussianFilterToCommandBuffer:bufferCopy source:textureCopy destination:toTexture[1]];
      [(FRCPyramid *)self encodeBicubicSubsampleTextureToCommandBuffer:bufferCopy source:toTexture[1] destination:toTexture[2]];
      [(FRCPyramid *)self encode3x3GaussianFilterToCommandBuffer:bufferCopy source:toTexture[2] destination:toTexture[3]];
      [(FRCPyramid *)self encodeResidueToCommandBuffer:bufferCopy base:v11 low:toTexture[3] destination:toTexture[1]];
      break;
    default:
...
- (void)blendWarpedResidueForward:(__CVBuffer *)forward backward:(__CVBuffer *)backward withGridNetOutput:(__CVBuffer *)output timeScale:(float)scale destination:(__CVBuffer *)destination callback:(id)callback
{
  ...
  *&v20 = scale;
  [(FRCPyramid *)self encodeLayerBlendToCommandBuffer:commandBuffer forwardResidue:v15 backwardResidue:v16 withBaseLayer:v17 timeScale:v18 destination:v20];
  if (callbackCopy)
  {
