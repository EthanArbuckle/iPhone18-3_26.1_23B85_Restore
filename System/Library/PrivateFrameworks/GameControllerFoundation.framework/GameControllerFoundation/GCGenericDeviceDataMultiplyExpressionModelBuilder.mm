...
- (GCGenericDeviceDataMultiplyExpressionModelBuilder)initWithDictionaryRepresentation:(id)representation error:(id *)error;
- (id)build;
- (void)build;
...
- (void)initializeWithModel:(id)model
{
  ...
  [(GCGenericDeviceDataMultiplyExpressionModelBuilder *)self setLeftExpression:leftExpression];

  rightExpression = [modelCopy rightExpression];

  [(GCGenericDeviceDataMultiplyExpressionModelBuilder *)self setRightExpression:rightExpression];
}
...
- (id)build
{
  ...
  leftExpression = [(GCGenericDeviceDataMultiplyExpressionModelBuilder *)self leftExpression];
  if (!leftExpression)
  {
  ...
  rightExpression = [(GCGenericDeviceDataMultiplyExpressionModelBuilder *)self rightExpression];
  if (!rightExpression)
  {
    [(GCGenericDeviceDataMultiplyExpressionModelBuilder *)a2 build];
  }
...
- (void)build
{
  ...
  [currentHandler handleFailureInMethod:self object:a2 file:@"GCGenericDeviceDataMultiplyExpressionModel.m" lineNumber:108 description:@"'rightExpression' can not be nil"];
}
