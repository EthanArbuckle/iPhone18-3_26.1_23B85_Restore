...
- (BOOL)runWithGraphManager:(id)manager withIncrementalChange:(id)change progressReporter:(id)reporter error:(id *)error
{
  ...
  {
    v12 = [objc_alloc(MEMORY[0x277D3B9D0]) initWithGraphManager:managerCopy];
    ...
    date = v13;
    v22 = date;
    [v12 applyChangesFromGraphUpdate:changeCopy progressReporter:reporterCopy completionHandler:v21];
  ...
  else
  {
    ...
    v12 = v17;
    v27 = v12;
    [managerCopy setGraphInfoDateOfLastIncrementalUpdateInvocationWithDate:date completionBlock:v26];
    ...
    {
      *buf = 0;
      _os_log_impl(&dword_22FA28000, loggingConnection, OS_LOG_TYPE_DEFAULT, "PHAGraphInternalConsistencyUpdateTask: IncrementalChange has nothing to do", buf, 2u);
    }
...
- (BOOL)runWithGraphManager:(id)manager progressReporter:(id)reporter error:(id *)error
{
  if (error)
  {
    *error = [MEMORY[0x277CCA9B8] pl_analysisErrorWithCode:14 localizedDescription:@"The task does not support graph full rebuild which is handled by PHAGraphRebuildTask"];
  }
...
- (BOOL)shouldRunIncrementallyWithGraphManager:(id)manager incrementalChange:(id)change timeIntervalSinceNonIncrementalRun:(double)run
{
  ...
  changeCopy = change;
  v18 = 0;
  v8 = [managerCopy isReadyWithError:&v18];
  v9 = v18;
  if (v8)
  {
