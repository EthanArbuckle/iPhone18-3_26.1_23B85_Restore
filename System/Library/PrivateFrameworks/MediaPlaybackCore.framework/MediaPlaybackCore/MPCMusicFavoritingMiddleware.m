@interface MPCMusicFavoritingMiddleware
- (id)_stateDumpObject;
- (id)operationsForPlayerRequest:(id)a3;
- (id)operationsForRequest:(id)a3;
- (id)requestingUserSubscriptionStatus:(id)a3 chain:(id)a4;
@end

@implementation MPCMusicFavoritingMiddleware

- (id)_stateDumpObject
{
  v13[3] = *MEMORY[0x1E69E9840];
  v12[0] = @"_obj";
  v3 = [MEMORY[0x1E696AEC0] stringWithFormat:@"<%@:%p>", objc_opt_class(), self];
  v13[0] = v3;
  v12[1] = @"requestingUserSubscriptionStatus";
  v4 = [(MPCMusicFavoritingMiddleware *)self requestingUserSubscriptionStatus];
  v5 = v4;
  if (v4)
  {
    v6 = v4;
  }

  else
  {
    v6 = @"<NULL>";
  }

  v13[1] = v6;
  v12[2] = @"invalidationObservers";
  v7 = [(MPCMusicFavoritingMiddleware *)self invalidationObservers];
  v8 = v7;
  if (v7)
  {
    v9 = v7;
  }

  else
  {
    v9 = @"<NULL>";
  }

  v13[2] = v9;
  v10 = [MEMORY[0x1E695DF20] dictionaryWithObjects:v13 forKeys:v12 count:3];

  return v10;
}

- (id)operationsForPlayerRequest:(id)a3
{
  v8[1] = *MEMORY[0x1E69E9840];
  v4 = a3;
  v5 = [[MPCMusicFavoritingMiddlewareOperation alloc] initWithMiddleware:self playerRequest:v4];

  v8[0] = v5;
  v6 = [MEMORY[0x1E695DEC8] arrayWithObjects:v8 count:1];

  return v6;
}

- (id)operationsForRequest:(id)a3
{
  v4 = a3;
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v5 = [(MPCMusicFavoritingMiddleware *)self operationsForPlayerRequest:v4];
  }

  else
  {
    v5 = MEMORY[0x1E695E0F0];
  }

  return v5;
}

- (id)requestingUserSubscriptionStatus:(id)a3 chain:(id)a4
{
  v6 = a4;
  v7 = a3;
  v8 = [v6 nextObject];
  v9 = [(MPCMusicFavoritingMiddleware *)self requestingUserSubscriptionStatus];
  v10 = v9;
  if (v9)
  {
    v11 = v9;
  }

  else
  {
    v11 = v7;
  }

  v12 = [v8 requestingUserSubscriptionStatus:v11 chain:v6];

  return v12;
}

@end