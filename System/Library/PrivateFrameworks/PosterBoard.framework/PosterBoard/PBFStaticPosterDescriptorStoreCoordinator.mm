...
- (id)_accessQueue_buildIdentityForVersion:(unint64_t)version supplement:(unint64_t)supplement
{
  ...
  if (supplement)
  {
    supplement = [MEMORY[0x277CCACA8] stringWithFormat:@"static descriptors don't support multiple supplements : provider=%@ descriptor=%@ role=%@ posterUUID=%@ version=%llu supplement=%llu", extensionIdentifier, descriptorIdentifier, role, posterUUID, version, supplement];
    if (os_log_type_enabled(MEMORY[0x277D86220], OS_LOG_TYPE_ERROR))
    {
      [(PBFStaticPosterDescriptorStoreCoordinator *)a2 _accessQueue_buildIdentityForVersion:supplement supplement:?];
    }
...
LABEL_14:
  ...
  {
    supplement = [MEMORY[0x277CCACA8] stringWithFormat:@"static descriptors missing component : provider=%@ descriptor=%@ role=%@ posterUUID=%@ version=%llu supplement=%llu", extensionIdentifier, descriptorIdentifier, role, posterUUID, version, 0];
    if (os_log_type_enabled(MEMORY[0x277D86220], OS_LOG_TYPE_ERROR))
    {
      [(PBFStaticPosterDescriptorStoreCoordinator *)a2 _accessQueue_buildIdentityForVersion:supplement supplement:?];
    }
  ...
  v13 = [MEMORY[0x277D3EB98] staticDescriptorIdentityWithProvider:extensionIdentifier identifier:descriptorIdentifier role:role posterUUID:posterUUID version:version];
