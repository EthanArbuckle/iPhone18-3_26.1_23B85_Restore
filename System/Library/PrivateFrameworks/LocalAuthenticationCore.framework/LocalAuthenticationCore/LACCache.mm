@interface LACCache
- (LACCache)initWithTimeout:(double)timeout;
...
- (id)cachedValueWithParameter:(id)parameter
{
  ...
  v6 = [(LACCache *)self _identifierForParameter:parameterCopy];
...
- (void)setCachedValue:(id)value withParameter:(id)parameter
{
  ...
  if (valueCopy)
  {
    v7 = [(LACCache *)self _identifierForParameter:parameter];
    os_unfair_lock_lock(&self->_cachedValuesLock);
    [(NSMutableDictionary *)self->_cachedValues setObject:valueCopy forKeyedSubscript:v7];
    os_unfair_lock_unlock(&self->_cachedValuesLock);
...
- (void)_invalidateCachedValue:(id)value identifier:(id)identifier
{
  ...
  valueCopy = value;
  os_unfair_lock_lock(&self->_cachedValuesLock);
  v7 = [(NSMutableDictionary *)self->_cachedValues objectForKeyedSubscript:identifierCopy];

  if (v7 == valueCopy)
  {
    [(NSMutableDictionary *)self->_cachedValues removeObjectForKey:identifierCopy];
  }
