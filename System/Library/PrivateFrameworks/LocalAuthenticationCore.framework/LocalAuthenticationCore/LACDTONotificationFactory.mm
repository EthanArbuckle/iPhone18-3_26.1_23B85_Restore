...
- (id)_securityDelayEndedNotificationWithIdentifier:(id)identifier body:(id)body callbackURL:(id)l after:(double)after maxAge:(double)age;
...
- (id)securityDelayEndedNotificationForPendingEvaluation:(id)evaluation after:(double)after validity:(double)validity
{
  ...
  callbackReason = [evaluationCopy callbackReason];
  v10 = callbackReason;
  if (callbackReason)
  {
    v11 = callbackReason;
  ...
  after = [LACLocalization dtoSecurityDelayEndedText:v11 duration:validity - after];

  identifier = [evaluationCopy identifier];
  callbackURL = [evaluationCopy callbackURL];

  v15 = [(LACDTONotificationFactory *)self _securityDelayEndedNotificationWithIdentifier:identifier body:after callbackURL:callbackURL after:after maxAge:-1.0];
...
- (id)_securityDelayEndedNotificationWithIdentifier:(id)identifier body:(id)body callbackURL:(id)l after:(double)after maxAge:(double)age
{
  ...
  [(LACUNMutableNotification *)v14 setIdentifier:identifierCopy];
  ...
  [(LACUNMutableNotification *)v14 setBody:bodyCopy];

  [(LACUNMutableNotification *)v14 setIsBodyLocalized:1];
  [(LACUNMutableNotification *)v14 setDefaultActionURL:lCopy];

  [(LACUNMutableNotification *)v14 setIsTimeSensitive:1];
  [(LACUNMutableNotification *)v14 setSystemIconName:@"lock.open.fill"];
  [(LACUNMutableNotification *)v14 setTriggerInterval:after];
  if (age > 0.0 && age != -1.0)
  {
    [(LACUNMutableNotification *)v14 setWithdrawInterval:fmax(age, 60.0)];
  }
