@interface NPKProtoPaymentWebServiceRegistrationRequest
- (BOOL)isEqual:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NPKProtoPaymentWebServiceRegistrationRequest

- (id)description
{
  v3 = MEMORY[0x277CCACA8];
  v8.receiver = self;
  v8.super_class = NPKProtoPaymentWebServiceRegistrationRequest;
  v4 = [(NPKProtoPaymentWebServiceRegistrationRequest *)&v8 description];
  v5 = [(NPKProtoPaymentWebServiceRegistrationRequest *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x277CBEB38] dictionary];
  v4 = v3;
  brokerURLString = self->_brokerURLString;
  if (brokerURLString)
  {
    [v3 setObject:brokerURLString forKey:@"brokerURLString"];
  }

  reason = self->_reason;
  if (reason)
  {
    [v4 setObject:reason forKey:@"reason"];
  }

  return v4;
}

- (void)writeTo:(id)a3
{
  v4 = a3;
  v5 = v4;
  if (self->_brokerURLString)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }

  if (self->_reason)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }
}

- (void)copyTo:(id)a3
{
  v4 = a3;
  v5 = v4;
  if (self->_brokerURLString)
  {
    [v4 setBrokerURLString:?];
    v4 = v5;
  }

  if (self->_reason)
  {
    [v5 setReason:?];
    v4 = v5;
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [objc_msgSend(objc_opt_class() allocWithZone:{a3), "init"}];
  v6 = [(NSString *)self->_brokerURLString copyWithZone:a3];
  v7 = v5[1];
  v5[1] = v6;

  v8 = [(NSString *)self->_reason copyWithZone:a3];
  v9 = v5[2];
  v5[2] = v8;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()] && ((brokerURLString = self->_brokerURLString, !(brokerURLString | v4[1])) || -[NSString isEqual:](brokerURLString, "isEqual:")))
  {
    reason = self->_reason;
    if (reason | v4[2])
    {
      v7 = [(NSString *)reason isEqual:?];
    }

    else
    {
      v7 = 1;
    }
  }

  else
  {
    v7 = 0;
  }

  return v7;
}

- (void)mergeFrom:(id)a3
{
  v4 = a3;
  v5 = v4;
  if (v4[1])
  {
    [(NPKProtoPaymentWebServiceRegistrationRequest *)self setBrokerURLString:?];
    v4 = v5;
  }

  if (v4[2])
  {
    [(NPKProtoPaymentWebServiceRegistrationRequest *)self setReason:?];
    v4 = v5;
  }
}

@end