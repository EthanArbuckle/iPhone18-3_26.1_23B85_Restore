...
- (id)description
{
  ...
  dictionaryRepresentation = [(NPKIDVRemoteDeviceProtoCasdCertificateResponse *)self dictionaryRepresentation];
...
- (id)copyWithZone:(_NSZone *)zone
{
  ...
  v8 = [(NSData *)self->_casdCertificate copyWithZone:zone];
  v9 = v5[1];
  v5[1] = v8;
...
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((errorData = self->_errorData, !(errorData | equalCopy[2])) || -[NSData isEqual:](errorData, "isEqual:")))
  {
    casdCertificate = self->_casdCertificate;
    if (casdCertificate | equalCopy[1])
    {
