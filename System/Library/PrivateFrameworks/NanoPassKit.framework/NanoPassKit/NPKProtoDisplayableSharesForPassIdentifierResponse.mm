...
- (id)description
{
  ...
  v8.receiver = self;
  v8.super_class = NPKProtoDisplayableSharesForPassIdentifierResponse;
  v4 = [(NPKProtoDisplayableSharesForPassIdentifierResponse *)&v8 description];
  dictionaryRepresentation = [(NPKProtoDisplayableSharesForPassIdentifierResponse *)self dictionaryRepresentation];
...
- (void)copyTo:(id)to
{
  ...
  if ([(NPKProtoDisplayableSharesForPassIdentifierResponse *)self sharesDatasCount])
  {
    ...
    sharesDatasCount = [(NPKProtoDisplayableSharesForPassIdentifierResponse *)self sharesDatasCount];
    if (sharesDatasCount)
    {
      v5 = sharesDatasCount;
      for (i = 0; i != v5; ++i)
      {
        v7 = [(NPKProtoDisplayableSharesForPassIdentifierResponse *)self sharesDataAtIndex:i];
  ...
  if (v5)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        [(NPKProtoDisplayableSharesForPassIdentifierResponse *)self addSharesData:*(*(&v10 + 1) + 8 * v8++), v10];
      }
