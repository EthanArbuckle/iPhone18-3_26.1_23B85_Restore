...
- (id)description
{
  ...
  dictionaryRepresentation = [(NPKProtoGenerateSEEncryptionCertificateForSubCredentialIdResponse *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (certificate)
  {
    [dictionary setObject:certificate forKey:@"certificate"];
  }
...
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((certificate = self->_certificate, !(certificate | equalCopy[1])) || -[NSData isEqual:](certificate, "isEqual:")))
  {
    errorData = self->_errorData;
