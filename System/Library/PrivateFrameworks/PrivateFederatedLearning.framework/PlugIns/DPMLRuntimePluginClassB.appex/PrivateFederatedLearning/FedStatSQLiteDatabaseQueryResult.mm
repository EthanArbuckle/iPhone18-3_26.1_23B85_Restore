  ...
  if (sqlite3_step([(FedStatSQLiteDatabaseQueryResult *)self queryStatement]) == 100)
  {
    ...
    if (v3 >= 1)
    {
      ...
      do
      {
        ...
        if ((v8 - 3) < 2)
        {
          v9 = [NSString stringWithUTF8String:sqlite3_column_text([(FedStatSQLiteDatabaseQueryResult *)self queryStatement], v5)];
        }

        else if (v8 == 2)
        {
          v9 = [NSNumber numberWithDouble:sqlite3_column_double([(FedStatSQLiteDatabaseQueryResult *)self queryStatement], v5)];
        }
