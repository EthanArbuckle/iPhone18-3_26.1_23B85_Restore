@interface MADRichLabelClassificationResult
- (MADRichLabelClassificationResult)initWithCoder:(id)coder;
- (MADRichLabelClassificationResult)initWithResultItems:(id)items;
- (id)description;
- (void)encodeWithCoder:(id)coder;
@end

@implementation MADRichLabelClassificationResult

- (MADRichLabelClassificationResult)initWithResultItems:(id)items
{
  itemsCopy = items;
  v9.receiver = self;
  v9.super_class = MADRichLabelClassificationResult;
  v6 = [(MADResult *)&v9 init];
  v7 = v6;
  if (v6)
  {
    objc_storeStrong(&v6->_resultItems, items);
  }

  return v7;
}

- (MADRichLabelClassificationResult)initWithCoder:(id)coder
{
  v13[2] = *MEMORY[0x1E69E9840];
  coderCopy = coder;
  v12.receiver = self;
  v12.super_class = MADRichLabelClassificationResult;
  v5 = [(MADResult *)&v12 initWithCoder:coderCopy];
  if (v5)
  {
    v6 = MEMORY[0x1E695DFD8];
    v13[0] = objc_opt_class();
    v13[1] = objc_opt_class();
    v7 = [MEMORY[0x1E695DEC8] arrayWithObjects:v13 count:2];
    v8 = [v6 setWithArray:v7];

    v9 = [coderCopy decodeObjectOfClasses:v8 forKey:@"ResultItems"];
    resultItems = v5->_resultItems;
    v5->_resultItems = v9;
  }

  return v5;
}

- (void)encodeWithCoder:(id)coder
{
  v5.receiver = self;
  v5.super_class = MADRichLabelClassificationResult;
  coderCopy = coder;
  [(MADResult *)&v5 encodeWithCoder:coderCopy];
  [coderCopy encodeObject:self->_resultItems forKey:{@"ResultItems", v5.receiver, v5.super_class}];
}

- (id)description
{
  string = [MEMORY[0x1E696AD60] string];
  v4 = objc_opt_class();
  v5 = NSStringFromClass(v4);
  [string appendFormat:@"<%@ %p, ", v5, self];

  [string appendFormat:@"resultItems: %@ >", self->_resultItems];

  return string;
}

@end