...
- (BOOL)_isHidingWindowLevel:(double)level;
- (SBWindowHidingManager)initWithWindowScene:(id)scene;
- (id)dumpHidingState;
- (id)dumpKnownWindows;
...
- (void)startHidingWindowsExclusivelyFromLevel:(double)level toLevel:(double)toLevel forContext:(id)context reason:(id)reason;
...
- (SBWindowHidingManager)initWithWindowScene:(id)scene
{
  ...
  if (v5)
  {
    ...
    v6->_hidingContextMap = strongToStrongObjectsMapTable;

    weakToStrongObjectsMapTable = [MEMORY[0x277CCAB00] weakToStrongObjectsMapTable];
    windowStateMap = v6->_windowStateMap;
    v6->_windowStateMap = weakToStrongObjectsMapTable;
    ...
    v16 = [objc_alloc(MEMORY[0x277D67970]) _initWithDefinition:v11 displayIdentity:displayIdentity];
    ...
    v21 = v6;
    v22 = sceneCopy;
    [windows enumerateObjectsUsingBlock:v20];
  }
...
- (void)_window:(id)_window willDetachFromScene:(id)scene
{
  ...
  v8 = [sceneCopy isEqual:WeakRetained];

  if (v8)
  {
    [(SBWindowHidingManager *)self _releaseWindow:_windowCopy];
  }
}
...
{
  windowCopy = window;
  v7 = [(NSMapTable *)self->_windowStateMap objectForKey:windowCopy];
  if (!v7)
  {
    v8 = SBLogCommon();
  ...
  {
LABEL_7:
    [windowCopy setAlphaAndObeyBecauseIAmTheWindowManager:alpha];
    goto LABEL_9;
  }

  [v7 setDesiredAlpha:alpha];
LABEL_9:
}
...
- (void)startHidingWindowsExclusivelyFromLevel:(double)level toLevel:(double)toLevel forContext:(id)context reason:(id)reason
{
  ...
  if (toLevel >= level)
  {
    v14 = [(NSMapTable *)self->_hidingContextMap objectForKey:contextCopy];
    if (v14)
    {
      v15 = SBLogCommon();
      if (os_log_type_enabled(v15, OS_LOG_TYPE_INFO))
      {
        ...
        v19 = [MEMORY[0x277CCACA8] stringWithFormat:@"(%.f, %.f)", *&level, *&toLevel];
        *buf = 134218754;
        v21 = contextCopy;
        v22 = 2112;
        v23 = contextCopy;
    ...
    toLevel = [SBWindowHideRequest hideRequestWithWindowLevelRange:reasonCopy reason:level, toLevel];

    [(NSMapTable *)self->_hidingContextMap setObject:toLevel forKey:contextCopy];
  ...
  {
    toLevel = SBLogCommon();
    if (os_log_type_enabled(toLevel, OS_LOG_TYPE_INFO))
    {
      v13 = [MEMORY[0x277CCACA8] stringWithFormat:@"(%.f, %.f)", *&level, *&toLevel];
      *buf = 134218498;
      v21 = contextCopy;
      v22 = 2112;
      v23 = contextCopy;
      v24 = 2112;
      v25 = v13;
      _os_log_impl(&dword_21ED4E000, toLevel, OS_LOG_TYPE_INFO, "Context %p (%@) passed an invalid window level range %@", buf, 0x20u);
    }
  }
}
...
- (void)stopHidingWindowsForContext:(id)context
{
  ...
  contextCopy = context;
  v5 = [(NSMapTable *)self->_hidingContextMap objectForKey:contextCopy];
  v6 = SBLogCommon();
  ...
  if (v5)
  {
    ...
    [(NSMapTable *)self->_hidingContextMap removeObjectForKey:contextCopy];
    [(SBWindowHidingManager *)self _recomputeHideRangeAndAdjustWindows];
  }
...
- (void)_recomputeHideRangeAndAdjustWindows
{
  ...
  if ([(SBWindowHidingManager *)self _isHidingWindows])
  {
    ...
    v17 = 0u;
    v18 = 0u;
    objectEnumerator = [(NSMapTable *)self->_hidingContextMap objectEnumerator];
    v4 = [objectEnumerator countByEnumeratingWithState:&v17 objects:v21 count:16];
    if (v4)
    {
      v5 = v4;
      v6 = *v18;
...
- (void)_adjustWindowsForActiveHideRange
{
  ...
  v29 = 0u;
  v30 = 0u;
  keyEnumerator = [(NSMapTable *)self->_windowStateMap keyEnumerator];
  v4 = [keyEnumerator countByEnumeratingWithState:&v29 objects:v33 count:16];
  if (v4)
  {
    v5 = v4;
    v6 = *v30;
...
LABEL_18:
      ...
      v5 = [keyEnumerator countByEnumeratingWithState:&v29 objects:v33 count:16];
    }

    while (v5);
  }

  sceneIfExists = [(SBSceneHandle *)self->_keyboardSceneHandle sceneIfExists];
  v18 = sceneIfExists;
  if (sceneIfExists)
  {
    ...
    uiPresentationManager = [v18 uiPresentationManager];
    ...
    else
    {
      ...
      v27 = [uiPresentationManager createPresenterWithIdentifier:@"SBWindowHidingManager" priority:-5];
...
- (void)_takeNoteOfWindow:(id)window onScene:(id)scene
{
  ...
  sceneCopy = scene;
  WeakRetained = objc_loadWeakRetained(&self->_windowScene);
  v9 = [sceneCopy isEqual:WeakRetained];
  ...
  if (v9)
  {
    ...
    {
      [(SBWindowHidingManager *)self _captureWindow:windowCopy];
    }
  ...
  {
    [(SBWindowHidingManager *)self _releaseWindow:windowCopy];
  }
}
...
- (void)_captureWindow:(id)window
{
  ...
  windowCopy = window;
  v5 = [(NSMapTable *)self->_windowStateMap objectForKey:windowCopy];
  v6 = SBLogCommon();
  v7 = os_log_type_enabled(v6, OS_LOG_TYPE_INFO);
  if (v5)
  {
  ...
  else
  {
    ...
    [(NSMapTable *)self->_windowStateMap setObject:v5 forKey:windowCopy];
  }
}
...
- (void)_releaseWindow:(id)window
{
  ...
  if (v5)
  {
    ...
    if ([v5 isHidden])
    {
      ...
      if (isKindOfClass)
      {
        [windowCopy setAlphaAndObeyBecauseIAmTheWindowManager:?];
      }
    ...
    [(NSMapTable *)self->_windowStateMap removeObjectForKey:windowCopy];
  }
}
...
- (id)dumpHidingState
{
  ...
  {
    [string appendFormat:@"Active hide range is: %@\n", @"OFF"];
  }
  ...
  v17 = 0u;
  v18 = 0u;
  keyEnumerator = [(NSMapTable *)self->_hidingContextMap keyEnumerator];
...
- (id)dumpKnownWindows
{
  ...
  string = [MEMORY[0x277CCAB68] string];
  [string appendString:@"These are our known windows:\n"];
  ...
  v25 = 0u;
  keyEnumerator = [(NSMapTable *)self->_windowStateMap keyEnumerator];
  v5 = [keyEnumerator countByEnumeratingWithState:&v24 objects:v28 count:16];
  if (v5)
  {
    v6 = *v25;
    do
...
- (void)sceneHandle:(id)handle didUpdateSettingsWithDiff:(id)diff previousSettings:(id)settings
{
  ...
  {
    _isHidingWindows = [(SBWindowHidingManager *)self _isHidingWindows];
