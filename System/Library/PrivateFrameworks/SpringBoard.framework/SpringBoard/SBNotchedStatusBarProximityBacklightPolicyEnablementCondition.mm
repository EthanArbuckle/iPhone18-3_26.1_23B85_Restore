...
- (SBNotchedStatusBarProximityBacklightPolicyEnablementCondition)initWithMainDisplaySceneManager:(id)manager windowSceneStatusBarManager:(id)barManager statusBarServer:(id)server orientationProvider:(id)provider;
- (SBNotchedStatusBarProximityBacklightPolicyEnablementConditionDelegate)delegate;
- (void)_evaluateEnablement;
- (void)_setEnabled:(BOOL)enabled statusBarHeight:(double)height;
- (void)dealloc;
- (void)invalidate;
...
- (SBNotchedStatusBarProximityBacklightPolicyEnablementCondition)init
{
  ...
  v8 = [(SBNotchedStatusBarProximityBacklightPolicyEnablementCondition *)self initWithMainDisplaySceneManager:v3 windowSceneStatusBarManager:statusBarManager statusBarServer:v7 orientationProvider:SBApp];
...
- (SBNotchedStatusBarProximityBacklightPolicyEnablementCondition)initWithMainDisplaySceneManager:(id)manager windowSceneStatusBarManager:(id)barManager statusBarServer:(id)server orientationProvider:(id)provider
{
  ...
  if (v15)
  {
    objc_storeStrong(&v15->_mainDisplaySceneManager, manager);
    [(SBMainDisplaySceneManager *)v16->_mainDisplaySceneManager addObserver:v16];
    objc_storeStrong(&v16->_windowSceneStatusBarManager, barManager);
    assertionManager = [barManagerCopy assertionManager];
    statusBarAssertionManager = v16->_statusBarAssertionManager;
    v16->_statusBarAssertionManager = assertionManager;

    [(SBWindowSceneStatusBarAssertionManager *)v16->_statusBarAssertionManager addObserver:v16];
    objc_storeStrong(&v16->_orientationProvider, provider);
    [(SpringBoard *)v16->_orientationProvider addActiveOrientationObserver:v16];
    objc_storeStrong(&v16->_statusBarConnection, server);
...
- (void)dealloc
{
  ...
  [currentHandler handleFailureInMethod:self object:a2 file:@"SBNotchedStatusBarProximityBacklightPolicyEnablementCondition.m" lineNumber:80 description:@"This object must be invalidated prior to being deallocated."];
}

- (void)invalidate
{
  currentHandler = [MEMORY[0x277CCA890] currentHandler];
  [currentHandler handleFailureInMethod:self object:a2 file:@"SBNotchedStatusBarProximityBacklightPolicyEnablementCondition.m" lineNumber:91 description:@"This object has already been invalidated."];
}
...
- (void)sceneHandle:(id)handle didUpdateSettingsWithDiff:(id)diff previousSettings:(id)settings
{
  ...
  [(UIApplicationSceneSettingsDiffInspector *)applicationSceneSettingsDiffInspector inspectDiff:diffCopy withContext:0];
}
...
LABEL_18:
    ...
    [(SBNotchedStatusBarProximityBacklightPolicyEnablementCondition *)selfCopy2 _setEnabled:v15 statusBarHeight:v13];
    return;
  }
  ...
  if (![(SBWindowSceneStatusBarManager *)self->_windowSceneStatusBarManager isStatusBarEffectivelyHidden])
  {
    ...
    currentStatusBarSettings = [(SBWindowSceneStatusBarAssertionManager *)self->_statusBarAssertionManager currentStatusBarSettings];
    backgroundActivitiesToSuppress = [currentStatusBarSettings backgroundActivitiesToSuppress];
    v19 = _STBackgroundActivityIdentifiersNotEmptyAfterSubtractingIdentifiers(currentBackgroundActivityIdentifiers, backgroundActivitiesToSuppress);
  ...
  v23 = 0u;
  v24 = 0u;
  externalForegroundApplicationSceneHandles = [(SBMainDisplaySceneManager *)self->_mainDisplaySceneManager externalForegroundApplicationSceneHandles];
  v4 = [externalForegroundApplicationSceneHandles countByEnumeratingWithState:&v23 objects:v27 count:16];
  v5 = 0.0;
  if (v4)
  {
    v6 = *v24;
    while (2)
    {
      for (i = 0; i != v4; ++i)
      {
        ...
        sceneIfExists = [*(*(&v23 + 1) + 8 * i) sceneIfExists];
        v9 = sceneIfExists;
        if (sceneIfExists)
        {
          uiSettings = [sceneIfExists uiSettings];
          if (([uiSettings statusBarDisabled] & 1) == 0 && (objc_msgSend(uiSettings, "forcedStatusBarForegroundTransparent") & 1) == 0)
          {
            v11 = self->_currentBackgroundActivityIdentifiers;
            [uiSettings statusBarStyleOverridesToSuppress];
            ...
            if (v11)
            {
              ...
              v4 = 1;
              [v20 heightForStyle:objc_msgSend(uiClientSettings orientation:{"statusBarStyle"), 1}];
              v5 = v22;
