...
- (NSData)jsonData;
- (VISDeviceStateConfig)initWithDictionary:(id)dictionary;
- (VISDeviceStateConfig)initWithJSON:(id)n;
...
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && (frame_count = self->_frame_count, frame_count == objc_msgSend(equalCopy, "frame_count")) && (anchor_depth = self->_anchor_depth, objc_msgSend(equalCopy, "anchor_depth"), anchor_depth == v7) && (set_zero_state_as_stable = self->_set_zero_state_as_stable, set_zero_state_as_stable == objc_msgSend(equalCopy, "set_zero_state_as_stable")) && (camera_small_movement_threshold = self->_camera_small_movement_threshold, objc_msgSend(equalCopy, "camera_small_movement_threshold"), camera_small_movement_threshold == v10))
  {
    camera_large_movement_threshold = self->_camera_large_movement_threshold;
    [equalCopy camera_large_movement_threshold];
...
- (id)dictionaryRepresentation
{
  ...
  if (self->_anchor_depth != 0.0)
  {
    ...
    [dictionary setObject:v5 forKeyedSubscript:@"anchorDepth"];
  }
  ...
  if (self->_camera_large_movement_threshold != 0.0)
  {
    ...
    [dictionary setObject:v7 forKeyedSubscript:@"cameraLargeMovementThreshold"];
  }
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(VISDeviceStateConfig *)self dictionaryRepresentation];
...
- (VISDeviceStateConfig)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    ...
    v7 = [dictionaryCopy objectForKeyedSubscript:@"anchorDepth"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      [v7 floatValue];
    ...
    v8 = [dictionaryCopy objectForKeyedSubscript:@"setZeroStateAsStable"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
    ...
    v9 = [dictionaryCopy objectForKeyedSubscript:@"cameraSmallMovementThreshold"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      [v9 floatValue];
    ...
    v10 = [dictionaryCopy objectForKeyedSubscript:@"cameraLargeMovementThreshold"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      [v10 floatValue];
