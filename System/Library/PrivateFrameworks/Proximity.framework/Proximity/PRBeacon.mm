@interface PRBeacon
- (PRBeacon)initWithDelegate:(id)delegate queue:(id)queue;
...
- (void)startBeaconingWithOptions:(id)options
{
  ...
  [v5 startBeaconing:1 options:optionsCopy reply:v6];
  objc_destroyWeak(&v7);
  objc_destroyWeak(&location);
}
...
- (void)rangingServiceDidUpdateState:(unint64_t)state cause:(int64_t)cause
{
  if (state <= 4)
  {
    [(PRBeacon *)self _updateState:qword_230EED738[state], cause];
  }
}
...
- (void)didReceiveNewSolutions:(id)solutions
{
  ...
  {
    [WeakRetained beacon:self didOutputRangeResults:solutionsCopy];
  }
}
...
- (void)rangingRequestDidUpdateStatus:(unint64_t)status
{
  if (status <= 3)
  {
    [(PRBeacon *)self _updateState:qword_230EED760[status]];
  }
}
...
- (void)_updateState:(unint64_t)state
{
  ...
  if (objc_opt_respondsToSelector())
  {
    [WeakRetained beacon:self didChangeState:state];
  }
}
