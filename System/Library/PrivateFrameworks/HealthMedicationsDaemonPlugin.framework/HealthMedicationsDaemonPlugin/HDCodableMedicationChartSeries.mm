@interface HDCodableMedicationChartSeries
- (BOOL)isEqual:(id)equal;
- (id)copyWithZone:(_NSZone *)zone;
- (id)description;
- (id)dictionaryRepresentation;
- (void)copyTo:(id)to;
- (void)mergeFrom:(id)from;
- (void)writeTo:(id)to;
@end

@implementation HDCodableMedicationChartSeries

- (id)description
{
  v3 = MEMORY[0x277CCACA8];
  v8.receiver = self;
  v8.super_class = HDCodableMedicationChartSeries;
  v4 = [(HDCodableMedicationChartSeries *)&v8 description];
  dictionaryRepresentation = [(HDCodableMedicationChartSeries *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, dictionaryRepresentation];

  return v6;
}

- (id)dictionaryRepresentation
{
  dictionary = [MEMORY[0x277CBEB38] dictionary];
  v4 = dictionary;
  monthData = self->_monthData;
  if (monthData)
  {
    [dictionary setObject:monthData forKey:@"monthData"];
  }

  sixMonthsData = self->_sixMonthsData;
  if (sixMonthsData)
  {
    [v4 setObject:sixMonthsData forKey:@"sixMonthsData"];
  }

  return v4;
}

- (void)writeTo:(id)to
{
  toCopy = to;
  v5 = toCopy;
  if (self->_monthData)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }

  if (self->_sixMonthsData)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }
}

- (void)copyTo:(id)to
{
  toCopy = to;
  v5 = toCopy;
  if (self->_monthData)
  {
    [toCopy setMonthData:?];
    toCopy = v5;
  }

  if (self->_sixMonthsData)
  {
    [v5 setSixMonthsData:?];
    toCopy = v5;
  }
}

- (id)copyWithZone:(_NSZone *)zone
{
  v5 = [objc_msgSend(objc_opt_class() allocWithZone:{zone), "init"}];
  v6 = [(NSData *)self->_monthData copyWithZone:zone];
  v7 = v5[1];
  v5[1] = v6;

  v8 = [(NSData *)self->_sixMonthsData copyWithZone:zone];
  v9 = v5[2];
  v5[2] = v8;

  return v5;
}

- (BOOL)isEqual:(id)equal
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((monthData = self->_monthData, !(monthData | equalCopy[1])) || -[NSData isEqual:](monthData, "isEqual:")))
  {
    sixMonthsData = self->_sixMonthsData;
    if (sixMonthsData | equalCopy[2])
    {
      v7 = [(NSData *)sixMonthsData isEqual:?];
    }

    else
    {
      v7 = 1;
    }
  }

  else
  {
    v7 = 0;
  }

  return v7;
}

- (void)mergeFrom:(id)from
{
  fromCopy = from;
  v5 = fromCopy;
  if (fromCopy[1])
  {
    [(HDCodableMedicationChartSeries *)self setMonthData:?];
    fromCopy = v5;
  }

  if (fromCopy[2])
  {
    [(HDCodableMedicationChartSeries *)self setSixMonthsData:?];
    fromCopy = v5;
  }
}

@end