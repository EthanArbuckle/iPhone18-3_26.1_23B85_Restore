...
- (id)createMapMatcherTestEventInsertStatementAndReturnError:(id *)a3;
...
{
  v4 = self;
  v5 = MNTrace.route(beforeTimestamp:)(a3);
...
- (id)routesWithDirectionsResponseID:(id)a3 selectedRouteIndex:(int64_t *)a4
{
  ...
  v11 = MNTrace.objcOnly_routesWithDirectionsResponseID(directionsResponseID:selectedRouteIndex:)(v8, v10, a4);
...
- (id)prepareStatement:(id)a3 outError:(id *)a4
{
  v5 = [MNTracePreparedStatement preparedStatementForTrace:self statement:a3 outError:a4];
  if (v5)
  {
...
- (id)_handleOpenErrorWithPath:(id)a3
{
  ...
  {
    v6 = [MEMORY[0x1E696AEC0] stringWithFormat:@"Can't open file because file does not have read permission set. Try running as root:\n> chmod u+r %@", v3];
    if (v6)
    {
...
- (int)mainTransportType
{
  ...
  {
    v4 = [v3 request];
    v5 = [v4 routeAttributes];
    v6 = [v5 mainTransportType];
  }
...
- (id)dateFromTimestamp:(double)a3
{
  ...
  {
    v6 = [(NSArray *)self->_locations firstObject];
    v7 = [v6 location];
    v8 = [v7 timestamp];
...
- (BOOL)startWritingTraceToPath:(id)a3
{
  ...
  self->_db = 0;
  v5 = sqlite3_open_v2([v4 fileSystemRepresentation], &self->_db, 6, 0);
  if (v5)
  {
  ...
  {
    LOBYTE(v13) = -1;
    setxattr([v4 UTF8String], "com.apple.runningboard.can-suspend-locked", &v13, 1uLL, 0, 0);
    v7 = [v4 copy];
...
- (BOOL)openTrace:(id)a3 outError:(id *)a4
{
  ...
  if (!self->_db)
  {
    v8 = sqlite3_open_v2([v6 fileSystemRepresentation], &self->_db, 2, 0);
    if (!v8)
    {
      v15 = [v7 copy];
      tracePath = self->_tracePath;
...
- (id)serializableBookmarks
{
  v3 = [(MNTrace *)self bookmarks];
  if ([v3 count])
  {
    v4 = MEMORY[0x1E695DF70];
    v5 = [(MNTrace *)self bookmarks];
    v6 = [v4 arrayWithCapacity:{objc_msgSend(v5, "count")}];

    v7 = [(MNTrace *)self bookmarks];
    v8 = [v7 count];
    ...
    if (v8)
    {
      ...
      do
      {
        v10 = [(MNTrace *)self bookmarks];
        v11 = [v10 objectAtIndex:v9];

        v12 = [(MNTrace *)self bookmarkImages];
