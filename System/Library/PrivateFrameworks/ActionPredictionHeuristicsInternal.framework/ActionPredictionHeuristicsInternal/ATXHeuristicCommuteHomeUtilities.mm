...
+ (id)heuristicResultToHomeWithLocation:(id)location validStartDate:(id)date validEndDate:(id)endDate heuristicDevice:(id)device;
+ (id)heuristicResultToHomeWithValidStartDate:(id)date validEndDate:(id)endDate heuristicDevice:(id)device;
@end
...
+ (id)heuristicResultToHomeWithLocation:(id)location validStartDate:(id)date validEndDate:(id)endDate heuristicDevice:(id)device
{
  ...
  else
  {
    ...
    v18 = [v17 localizedStringForKey:@"DESTINATION_NAME_HOME" value:&stru_2850AD368 table:0];

    v19 = [MEMORY[0x277CBFC40] placemarkWithLocation:locationCopy name:v18 postalAddress:0];
    LOBYTE(v27) = 0;
    v20 = [ATXHeuristicNavigationUtilities navigationSuggestionActionForDestination:v19 event:0 transportType:@"AUTOMOBILE" schemaForEvent:0 predictionReasons:0x2000000000 heuristicDevice:deviceCopy score:50.0 shouldClearOnEngagement:v27 validStartDate:dateCopy validEndDate:endDateCopy];
    if (v20)
    {
      v28 = v18;
      v29 = locationCopy;
...
+ (id)heuristicResultToHomeWithValidStartDate:(id)date validEndDate:(id)endDate heuristicDevice:(id)device
{
  ...
  else
  {
    ...
    v15 = [v14 localizedStringForKey:@"DESTINATION_NAME_HOME" value:&stru_2850AD368 table:0];

    v16 = [ATXHeuristicNavigationUtilities navigationSuggestionActionForLOI:0 destinationName:v15 schemaForEvent:0 withScore:@"AUTOMOBILE" transportType:0x2000000000 predictionReason:dateCopy validStartDate:50.0 validEndDate:endDateCopy heuristicDevice:deviceCopy];
    if (v16)
    {
      [v10 addObject:v16];
...
+ (void)heuristicResultToHomeWithLocation:validStartDate:validEndDate:heuristicDevice:.cold.1()
{
  ...
  OUTLINED_FUNCTION_3_1(&dword_23E3EA000, v0, v1, "ATXHeuristicCommuteHomeUtilities: validStartDate %@ is after validEndDate %@. Skipping event");
