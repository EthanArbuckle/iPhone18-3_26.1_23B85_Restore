...
+ (BOOL)isZipArchiveAtPath:(id)a3
{
  ...
  v6 = [v4 readToBuffer:v8 size:4] == 4 && v8[0] == 67324752;
  [v4 close];
...
- (void)setCryptoKey:(id)a3
{
  ...
  [v6 makeObjectsPerformSelector:"setCryptoKey:" withObject:mCryptoKey];
}
...
- (BOOL)decompressAtPath:(id)a3 wasEmpty:(BOOL *)a4
{
  ...
  v23 = 0;
  if ([(NSFileManager *)v6 fileExistsAtPath:a3 isDirectory:&v23])
  {
    if ((v23 & 1) == 0)
    {
...
LABEL_27:
  ...
  else if (![(NSFileManager *)v6 createDirectoryAtPath:a3 withIntermediateDirectories:1 attributes:0 error:0])
  {
    ...
    [TSUAssertionHandler handleFailureInFunction:[NSString stringWithUTF8String:"[SFUZipArchive decompressAtPath:wasEmpty:]" file:a4] lineNumber:[NSString stringWithUTF8String:"/Library/Caches/com.apple.xbs/Sources/iWorkXPC/shared/utility/sf/SFUZipArchive.m"] isFatal:217 description:0, "Can't create output directory"];
    +[TSUAssertionHandler logBacktraceThrottled];
    goto LABEL_27;
  }

  v7 = [-[SFUZipArchive allEntryNames](self allEntryNames];
  v21 = 0u;
  v22 = 0u;
  v19 = 0u;
  v20 = 0u;
        ...
        if (v8)
        {
          ...
          if ([(NSMutableArray *)v8 count])
          {
            do
            {
              ...
              if ([-[NSMutableArray objectAtIndexedSubscript:](v8 objectAtIndexedSubscript:{v11), "compare:options:", objc_msgSend(v10, "objectAtIndexedSubscript:", v11), v3 | 2}])
              {
                break;
              }
    ...
    while (1)
    {
      ...
      if (v14 > 0x636D)
      {
        if (v14 == 25454)
        {
          ...
          if (v15 && *v12 == v18[0])
          {
            v16 = [[NSData alloc] initWithBytes:v8 + 8 length:(v11 - 4)];
        ...
        {
          self->mEncryptedDocumentUuid = [[NSData alloc] initWithBytes:v8 + 8 length:(v11 - 4)];
        }
      }
      ...
      {
        self->mPassphraseVerifier = [[NSData alloc] initWithBytes:v8 + 8 length:(v11 - 4)];
      }
