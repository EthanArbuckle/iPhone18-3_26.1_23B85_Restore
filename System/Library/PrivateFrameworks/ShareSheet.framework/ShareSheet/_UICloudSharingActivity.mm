...
- (BOOL)_isURLEligibleForSharing:(id)sharing isInitial:(BOOL *)initial isFolder:(BOOL *)folder;
- (BOOL)canPerformWithActivityItems:(id)items;
- (BOOL)eligibleTypeForActivity:(id)activity;
...
- (void)_showSharingControllerForURL:(id)l completion:(id)completion;
@end
...
- (void)_setupForInitialSharing
{
  activityItemIsFolder = [(_UICloudSharingActivity *)self activityItemIsFolder];
  v4 = @"Share File in iCloud";
  if (activityItemIsFolder)
  {
  ...
  if (v6)
  {
    ...
    while (2)
    {
      for (i = 0; i != v7; ++i)
      {
        ...
        if (UTTypeConformsTo(activityCopy, *(*(&v11 + 1) + 8 * i)))
        {
          v4 = 0;
          goto LABEL_11;
        }
...
- (BOOL)_isURLEligibleForSharing:(id)sharing isInitial:(BOOL *)initial isFolder:(BOOL *)folder
{
  ...
  initialCopy = initial;
  folderCopy = folder;
  [defaultManager synchronouslyGetFileProviderServicesForItemAtURL:v10 completionHandler:v12];

  LOBYTE(folder) = *(v19 + 24);
...
- (void)_prepareWithActivityItems:(id)items completion:(id)completion
{
  ...
  v9 = completionCopy;
  v7 = completionCopy;
  [(UIActivity *)self _loadItemProvidersFromActivityItems:items wantsSendCopyRepresentation:0 completion:v8];
}
...
- (void)_showSharingControllerForURL:(id)l completion:(id)completion
{
  ...
  lCopy = l;
  v15 = 0;
  v9 = [[v6 alloc] initWithFileURL:lCopy error:&v15];
  ...
  else
  {
    ...
    v14 = [MEMORY[0x1E69DC650] alertControllerWithTitle:v12 message:localizedDescription preferredStyle:1];
