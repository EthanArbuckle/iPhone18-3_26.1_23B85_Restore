...
- (PXEditBatchEnhanceAction)initWithPresetManager:(id)manager syncProgress:(id)progress asyncProgress:(id)asyncProgress asyncLoadingStatusManager:(id)statusManager forAssets:(id)assets;
- (void)_syncEnhanceDidFinishWithResults:(id)results didCancel:(BOOL)cancel completionHandler:(id)handler;
...
- (void)_syncEnhanceDidFinishWithResults:(id)results didCancel:(BOOL)cancel completionHandler:(id)handler
{
  ...
  else
  {
    ...
    enabled = [(PXEditBatchEnhanceAction *)self enabled];
    ...
    v17 = handlerCopy;
    [manager autoEnhanceAssets:v11 enabled:enabled async:1 progress:asyncProgress completion:v15];
...
- (void)performAction:(id)action
{
  ...
  if ([assets count])
  {
    ...
    else
    {
      ...
      v13 = actionCopy;
      [manager autoEnhanceAssets:syncAssets enabled:enabled async:0 progress:syncProgress completion:v12];
