...
- (void)_promptDeleteMemoryConfirmationWithCompletionHandler:(id)handler;
...
- (void)performBackgroundTask
{
  ...
  if (assetCollection)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXPhotoKitAssetCollectionActionManager.m" lineNumber:1070 description:{@"%@ should be an instance inheriting from %@, but it is %@", @"self.assetCollection", v17, px_descriptionForAssertionMessage}];
  }
  ...
  else
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXPhotoKitAssetCollectionActionManager.m" lineNumber:1070 description:{@"%@ should be an instance inheriting from %@, but it is nil", @"self.assetCollection", v17}];
  }
...
LABEL_3:
  ...
  v21 = assetCollection;
  v14 = assetCollection;
  [(PXAction *)v12 executeWithUndoManager:undoManager completionHandler:v20];
}
...
- (void)_promptDeleteMemoryConfirmationWithCompletionHandler:(id)handler
{
  ...
  if (!assetCollection)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXPhotoKitAssetCollectionActionManager.m" lineNumber:1050 description:{@"%@ should be an instance inheriting from %@, but it is nil", @"self.assetCollection", v11}];
LABEL_6:
  ...
  if ((objc_opt_isKindOfClass() & 1) == 0)
  {
    ...
    px_descriptionForAssertionMessage = [assetCollection px_descriptionForAssertionMessage];
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXPhotoKitAssetCollectionActionManager.m" lineNumber:1050 description:{@"%@ should be an instance inheriting from %@, but it is %@", @"self.assetCollection", v11, px_descriptionForAssertionMessage}];

    goto LABEL_6;
  }

LABEL_3:
  v14[0] = assetCollection;
  v7 = [MEMORY[0x1E695DEC8] arrayWithObjects:v14 count:1];
  presentationEnvironment = [(PXActionPerformer *)self presentationEnvironment];
  PXMemoriesDeleteConfirmationAlertShow(v7, presentationEnvironment, handlerCopy);
}
