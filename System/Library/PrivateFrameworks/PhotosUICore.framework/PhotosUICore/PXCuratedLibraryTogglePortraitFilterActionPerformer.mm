...
- (PXCuratedLibraryTogglePortraitFilterActionPerformer)initWithViewModel:(id)model;
  ...
  if (case == 1)
  {
    ...
    if (allPhotosContentFilterState)
    {
      ...
      if ((objc_opt_isKindOfClass() & 1) == 0)
      {
        ...
        [currentHandler handleFailureInMethod:a2 object:self file:@"PXCuratedLibraryTogglePortraitFilterActionPerformer.m" lineNumber:43 description:{@"%@ should be nil or an instance inheriting from %@, but it is %@", @"self.viewModel.allPhotosContentFilterState", v12, px_descriptionForAssertionMessage}];
      }
    }

    if ([allPhotosContentFilterState portrait])
    {
...
- (PXCuratedLibraryTogglePortraitFilterActionPerformer)initWithViewModel:(id)model
{
  ...
  return [(PXCuratedLibraryActionPerformer *)&v4 initWithActionType:@"PXCuratedLibraryActionTogglePortraitFilter" viewModel:model];
}
