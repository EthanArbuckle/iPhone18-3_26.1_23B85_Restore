...
- (PXCPLPhotoLibrarySource)initWithPhotoLibrary:(id)library;
...
- (void)_updateIsCloudPhotosEnabled:(BOOL)enabled;
- (void)_updateIsKeepOriginalsEnabled:(BOOL)enabled;
- (void)_updateIsLocalModeEnabled;
- (void)_updateIsRebuildingThumbnails;
- (void)cplSettingsDidChange:(id)change;
- (void)dealloc;
- (void)observeValueForKeyPath:(id)path ofObject:(id)object change:(id)change context:(void *)context;
- (void)setFractionCompletedForRebuild:(float)rebuild;
- (void)setIsCloudPhotosEnabled:(BOOL)enabled;
- (void)setIsKeepOriginalsEnabled:(BOOL)enabled;
- (void)setIsLocalModeEnabled:(BOOL)enabled;
- (void)setIsRebuildingThumbnails:(BOOL)thumbnails;
@end
...
- (void)observeValueForKeyPath:(id)path ofObject:(id)object change:(id)change context:(void *)context
{
  if (_PostOpenProgressObservationContext == context)
  {
  ...
  else
  {
    ...
    [(PXCPLPhotoLibrarySource *)&v6 observeValueForKeyPath:path ofObject:object change:change context:?];
  }
}
...
- (void)_updateFractionCompletedForRebuild
{
  ...
  dispatch_assert_queue_V2(MEMORY[0x1E69E96A0]);
  postOpenProgress = [(PXCPLPhotoLibrarySource *)self postOpenProgress];
  [postOpenProgress fractionCompleted];
  v5 = v4 >= 1.0 || v4 <= 0.0;
  v6 = v4;
  v9 = 3221225472;
  ...
  if ([postOpenProgress isFinished])
  {
...
- (void)_removePostOpenProgressObserverIfNeeded
{
  ...
  if (postOpenProgress)
  {
    ...
    [postOpenProgress removeObserver:self forKeyPath:@"fractionCompleted" context:_PostOpenProgressObservationContext];
  }
}
...
- (void)_queue_subscribeToPostOpenProgressForPhotoLibrary:(id)library
{
  ...
  if (!libraryCopy)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXCPLPhotoLibrarySource.m" lineNumber:171 description:{@"Invalid parameter not satisfying: %@", @"photoLibrary"}];
  }

  postOpenProgress = [libraryCopy postOpenProgress];
  v7 = postOpenProgress;
  if (!postOpenProgress)
  {
...
LABEL_10:
  ...
  isFinished = [postOpenProgress isFinished];
  v9 = PLUserStatusGetLog();
...
- (void)setIsRebuildingThumbnails:(BOOL)thumbnails
{
  ...
  dispatch_assert_queue_V2(MEMORY[0x1E69E96A0]);
  if (self->_isRebuildingThumbnails != thumbnailsCopy)
  {
    self->_isRebuildingThumbnails = thumbnailsCopy;
...
- (void)setFractionCompletedForRebuild:(float)rebuild
{
  ...
  v7 = -1;
  v8 = vcvtps_s32_f32(rebuild * 100.0);
  if (v8 >= 100)
  {
    v8 = 100;
  }
...
- (void)setIsKeepOriginalsEnabled:(BOOL)enabled
{
  enabledCopy = enabled;
  dispatch_assert_queue_V2(MEMORY[0x1E69E96A0]);
  if (self->_isKeepOriginalsEnabled != enabledCopy)
  {
    self->_isKeepOriginalsEnabled = enabledCopy;
...
- (void)setIsLocalModeEnabled:(BOOL)enabled
{
  enabledCopy = enabled;
  dispatch_assert_queue_V2(MEMORY[0x1E69E96A0]);
  if (self->_isLocalModeEnabled != enabledCopy)
  {
    self->_isLocalModeEnabled = enabledCopy;
...
- (void)setIsCloudPhotosEnabled:(BOOL)enabled
{
  enabledCopy = enabled;
  dispatch_assert_queue_V2(MEMORY[0x1E69E96A0]);
  if (self->_isCloudPhotosEnabled != enabledCopy)
  {
    self->_isCloudPhotosEnabled = enabledCopy;
...
- (PXCPLPhotoLibrarySource)initWithPhotoLibrary:(id)library
{
  ...
  {
    currentHandler = [MEMORY[0x1E696AAA8] currentHandler];
    [currentHandler handleFailureInMethod:a2 object:self file:@"PXCPLPhotoLibrarySource.m" lineNumber:72 description:{@"Invalid parameter not satisfying: %@", @"photoLibrary"}];
  }
  ...
  if (v7)
  {
    ...
    v11 = [v9 initWithLibraryBundle:photoLibraryBundle];
    ...
    pathManager = [(PHPhotoLibrary *)v8->_photoLibrary pathManager];
    v8->_isRebuildingThumbnails = [MEMORY[0x1E69BE848] hasRebuildingThumbnailsIndicatorAndRebuildingWithPathManager:pathManager];
...
{
  currentHandler = [MEMORY[0x1E696AAA8] currentHandler];
  [currentHandler handleFailureInMethod:a2 object:self file:@"PXCPLPhotoLibrarySource.m" lineNumber:68 description:{@"%s is not available as initializer", "-[PXCPLPhotoLibrarySource init]"}];
