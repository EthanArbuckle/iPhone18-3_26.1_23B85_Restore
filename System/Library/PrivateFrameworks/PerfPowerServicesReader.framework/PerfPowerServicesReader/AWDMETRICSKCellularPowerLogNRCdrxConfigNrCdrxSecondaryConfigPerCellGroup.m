...
- (id)description
{
  ...
  v5 = [(AWDMETRICSKCellularPowerLogNRCdrxConfigNrCdrxSecondaryConfigPerCellGroup *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 4) != 0)
  {
    ...
    [v3 setObject:v7 forKey:@"on_duration_timer_ms"];
...
LABEL_3:
  ...
  [v3 setObject:v8 forKey:@"on_duration_timer_fraction"];

  if (*&self->_has)
  {
LABEL_4:
    v5 = [MEMORY[0x277CCABB0] numberWithUnsignedInt:self->_inactivityTimerMs];
    [v3 setObject:v5 forKey:@"inactivity_timer_ms"];
  }
...
LABEL_3:
  ...
  v4[3] = self->_onDurationTimerFraction;
  *(v4 + 20) |= 2u;
  if (*&self->_has)
  {
LABEL_4:
    v4[2] = self->_inactivityTimerMs;
    *(v4 + 20) |= 1u;
  }
...
- (BOOL)isEqual:(id)a3
{
  ...
  if ((*&self->_has & 4) != 0)
  {
    if ((*(v4 + 20) & 4) == 0 || self->_onDurationTimerMs != *(v4 + 4))
    {
      goto LABEL_16;
    }
  }
...
LABEL_16:
  ...
  if ((*&self->_has & 2) != 0)
  {
    if ((*(v4 + 20) & 2) == 0 || self->_onDurationTimerFraction != *(v4 + 3))
    {
      goto LABEL_16;
    }
  }
  ...
  if (*&self->_has)
  {
    if ((*(v4 + 20) & 1) == 0 || self->_inactivityTimerMs != *(v4 + 2))
    {
      goto LABEL_16;
    }
...
LABEL_3:
  ...
  self->_onDurationTimerFraction = *(v4 + 3);
  *&self->_has |= 2u;
  if (*(v4 + 20))
  {
LABEL_4:
    self->_inactivityTimerMs = *(v4 + 2);
    *&self->_has |= 1u;
  }
