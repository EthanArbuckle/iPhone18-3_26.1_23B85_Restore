...
- (void)addBwpInfo:(id)info
{
  ...
  [(NSMutableArray *)bwpInfos addObject:infoCopy];
}
...
- (id)description
{
  ...
  dictionaryRepresentation = [(AWDMETRICSCellularNrRadioStats *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ([(NSMutableArray *)self->_bwpInfos count])
  {
    ...
    [dictionary setObject:v5 forKey:@"Bwp_info"];
  }
...
- (void)copyTo:(id)to
{
  ...
  {
    [v10 clearBwpInfos];
    bwpInfosCount = [(AWDMETRICSCellularNrRadioStats *)self bwpInfosCount];
    if (bwpInfosCount)
    {
      v6 = bwpInfosCount;
      for (i = 0; i != v6; ++i)
...
LABEL_19:
  ...
  if ((has & 4) != 0)
  {
    if ((*(equalCopy + 32) & 4) == 0 || self->_subsId != *(equalCopy + 7))
    {
      goto LABEL_19;
    }
  }
