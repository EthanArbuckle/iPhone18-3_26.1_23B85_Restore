@interface AEWrappedAssetsDataSource
- (AEWrappedAssetsDataSource)initWithReviewAssetsDataSource:(id)source;
- (PXSimpleIndexPath)indexPathForObjectReference:(SEL)reference;
- (id)objectAtIndexPath:(PXSimpleIndexPath *)path;
- (int64_t)numberOfItemsInSection:(int64_t)section;
- (int64_t)numberOfSections;
@end

@implementation AEWrappedAssetsDataSource

- (PXSimpleIndexPath)indexPathForObjectReference:(SEL)reference
{
  v6 = a4;
  *&retstr->dataSourceIdentifier = 0u;
  *&retstr->item = 0u;
  v15.receiver = self;
  v15.super_class = AEWrappedAssetsDataSource;
  [(PXSimpleIndexPath *)&v15 indexPathForObjectReference:v6];
  if (retstr->dataSourceIdentifier == *MEMORY[0x277D3CF78])
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      itemObject = [v6 itemObject];
      _reviewAssetsDataSource = [(AEWrappedAssetsDataSource *)self _reviewAssetsDataSource];
      objc_opt_class();
      if (objc_opt_isKindOfClass())
      {
        objc_opt_class();
        if (objc_opt_isKindOfClass())
        {
          v9 = _reviewAssetsDataSource;
          identifier = [itemObject identifier];
          v11 = [v9 indexPathForAssetWithIdentifier:identifier];

          [(AEWrappedAssetsDataSource *)self identifier];
          PXSimpleIndexPathFromIndexPath();
          *&retstr->dataSourceIdentifier = v13;
          *&retstr->item = v14;
        }
      }
    }
  }

  return result;
}

- (id)objectAtIndexPath:(PXSimpleIndexPath *)path
{
  _reviewAssetsDataSource = [(AEWrappedAssetsDataSource *)self _reviewAssetsDataSource];
  v8 = *path;
  v5 = PXIndexPathFromSimpleIndexPath();
  v6 = [_reviewAssetsDataSource assetAtIndexPath:{v5, *&v8.dataSourceIdentifier, *&v8.item}];

  return v6;
}

- (int64_t)numberOfItemsInSection:(int64_t)section
{
  _reviewAssetsDataSource = [(AEWrappedAssetsDataSource *)self _reviewAssetsDataSource];
  v4 = [objc_alloc(MEMORY[0x277CCAA70]) initWithIndex:0];
  v5 = [_reviewAssetsDataSource numberOfSubItemsAtIndexPath:v4];

  return v5;
}

- (int64_t)numberOfSections
{
  _reviewAssetsDataSource = [(AEWrappedAssetsDataSource *)self _reviewAssetsDataSource];
  v3 = objc_alloc_init(MEMORY[0x277CCAA70]);
  v4 = [_reviewAssetsDataSource numberOfSubItemsAtIndexPath:v3];

  return v4;
}

- (AEWrappedAssetsDataSource)initWithReviewAssetsDataSource:(id)source
{
  sourceCopy = source;
  v9.receiver = self;
  v9.super_class = AEWrappedAssetsDataSource;
  v6 = [(AEWrappedAssetsDataSource *)&v9 init];
  v7 = v6;
  if (v6)
  {
    objc_storeStrong(&v6->__reviewAssetsDataSource, source);
  }

  return v7;
}

@end