...
- (TNMutableChartFormulaStorage)initWithChartFormulaStorage:(id)storage;
- (id)mutableFormulaListForType:(unint64_t)type;
- (unint64_t)numberOfGroupsWithCalcEngine:(id)engine inEntity:(TSKUIDStruct)entity shouldSkipHiddenData:(BOOL)data;
- (void)addFormula:(id)formula withFormulaID:(TSUCellCoord)d;
- (void)clearFormulaListForKey:(unint64_t)key;
- (void)setDirection:(int)direction;
- (void)setFormulaList:(id)list forKey:(unint64_t)key;
@end
...
{
  storageCopy = storage;
  v7 = objc_msgSend_direction(storageCopy, v5, v6);
  v10 = objc_msgSend_scheme(storageCopy, v8, v9);
  v14 = objc_msgSend_initWithDirection_scheme_(self, v11, v7, v10);
  if (v14)
  {
    v15 = objc_msgSend_formulaTypeEnumerator(storageCopy, v12, v13);
    for (i = objc_msgSend_nextFormulaType(v15, v16, v17); i != 0x7FFFFFFFFFFFFFFFLL; i = objc_msgSend_nextFormulaType(v15, v25, v26))
    {
      v20 = objc_msgSend_formulaListForType_(storageCopy, v18, i);
...
- (void)clearFormulaListForKey:(unint64_t)key
{
  v5 = objc_msgSend_mutableFormulaListForType_(self, a2, key);
...
- (void)setFormulaList:(id)list forKey:(unint64_t)key
{
  listCopy = list;
  storage = self->super._storage;
  v8 = objc_msgSend_numberWithInteger_(MEMORY[0x277CCABB0], v7, key);
  objc_msgSend_setObject_forKey_(storage, v9, listCopy, v8);
}

- (void)addFormula:(id)formula withFormulaID:(TSUCellCoord)d
{
  formulaCopy = formula;
  v9 = objc_msgSend_mutableFormulaListForType_(self, v6, d.column);
  if (!v9)
  {
  ...
  if (v17 <= LOWORD(d.row))
  {
    if (v17 == LOWORD(d.row))
    {
      objc_msgSend_addObject_(v9, v18, formulaCopy);
    }

    else if (v17 < LOWORD(d.row))
    {
  ...
  {
    objc_msgSend_replaceObjectAtIndex_withObject_(v9, v18, LOWORD(d.row), formulaCopy);
  }
}
...
- (unint64_t)numberOfGroupsWithCalcEngine:(id)engine inEntity:(TSKUIDStruct)entity shouldSkipHiddenData:(BOOL)data
{
  ...
  return [(TNChartFormulaStorage *)&v6 numberOfGroupsWithCalcEngine:engine inEntity:entity._lower shouldSkipHiddenData:entity._upper, data];
}
