...
- (SFUMoveableFileOutputStream)initWithTemporaryFile:(id)file
{
  ...
  if (v4)
  {
    ...
    if (v9 == -1)
    {
      [MEMORY[0x277CBEAD8] sfu_errnoRaise:@"SFUMkstempError" format:{@"Cannot create temporary file with template %@", v6}];
    }
...
- (SFUMoveableFileOutputStream)initWithPath:(id)path
{
  ...
  if (v4)
  {
    ...
    v5->mFd = sub_27709E424(path, 1537, 438);
    v5->mPath = [path copy];
  }
...
- (int64_t)offset
{
  ...
  if (v2 == -1)
  {
    [MEMORY[0x277CBEAD8] sfu_errnoRaise:@"SFUTellError" format:@"Could not tell"];
  }
...
- (void)seekToOffset:(int64_t)offset whence:(int)whence
{
  if (lseek(self->mFd, offset, whence) == -1)
  {
    ...
    [v4 sfu_errnoRaise:@"SFUSeekError" format:@"Could not seek"];
  }
}
...
- (BOOL)moveToPath:(id)path
{
  ...
  if (v6)
  {
    ...
    self->mPath = v7;
    v8 = sub_27709E424(v7, 1, 0);
    self->mFd = v8;
    if (lseek(v8, 0, 2) == -1)
    {
      [MEMORY[0x277CBEAD8] sfu_errnoRaise:@"SFUSeekError" format:@"Could not seek"];
    }
  }
...
- (void)truncateToLength:(int64_t)length
{
  ...
  if (ftruncate(self->mFd, length))
  {
    ...
    [v5 sfu_errnoRaise:@"SFUTruncateError" format:@"Could not ftruncate."];
  }
}
...
- (void)flush
{
  if (fsync(self->mFd))
  {
    ...
    [v2 sfu_errnoRaise:@"SFUFlushError" format:@"Could not fsync."];
  }
}
