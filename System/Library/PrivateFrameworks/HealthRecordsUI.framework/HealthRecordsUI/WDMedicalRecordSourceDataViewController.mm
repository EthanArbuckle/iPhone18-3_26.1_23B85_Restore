...
- (WDMedicalRecordSourceDataViewController)initWithProfile:(id)profile medicalRecord:(id)record;
...
- (void)loadView
{
  ...
  [(UITextView *)self->_sourceView setTextColor:secondaryLabelColor];

  [(UITextView *)self->_sourceView setTextAlignment:0];
  [(UITextView *)self->_sourceView setEditable:0];
  view = [(WDMedicalRecordSourceDataViewController *)self view];
  [view addSubview:self->_sourceView];

  v8 = self->_sourceView;
  view2 = [(WDMedicalRecordSourceDataViewController *)self view];
  [(UITextView *)v8 hk_alignConstraintsWithView:view2];
  ...
  if ([MEMORY[0x1E696C608] isAppleInternalInstall])
  {
    ...
    shareItem = [(WDMedicalRecordSourceDataViewController *)self shareItem];
    [v10 addObject:shareItem];

    shareItem2 = [(WDMedicalRecordSourceDataViewController *)self shareItem];
    [shareItem2 setEnabled:0];
    ...
    navigationItem = [(WDMedicalRecordSourceDataViewController *)self navigationItem];
...
- (void)share:(id)share
{
  ...
  medicalRecord = [(WDMedicalRecordSourceDataViewController *)self medicalRecord];
  v10 = objc_opt_class();
  ...
  sourceView = [(WDMedicalRecordSourceDataViewController *)self sourceView];
  text = [sourceView text];
  v19 = [text dataUsingEncoding:4];

  v27 = 0;
  LOBYTE(text) = [v19 writeToURL:v16 options:0 error:&v27];
  v20 = v27;
  if (text)
  {
...
- (void)viewWillAppear:(BOOL)appear
{
  ...
  [(WDMedicalRecordSourceDataViewController *)&v4 viewWillAppear:appear];
...
- (void)_fetchAndDisplaySourceStringIfNecessary
{
  ...
  if (![(WDMedicalRecordSourceDataViewController *)self hasFetchedSourceString])
  {
    ...
    profile = [(WDMedicalRecordSourceDataViewController *)self profile];
    ...
    v6[4] = self;
    [healthRecordsStore fetchRawSourceStringForHealthRecord:medicalRecord completion:v6];
...
- (void)displaySourceString:(id)string
{
  ...
  traitCollection = [(WDMedicalRecordSourceDataViewController *)self traitCollection];
  v8 = [v5 preferredFontForTextStyle:v6 compatibleWithTraitCollection:traitCollection];
  sourceView = [(WDMedicalRecordSourceDataViewController *)self sourceView];
  [sourceView setFont:v8];

  sourceView2 = [(WDMedicalRecordSourceDataViewController *)self sourceView];
  [sourceView2 setText:stringCopy];
}
