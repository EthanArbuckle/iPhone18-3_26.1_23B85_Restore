@interface DMFInstallAppRequest
- (DMFInstallAppRequest)initWithCoder:(id)coder;
- (void)encodeWithCoder:(id)coder;
@end

@implementation DMFInstallAppRequest

- (DMFInstallAppRequest)initWithCoder:(id)coder
{
  coderCopy = coder;
  v9.receiver = self;
  v9.super_class = DMFInstallAppRequest;
  v5 = [(DMFAppRequest *)&v9 initWithCoder:coderCopy];
  if (v5)
  {
    v6 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"licenseType"];
    v5->_licenseType = [v6 integerValue];

    v7 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"allowFreePurchases"];
    v5->_allowFreePurchases = [v7 BOOLValue];
  }

  return v5;
}

- (void)encodeWithCoder:(id)coder
{
  v7.receiver = self;
  v7.super_class = DMFInstallAppRequest;
  coderCopy = coder;
  [(DMFAppRequest *)&v7 encodeWithCoder:coderCopy];
  v5 = [MEMORY[0x1E696AD98] numberWithUnsignedInteger:{-[DMFInstallAppRequest licenseType](self, "licenseType", v7.receiver, v7.super_class)}];
  [coderCopy encodeObject:v5 forKey:@"licenseType"];

  v6 = [MEMORY[0x1E696AD98] numberWithBool:{-[DMFInstallAppRequest allowFreePurchases](self, "allowFreePurchases")}];
  [coderCopy encodeObject:v6 forKey:@"allowFreePurchases"];
}

@end