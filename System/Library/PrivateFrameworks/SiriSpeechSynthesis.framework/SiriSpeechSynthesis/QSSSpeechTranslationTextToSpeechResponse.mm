...
- (QSSSpeechTranslationTextToSpeechResponse)initWithFlatbuffData:(id)data root:(const SpeechTranslationTextToSpeechResponse *)root verify:(BOOL)verify;
...
- (Offset<siri::speech::schema_fb::SpeechTranslationTextToSpeechResponse>)addObjectToBuffer:(void *)buffer
{
  conversation_id = [(QSSSpeechTranslationTextToSpeechResponse *)self conversation_id];
  v6 = conversation_id;
  if (!conversation_id)
  {
  ...
  request_id = [(QSSSpeechTranslationTextToSpeechResponse *)self request_id];
  v11 = request_id;
  if (!request_id)
  {
  ...
  target_locale = [(QSSSpeechTranslationTextToSpeechResponse *)self target_locale];
  v16 = target_locale;
  if (!target_locale)
  {
    target_locale = &stru_2879AE8E0;
  }
  ...
  text_to_speech_response = [(QSSSpeechTranslationTextToSpeechResponse *)self text_to_speech_response];
...
- (QSSSpeechTranslationTextToSpeechResponse)initWithFlatbuffData:(id)data root:(const SpeechTranslationTextToSpeechResponse *)root verify:(BOOL)verify
{
  ...
  dataCopy = data;
  v29.receiver = self;
  v29.super_class = QSSSpeechTranslationTextToSpeechResponse;
  v10 = [(QSSSpeechTranslationTextToSpeechResponse *)&v29 init];
  v11 = v10;
  if (v10)
  {
    ...
    if (verifyCopy)
    {
      ...
      if (v19)
      {
        if (!siri::speech::schema_fb::SpeechTranslationTextToSpeechResponse::Verify(v19, &v24))
        {
LABEL_16:
          v22 = 0;
          goto LABEL_17;
