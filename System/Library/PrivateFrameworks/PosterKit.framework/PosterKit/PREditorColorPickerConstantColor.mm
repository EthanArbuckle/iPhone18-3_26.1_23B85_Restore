...
- (PREditorColorPickerConstantColor)colorWithVariation:(double)variation;
- (PREditorColorPickerConstantColor)init;
- (PREditorColorPickerConstantColor)initWithBSXPCCoder:(id)coder;
- (PREditorColorPickerConstantColor)initWithBaseUIColor:(id)color initialVariation:(double)variation;
- (PREditorColorPickerConstantColor)initWithCoder:(id)coder;
- (PREditorColorPickerConstantColor)initWithColor:(id)color initialVariation:(double)variation;
...
- (PREditorColorPickerConstantColor)init
{
  ...
  v4 = [(PREditorColorPickerConstantColor *)self initWithBaseUIColor:whiteColor];
...
{
  colorCopy = color;
  v7 = [[PRPosterColor alloc] initWithColor:colorCopy preferredStyle:0];

  v8 = [(PREditorColorPickerConstantColor *)self initWithColor:v7 initialVariation:variation];
  return v8;
}
...
- (double)variationForLuminance:(double)luminance
{
  baseColor = [(PREditorColorPickerConstantColor *)self baseColor];
  hslValues = [baseColor hslValues];
  [hslValues luminance];
  v8 = v7;

  leftSliderColor = [(PREditorColorPickerConstantColor *)self leftSliderColor];
  ...
  rightSliderColor = [(PREditorColorPickerConstantColor *)self rightSliderColor];
...
- (BOOL)isEqual:(id)equal
{
  ...
  else
  {
    ...
    if (isKindOfClass)
    {
      v7 = equalCopy;
      baseColor = [(PREditorColorPickerConstantColor *)self baseColor];
      baseColor2 = [(PREditorColorPickerConstantColor *)v7 baseColor];
      v10 = BSEqualObjects();
      ...
      {
        localizedName = [(PREditorColorPickerConstantColor *)self localizedName];
        localizedName2 = [(PREditorColorPickerConstantColor *)v7 localizedName];
        v14 = BSEqualStrings();
      }
...
- (unint64_t)hash
{
  ...
  color = [(PREditorColorPickerConstantColor *)self color];
