...
+ (id)eventStreamWithFileURL:(id)l error:(id *)error;
+ (id)eventStreamWithStudyLogFileURL:(id)l error:(id *)error;
- (RCPEventStream)init;
...
- (void)enumerateEventsUsingBlock:(id)block;
- (void)filterEventsUsingBlock:(id)block;
- (void)setEvents:(id)events;
- (void)transformDigitizerEventLocationsWithTransform:(CGAffineTransform *)transform;
...
- (void)setEvents:(id)events
{
  ...
  [(NSMutableArray *)self->_rawEvents addObjectsFromArray:eventsCopy];
}

- (id)copyWithZone:(_NSZone *)zone
{
  v4 = objc_opt_class();
  dataRepresentation = [(RCPEventStream *)self dataRepresentation];
  v9 = 0;
  v6 = [v4 eventStreamWithData:dataRepresentation error:&v9];
  v7 = v9;
...
- (void)writeToURL:(id)l
{
  lCopy = l;
  dataRepresentation = [(RCPEventStream *)self dataRepresentation];
  [dataRepresentation writeToURL:lCopy options:1 error:0];
}
...
LABEL_12:
  ...
  environment = [(RCPEventStream *)self environment];
  [(RCPEventStream *)v5 setEnvironment:environment];
...
- (id)streamByAddingEventsFromStream:(id)stream withDistance:(double)distance
{
  ...
  v7 = objc_opt_new();
  environment = [(RCPEventStream *)self environment];
  [v7 setEnvironment:environment];
  events = [(RCPEventStream *)self events];
  [v7 setEvents:events];

  environment2 = [(RCPEventStream *)self environment];
  lastObject = [(NSMutableArray *)self->_rawEvents lastObject];
  [environment2 timeIntervalForMachAbsoluteTime:{objc_msgSend(lastObject, "deliveryTimestamp")}];
  v13 = v12;
  environment3 = [streamCopy environment];
  events2 = [streamCopy events];
  firstObject = [events2 firstObject];
  [environment3 timeIntervalForMachAbsoluteTime:{objc_msgSend(firstObject, "deliveryTimestamp")}];
  v18 = v13 - v17 + distance;
  ...
  v20 = v19;
  v21 = environment;
  [streamCopy enumerateEventsUsingBlock:v25];
...
+ (id)eventStreamWithData:(id)data error:(id *)error
{
  ...
  {
    v7 = [objc_alloc(MEMORY[0x277CCACA8]) initWithBytes:objc_msgSend(dataCopy length:"bytes") encoding:{v6, 4}];
...
+ (id)eventStreamWithFileURL:(id)l error:(id *)error
{
  ...
  if (v6)
  {
    v7 = [self eventStreamWithData:v6 error:error];
  }
...
+ (id)eventStreamWithStudyLogFileURL:(id)l error:(id *)error
{
  ...
  lCopy = l;
  v6 = [objc_alloc(MEMORY[0x277CBEAE0]) initWithURL:lCopy];
  [v6 open];
  v7 = [MEMORY[0x277CCAAA0] JSONObjectWithStream:v6 options:4 error:error];
  [v6 close];
  if (v7)
  {
    v38 = v6;
    ...
    if (v42)
    {
      ...
      do
      {
        ...
        do
        {
          ...
          environment = [(RCPEventStream *)v46 environment];
          v17 = MEMORY[0x277CBEAA8];
          [v13 doubleValue];
          v18 = [v17 dateWithTimeIntervalSince1970:?];
          [environment setStartDate:v18 machAbsoluteTime:{objc_msgSend(v15, "unsignedLongLongValue")}];
...
- (id)statistics
{
  ...
  v9 = [MEMORY[0x277CCACA8] stringWithFormat:@"%ld Events", objc_msgSend(rawEvents, "count")];
  [v3 addObject:v9];

  v10 = MEMORY[0x277CCACA8];
  lastObject = [rawEvents lastObject];
  deliveryTimestamp = [lastObject deliveryTimestamp];
  firstObject = [rawEvents firstObject];
  v14 = [v10 stringWithFormat:@"%0.4fs total duration (%0.4fx original)", RCPTimeIntervalFromMachTimestamp(deliveryTimestamp - objc_msgSend(firstObject, "deliveryTimestamp")) / v8, 1.0 / v8];
  [v3 addObject:v14];
...
- (id)timingStatistics
{
  ...
  v13 = v9;
  v15 = v7;
  [rawEvents enumerateObjectsUsingBlock:v12];
...
- (void)enumerateEventsUsingBlock:(id)block
{
  ...
  v8 = blockCopy;
  v6 = blockCopy;
  [rawEvents enumerateObjectsUsingBlock:v7];
}
  ...
  if (v5)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        hidEvent = [*(*(&v12 + 1) + 8 * v8) hidEvent];
        v10 = *&transform->c;
        v11[0] = *&transform->a;
        v11[1] = v10;
        v11[2] = *&transform->tx;
        RCPHIDEventTransformLocation(hidEvent, v11);
        ++v8;
      }
