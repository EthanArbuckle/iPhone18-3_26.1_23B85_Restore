...
- (PLClickPresentationTransition)initWithTransitionDelegate:(id)delegate presentingViewController:(id)controller presentedViewController:(id)viewController completion:(id)completion;
...
- (PLClickPresentationTransition)initWithTransitionDelegate:(id)delegate presentingViewController:(id)controller presentedViewController:(id)viewController completion:(id)completion
{
  ...
  if (v11)
  {
    objc_storeStrong(&v11->_transitionDelegate, delegate);
...
- (void)updateBackgroundViewForTransition
{
  ...
  [currentHandler handleFailureInMethod:a2 object:self file:@"PLClickPresentationInteractionManager.m" lineNumber:420 description:@"Abstract"];
}
...
- (void)prepareTransitionFromView:(id)view toView:(id)toView containerView:(id)containerView
{
  ...
  [(PLClickPresentationTransition *)self _configureTransitionContextWithFromView:viewCopy toView:toViewCopy containerView:containerViewCopy];
  _animator = [(PLClickPresentationTransition *)self _animator];
  [_animator prepareForTransitionWithContext:self->_transitionContext];
}
...
- (UIViewPropertyAnimator)customAnimator
{
  ...
  {
    _animator2 = [(PLClickPresentationTransition *)self _animator];
    propertyAnimator = [_animator2 propertyAnimator];
  }
...
{
  currentHandler = [MEMORY[0x277CCA890] currentHandler];
  [currentHandler handleFailureInMethod:a2 object:self file:@"PLClickPresentationInteractionManager.m" lineNumber:454 description:@"Abstract"];
...
- (id)_animator
{
  ...
  if (!animator)
  {
    _newAnimator = [(PLClickPresentationTransition *)self _newAnimator];
    v5 = self->_animator;
    self->_animator = _newAnimator;
...
- (void)_configureTransitionContextWithFromView:(id)view toView:(id)toView containerView:(id)containerView
{
  ...
  viewCopy = view;
  _animator = [(PLClickPresentationTransition *)self _animator];
  [(_UIViewControllerTransitionContext *)self->_transitionContext _setAnimator:_animator];
  [(_UIViewControllerTransitionContext *)self->_transitionContext _setIsAnimated:1];
  [(_UIViewControllerOneToOneTransitionContext *)self->_transitionContext _setFromView:viewCopy];

  [(_UIViewControllerOneToOneTransitionContext *)self->_transitionContext _setToView:toViewCopy];
  [(_UIViewControllerTransitionContext *)self->_transitionContext _setContainerView:containerViewCopy];
