...
- (void)addBulletinIdentifier:(id)identifier
{
  ...
  [(NSMutableArray *)bulletinIdentifiers addObject:identifierCopy];
}
...
- (id)description
{
  ...
  dictionaryRepresentation = [(BLTPBSectionBulletinList *)self dictionaryRepresentation];
...
- (void)copyTo:(id)to
{
  ...
  {
    [toCopy clearBulletinIdentifiers];
    bulletinIdentifiersCount = [(BLTPBSectionBulletinList *)self bulletinIdentifiersCount];
    if (bulletinIdentifiersCount)
    {
      v5 = bulletinIdentifiersCount;
      for (i = 0; i != v5; ++i)
      {
        v7 = [(BLTPBSectionBulletinList *)self bulletinIdentifierAtIndex:i];
        [toCopy addBulletinIdentifier:v7];
      }
    }
  }
}
...
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((sectionID = self->_sectionID, !(sectionID | equalCopy[2])) || -[NSString isEqual:](sectionID, "isEqual:")))
  {
    bulletinIdentifiers = self->_bulletinIdentifiers;
    if (bulletinIdentifiers | equalCopy[1])
    {
