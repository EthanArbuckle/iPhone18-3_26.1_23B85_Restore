...
uint64_t pg_query_parse@<X0>(uint64_t a1@<X8>)
{
  pg_query_enter_memory_context();
  pg_query_raw_parse();
  *(a1 + 8) = v5;
  nodes_to_json = pg_query_nodes_to_json(v4);
  *a1 = strdup(nodes_to_json);
  pfree(nodes_to_json);

  return pg_query_exit_memory_context();
}

uint64_t pg_query_parse_protobuf@<X0>(uint64_t a1@<X8>)
{
  pg_query_enter_memory_context();
  pg_query_raw_parse();
  *(a1 + 16) = v5;
  *a1 = pg_query_nodes_to_protobuf(v4);
  *(a1 + 8) = v2;
