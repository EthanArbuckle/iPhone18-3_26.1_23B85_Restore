...
- (NSData)jsonData;
- (SISchemaBluetoothCarInvocationContext)initWithDictionary:(id)dictionary;
- (SISchemaBluetoothCarInvocationContext)initWithJSON:(id)n;
...
- (SISchemaBluetoothCarInvocationContext)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"route"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(SISchemaBluetoothCarInvocationContext *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    route = [(SISchemaBluetoothCarInvocationContext *)self route];
    ...
    [dictionary setObject:v6 forKeyedSubscript:@"route"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (route = self->_route, route == [equalCopy route]))
      {
        v6 = 1;
      }
    }
