...
- (NSData)jsonData;
- (READSchemaREADPlaybackSessionFailed)initWithDictionary:(id)dictionary;
- (READSchemaREADPlaybackSessionFailed)initWithJSON:(id)n;
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(READSchemaREADPlaybackSessionFailed *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    ...
    [dictionary setObject:v4 forKeyedSubscript:@"errorCodes"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (errorCodes = self->_errorCodes, errorCodes == [equalCopy errorCodes]))
      {
        v6 = 1;
      }
    }
