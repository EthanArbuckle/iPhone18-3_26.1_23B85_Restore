...
- (NSData)jsonData;
- (PRSiriSchemaPRJointResolverCallStarted)initWithDictionary:(id)dictionary;
- (PRSiriSchemaPRJointResolverCallStarted)initWithJSON:(id)n;
...
- (PRSiriSchemaPRJointResolverCallStarted)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"callType"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(PRSiriSchemaPRJointResolverCallStarted *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    callType = [(PRSiriSchemaPRJointResolverCallStarted *)self callType];
    ...
    [dictionary setObject:v6 forKeyedSubscript:@"callType"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (callType = self->_callType, callType == [equalCopy callType]))
      {
        v6 = 1;
      }
    }
