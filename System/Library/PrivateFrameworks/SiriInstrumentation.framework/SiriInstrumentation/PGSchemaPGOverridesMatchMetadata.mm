...
- (NSData)jsonData;
- (PGSchemaPGOverridesMatchMetadata)initWithDictionary:(id)dictionary;
- (PGSchemaPGOverridesMatchMetadata)initWithJSON:(id)n;
...
- (PGSchemaPGOverridesMatchMetadata)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"isMatched"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(PGSchemaPGOverridesMatchMetadata *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*(&self->_isMatched + 1))
  {
    ...
    [dictionary setObject:v4 forKeyedSubscript:@"isMatched"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    if (*(&self->_isMatched + 1) == (equalCopy[9] & 1))
    {
      if (!*(&self->_isMatched + 1) || (isMatched = self->_isMatched, isMatched == [equalCopy isMatched]))
      {
        v6 = 1;
      }
    }
