...
- (NSData)jsonData;
- (PLUSSchemaPLUSInferencePluginStartedOrChanged)initWithDictionary:(id)dictionary;
- (PLUSSchemaPLUSInferencePluginStartedOrChanged)initWithJSON:(id)n;
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(PLUSSchemaPLUSInferencePluginStartedOrChanged *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    triggerReason = [(PLUSSchemaPLUSInferencePluginStartedOrChanged *)self triggerReason];
    ...
    [dictionary setObject:v6 forKeyedSubscript:@"triggerReason"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (triggerReason = self->_triggerReason, triggerReason == [equalCopy triggerReason]))
      {
        v6 = 1;
      }
    }
