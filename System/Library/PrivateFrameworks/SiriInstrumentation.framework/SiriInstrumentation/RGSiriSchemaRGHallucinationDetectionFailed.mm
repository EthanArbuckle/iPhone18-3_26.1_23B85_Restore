...
- (NSData)jsonData;
- (RGSiriSchemaRGHallucinationDetectionFailed)initWithDictionary:(id)dictionary;
- (RGSiriSchemaRGHallucinationDetectionFailed)initWithJSON:(id)n;
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(RGSiriSchemaRGHallucinationDetectionFailed *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (self->_error)
  {
    error = [(RGSiriSchemaRGHallucinationDetectionFailed *)self error];
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    if ((*&self->_has & 1) == (equalCopy[24] & 1))
    {
      if ((*&self->_has & 1) == 0 || (fatalError = self->_fatalError, fatalError == [equalCopy fatalError]))
      {
        ...
        if ((error != 0) != (error2 == 0))
        {
          error3 = [(RGSiriSchemaRGHallucinationDetectionFailed *)self error];
          if (!error3)
          {
...
LABEL_13:
          ...
          v10 = error3;
          error4 = [(RGSiriSchemaRGHallucinationDetectionFailed *)self error];
...
- (void)writeTo:(id)to
{
  ...
  if (error)
  {
    error2 = [(RGSiriSchemaRGHallucinationDetectionFailed *)self error];
