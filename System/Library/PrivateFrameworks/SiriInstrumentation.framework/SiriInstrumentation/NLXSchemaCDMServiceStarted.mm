...
- (NLXSchemaCDMServiceStarted)initWithDictionary:(id)dictionary;
- (NLXSchemaCDMServiceStarted)initWithJSON:(id)n;
- (NSData)jsonData;
...
- (NLXSchemaCDMServiceStarted)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"serviceName"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(NLXSchemaCDMServiceStarted *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    ...
    [dictionary setObject:v5 forKeyedSubscript:@"serviceName"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (serviceName = self->_serviceName, serviceName == [equalCopy serviceName]))
      {
        v6 = 1;
      }
    }
