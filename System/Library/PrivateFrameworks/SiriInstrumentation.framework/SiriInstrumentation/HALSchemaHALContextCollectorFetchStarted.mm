...
- (HALSchemaHALContextCollectorFetchStarted)initWithDictionary:(id)dictionary;
- (HALSchemaHALContextCollectorFetchStarted)initWithJSON:(id)n;
- (NSData)jsonData;
...
- (HALSchemaHALContextCollectorFetchStarted)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"isContextCollectorLocal"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(HALSchemaHALContextCollectorFetchStarted *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*(&self->_isContextCollectorLocal + 1))
  {
    ...
    [dictionary setObject:v4 forKeyedSubscript:@"isContextCollectorLocal"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    if (*(&self->_isContextCollectorLocal + 1) == (equalCopy[9] & 1))
    {
      if (!*(&self->_isContextCollectorLocal + 1) || (isContextCollectorLocal = self->_isContextCollectorLocal, isContextCollectorLocal == [equalCopy isContextCollectorLocal]))
      {
        v6 = 1;
      }
    }
