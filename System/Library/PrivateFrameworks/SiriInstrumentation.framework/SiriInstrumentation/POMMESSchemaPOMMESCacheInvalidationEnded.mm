...
- (NSData)jsonData;
- (POMMESSchemaPOMMESCacheInvalidationEnded)initWithDictionary:(id)dictionary;
- (POMMESSchemaPOMMESCacheInvalidationEnded)initWithJSON:(id)n;
...
- (POMMESSchemaPOMMESCacheInvalidationEnded)initWithDictionary:(id)dictionary
{
  ...
  if (v5)
  {
    v6 = [dictionaryCopy objectForKeyedSubscript:@"numberOfCacheKeysInvalidated"];
    objc_opt_class();
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(POMMESSchemaPOMMESCacheInvalidationEnded *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    ...
    [dictionary setObject:v4 forKeyedSubscript:@"numberOfCacheKeysInvalidated"];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([equalCopy isMemberOfClass:objc_opt_class()])
  {
    ...
    {
      if ((*&self->_has & 1) == 0 || (numberOfCacheKeysInvalidated = self->_numberOfCacheKeysInvalidated, numberOfCacheKeysInvalidated == [equalCopy numberOfCacheKeysInvalidated]))
      {
        v6 = 1;
      }
    }
