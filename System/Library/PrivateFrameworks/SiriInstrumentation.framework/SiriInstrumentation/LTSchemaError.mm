...
- (LTSchemaError)initWithDictionary:(id)dictionary;
- (LTSchemaError)initWithJSON:(id)n;
- (NSData)jsonData;
...
- (NSData)jsonData
{
  dictionaryRepresentation = [(LTSchemaError *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    v4 = [MEMORY[0x1E696AD98] numberWithUnsignedInt:{-[LTSchemaError code](self, "code")}];
    [dictionary setObject:v4 forKeyedSubscript:@"code"];
  }

  if (self->_domain)
  {
    domain = [(LTSchemaError *)self domain];
    v6 = [domain copy];
...
LABEL_12:
  ...
  domain3 = [(LTSchemaError *)self domain];
  if (domain3)
  {
    v9 = domain3;
    domain4 = [(LTSchemaError *)self domain];
