@interface DYGTCAMetalLayerInfo
- (CGRect)frame;
- (DYGTCAMetalLayerInfo)initWithCoder:(id)coder;
- (void)encodeWithCoder:(id)coder;
@end

@implementation DYGTCAMetalLayerInfo

- (DYGTCAMetalLayerInfo)initWithCoder:(id)coder
{
  coderCopy = coder;
  v13.receiver = self;
  v13.super_class = DYGTCAMetalLayerInfo;
  v5 = [(DYGTCAMetalLayerInfo *)&v13 init];
  if (v5)
  {
    v5->_deviceAddress = [coderCopy decodeInt64ForKey:@"deviceAddress"];
    v5->_deviceStreamRef = [coderCopy decodeInt64ForKey:@"deviceStreamRef"];
    v5->_layerAddress = [coderCopy decodeInt64ForKey:@"layerAddress"];
    v5->_layerStreamRef = [coderCopy decodeInt64ForKey:@"layerStreamRef"];
    [coderCopy decodeRectForKey:@"frame"];
    v5->_frame.origin.x = v6;
    v5->_frame.origin.y = v7;
    v5->_frame.size.width = v8;
    v5->_frame.size.height = v9;
    v10 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"name"];
    name = v5->_name;
    v5->_name = v10;
  }

  return v5;
}

- (void)encodeWithCoder:(id)coder
{
  deviceAddress = self->_deviceAddress;
  coderCopy = coder;
  [coderCopy encodeInt64:deviceAddress forKey:@"deviceAddress"];
  [coderCopy encodeInt64:self->_deviceStreamRef forKey:@"deviceStreamRef"];
  [coderCopy encodeInt64:self->_layerAddress forKey:@"layerAddress"];
  [coderCopy encodeInt64:self->_layerStreamRef forKey:@"layerStreamRef"];
  [coderCopy encodeRect:@"frame" forKey:{self->_frame.origin.x, self->_frame.origin.y, self->_frame.size.width, self->_frame.size.height}];
  [coderCopy encodeObject:self->_name forKey:@"name"];
}

- (CGRect)frame
{
  x = self->_frame.origin.x;
  y = self->_frame.origin.y;
  width = self->_frame.size.width;
  height = self->_frame.size.height;
  result.size.height = height;
  result.size.width = width;
  result.origin.y = y;
  result.origin.x = x;
  return result;
}

@end