...
+ (id)sharedSingleton
{
  ...
  block[1] = 3221225472;
  block[2] = __47__MRAudioCodecComponentHandler_sharedSingleton__block_invoke;
  block[3] = &__block_descriptor_40_e5_v8__0l;
  block[4] = self;
  if (sharedSingleton_once_7 != -1)
  {
    dispatch_once(&sharedSingleton_once_7, block);
  }

  v2 = sharedSingleton_sharedInstance_7;

  return v2;
}

uint64_t __47__MRAudioCodecComponentHandler_sharedSingleton__block_invoke(uint64_t a1)
{
  sharedSingleton_sharedInstance_7 = [objc_alloc(*(a1 + 32)) _init];
