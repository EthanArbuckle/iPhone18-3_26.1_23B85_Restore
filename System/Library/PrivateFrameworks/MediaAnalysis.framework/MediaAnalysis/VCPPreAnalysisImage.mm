...
- (VCPPreAnalysisImage)initWithPixelBuffer:(__CVBuffer *)buffer;
- (int)pixelBuffer:(__CVBuffer *)buffer width:(unint64_t)width height:(unint64_t)height;
- (int)preWarmWidth:(unint64_t)width andHeight:(unint64_t)height;
@end
...
- (VCPPreAnalysisImage)initWithPixelBuffer:(__CVBuffer *)buffer
{
  ...
  if (v4)
  {
    ...
    v4->_scaledPixelBuffers = dictionary;

    v7 = MEMORY[0x1E696B098];
    Width = CVPixelBufferGetWidth(buffer);
    v9 = [v7 valueWithSize:{Width, CVPixelBufferGetHeight(buffer)}];
...
- (int)preWarmWidth:(unint64_t)width andHeight:(unint64_t)height
{
  ...
  v7 = [MEMORY[0x1E696B098] valueWithSize:{width, height}];
  v8 = [(NSMutableDictionary *)selfCopy->_scaledPixelBuffers objectForKeyedSubscript:v7];
  if (!v8)
  {
    v14 = 0;
    v10 = +[VCPSceneProcessingImageManager imageManager];
    v9 = [v10 scalePixelBuffer:selfCopy->_sourcePixelBuffer.value_ toPixelBuffer:&v14 width:width height:height];
    ...
    [(NSMutableDictionary *)selfCopy->_scaledPixelBuffers setObject:v13 forKeyedSubscript:v7];
...
- (int)pixelBuffer:(__CVBuffer *)buffer width:(unint64_t)width height:(unint64_t)height
{
  ...
  selfCopy = self;
  objc_sync_enter(selfCopy);
  v9 = [MEMORY[0x1E696B098] valueWithSize:{width, height}];
  v10 = [(NSMutableDictionary *)selfCopy->_scaledPixelBuffers objectForKeyedSubscript:v9];
  if (!v10)
  {
    ...
    v11 = [v12 scalePixelBuffer:selfCopy->_sourcePixelBuffer.value_ toPixelBuffer:&v16 width:width height:height];
    ...
    scaledPixelBuffers = selfCopy->_scaledPixelBuffers;
  ...
  *buffer = CFRetain([(VCPPreAnalysisImageEntry *)v10 pixelBuffer]);
