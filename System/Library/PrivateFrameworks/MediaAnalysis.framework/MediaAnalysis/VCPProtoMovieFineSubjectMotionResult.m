...
- (id)description
{
  ...
  v8.receiver = self;
  v8.super_class = VCPProtoMovieFineSubjectMotionResult;
...
{
  v4 = a3;
  v6 = [v4 isMemberOfClass:objc_opt_class()] && ((timeRange = self->_timeRange, !(timeRange | *(v4 + 2))) || -[VCPProtoTimeRange isEqual:](timeRange, "isEqual:")) && self->_actionScore == *(v4 + 2);
...
- (void)mergeFrom:(id)a3
{
  ...
  else
  {
    ...
    v7 = v4;
    [(VCPProtoMovieFineSubjectMotionResult *)self setTimeRange:?];
  }

  v4 = v7;
LABEL_7:
  self->_actionScore = *(v4 + 2);
}
    ...
    if ((v10.duration.flags & 1) != 0 && !v10.duration.epoch)
    {
      ...
      if ((v10.duration.value & 0x8000000000000000) == 0)
      {
        if (v4)
        {
          ...
          [(VCPProtoMovieFineSubjectMotionResult *)v6 setTimeRange:v8];

          [v5 floatValue];
          [(VCPProtoMovieFineSubjectMotionResult *)v6 setActionScore:?];
        }
      }
    }
  }
...
- (id)exportToLegacyDictionary
{
  v3 = [(VCPProtoMovieFineSubjectMotionResult *)self timeRange];
  v4 = v3;
  if (v3)
  {
    [v3 timeRangeValue];
