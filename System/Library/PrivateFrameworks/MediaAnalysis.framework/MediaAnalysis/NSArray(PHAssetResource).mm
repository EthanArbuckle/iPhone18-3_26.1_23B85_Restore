...
- (uint64_t)vcp_isOriginalLocal
{
  vcp_originalResource = [self vcp_originalResource];
  vcp_isLocallyAvailable = [vcp_originalResource vcp_isLocallyAvailable];

  return vcp_isLocallyAvailable;
}
...
- (uint64_t)vcp_hasLocalAdjustments
{
  ...
  v10 = 0u;
  selfCopy = self;
  vcp_isLocallyAvailable = [selfCopy countByEnumeratingWithState:&v7 objects:v11 count:16];
  if (vcp_isLocallyAvailable)
  {
    v3 = *v8;
    while (2)
    {
      for (i = 0; i != vcp_isLocallyAvailable; ++i)
      {
        if (*v8 != v3)
        {
          objc_enumerationMutation(selfCopy);
        ...
        if ([v5 type] == 7)
        {
          vcp_isLocallyAvailable = [v5 vcp_isLocallyAvailable];
          goto LABEL_11;
        }
      }
...
- (uint64_t)vcp_hasLocalSlowmo:()PHAssetResource
{
  if (a3 & 1) == 0 && ([self vcp_isOriginalLocal])
  {
    return 1;
  }

  result = [self vcp_hasLocalMovie:a3];
  if (result)
  {

    return [self vcp_hasLocalAdjustments];
  }
...
{
  v13 = *MEMORY[0x1E69E9840];
  v1 = [self vcp_localMovieResourcesSorted:?];
  v10 = 0u;
  v11 = 0u;
  v8 = 0u;
  v9 = 0u;
  reverseObjectEnumerator = [v1 reverseObjectEnumerator];
  v3 = [reverseObjectEnumerator countByEnumeratingWithState:&v8 objects:v12 count:16];
  if (v3)
  {
    v4 = *v9;
LABEL_3:
    ...
    while (1)
    {
      ...
      vcp_avAsset = [*(*(&v8 + 1) + 8 * v5) vcp_avAsset];
      if (vcp_avAsset)
      {
        break;
      }
...
{
  v2 = 1.0;
  if ([self vcp_hasLocalAdjustments])
  {
    vcp_adjustmentsResource = [self vcp_adjustmentsResource];
    privateFileURL = [vcp_adjustmentsResource privateFileURL];

    v5 = [objc_alloc(MEMORY[0x1E69C0908]) initWithURL:privateFileURL];
    v6 = v5;
...
- (id)vcp_highResImageResourcesForAsset:()PHAssetResource
{
  ...
  else
  {
    ...
    v5 = [photoLibrary vcp_isSyndicationLibrary] ^ 1;
  }
  ...
  vcp_ascendingSizeComparator = [MEMORY[0x1E69786D0] vcp_ascendingSizeComparator];
  [array sortUsingComparator:vcp_ascendingSizeComparator];
