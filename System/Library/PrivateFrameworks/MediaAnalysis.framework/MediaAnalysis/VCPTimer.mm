...
- (VCPTimer)initWithIntervalNanoseconds:(unint64_t)nanoseconds isOneShot:(BOOL)shot andBlock:(id)block;
- (void)dealloc;
- (void)destroy;
...
+ (id)timerWithInterval:(unint64_t)interval unit:(unint64_t)unit oneShot:(BOOL)shot andBlock:(id)block
{
  ...
  if (unit <= 3)
  {
    v10 = [objc_alloc(objc_opt_class()) initWithIntervalNanoseconds:+[VCPTimer timerWithInterval:unit:oneShot:andBlock:]::kTimeScaler[unit] * interval isOneShot:shotCopy andBlock:blockCopy];
  }
