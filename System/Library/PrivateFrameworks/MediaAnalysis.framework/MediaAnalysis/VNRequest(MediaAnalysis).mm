...
- (id)vcp_idealDimension
{
  ...
  {
    supportedImageSizeSet = [self supportedImageSizeSet];
    if ([supportedImageSizeSet count])
    {
      v3 = MEMORY[0x1E696B098];
      v4 = [supportedImageSizeSet objectAtIndexedSubscript:0];
      pixelsWideRange = [v4 pixelsWideRange];
      idealDimension = [pixelsWideRange idealDimension];
      v7 = [supportedImageSizeSet objectAtIndexedSubscript:0];
      pixelsHighRange = [v7 pixelsHighRange];
      v9 = [v3 valueWithSize:{idealDimension, objc_msgSend(pixelsHighRange, "idealDimension")}];
    }
    ...
    else
    {
      ...
      {
        [(VNRequest(MediaAnalysis) *)self vcp_idealDimension];
      }
