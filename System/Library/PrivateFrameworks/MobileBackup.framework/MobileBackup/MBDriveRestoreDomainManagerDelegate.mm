...
+ (id)domainForName:(id)name containerID:(id)d engine:(id)engine
{
  ...
  if (!persona)
  {
    __assert_rtn("+[MBDriveRestoreDomainManagerDelegate domainForName:containerID:engine:]", "MBDriveRestoreDomainManagerDelegate.m", 24, "persona");
  }

  if (!dCopy)
  {
    __assert_rtn("+[MBDriveRestoreDomainManagerDelegate domainForName:containerID:engine:]", "MBDriveRestoreDomainManagerDelegate.m", 25, "containerID");
  }
...
LABEL_11:
    ...
    v19 = [NSSet setWithObject:&stru_1003C3430];
