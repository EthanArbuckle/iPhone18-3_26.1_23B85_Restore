...
+ (id)_readDomainsFromPlist:(id)plist accountType:(int64_t)type volumeMountPoint:(id)point error:(id *)error
{
  ...
  if (type <= 1)
  {
    if (type != 1)
    {
      if (!type)
      {
        __assert_rtn("+[MBDomainManager _readDomainsFromPlist:accountType:volumeMountPoint:error:]", "MBDomainManager.m", 128, "accountType != MBAccountTypeUnspecified");
      }
...
- (id)_initWithAccountType:(int64_t)type volumeMountPoint:(id)point plistPath:(id)path error:(id *)error
{
  ...
  if (!error)
  {
    __assert_rtn("[MBDomainManager _initWithAccountType:volumeMountPoint:plistPath:error:]", "MBDomainManager.m", 159, "error");
  }
...
- (id)domainForName:(id)name
{
  ...
  if (!nameCopy)
  {
    ...
    [v15 handleFailureInMethod:a2 object:self file:@"MBDomainManager.m" lineNumber:229 description:@"Null name"];
  }
  ...
  else
  {
    ...
    v13 = v11;
    [(NSMutableDictionary *)self->_domainsByName setObject:v11 forKeyedSubscript:nameCopy, v16];
    v7 = v13;
  }
...
- (id)redirectDomain:(id)domain forRelativePath:(id)path
{
  ...
  v10 = [relativePathDomainRedirects countByEnumeratingWithState:&v23 objects:v33 count:16];
  if (v10)
  {
    v11 = v10;
    v12 = *v24;
    while (2)
    {
      for (i = 0; i != v11; i = i + 1)
      {
        ...
        if ([pathCopy hasPrefix:v14])
        {
          ...
          if (!v15)
          {
            ...
            [v21 handleFailureInMethod:a2 object:self file:@"MBDomainManager.m" lineNumber:276 description:{@"Domain not found for redirect: %@", v17}];
          }
      ...
      v11 = [relativePathDomainRedirects countByEnumeratingWithState:&v23 objects:v33 count:16];
      if (v11)
      {
        continue;
      }
...
- (void)addDomain:(id)domain
{
  ...
  if (v6)
  {
    ...
    [v7 handleFailureInMethod:a2 object:self file:@"MBDomainManager.m" lineNumber:285 description:{@"Domain already exists: %@", name}];
  }
