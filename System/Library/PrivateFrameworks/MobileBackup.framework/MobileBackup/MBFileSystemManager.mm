...
+ (unint64_t)fileSystemCapacity
{
  unsignedLongLongValue = atomic_load(&qword_100421A28);
  if (!unsignedLongLongValue)
  {
    ...
    if (v4)
    {
      ...
      atomic_store(unsignedLongLongValue, &qword_100421A28);
    }
...
+ (id)createAndMountSnapshotForVolume:(id)volume name:(id)name atFirstAvailableMountPoint:(id)point error:(id *)error cancelationHandler:(id)handler
{
  ...
  if (![pointCopy count])
  {
    __assert_rtn("+[MBFileSystemManager createAndMountSnapshotForVolume:name:atFirstAvailableMountPoint:error:cancelationHandler:]", "MBFileSystemManager.m", 162, "mountPoints.count > 0");
  }
...
+ (BOOL)deleteAllSnapshotsForVolume:(id)volume withPrefix:(id)prefix latestCreationDate:(id)date error:(id *)error
{
  ...
  if (!dateCopy)
  {
    __assert_rtn("+[MBFileSystemManager deleteAllSnapshotsForVolume:withPrefix:latestCreationDate:error:]", "MBFileSystemManager.m", 431, "latestCreationDate");
  }
...
+ (void)removeAbandonedDriveBackupDirectoriesWithLatestCreationDate:(id)date persona:(id)persona
{
  ...
  v51 = v11;
  v52 = v7;
  [v9 enumeratorAtURL:v7 includingPropertiesForKeys:v11 options:1 errorHandler:&stru_1003C15B8];
  v74 = 0u;
  v75 = 0u;
  v76 = 0u;
  v17 = v77 = 0u;
...
+ (id)listSnapshotsForVolume:(id)volume error:(id *)error
{
  ...
  if (v43 != -1)
  {
    ...
    while (1)
    {
      *&v47.bitmapcount = xmmword_1002BA1A8;
      *&v47.fileattr = 0;
...
LABEL_45:
      ...
      do
      {
        ...
        if ((v12 & 1) != 0 && (v21 = &buf[v11], v11 += 8, (v22 = *(v21 + 1)) != 0))
        {
          ...
          if (v23[v24])
          {
            __assert_rtn("+[MBFileSystemManager listSnapshotsForVolume:error:]", "MBFileSystemManager.m", 555, "namePtr[nameLength - 1] == '\\0'");
          }
...
LABEL_27:
        if (v11 >= 1025)
        {
          __assert_rtn("+[MBFileSystemManager listSnapshotsForVolume:error:]", "MBFileSystemManager.m", 576, "p <= attrBuf + sizeof(attrBuf)");
        }
...
+ (BOOL)startFilesystemKeyRollingWithPath:(id)path error:(id *)error
{
  ...
  if (!pathCopy)
  {
    __assert_rtn("+[MBFileSystemManager startFilesystemKeyRollingWithPath:error:]", "MBFileSystemManager.m", 627, "path");
  }

  v6 = pathCopy;
  v12 = xmmword_1002BA1E0;
...
+ (BOOL)stopFilesystemKeyRollingWithPath:(id)path error:(id *)error
{
  ...
  if (!pathCopy)
  {
    __assert_rtn("+[MBFileSystemManager stopFilesystemKeyRollingWithPath:error:]", "MBFileSystemManager.m", 660, "path");
  }

  v6 = pathCopy;
  v12 = xmmword_1002BA1F0;
  v7 = fsctl([pathCopy fileSystemRepresentation], 0xC0104A3DuLL, &v12, 0);
  if (v7 == -1)
  {
    v9 = *__error();
...
+ (id)volumeUUIDWithVolumeMountPoint:(id)point error:(id *)error
{
  ...
  if (!pointCopy)
  {
    __assert_rtn("+[MBFileSystemManager volumeUUIDWithVolumeMountPoint:error:]", "MBFileSystemManager.m", 721, "volumeMountPoint");
  }

  if (!error)
  {
    __assert_rtn("+[MBFileSystemManager volumeUUIDWithVolumeMountPoint:error:]", "MBFileSystemManager.m", 722, "error");
  }
...
LABEL_9:
  ...
  if (!v12)
  {
    __assert_rtn("+[MBFileSystemManager volumeUUIDWithVolumeMountPoint:error:]", "MBFileSystemManager.m", 751, "volumeUUID");
  }
