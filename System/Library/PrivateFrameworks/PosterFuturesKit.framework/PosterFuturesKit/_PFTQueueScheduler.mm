...
- (_PFTQueueScheduler)initWithQueue:(id)queue;
...
{
  blockCopy = block;
  v7 = [(_PFTQueueScheduler *)self _queueForQualityOfService:service];
  v8 = blockCopy;
  v13 = v8;
  if (service <= 2)
  {
...
- (id)_queueForQualityOfService:(unint64_t)service
{
  queue = [(_PFTQueueScheduler *)self queue];

  if (queue)
  {
    queue2 = [(_PFTQueueScheduler *)self queue];
  }
...
- (id)performCancellableBlock:(id)block qualityOfService:(unint64_t)service
{
  ...
  v8 = [(_PFTQueueScheduler *)self _queueForQualityOfService:service];
...
{
  blockCopy = block;
  v9 = [(_PFTQueueScheduler *)self _queueForQualityOfService:service];
