...
- (PFTCoalescingSchedulerDecorator)initWithScheduler:(id)a3 coalescingWindow:(double)a4;
- (double)timestamp;
...
- (unint64_t)getAndIncrementState
{
  v2 = self;
  objc_sync_enter(v2);
  if ([(PFTCoalescingSchedulerDecorator *)v2 state])
  {
    v3 = 1;
  }
  ...
  {
    [(PFTCoalescingSchedulerDecorator *)v2 setState:1];
    v3 = 0;
  }
...
- (void)flushDelayedBlock
{
  v2 = self;
  objc_sync_enter(v2);
  v3 = [(PFTCoalescingSchedulerDecorator *)v2 delayedBlocks];
  v4 = [v3 count];

  if (v4)
  {
    v5 = [(PFTCoalescingSchedulerDecorator *)v2 delayedBlocks];
    v6 = [v5 pop];

    v7 = [(PFTCoalescingSchedulerDecorator *)v2 delayedBlocks];
    v8 = [v7 popAll];

    v9 = [(PFTCoalescingSchedulerDecorator *)v2 scheduler];
    [(PFTCoalescingSchedulerDecorator *)v2 coalescingWindow];
  ...
  {
    [(PFTCoalescingSchedulerDecorator *)v2 setState:0];
    v6 = 0;
  }
...
- (void)removeDelayedBlock:(id)a3
{
  ...
  v4 = self;
  objc_sync_enter(v4);
  v5 = [(PFTCoalescingSchedulerDecorator *)v4 delayedBlocks];
  v6 = MEMORY[0x25F8CF560](v7);
  [v5 removeObject:v6];
...
- (id)performCancellableBlock:(id)a3 qualityOfService:(unint64_t)a4
{
  ...
  v8 = [(PFTCoalescingSchedulerDecorator *)self getAndIncrementState];
  if (v8 == 1)
  {
    v10 = self;
    objc_sync_enter(v10);
    ...
    v24[1] = a4;
    v13 = [v21 copy];
    v14 = [(PFTCoalescingSchedulerDecorator *)v10 delayedBlocks];
  ...
  else
  {
    ...
    v9 = [(PFTCoalescingSchedulerDecorator *)self scheduler];
    v10 = [v9 performCancellableBlock:v6 qualityOfService:a4];

    v11 = [(PFTCoalescingSchedulerDecorator *)self scheduler];
    ...
    [(PFTCancellationToken *)v7 addCancellable:v10];
...
{
  v6 = a4;
  v7 = [(PFTCoalescingSchedulerDecorator *)self scheduler];
  v11[0] = MEMORY[0x277D85DD0];
  v11[1] = 3221225472;
...
- (double)timestamp
{
  v2 = [(PFTCoalescingSchedulerDecorator *)self scheduler];
  [v2 timestamp];
  v4 = v3;
