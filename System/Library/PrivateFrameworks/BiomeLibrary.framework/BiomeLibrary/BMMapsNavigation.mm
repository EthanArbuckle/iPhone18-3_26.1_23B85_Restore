@interface BMMapsNavigation
+ (id)columns;
+ (id)eventWithData:(id)data dataVersion:(unsigned int)version;
+ (id)protoFields;
- (BMMapsNavigation)initWithJSONDictionary:(id)dictionary error:(id *)error;
- (BMMapsNavigation)initWithStarting:(id)starting;
...
@implementation BMMapsNavigation
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    v5 = equalCopy;
    if (-[BMMapsNavigation hasStarting](self, "hasStarting") || [v5 hasStarting])
    {
      if (-[BMMapsNavigation hasStarting](self, "hasStarting") && [v5 hasStarting])
      {
        starting = [(BMMapsNavigation *)self starting];
...
- (id)jsonDictionary
{
  ...
  if ([(BMMapsNavigation *)self hasStarting])
  {
    v3 = [MEMORY[0x1E696AD98] numberWithBool:{-[BMMapsNavigation starting](self, "starting")}];
    v10 = @"starting";
    if (v3)
    {
      v4 = v3;
...
- (BMMapsNavigation)initWithJSONDictionary:(id)dictionary error:(id *)error
{
  ...
  if (!v6 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0))
  {
    error = 0;
LABEL_4:
    self = [(BMMapsNavigation *)self initWithStarting:error];
    selfCopy = self;
    goto LABEL_5;
  }
...
{
  v3 = objc_opt_new();
  [(BMMapsNavigation *)self writeTo:v3];
...
- (id)initByReadFrom:(id)from
{
  fromCopy = from;
  v25.receiver = self;
  v25.super_class = BMMapsNavigation;
...
- (NSString)description
{
  ...
  v4 = [MEMORY[0x1E696AD98] numberWithBool:{-[BMMapsNavigation starting](self, "starting")}];
  v5 = [v3 initWithFormat:@"BMMapsNavigation with starting: %@", v4];
...
- (BMMapsNavigation)initWithStarting:(id)starting
{
  ...
  v7.receiver = self;
  v7.super_class = BMMapsNavigation;
...
+ (id)eventWithData:(id)data dataVersion:(unsigned int)version
{
  ...
  else
  {
    ...
    v8 = [[BMMapsNavigation alloc] initByReadFrom:v7];
    v4 = v8;
    if (v8)
    {
      v8[5] = 0;
