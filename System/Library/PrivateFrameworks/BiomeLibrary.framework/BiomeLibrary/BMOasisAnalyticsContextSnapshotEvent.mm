...
+ (id)protoFields;
- (BMOasisAnalyticsContextSnapshotEvent)initWithJSONDictionary:(id)dictionary error:(id *)error;
- (BMOasisAnalyticsContextSnapshotEvent)initWithQualityTrace:(id)trace metadata:(id)metadata payload:(id)payload;
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    v5 = equalCopy;
    qualityTrace = [(BMOasisAnalyticsContextSnapshotEvent *)self qualityTrace];
    ...
    {
      qualityTrace3 = [(BMOasisAnalyticsContextSnapshotEvent *)self qualityTrace];
    ...
    metadata = [(BMOasisAnalyticsContextSnapshotEvent *)self metadata];
    ...
    {
      metadata3 = [(BMOasisAnalyticsContextSnapshotEvent *)self metadata];
...
LABEL_15:
    ...
    payload = [(BMOasisAnalyticsContextSnapshotEvent *)self payload];
    payload2 = [v5 payload];
    if (payload == payload2)
    {
      v12 = 1;
    ...
    {
      payload3 = [(BMOasisAnalyticsContextSnapshotEvent *)self payload];
      payload4 = [v5 payload];
...
LABEL_34:
      ...
      if (objc_opt_isKindOfClass())
      {
        v16 = v11;
        v27 = 0;
        v12 = [[BMOasisAnalyticsContextSnapshotType alloc] initWithJSONDictionary:v16 error:&v27];
        v17 = v27;
        if (!v17)
        {
...
- (void)writeTo:(id)to
{
  ...
  {
    PBDataWriterPlaceMark();
    [(BMOasisAnalyticsContextSnapshotType *)self->_payload writeTo:toCopy];
    PBDataWriterRecallMark();
  }
}
...
- (id)initByReadFrom:(id)from
{
  fromCopy = from;
  v22.receiver = self;
  v22.super_class = BMOasisAnalyticsContextSnapshotEvent;
...
LABEL_16:
      ...
      else if (v15 == 2)
      {
        ...
        v16 = [[BMOasisAnalyticsContextSnapshotType alloc] initByReadFrom:fromCopy];
        if (!v16)
        {
          goto LABEL_34;
        }
...
- (NSString)description
{
  ...
  metadata = [(BMOasisAnalyticsContextSnapshotEvent *)self metadata];
  payload = [(BMOasisAnalyticsContextSnapshotEvent *)self payload];
  v7 = [v3 initWithFormat:@"BMOasisAnalyticsContextSnapshotEvent with qualityTrace: %@, metadata: %@, payload: %@", qualityTrace, metadata, payload];
...
- (BMOasisAnalyticsContextSnapshotEvent)initWithQualityTrace:(id)trace metadata:(id)metadata payload:(id)payload
{
  ...
  payloadCopy = payload;
  v14.receiver = self;
  v14.super_class = BMOasisAnalyticsContextSnapshotEvent;
...
{
  v8[3] = *MEMORY[0x1E69E9840];
  v2 = [objc_alloc(MEMORY[0x1E698F2D8]) initWithName:@"qualityTrace_json" dataType:5 requestOnly:1 extractBlock:&__block_literal_global_339];
  v3 = [objc_alloc(MEMORY[0x1E698F2D8]) initWithName:@"metadata_json" dataType:5 requestOnly:1 extractBlock:&__block_literal_global_341];
  v4 = [objc_alloc(MEMORY[0x1E698F2D8]) initWithName:@"payload_json" dataType:5 requestOnly:1 extractBlock:&__block_literal_global_343_96159];
  v8[0] = v3;
  v8[1] = v4;
  v8[2] = v2;
  v5 = [MEMORY[0x1E695DEC8] arrayWithObjects:v8 count:3];
