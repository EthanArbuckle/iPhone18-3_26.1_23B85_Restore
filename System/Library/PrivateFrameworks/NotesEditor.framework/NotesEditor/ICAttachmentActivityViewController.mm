...
+ (id)makeWithAttachment:(id)attachment quickLookItemDelegate:(id)delegate completion:(id)completion;
- (BOOL)inhibitPDFGenerationForActivityType:(id)type;
- (BOOL)writeGalleryPDFDataToPasteboardIfNecessaryForActivity:(id)activity attachments:(id)attachments;
...
+ (id)makeWithAttachment:(id)attachment quickLookItemDelegate:(id)delegate completion:(id)completion
{
  ...
  activityItems = [attachmentCopy activityItems];
  if (activityItems)
  {
    v11 = activityItems;
    media = [attachmentCopy media];
    v13 = media;
    if (media && [media hasFile] && (objc_msgSend(v13, "isPasswordProtected") & 1) == 0 && (objc_msgSend(MEMORY[0x277D75418], "ic_isVision") & 1) == 0)
    {
      v28 = objc_alloc_init(NotesQuickLookActivityItem);
      [(NotesQuickLookActivityItem *)v28 setDelegate:delegateCopy];
      v35[0] = attachmentCopy;
    ...
    if (ICDebugModeEnabled())
    {
      ...
      v19 = [objc_alloc(MEMORY[0x277D36758]) initWithObject:attachmentCopy activityType:1];
    ...
    v21 = [MEMORY[0x277CBEA60] ic_arrayFromNonNilObject:attachmentCopy];
    [(ICAttachmentActivityViewController *)v20 setAttachments:v21];

    note = [attachmentCopy note];
    account = [note account];
    -[ICAttachmentActivityViewController setIsContentManaged:](v20, "setIsContentManaged:", [account isManaged]);
...
- (void)_performActivity:(id)activity
{
  ...
  v6 = [(ICAttachmentActivityViewController *)self inhibitPDFGenerationForActivityType:activityType];

  if (v6)
  {
    v11.receiver = self;
    v11.super_class = ICAttachmentActivityViewController;
    [(ICAttachmentActivityViewController *)&v11 _performActivity:activityCopy];
  }

  else
  {
    v7 = MEMORY[0x277D36788];
    attachments = [(ICAttachmentActivityViewController *)self attachments];
    view = [(ICAttachmentActivityViewController *)self view];
    window = [view window];
    ...
    v12[4] = self;
    v13 = activityCopy;
    [v7 generatePDFsIfNecessaryForGalleryAttachments:attachments displayWindow:window presentingViewController:self completionHandler:v12];
  }
}
...
{
  typeCopy = type;
  if (-[ICAttachmentActivityViewController ignoreAttachmentsForCopyToPasteboard](self, "ignoreAttachmentsForCopyToPasteboard") && ([typeCopy isEqualToString:*MEMORY[0x277D54720]] & 1) != 0)
  {
    v5 = 1;
  }
  ...
  {
    activityTypesThatInhibitPDFGeneration = [(ICAttachmentActivityViewController *)self activityTypesThatInhibitPDFGeneration];
      ...
      do
      {
        for (i = 0; i != v9; ++i)
        {
          ...
          if ([v13 attachmentType] == 11)
          {
            ...
            pasteboardData = [v13 pasteboardData];
            persistenceData = [pasteboardData persistenceData];

            v21 = objc_alloc_init(MEMORY[0x277CBEB38]);
            [v21 ic_setNonNilObject:persistenceData forKey:v27];
            identifier = [v26 identifier];
            [v21 ic_setNonNilObject:v18 forKey:identifier];
          ...
          generalPasteboard = [*(v11 + 2064) generalPasteboard];
          [generalPasteboard setItems:v29];
