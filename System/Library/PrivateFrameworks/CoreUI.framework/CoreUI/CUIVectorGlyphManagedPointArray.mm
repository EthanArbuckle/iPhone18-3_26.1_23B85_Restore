...
+ (id)createManagedPointArrayWrapping:(id)wrapping;
+ (id)createWithPointCount:(unint64_t)count;
...
+ (id)createWithPointCount:(unint64_t)count
{
  ...
  v4->_rawArray.pointComponents = malloc_type_calloc(2 * count, 8uLL, 0x100004000313F17uLL);
  v4->_rawArray.numPoints = count;
  return v4;
}
