  ...
  if (!notificationClient)
  {
    ...
    if (os_log_type_enabled(v16, OS_LOG_TYPE_DEBUG))
    {
      ...
      *(&buf + 4) = hashedDescription;
      _os_log_debug_impl(&_mh_execute_header, v16, OS_LOG_TYPE_DEBUG, "Update request received auth challenge for:%{public}@ but found no notification client to pass on to. Attempting internal auth", &buf, 0xCu);
