@interface TLKRichTextField
- (CGRect)containerView:(id)a3 layoutFrameForArrangedSubview:(id)a4 withProposedFrame:(CGRect)a5;
- (NSString)description;
- (TLKObserver)observer;
- (TLKRichTextField)init;
- (UIFont)font;
- (id)attributedString;
- (unint64_t)prominence;
- (void)propertiesDidChange;
- (void)setFont:(id)a3;
- (void)setProminence:(unint64_t)a3;
- (void)setRichText:(id)a3;
- (void)setRoundedCornerLabelSizeConfiguration:(unint64_t)a3;
- (void)setScaleIconsToFont:(BOOL)a3;
- (void)setUseCompactMode:(BOOL)a3;
- (void)updateIconSizing;
- (void)updateIcons:(id)a3;
- (void)updateRoundedCornerLabels:(id)a3;
- (void)updateStarRating:(id)a3;
- (void)updateWithRichText:(id)a3;
@end

@implementation TLKRichTextField

- (TLKRichTextField)init
{
  v9.receiver = self;
  v9.super_class = TLKRichTextField;
  v2 = [(TLKRichTextField *)&v9 init];
  v3 = v2;
  if (v2)
  {
    [(TLKStackView *)v2 setAxis:0];
    [(TLKStackView *)v3 setAlignment:5];
    v4 = objc_opt_new();
    [(TLKRichTextField *)v3 setTextLabel:v4];

    v5 = [(TLKRichTextField *)v3 textLabel];
    [v5 setSupportsColorGlyphs:1];

    v6 = +[TLKFontUtilities shortFootnoteFont];
    [(TLKRichTextField *)v3 setFont:v6];

    v7 = [(TLKRichTextField *)v3 textLabel];
    [(TLKStackView *)v3 addArrangedSubview:v7];

    [(TLKRichTextField *)v3 setDelegate:v3];
    [(TLKRichTextField *)v3 setObserver:v3];
  }

  return v3;
}

- (void)updateIconSizing
{
  if (!-[TLKRichTextField scaleIconsToFont](self, "scaleIconsToFont") || ((-[TLKRichTextField richText](self, "richText"), v3 = objc_claimAutoreleasedReturnValue(), [v3 icons], v4 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v4, "firstObject"), v5 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v5, "image"), v6 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v6, "size"), v8 = v7, v10 = v9, v11 = *MEMORY[0x1E695F060], v12 = *(MEMORY[0x1E695F060] + 8), v6, v5, v4, v3, v8 == v11) ? (v13 = v10 == v12) : (v13 = 0), v13))
  {
    v20 = [(TLKRichTextField *)self iconView];
    [v20 setMinimumLayoutSize:{0.0, 0.0}];

    v18 = [(TLKRichTextField *)self iconView];
    v21 = v18;
    v19 = 1.79769313e308;
  }

  else
  {
    v14 = [(TLKRichTextField *)self font];
    [v14 capHeight];
    v16 = v15;

    v17 = [(TLKRichTextField *)self iconView];
    [v17 setMinimumLayoutSize:{0.0, v16}];

    v18 = [(TLKRichTextField *)self iconView];
    v21 = v18;
    v19 = v16;
  }

  [v18 setMaximumLayoutSize:{1.79769313e308, v19}];
}

- (TLKObserver)observer
{
  WeakRetained = objc_loadWeakRetained(&self->observer);

  return WeakRetained;
}

- (void)propertiesDidChange
{
  v3 = [(TLKRichTextField *)self richText];
  [(TLKRichTextField *)self updateWithRichText:v3];
}

- (void)setRichText:(id)a3
{
  v34 = *MEMORY[0x1E69E9840];
  v5 = a3;
  if (objc_opt_respondsToSelector())
  {
    [(TLKObject *)self->_richText setObserver:0];
  }

  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v23 = v5;
      v30 = 0u;
      v31 = 0u;
      v28 = 0u;
      v29 = 0u;
      v6 = self->_richText;
      v7 = [(TLKRichText *)v6 countByEnumeratingWithState:&v28 objects:v33 count:16];
      if (v7)
      {
        v8 = v7;
        v9 = *v29;
        do
        {
          for (i = 0; i != v8; ++i)
          {
            if (*v29 != v9)
            {
              objc_enumerationMutation(v6);
            }

            v11 = *(*(&v28 + 1) + 8 * i);
            if (objc_opt_respondsToSelector())
            {
              [v11 setObserver:0];
            }
          }

          v8 = [(TLKRichText *)v6 countByEnumeratingWithState:&v28 objects:v33 count:16];
        }

        while (v8);
      }

      v5 = v23;
    }
  }

  objc_storeStrong(&self->_richText, a3);
  if (objc_opt_respondsToSelector())
  {
    [(TLKObject *)self->_richText setObserver:self];
  }

  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v26 = 0u;
      v27 = 0u;
      v24 = 0u;
      v25 = 0u;
      v12 = self->_richText;
      v13 = [(TLKRichText *)v12 countByEnumeratingWithState:&v24 objects:v32 count:16];
      if (v13)
      {
        v14 = v13;
        v15 = *v25;
        do
        {
          for (j = 0; j != v14; ++j)
          {
            if (*v25 != v15)
            {
              objc_enumerationMutation(v12);
            }

            v17 = *(*(&v24 + 1) + 8 * j);
            if (objc_opt_respondsToSelector())
            {
              [v17 setObserver:self];
            }
          }

          v14 = [(TLKRichText *)v12 countByEnumeratingWithState:&v24 objects:v32 count:16];
        }

        while (v14);
      }
    }
  }

  v18 = [(TLKRichTextField *)self observer];
  if (v18)
  {
    v19 = v18;
    v20 = [(TLKRichTextField *)self observer];
    v21 = [v20 batchUpdateCount];

    if (!v21)
    {
      v22 = [(TLKRichTextField *)self observer];
      [v22 propertiesDidChange];
    }
  }
}

- (void)updateWithRichText:(id)a3
{
  v4 = a3;
  v6[0] = MEMORY[0x1E69E9820];
  v6[1] = 3221225472;
  v6[2] = __39__TLKRichTextField_updateWithRichText___block_invoke;
  v6[3] = &unk_1E7FD8DA8;
  v6[4] = self;
  v7 = v4;
  v5 = v4;
  [(TLKRichTextField *)self performBatchUpdates:v6];
}

void __39__TLKRichTextField_updateWithRichText___block_invoke(uint64_t a1)
{
  v2 = *(a1 + 40);
  v3 = [*(a1 + 32) textLabel];
  [v3 setRichText:v2];

  LODWORD(v2) = [*(a1 + 40) hasContent];
  v4 = [*(a1 + 32) textLabel];
  [v4 setHidden:v2 ^ 1];

  v5 = *(a1 + 32);
  v6 = [*(a1 + 40) stars];
  [v5 updateStarRating:v6];

  v7 = *(a1 + 32);
  v8 = [*(a1 + 40) icons];
  [v7 updateIcons:v8];

  v9 = *(a1 + 32);
  v10 = [*(a1 + 40) inlineRoundedText];
  [v9 updateRoundedCornerLabels:v10];
}

- (UIFont)font
{
  v2 = [(TLKRichTextField *)self textLabel];
  v3 = [v2 font];

  return v3;
}

- (void)setFont:(id)a3
{
  v4 = a3;
  v5 = [(TLKRichTextField *)self textLabel];
  [v5 setFont:v4];

  v6 = [(TLKRichTextField *)self starRatingView];
  [v6 setFont:v4];

  v7 = [(TLKRichTextField *)self iconView];
  [v7 setSymbolFont:v4];

  [(TLKRichTextField *)self updateIconSizing];
}

- (void)setScaleIconsToFont:(BOOL)a3
{
  if (self->_scaleIconsToFont != a3)
  {
    self->_scaleIconsToFont = a3;
    [(TLKRichTextField *)self updateIconSizing];
  }
}

- (void)setRoundedCornerLabelSizeConfiguration:(unint64_t)a3
{
  self->_roundedCornerLabelSizeConfiguration = a3;
  v4 = [(TLKRichTextField *)self roundedCornerLabels];
  [v4 setSizeConfiguration:a3];
}

- (unint64_t)prominence
{
  v2 = [(TLKRichTextField *)self textLabel];
  v3 = [v2 prominence];

  return v3;
}

- (void)setProminence:(unint64_t)a3
{
  v5 = [(TLKRichTextField *)self textLabel];
  [v5 setProminence:a3];

  v6 = [(TLKRichTextField *)self iconView];
  [v6 setProminence:a3];

  v7 = [(TLKRichTextField *)self roundedCornerLabels];
  [v7 setProminence:a3];
}

- (void)setUseCompactMode:(BOOL)a3
{
  v3 = a3;
  self->_useCompactMode = a3;
  v4 = [(TLKRichTextField *)self starRatingView];
  [v4 setUseCompactMode:v3];
}

- (void)updateStarRating:(id)a3
{
  v18 = a3;
  v4 = [(TLKRichTextField *)self starRatingView];
  if (v18)
  {

    if (!v4)
    {
      v5 = objc_opt_new();
      [(TLKRichTextField *)self setStarRatingView:v5];

      v6 = [(TLKRichTextField *)self font];
      v7 = [(TLKRichTextField *)self starRatingView];
      [v7 setFont:v6];

      v8 = [(TLKRichTextField *)self useCompactMode];
      v9 = [(TLKRichTextField *)self starRatingView];
      [v9 setUseCompactMode:v8];

      v10 = [(TLKRichTextField *)self starRatingView];
      [(TLKRichTextField *)self insertArrangedSubview:v10 atIndex:0];

      v11 = [(TLKRichTextField *)self starRatingView];
      [(TLKRichTextField *)self setMinimumSpacing:v11 adjacentToView:0.0, 0.0, 0.0, 5.5];
    }

    v12 = [v18 firstObject];
    [v12 starRating];
    v14 = v13;
    v15 = [(TLKRichTextField *)self starRatingView];
    [v15 setStarRating:v14];

    v16 = [(TLKRichTextField *)self starRatingView];
    v4 = v16;
    v17 = 0;
  }

  else
  {
    v16 = v4;
    v17 = 1;
  }

  [v16 setHidden:v17];
}

- (void)updateIcons:(id)a3
{
  v17 = a3;
  v4 = [(TLKRichTextField *)self iconView];
  if (v17)
  {

    if (!v4)
    {
      v5 = objc_opt_new();
      [(TLKRichTextField *)self setIconView:v5];

      v6 = [(TLKRichTextField *)self prominence];
      v7 = [(TLKRichTextField *)self iconView];
      [v7 setProminence:v6];

      v8 = [(TLKRichTextField *)self font];
      v9 = [(TLKRichTextField *)self iconView];
      [v9 setSymbolFont:v8];

      v10 = [(TLKRichTextField *)self iconView];
      [TLKLayoutUtilities requireIntrinsicSizeForView:v10];

      [(TLKRichTextField *)self updateIconSizing];
      v11 = [(TLKRichTextField *)self iconView];
      v12 = [(TLKRichTextField *)self arrangedSubviews];
      -[TLKRichTextField insertArrangedSubview:atIndex:](self, "insertArrangedSubview:atIndex:", v11, [v12 count] - 1);

      v13 = [(TLKRichTextField *)self iconView];
      [(TLKRichTextField *)self setMinimumSpacing:v13 adjacentToView:0.0, 0.0, 0.0, 4.0];
    }

    v14 = [(TLKRichTextField *)self iconView];
    [v14 updateIcons:v17];

    v15 = [(TLKRichTextField *)self iconView];
    v4 = v15;
    v16 = 0;
  }

  else
  {
    v15 = v4;
    v16 = 1;
  }

  [v15 setHidden:v16];
}

- (void)updateRoundedCornerLabels:(id)a3
{
  v17 = a3;
  v4 = [(TLKRichTextField *)self roundedCornerLabels];
  if (v17)
  {

    if (!v4)
    {
      v5 = objc_opt_new();
      [(TLKRichTextField *)self setRoundedCornerLabels:v5];

      v6 = [(TLKRichTextField *)self prominence];
      v7 = [(TLKRichTextField *)self roundedCornerLabels];
      [v7 setProminence:v6];

      v8 = [(TLKRichTextField *)self roundedCornerLabelSizeConfiguration];
      v9 = [(TLKRichTextField *)self roundedCornerLabels];
      [v9 setSizeConfiguration:v8];

      v10 = [(TLKRichTextField *)self roundedCornerLabels];
      [TLKLayoutUtilities requireIntrinsicSizeForView:v10];

      v11 = [(TLKRichTextField *)self roundedCornerLabels];
      v12 = [(TLKRichTextField *)self starRatingView];
      [(TLKRichTextField *)self insertArrangedSubview:v11 atIndex:v12 != 0];

      v13 = [(TLKRichTextField *)self roundedCornerLabels];
      [(TLKRichTextField *)self setMinimumSpacing:v13 adjacentToView:0.0, 0.0, 0.0, 10.0];
    }

    v14 = [(TLKRichTextField *)self roundedCornerLabels];
    [v14 updateRoundedText:v17];

    v15 = [(TLKRichTextField *)self roundedCornerLabels];
    v4 = v15;
    v16 = 0;
  }

  else
  {
    v15 = v4;
    v16 = 1;
  }

  [v15 setHidden:v16];
}

- (CGRect)containerView:(id)a3 layoutFrameForArrangedSubview:(id)a4 withProposedFrame:(CGRect)a5
{
  height = a5.size.height;
  width = a5.size.width;
  y = a5.origin.y;
  x = a5.origin.x;
  v10 = a4;
  v11 = [(TLKRichTextField *)self starRatingView];

  if (v11 != v10)
  {
    v12 = [(TLKRichTextField *)self roundedCornerLabels];

    if (v12 == v10)
    {
      x = x + 1.0;
    }

    else
    {
      v13 = [(TLKRichTextField *)self iconView];

      if (v13 == v10)
      {
        v14 = [(TLKRichTextField *)self iconView];
        v15 = [v14 imageViews];
        v16 = [v15 firstObject];
        [v16 intrinsicContentSize];
        v18 = v17;

        if (v18 < 20.0)
        {
          [TLKLayoutUtilities deviceScaledRoundedValue:self forView:0.5];
          y = y - v19;
        }
      }
    }
  }

  v20 = x;
  v21 = y;
  v22 = width;
  v23 = height;
  result.size.height = v23;
  result.size.width = v22;
  result.origin.y = v21;
  result.origin.x = v20;
  return result;
}

- (id)attributedString
{
  v2 = [(TLKRichTextField *)self textLabel];
  v3 = [v2 attributedText];

  return v3;
}

- (NSString)description
{
  v3 = MEMORY[0x1E696AD60];
  v9.receiver = self;
  v9.super_class = TLKRichTextField;
  v4 = [(TLKRichTextField *)&v9 description];
  v5 = [v3 stringWithString:v4];

  v6 = [(TLKRichTextField *)self textLabel];
  v7 = [v6 description];

  if (v7)
  {
    [v5 appendString:v7];
  }

  return v5;
}

@end