...
- (FTTextToSpeechResource)initWithFlatbuffData:(id)data root:(const TextToSpeechResource *)root verify:(BOOL)verify;
- (NSString)identifier;
- (NSString)language;
- (NSString)version;
...
- (FTTextToSpeechResource)initWithFlatbuffData:(id)data root:(const TextToSpeechResource *)root verify:(BOOL)verify
{
  ...
  dataCopy = data;
  v25.receiver = self;
  v25.super_class = FTTextToSpeechResource;
  v10 = [(FTTextToSpeechResource *)&v25 init];
  if (!v10)
  {
    goto LABEL_13;
  }
...
- (Offset<siri::speech::schema_fb::TextToSpeechResource>)addObjectToBuffer:(void *)buffer
{
  language = [(FTTextToSpeechResource *)self language];
  v6 = language;
  if (!language)
  {
  ...
  version = [(FTTextToSpeechResource *)self version];
  v11 = version;
  if (!version)
  {
  ...
  identifier = [(FTTextToSpeechResource *)self identifier];
  v16 = identifier;
  if (!identifier)
  {
  ...
  uTF8String3 = [(__CFString *)identifier UTF8String];
...
- (id)flatbuffData
{
  ...
  v10 = 256;
  v11 = 0;
  apple::aiml::flatbuffers2::FlatBufferBuilder::Finish(&v3, [(FTTextToSpeechResource *)self addObjectToBuffer:&v3], 0, 0);
  operator new();
}
