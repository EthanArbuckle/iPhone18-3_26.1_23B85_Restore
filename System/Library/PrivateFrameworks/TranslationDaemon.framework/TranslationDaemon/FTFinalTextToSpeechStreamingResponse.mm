...
- (FTFinalTextToSpeechStreamingResponse)initWithFlatbuffData:(id)data root:(const FinalTextToSpeechStreamingResponse *)root verify:(BOOL)verify;
...
- (Offset<siri::speech::schema_fb::FinalTextToSpeechStreamingResponse>)addObjectToBuffer:(void *)buffer
{
  speech_id = [(FTFinalTextToSpeechStreamingResponse *)self speech_id];
  v6 = speech_id;
  if (!speech_id)
  {
  ...
  session_id = [(FTFinalTextToSpeechStreamingResponse *)self session_id];
  v11 = session_id;
  if (!session_id)
  {
  ...
  error_code = [(FTFinalTextToSpeechStreamingResponse *)self error_code];
  error_str = [(FTFinalTextToSpeechStreamingResponse *)self error_str];
  v17 = error_str;
  if (!error_str)
  {
  ...
  stream_id = [(FTFinalTextToSpeechStreamingResponse *)self stream_id];
  v22 = stream_id;
  if (!stream_id)
  {
  ...
  total_pkt_number = [(FTFinalTextToSpeechStreamingResponse *)self total_pkt_number];
  dev_data = [(FTFinalTextToSpeechStreamingResponse *)self dev_data];
  ...
  *(buffer + 70) = 1;
  ...
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddOffset<apple::aiml::flatbuffers2::String>(buffer, 12, uTF8String4);
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddElement<unsigned int>(buffer, 14, total_pkt_number, 0);
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddOffset<apple::aiml::flatbuffers2::String>(buffer, 16, v27);

  return apple::aiml::flatbuffers2::FlatBufferBuilder::EndTable(buffer, v29 + v28);
}
