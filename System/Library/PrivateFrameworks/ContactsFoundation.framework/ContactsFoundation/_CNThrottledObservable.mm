...
- (_CNThrottledObservable)initWithInterval:(double)interval options:(unint64_t)options observable:(id)observable schedulerProvider:(id)provider;
...
- (_CNThrottledObservable)initWithInterval:(double)interval options:(unint64_t)options observable:(id)observable schedulerProvider:(id)provider
{
  ...
  if (v13)
  {
    ...
    v13->_options = optionsCopy;
    objc_storeStrong(&v13->_observable, observable);
    v16 = [providerCopy newSerialSchedulerWithName:@"com.apple.contacts.reactive.throttle.downstream"];
...
- (void)dealloc
{
  delayToken = [(_CNThrottledObservable *)self delayToken];
  [delayToken cancel];
...
- (id)subscribe:(id)subscribe
{
  ...
  resourceLock = [(_CNThrottledObservable *)self resourceLock];
  ...
  observable = [(_CNThrottledObservable *)self observable];
  v14 = [observable subscribe:v12];
...
- (void)observerScheduler_sendResultToObserver:(id)observer
{
  ...
  v6[4] = self;
  v6[5] = &v7;
  CNRunWithLock(resourceLock, v6);

  if (v8[5])
  {
    [observerCopy observerDidReceiveResult:?];
  }
