...
- (ATXBiomeLightweightClientModelCacheUpdateStream)initWithStoreConfig:(id)config;
...
- (ATXBiomeLightweightClientModelCacheUpdateStream)initWithStoreConfig:(id)config
{
  ...
  if (v5)
  {
    ...
    v7 = atx_storeConfig;
    v8 = [objc_alloc(MEMORY[0x1E698F318]) initWithPrivateStreamIdentifier:@"lightweightClientModelCacheUpdates" storeConfig:atx_storeConfig eventDataClass:objc_opt_class()];
    inner = v5->_inner;
    v5->_inner = v8;
  }
...
{
  eventCopy = event;
  source = [(ATXBiomeLightweightClientModelCacheUpdateStream *)self source];
