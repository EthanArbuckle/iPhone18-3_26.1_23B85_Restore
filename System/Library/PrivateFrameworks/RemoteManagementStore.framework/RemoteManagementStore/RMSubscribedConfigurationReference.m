@interface RMSubscribedConfigurationReference
- (BOOL)isEqual:(id)a3;
- (BOOL)isEqualToReference:(id)a3;
- (RMSubscribedConfigurationReference)initWithCoder:(id)a3;
- (RMSubscribedConfigurationReference)initWithDeclaration:(id)a3 subscriberStore:(id)a4 assets:(id)a5;
- (id)_resolveDynamicDeclaration:(id)a3;
- (id)assetWithIdentifier:(id)a3;
- (void)encodeWithCoder:(id)a3;
@end

@implementation RMSubscribedConfigurationReference

- (RMSubscribedConfigurationReference)initWithDeclaration:(id)a3 subscriberStore:(id)a4 assets:(id)a5
{
  v8 = a3;
  v9 = a4;
  v10 = a5;
  v16.receiver = self;
  v16.super_class = RMSubscribedConfigurationReference;
  v11 = [(RMSubscribedConfigurationReference *)&v16 init];
  v12 = v11;
  if (v11)
  {
    v13 = [(RMSubscribedConfigurationReference *)v11 _resolveDynamicDeclaration:v8];
    declaration = v12->_declaration;
    v12->_declaration = v13;

    objc_storeStrong(&v12->_store, a4);
    objc_storeStrong(&v12->_assets, a5);
  }

  return v12;
}

- (id)_resolveDynamicDeclaration:(id)a3
{
  v3 = a3;
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v4 = [v3 serializeWithType:0];
    v12 = 0;
    v5 = [MEMORY[0x277D46018] load:v4 serializationType:0 error:&v12];
    v6 = v12;
    v7 = [MEMORY[0x277D45F58] subscribedConfigurationReference];
    v8 = v7;
    if (v5)
    {
      v9 = v5;
      if (os_log_type_enabled(v7, OS_LOG_TYPE_DEBUG))
      {
        [(RMSubscribedConfigurationReference *)v5 _resolveDynamicDeclaration:v8];
        v9 = v5;
      }
    }

    else
    {
      v9 = v3;
      if (os_log_type_enabled(v7, OS_LOG_TYPE_ERROR))
      {
        [(RMSubscribedConfigurationReference *)v6 _resolveDynamicDeclaration:v8];
        v9 = v3;
      }
    }

    v10 = v9;
  }

  else
  {
    v10 = v3;
  }

  return v10;
}

- (id)assetWithIdentifier:(id)a3
{
  v19 = *MEMORY[0x277D85DE8];
  v4 = a3;
  v14 = 0u;
  v15 = 0u;
  v16 = 0u;
  v17 = 0u;
  v5 = [(RMSubscribedConfigurationReference *)self assets];
  v6 = [v5 countByEnumeratingWithState:&v14 objects:v18 count:16];
  if (v6)
  {
    v7 = *v15;
    while (2)
    {
      for (i = 0; i != v6; i = i + 1)
      {
        if (*v15 != v7)
        {
          objc_enumerationMutation(v5);
        }

        v9 = *(*(&v14 + 1) + 8 * i);
        v10 = [v9 declarationIdentifier];
        v11 = [v10 isEqualToString:v4];

        if (v11)
        {
          v6 = v9;
          goto LABEL_11;
        }
      }

      v6 = [v5 countByEnumeratingWithState:&v14 objects:v18 count:16];
      if (v6)
      {
        continue;
      }

      break;
    }
  }

LABEL_11:

  v12 = *MEMORY[0x277D85DE8];

  return v6;
}

- (RMSubscribedConfigurationReference)initWithCoder:(id)a3
{
  v4 = a3;
  v5 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"declaration"];
  v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"store"];
  v7 = MEMORY[0x277CBEB98];
  v8 = objc_opt_class();
  v9 = [v7 setWithObjects:{v8, objc_opt_class(), 0}];
  v10 = [v4 decodeObjectOfClasses:v9 forKey:@"assets"];

  v11 = [(RMSubscribedConfigurationReference *)self initWithDeclaration:v5 subscriberStore:v6 assets:v10];
  return v11;
}

- (void)encodeWithCoder:(id)a3
{
  v4 = a3;
  v5 = [(RMSubscribedConfigurationReference *)self declaration];
  [v4 encodeObject:v5 forKey:@"declaration"];

  v6 = [(RMSubscribedConfigurationReference *)self store];
  [v4 encodeObject:v6 forKey:@"store"];

  v7 = [(RMSubscribedConfigurationReference *)self assets];
  [v4 encodeObject:v7 forKey:@"assets"];
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if (self == v4)
  {
    v5 = 1;
  }

  else
  {
    objc_opt_class();
    v5 = (objc_opt_isKindOfClass() & 1) != 0 && [(RMSubscribedConfigurationReference *)self isEqualToReference:v4];
  }

  return v5;
}

- (BOOL)isEqualToReference:(id)a3
{
  v4 = a3;
  v5 = [(RMSubscribedConfigurationReference *)self declaration];
  v6 = [v5 declarationType];
  v7 = [v4 declaration];
  v8 = [v7 declarationType];
  if ([v6 isEqualToString:v8])
  {
    v9 = [(RMSubscribedConfigurationReference *)self declaration];
    v10 = [v9 declarationIdentifier];
    v11 = [v4 declaration];
    v12 = [v11 declarationIdentifier];
    if ([v10 isEqualToString:v12])
    {
      v24 = v9;
      v26 = [(RMSubscribedConfigurationReference *)self declaration];
      v13 = [v26 declarationServerToken];
      v23 = [v4 declaration];
      [v23 declarationServerToken];
      v22 = v25 = v13;
      if ([v13 isEqualToString:?])
      {
        v14 = [(RMSubscribedConfigurationReference *)self store];
        v20 = [v4 store];
        v21 = v14;
        if ([v14 isEqual:?])
        {
          v15 = [(RMSubscribedConfigurationReference *)self assets];
          v16 = [v15 count];
          v19 = [v4 assets];
          v17 = v16 == [v19 count];
        }

        else
        {
          v17 = 0;
        }

        v9 = v24;
      }

      else
      {
        v17 = 0;
        v9 = v24;
      }
    }

    else
    {
      v17 = 0;
    }
  }

  else
  {
    v17 = 0;
  }

  return v17;
}

- (void)_resolveDynamicDeclaration:(uint64_t)a1 .cold.1(uint64_t a1, NSObject *a2)
{
  v8 = *MEMORY[0x277D85DE8];
  v3 = objc_opt_class();
  v4 = NSStringFromClass(v3);
  v6 = 138543362;
  v7 = v4;
  _os_log_debug_impl(&dword_261E36000, a2, OS_LOG_TYPE_DEBUG, "Resolved dynamic configuration: %{public}@", &v6, 0xCu);

  v5 = *MEMORY[0x277D85DE8];
}

- (void)_resolveDynamicDeclaration:(uint64_t)a1 .cold.2(uint64_t a1, NSObject *a2)
{
  v5 = *MEMORY[0x277D85DE8];
  v3 = 138543362;
  v4 = a1;
  _os_log_error_impl(&dword_261E36000, a2, OS_LOG_TYPE_ERROR, "Failed to resolve dynamic configuration: %{public}@", &v3, 0xCu);
  v2 = *MEMORY[0x277D85DE8];
}

@end