...
+ (unint64_t)sourceStatsNeededForBytecode:(id)bytecode;
- (PPStreamingNamedEntityScorer)initWithScoringDate:(id)date perRecordDecayRate:(double)rate sourceStats:(id)stats;
- (float)getFinalScoreWithAggregationResultOut:(id *)out finalResultOut:(id *)resultOut;
...
- (float)getFinalScoreWithAggregationResultOut:(id *)out finalResultOut:(id *)resultOut
{
  ...
  if (out)
  {
    objc_storeStrong(out, self->_currentAggResult);
  }

  [(PPScoreInterpreter *)self->_aggregationScorer cleanupReusableComponents:out];
...
- (void)startNewClusterWithDecayedFeedbackCounts:(id)counts mostRelevantRecord:(id)record dominantEntityName:(id)name
{
  ...
  *&v15 = v14;
  ...
  latestDate = [countsCopy latestDate];

  [(PPScoreDict *)v7 setObject:latestDate forIndex:12];
}
...
+ (unint64_t)sourceStatsNeededForBytecode:(id)bytecode
{
  ...
  scalarScoreInputsNeeded = [bytecodeCopy scalarScoreInputsNeeded];
  objectScoreInputsNeeded = [bytecodeCopy objectScoreInputsNeeded];

  BitAtIndex = CFBitVectorGetBitAtIndex(scalarScoreInputsNeeded, 7);
  v7 = CFBitVectorGetBitAtIndex(scalarScoreInputsNeeded, 8);
  v8 = 2;
  if (BitAtIndex)
  {
    v8 = 3;
  ...
  if (CFBitVectorGetBitAtIndex(objectScoreInputsNeeded, 11))
  {
    return v10 | 0x20;
  }
