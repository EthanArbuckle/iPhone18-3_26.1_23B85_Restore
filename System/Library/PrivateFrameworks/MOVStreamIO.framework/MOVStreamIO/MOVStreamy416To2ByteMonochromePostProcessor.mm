...
- (__CVBuffer)processedPixelBufferFrom:(__CVBuffer *)from metadata:(id)metadata error:(id *)error
{
  ...
  pool = self->_pool;
  if (pool || (Width = CVPixelBufferGetWidth(from), [MIOPixelBufferPool createMIOPixelBufferPoolWithWidth:Width height:CVPixelBufferGetHeight(from) pixelFormat:[(MOVStreamDefaultPostProcessor *)self originalPixelFormat] extendedPixelsPerRow:0 minBufferCount:10 bufferCacheMode:[(MOVStreamDefaultPostProcessor *)self bufferCacheMode]], v11 = objc_claimAutoreleasedReturnValue(), v12 = self->_pool, self->_pool = v11, v12, (pool = self->_pool) != 0))
  {
    getPixelBuffer = [(MIOPixelBufferPool *)pool getPixelBuffer];
    [MIOPixelBufferUtility copyAlphaChannelOfBuffer:from toMonochromeBuffer:getPixelBuffer];
