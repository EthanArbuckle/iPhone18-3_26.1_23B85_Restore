...
- (MOVStreamRawBayerPostProcessor)initWithOriginalPixelFormat:(unsigned int)format bufferCacheMode:(int)mode rawBayerMSBReplication:(BOOL)replication;
- (MOVStreamRawBayerPostProcessor)initWithOriginalPixelFormat:(unsigned int)format rawBayerMSBReplication:(BOOL)replication;
...
- (__CVBuffer)processedPixelBufferFrom:(__CVBuffer *)from metadata:(id)metadata error:(id *)error
{
  ...
  processedPixelFormat = [(MOVStreamRawBayerPostProcessor *)self processedPixelFormat];
  LODWORD(v17) = 1;
  HIDWORD(v17) = [(MOVStreamDefaultPostProcessor *)self bufferCacheMode];
  v12 = [MIOPixelBufferUtility createRawPixelBufferWithWidth:Width height:Height extendedRows:0 extendedPixelsPerRow:0 pixelFormat:processedPixelFormat bytesPerRowAlignment:1 planeAlignment:v17 bufferCacheMode:?];
  if ([MIOPixelBufferUtility joinWarholBuffer:from intoBayerBuffer:v12 shiftBitsRightBy:2 msbReplication:self->_rawBayerMSBReplication])
  {
