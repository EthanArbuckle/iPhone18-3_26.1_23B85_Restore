@interface AXSSMotionTrackingHIDInput
+ (id)motionTrackingHIDInputWithDevice:(id)device;
- (AXSSMotionTrackingHIDInput)initWithCoder:(id)coder;
- (AXSSMotionTrackingHIDInput)initWithDeviceUniqueID:(id)d productID:(id)iD vendorID:(id)vendorID deviceName:(id)name mfiAuthenticated:(BOOL)authenticated;
- (AXSSMotionTrackingHIDInput)initWithPlistDictionary:(id)dictionary;
- (NSDictionary)hidMatchingDictionary;
- (id)description;
- (id)plistDictionary;
- (id)uniqueIdentifier;
- (void)encodeWithCoder:(id)coder;
@end

@implementation AXSSMotionTrackingHIDInput

+ (id)motionTrackingHIDInputWithDevice:(id)device
{
  deviceCopy = device;
  aBlock[0] = MEMORY[0x1E69E9820];
  aBlock[1] = 3221225472;
  aBlock[2] = __63__AXSSMotionTrackingHIDInput_motionTrackingHIDInputWithDevice___block_invoke;
  aBlock[3] = &unk_1E8134E70;
  v4 = deviceCopy;
  v21 = v4;
  v5 = _Block_copy(aBlock);
  v15 = MEMORY[0x1E69E9820];
  v16 = 3221225472;
  v17 = __63__AXSSMotionTrackingHIDInput_motionTrackingHIDInputWithDevice___block_invoke_2;
  v18 = &unk_1E8134E98;
  v6 = v4;
  v19 = v6;
  v7 = _Block_copy(&v15);
  v8 = v5[2](v5, @"PhysicalDeviceUniqueID");
  v9 = v7[2](v7, @"ProductID");
  v10 = v7[2](v7, @"VendorID");
  v11 = v5[2](v5, @"Product");
  v12 = [AXSSMotionTrackingUtilities axss_HIDDeviceIsMFiAuthenticated:v6, v15, v16, v17, v18];
  if ([v8 length] || v9 || v10 || objc_msgSend(v11, "length"))
  {
    v13 = [[AXSSMotionTrackingHIDInput alloc] initWithDeviceUniqueID:v8 productID:v9 vendorID:v10 deviceName:v11 mfiAuthenticated:v12];
  }

  else
  {
    v13 = 0;
  }

  return v13;
}

id __63__AXSSMotionTrackingHIDInput_motionTrackingHIDInputWithDevice___block_invoke(uint64_t a1, uint64_t a2)
{
  v2 = [*(a1 + 32) propertyForKey:a2];
  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) != 0 && [v2 length])
  {
    v3 = v2;
  }

  else
  {
    v3 = 0;
  }

  return v3;
}

id __63__AXSSMotionTrackingHIDInput_motionTrackingHIDInputWithDevice___block_invoke_2(uint64_t a1, uint64_t a2)
{
  v2 = [*(a1 + 32) propertyForKey:a2];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v3 = v2;
  }

  else
  {
    v3 = 0;
  }

  return v3;
}

- (AXSSMotionTrackingHIDInput)initWithDeviceUniqueID:(id)d productID:(id)iD vendorID:(id)vendorID deviceName:(id)name mfiAuthenticated:(BOOL)authenticated
{
  dCopy = d;
  iDCopy = iD;
  vendorIDCopy = vendorID;
  nameCopy = name;
  v16 = [dCopy length];
  if (iDCopy || vendorIDCopy || v16 || [nameCopy length])
  {
    v28.receiver = self;
    v28.super_class = AXSSMotionTrackingHIDInput;
    v17 = [(AXSSMotionTrackingInput *)&v28 init];
    if (v17)
    {
      v18 = [dCopy copy];
      deviceUniqueID = v17->_deviceUniqueID;
      v17->_deviceUniqueID = v18;

      v20 = [iDCopy copy];
      productID = v17->_productID;
      v17->_productID = v20;

      v22 = [vendorIDCopy copy];
      vendorID = v17->_vendorID;
      v17->_vendorID = v22;

      v24 = [nameCopy copy];
      deviceName = v17->_deviceName;
      v17->_deviceName = v24;

      v17->_mfiAuthenticated = authenticated;
    }

    self = v17;
    selfCopy = self;
  }

  else
  {
    selfCopy = 0;
  }

  return selfCopy;
}

- (id)uniqueIdentifier
{
  array = [MEMORY[0x1E695DF70] array];
  productID = [(AXSSMotionTrackingHIDInput *)self productID];

  if (productID)
  {
    v5 = MEMORY[0x1E696AEC0];
    productID2 = [(AXSSMotionTrackingHIDInput *)self productID];
    v7 = [v5 stringWithFormat:@"%@", productID2];
    [array addObject:v7];
  }

  vendorID = [(AXSSMotionTrackingHIDInput *)self vendorID];

  if (vendorID)
  {
    v9 = MEMORY[0x1E696AEC0];
    vendorID2 = [(AXSSMotionTrackingHIDInput *)self vendorID];
    v11 = [v9 stringWithFormat:@"%@", vendorID2];
    [array addObject:v11];
  }

  deviceName = [(AXSSMotionTrackingHIDInput *)self deviceName];
  v13 = [deviceName length];

  if (v13)
  {
    deviceName2 = [(AXSSMotionTrackingHIDInput *)self deviceName];
    [array addObject:deviceName2];
  }

  v15 = MEMORY[0x1E696AEC0];
  v16 = [MEMORY[0x1E696AD98] numberWithBool:{-[AXSSMotionTrackingHIDInput mfiAuthenticated](self, "mfiAuthenticated")}];
  v17 = [v15 stringWithFormat:@"%@", v16];
  [array addObject:v17];

  if ([array count])
  {
    v18 = [array componentsJoinedByString:@"-"];
  }

  else
  {
    v18 = 0;
  }

  return v18;
}

- (NSDictionary)hidMatchingDictionary
{
  dictionary = [MEMORY[0x1E695DF90] dictionary];
  aBlock[0] = MEMORY[0x1E69E9820];
  aBlock[1] = 3221225472;
  aBlock[2] = __51__AXSSMotionTrackingHIDInput_hidMatchingDictionary__block_invoke;
  aBlock[3] = &unk_1E8134EC0;
  v4 = dictionary;
  v20 = v4;
  v5 = _Block_copy(aBlock);
  v14 = MEMORY[0x1E69E9820];
  v15 = 3221225472;
  v16 = __51__AXSSMotionTrackingHIDInput_hidMatchingDictionary__block_invoke_2;
  v17 = &unk_1E8134EE8;
  v18 = v4;
  v6 = v4;
  v7 = _Block_copy(&v14);
  v8 = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID:v14];
  v5[2](v5, v8, "PhysicalDeviceUniqueID");

  productID = [(AXSSMotionTrackingHIDInput *)self productID];
  v7[2](v7, productID, "ProductID");

  vendorID = [(AXSSMotionTrackingHIDInput *)self vendorID];
  v7[2](v7, vendorID, "VendorID");

  deviceName = [(AXSSMotionTrackingHIDInput *)self deviceName];
  v5[2](v5, deviceName, "Product");

  v12 = [v6 copy];

  return v12;
}

uint64_t __51__AXSSMotionTrackingHIDInput_hidMatchingDictionary__block_invoke(uint64_t a1, void *a2, uint64_t a3)
{
  v5 = a2;
  if (v5)
  {
    v9 = v5;
    if ([v5 length])
    {
      v6 = *(a1 + 32);
      v7 = [MEMORY[0x1E696AEC0] stringWithUTF8String:a3];
      [v6 setObject:v9 forKeyedSubscript:v7];
    }
  }

  return MEMORY[0x1EEE66BB8]();
}

void __51__AXSSMotionTrackingHIDInput_hidMatchingDictionary__block_invoke_2(uint64_t a1, void *a2, uint64_t a3)
{
  if (a2)
  {
    v4 = *(a1 + 32);
    v5 = MEMORY[0x1E696AEC0];
    v6 = a2;
    v7 = [v5 stringWithUTF8String:a3];
    [v4 setObject:v6 forKeyedSubscript:v7];
  }
}

- (void)encodeWithCoder:(id)coder
{
  coderCopy = coder;
  deviceUniqueID = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID];

  if (deviceUniqueID)
  {
    deviceUniqueID2 = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID];
    v6 = NSStringFromSelector(sel_deviceUniqueID);
    [coderCopy encodeObject:deviceUniqueID2 forKey:v6];
  }

  productID = [(AXSSMotionTrackingHIDInput *)self productID];

  if (productID)
  {
    productID2 = [(AXSSMotionTrackingHIDInput *)self productID];
    v9 = NSStringFromSelector(sel_productID);
    [coderCopy encodeObject:productID2 forKey:v9];
  }

  vendorID = [(AXSSMotionTrackingHIDInput *)self vendorID];

  if (vendorID)
  {
    vendorID2 = [(AXSSMotionTrackingHIDInput *)self vendorID];
    v12 = NSStringFromSelector(sel_vendorID);
    [coderCopy encodeObject:vendorID2 forKey:v12];
  }

  deviceName = [(AXSSMotionTrackingHIDInput *)self deviceName];

  if (deviceName)
  {
    deviceName2 = [(AXSSMotionTrackingHIDInput *)self deviceName];
    v15 = NSStringFromSelector(sel_deviceName);
    [coderCopy encodeObject:deviceName2 forKey:v15];
  }

  v16 = [MEMORY[0x1E696AD98] numberWithBool:{-[AXSSMotionTrackingHIDInput mfiAuthenticated](self, "mfiAuthenticated")}];
  v17 = NSStringFromSelector(sel_mfiAuthenticated);
  [coderCopy encodeObject:v16 forKey:v17];
}

- (AXSSMotionTrackingHIDInput)initWithCoder:(id)coder
{
  coderCopy = coder;
  v5 = objc_opt_class();
  v6 = NSStringFromSelector(sel_deviceUniqueID);
  v7 = [coderCopy decodeObjectOfClass:v5 forKey:v6];

  v8 = objc_opt_class();
  v9 = NSStringFromSelector(sel_productID);
  v10 = [coderCopy decodeObjectOfClass:v8 forKey:v9];

  v11 = objc_opt_class();
  v12 = NSStringFromSelector(sel_vendorID);
  v13 = [coderCopy decodeObjectOfClass:v11 forKey:v12];

  v14 = objc_opt_class();
  v15 = NSStringFromSelector(sel_deviceName);
  v16 = [coderCopy decodeObjectOfClass:v14 forKey:v15];

  v17 = objc_opt_class();
  v18 = NSStringFromSelector(sel_mfiAuthenticated);
  v19 = [coderCopy decodeObjectOfClass:v17 forKey:v18];

  bOOLValue = [v19 BOOLValue];
  v21 = [(AXSSMotionTrackingHIDInput *)self initWithDeviceUniqueID:v7 productID:v10 vendorID:v13 deviceName:v16 mfiAuthenticated:bOOLValue];

  return v21;
}

- (id)description
{
  v3 = MEMORY[0x1E696AEC0];
  v12.receiver = self;
  v12.super_class = AXSSMotionTrackingHIDInput;
  v4 = [(AXSSMotionTrackingHIDInput *)&v12 description];
  deviceUniqueID = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID];
  productID = [(AXSSMotionTrackingHIDInput *)self productID];
  vendorID = [(AXSSMotionTrackingHIDInput *)self vendorID];
  deviceName = [(AXSSMotionTrackingHIDInput *)self deviceName];
  v9 = [MEMORY[0x1E696AD98] numberWithBool:{-[AXSSMotionTrackingHIDInput mfiAuthenticated](self, "mfiAuthenticated")}];
  v10 = [v3 stringWithFormat:@"%@ <deviceUniqueID: %@, productID: %@, vendorID: %@, deviceName: %@, MFi authenticated: %@>", v4, deviceUniqueID, productID, vendorID, deviceName, v9];

  return v10;
}

- (id)plistDictionary
{
  dictionary = [MEMORY[0x1E695DF90] dictionary];
  v21.receiver = self;
  v21.super_class = AXSSMotionTrackingHIDInput;
  plistDictionary = [(AXSSMotionTrackingInput *)&v21 plistDictionary];
  if ([plistDictionary count])
  {
    [dictionary addEntriesFromDictionary:plistDictionary];
  }

  deviceUniqueID = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID];

  if (deviceUniqueID)
  {
    deviceUniqueID2 = [(AXSSMotionTrackingHIDInput *)self deviceUniqueID];
    v7 = NSStringFromSelector(sel_deviceUniqueID);
    [dictionary setObject:deviceUniqueID2 forKeyedSubscript:v7];
  }

  productID = [(AXSSMotionTrackingHIDInput *)self productID];

  if (productID)
  {
    productID2 = [(AXSSMotionTrackingHIDInput *)self productID];
    v10 = NSStringFromSelector(sel_productID);
    [dictionary setObject:productID2 forKeyedSubscript:v10];
  }

  vendorID = [(AXSSMotionTrackingHIDInput *)self vendorID];

  if (vendorID)
  {
    vendorID2 = [(AXSSMotionTrackingHIDInput *)self vendorID];
    v13 = NSStringFromSelector(sel_vendorID);
    [dictionary setObject:vendorID2 forKeyedSubscript:v13];
  }

  deviceName = [(AXSSMotionTrackingHIDInput *)self deviceName];

  if (deviceName)
  {
    deviceName2 = [(AXSSMotionTrackingHIDInput *)self deviceName];
    v16 = NSStringFromSelector(sel_deviceName);
    [dictionary setObject:deviceName2 forKeyedSubscript:v16];
  }

  v17 = [MEMORY[0x1E696AD98] numberWithBool:{-[AXSSMotionTrackingHIDInput mfiAuthenticated](self, "mfiAuthenticated")}];
  v18 = NSStringFromSelector(sel_mfiAuthenticated);
  [dictionary setObject:v17 forKeyedSubscript:v18];

  v19 = [dictionary copy];

  return v19;
}

- (AXSSMotionTrackingHIDInput)initWithPlistDictionary:(id)dictionary
{
  dictionaryCopy = dictionary;
  aBlock[0] = MEMORY[0x1E69E9820];
  aBlock[1] = 3221225472;
  aBlock[2] = __54__AXSSMotionTrackingHIDInput_initWithPlistDictionary___block_invoke;
  aBlock[3] = &unk_1E8134E70;
  v4 = dictionaryCopy;
  v24 = v4;
  v5 = _Block_copy(aBlock);
  v21[0] = MEMORY[0x1E69E9820];
  v21[1] = 3221225472;
  v21[2] = __54__AXSSMotionTrackingHIDInput_initWithPlistDictionary___block_invoke_2;
  v21[3] = &unk_1E8134E98;
  v22 = v4;
  v20 = v4;
  v6 = _Block_copy(v21);
  v17 = NSStringFromSelector(sel_deviceUniqueID);
  v7 = v5[2](v5, v17);
  v16 = NSStringFromSelector(sel_productID);
  v8 = v6[2](v6, v16);
  v9 = NSStringFromSelector(sel_vendorID);
  v10 = v6[2](v6, v9);
  v11 = NSStringFromSelector(sel_deviceName);
  v12 = v5[2](v5, v11);
  v13 = NSStringFromSelector(sel_mfiAuthenticated);
  v14 = v6[2](v6, v13);
  v19 = -[AXSSMotionTrackingHIDInput initWithDeviceUniqueID:productID:vendorID:deviceName:mfiAuthenticated:](self, "initWithDeviceUniqueID:productID:vendorID:deviceName:mfiAuthenticated:", v7, v8, v10, v12, [v14 BOOLValue]);

  return v19;
}

id __54__AXSSMotionTrackingHIDInput_initWithPlistDictionary___block_invoke(uint64_t a1, uint64_t a2)
{
  v2 = [*(a1 + 32) objectForKeyedSubscript:a2];
  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) != 0 && [v2 length])
  {
    v3 = v2;
  }

  else
  {
    v3 = 0;
  }

  return v3;
}

id __54__AXSSMotionTrackingHIDInput_initWithPlistDictionary___block_invoke_2(uint64_t a1, uint64_t a2)
{
  v2 = [*(a1 + 32) objectForKeyedSubscript:a2];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v3 = v2;
  }

  else
  {
    v3 = 0;
  }

  return v3;
}

@end