...
- (AXDeviceMonitor)initWithMatchingDictionary:(id)dictionary callbackRunLoop:(id)loop;
- (AXDeviceMonitor)initWithMatchingMultiple:(id)multiple callbackRunLoop:(id)loop;
- (AXDeviceMonitorDelegate)delegate;
- (id)_existingDevicesForDevice:(__IOHIDDevice *)device;
- (id)copyDevices;
- (void)_commonInitWithRunLoop:(id)loop;
- (void)begin;
- (void)dealloc;
- (void)didAddDevice:(__IOHIDDevice *)device;
- (void)didRemoveDevice:(__IOHIDDevice *)device;
- (void)informDelegate;
- (void)invalidate;
@end
...
- (AXDeviceMonitor)initWithMatchingDictionary:(id)dictionary callbackRunLoop:(id)loop
{
  ...
  if (v8)
  {
    [(AXDeviceMonitor *)v8 _commonInitWithRunLoop:loopCopy];
    IOHIDManagerSetDeviceMatching(v9->_hidManager, dictionaryCopy);
  }
...
- (AXDeviceMonitor)initWithMatchingMultiple:(id)multiple callbackRunLoop:(id)loop
{
  ...
  {
    [(AXDeviceMonitor *)v8 _commonInitWithRunLoop:loopCopy];
    IOHIDManagerSetDeviceMatchingMultiple(v9->_hidManager, multipleCopy);
  }
...
- (void)_commonInitWithRunLoop:(id)loop
{
  objc_storeStrong(&self->_targetRunLoop, loop);
  loopCopy = loop;
...
- (void)begin
{
  ...
  getCFRunLoop = [(NSRunLoop *)self->_targetRunLoop getCFRunLoop];
  IOHIDManagerScheduleWithRunLoop(hidManager, getCFRunLoop, *MEMORY[0x1E695E8E0]);
  v5 = self->_hidManager;
...
- (void)invalidate
{
  ...
  hidManager = self->_hidManager;
  getCFRunLoop = [(NSRunLoop *)self->_targetRunLoop getCFRunLoop];
  IOHIDManagerUnscheduleFromRunLoop(hidManager, getCFRunLoop, *MEMORY[0x1E695E8E0]);
  v5 = self->_hidManager;
...
- (void)didAddDevice:(__IOHIDDevice *)device
{
  ...
  {
    os_unfair_lock_lock(&self->_devicesLock);
    [(NSMutableSet *)self->_devices addObject:device];
    os_unfair_lock_unlock(&self->_devicesLock);
...
- (void)didRemoveDevice:(__IOHIDDevice *)device
{
  v4 = [(AXDeviceMonitor *)self _existingDevicesForDevice:device];
  if ([v4 count])
  {
    os_unfair_lock_lock(&self->_devicesLock);
...
- (id)_existingDevicesForDevice:(__IOHIDDevice *)device
{
  v5 = IOHIDDeviceGetProperty(device, @"UniqueID");
  os_unfair_lock_lock(&self->_devicesLock);
  devices = self->_devices;
...
- (void)informDelegate
{
  delegate = [(AXDeviceMonitor *)self delegate];
  [delegate deviceMonitorDidDetectDeviceEvent:self];
}
