@interface AXSetBackgroundSoundIntentResponse
- (AXSetBackgroundSoundIntentResponse)initWithCode:(int64_t)code userActivity:(id)activity;
@end

@implementation AXSetBackgroundSoundIntentResponse

- (AXSetBackgroundSoundIntentResponse)initWithCode:(int64_t)code userActivity:(id)activity
{
  activityCopy = activity;
  v10.receiver = self;
  v10.super_class = AXSetBackgroundSoundIntentResponse;
  v7 = [(AXSetBackgroundSoundIntentResponse *)&v10 init];
  v8 = v7;
  if (v7)
  {
    v7->_code = code;
    [(AXSetBackgroundSoundIntentResponse *)v7 setUserActivity:activityCopy];
  }

  return v8;
}

@end