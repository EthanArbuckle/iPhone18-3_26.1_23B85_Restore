@interface AAPhotoResponse
- (AAPhotoResponse)initWithHTTPResponse:(id)response data:(id)data;
@end

@implementation AAPhotoResponse

- (AAPhotoResponse)initWithHTTPResponse:(id)response data:(id)data
{
  responseCopy = response;
  v40.receiver = self;
  v40.super_class = AAPhotoResponse;
  v7 = [(AAResponse *)&v40 initWithHTTPResponse:responseCopy data:data];
  v8 = v7;
  if (v7)
  {
    data = [(AAResponse *)v7 data];
    photoData = v8->_photoData;
    v8->_photoData = data;

    allHeaderFields = [responseCopy allHeaderFields];
    v12 = [allHeaderFields objectForKeyedSubscript:@"X-MMe-Setup-Family-Photo"];

    if (v12)
    {
      v13 = [v12 componentsSeparatedByString:{@", "}];
      if ([v13 count] == 4)
      {
        v14 = CGImageSourceCreateWithData(v8->_photoData, 0);
        if (v14)
        {
          v15 = v14;
          v16 = CGImageSourceCopyPropertiesAtIndex(v14, 0, 0);
          v17 = *MEMORY[0x1E696DEC8];
          v18 = [(__CFDictionary *)v16 objectForKeyedSubscript:*MEMORY[0x1E696DEC8]];
          unsignedIntegerValue = [v18 unsignedIntegerValue];

          v20 = [v13 objectAtIndexedSubscript:1];
          integerValue = [v20 integerValue];
          v22 = [v13 objectAtIndexedSubscript:3];
          v23 = unsignedIntegerValue - (integerValue + [v22 integerValue]);

          v24 = [v13 objectAtIndexedSubscript:0];
          integerValue2 = [v24 integerValue];
          v26 = v23;
          v27 = [v13 objectAtIndexedSubscript:2];
          integerValue3 = [v27 integerValue];
          v29 = [v13 objectAtIndexedSubscript:3];
          v36 = integerValue2;
          v37 = v26;
          v38 = integerValue3;
          integerValue4 = [v29 integerValue];

          v30 = [MEMORY[0x1E696B098] valueWithBytes:&v36 objCType:"{CGRect={CGPoint=dd}{CGSize=dd}}"];
          cropRect = v8->_cropRect;
          v8->_cropRect = v30;

          CFRelease(v15);
        }
      }
    }

    allHeaderFields2 = [responseCopy allHeaderFields];
    v33 = [allHeaderFields2 objectForKeyedSubscript:@"ETag"];
    serverCacheTag = v8->_serverCacheTag;
    v8->_serverCacheTag = v33;
  }

  return v8;
}

@end