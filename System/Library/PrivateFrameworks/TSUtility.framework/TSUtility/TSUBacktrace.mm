...
- (id)backtraceString
{
  ...
  v4 = [MEMORY[0x277CCAB68] stringWithString:@"Backtrace:\n"];
  initAdjustment = self->_initAdjustment;
  if (initAdjustment < self->_frames - initAdjustment)
  {
    do
    {
      [v4 appendFormat:@"\t%s\n", v3[initAdjustment++]];
    ...
    while (initAdjustment < self->_frames - self->_initAdjustment);
  }
...
- (id)callerAtIndex:(int64_t)index
{
  ...
  {
    return [MEMORY[0x277CCACA8] stringWithFormat:@"(%s @ %p)", v6.dli_fname, *(&self->_callstack[self->_initAdjustment] + index)];
  }
}
