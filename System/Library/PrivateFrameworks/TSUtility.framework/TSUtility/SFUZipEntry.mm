...
- (BOOL)hasSameLocationAs:(id)as
{
  ...
  else
  {
    ...
    v5 = [(SFUZipArchiveDataRepresentation *)self->mArchiveDataRepresentation hasSameLocationAs:*(as + 4)];
    if (!v5)
    {
      return v5;
    }
...
LABEL_6:
    ...
    LOBYTE(v5) = self->mUncompressedSize == *(as + 7);
  }
