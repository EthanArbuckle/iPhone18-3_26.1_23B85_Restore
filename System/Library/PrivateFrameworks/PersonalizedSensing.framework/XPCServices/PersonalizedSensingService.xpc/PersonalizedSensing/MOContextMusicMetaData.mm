...
- (MOContextMusicMetaData)initWithMusicMetaDataMO:(id)o;
...
- (MOContextMusicMetaData)initWithMusicMetaDataMO:(id)o
{
  ...
  {
    oCopy = o;
    musicString = [oCopy musicString];
    self = [(MOContextMusicMetaData *)self initWithMusicString:musicString];

    artist = [oCopy artist];
    [(MOContextMusicMetaData *)self setArtist:artist];

    title = [oCopy title];

    [(MOContextMusicMetaData *)self setTitle:title];
    selfCopy = self;
  }
...
- (id)description
{
  ...
  artist = [(MOContextMusicMetaData *)self artist];
  title = [(MOContextMusicMetaData *)self title];
  v7 = [NSString stringWithFormat:@"music string, %@, artist, %@, title, %@", mask, artist, title];
