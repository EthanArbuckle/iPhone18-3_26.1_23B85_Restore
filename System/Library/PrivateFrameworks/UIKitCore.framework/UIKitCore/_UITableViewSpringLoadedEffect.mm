...
{
  v5.receiver = self;
  v5.super_class = _UITableViewSpringLoadedEffect;
...
- (void)interaction:(id)interaction didChangeWithContext:(id)context
{
  ...
  {
    v15 = realCellForRowAtIndexPath(view, overrideTargetItem);
  ...
  else
  {
    ...
    {
      v22 = realCellForRowAtIndexPath(view, overrideTargetItem);
    }
  ...
  if (state == 2)
  {
    if (v23)
    {
      ...
      [(_UITableViewSpringLoadedEffect *)self setStateBlinkTimer:v29, v32, v33, v34, v35];
  ...
  if (state == 1)
  {
    ...
    [view _highlightSpringLoadedRowAtIndexPath:overrideTargetItem];
LABEL_28:
    blinkEffect = [(_UITableViewSpringLoadedEffect *)self blinkEffect];
  ...
  if (state)
  {
    stateBlinkTimer = [(_UITableViewSpringLoadedEffect *)self stateBlinkTimer];
  ...
  {
    stateBlinkTimer2 = [(_UITableViewSpringLoadedEffect *)self stateBlinkTimer];
    [stateBlinkTimer2 invalidate];

    [(_UITableViewSpringLoadedEffect *)self setStateBlinkTimer:0];
    [view _unhighlightSpringLoadedRow];
