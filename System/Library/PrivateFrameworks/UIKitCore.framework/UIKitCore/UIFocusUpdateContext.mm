...
{
  previouslyFocusedItem = [(UIFocusUpdateContext *)self previouslyFocusedItem];
  v3 = _UIFocusEnvironmentContainingView(previouslyFocusedItem);
...
{
  nextFocusedItem = [(UIFocusUpdateContext *)self nextFocusedItem];
  v3 = _UIFocusEnvironmentContainingView(nextFocusedItem);
...
- (id)_initWithFocusUpdateRequest:(id)request
{
  ...
  if (!requestCopy)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"UIFocusUpdateContext.m" lineNumber:66 description:{@"Invalid parameter not satisfying: %@", @"request"}];
  }
  ...
  if (v7)
  {
    ...
    v10 = [_UIFocusItemInfo infoWithItem:focusedItem];
    ...
    else
    {
      ...
      {
        objc_storeWeak(&v7->_initialDestinationEnvironment, destinationEnvironment);
      }

      else
      {
        environment = [requestCopy environment];
        objc_storeWeak(&v7->_initialDestinationEnvironment, environment);
      }
    }
...
- (id)_initWithFocusMovementRequest:(id)request nextFocusedItem:(id)item
{
  ...
  {
    currentHandler = [MEMORY[0x1E696AAA8] currentHandler];
    [currentHandler handleFailureInMethod:a2 object:self file:@"UIFocusUpdateContext.m" lineNumber:84 description:{@"Invalid parameter not satisfying: %@", @"request"}];
  }
...
- (id)_initWithContext:(id)context
{
  ...
  if (v5)
  {
    objc_storeStrong(&v5->_request, contextCopy[15]);
    objc_storeStrong(&v6->_sourceItemInfo, contextCopy[9]);
    objc_storeStrong(&v6->_destinationItemInfo, contextCopy[3]);
    objc_storeStrong(&v6->_focusMovement, contextCopy[16]);
    WeakRetained = objc_loadWeakRetained(contextCopy + 11);
