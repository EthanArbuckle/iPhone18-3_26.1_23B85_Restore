...
- (UIView)previouslyFocusedView
{
  v2 = [(UIFocusUpdateContext *)self previouslyFocusedItem];
  v3 = _UIFocusEnvironmentContainingView(v2);
...
- (UIView)nextFocusedView
{
  v2 = [(UIFocusUpdateContext *)self nextFocusedItem];
  v3 = _UIFocusEnvironmentContainingView(v2);
...
- (UIFocusUpdateContext)init
{
  if (os_variant_has_internal_diagnostics())
  {
    ...
    {
      *buf = 0;
      _os_log_fault_impl(&dword_188A29000, v9, OS_LOG_TYPE_FAULT, "-init is not a useful initializer for this class. Use one of the designated initializers instead.", buf, 2u);
    }
  }

  else
  {
    v3 = *(__UILogGetCategoryCachedImpl("Assert", &init___s_category_0) + 8);
    if (os_log_type_enabled(v3, OS_LOG_TYPE_ERROR))
    {
      *v10 = 0;
      _os_log_impl(&dword_188A29000, v3, OS_LOG_TYPE_ERROR, "-init is not a useful initializer for this class. Use one of the designated initializers instead.", v10, 2u);
    }
  }
...
- (id)_initWithFocusUpdateRequest:(id)a3
{
  ...
  if (!v6)
  {
    ...
    [v15 handleFailureInMethod:a2 object:self file:@"UIFocusUpdateContext.m" lineNumber:66 description:{@"Invalid parameter not satisfying: %@", @"request"}];
  }

  v16.receiver = self;
  v16.super_class = UIFocusUpdateContext;
  v7 = [(UIFocusUpdateContext *)&v16 init];
  if (v7)
  {
    ...
    v8 = [v6 focusSystem];
    v9 = [v8 focusedItem];
    v10 = [_UIFocusItemInfo infoWithItem:v9];
    sourceItemInfo = v7->_sourceItemInfo;
    v7->_sourceItemInfo = v10;

    if ([v6 isFocusRemovalRequest])
    {
      objc_storeWeak(&v7->_initialDestinationEnvironment, 0);
    }

    else
    {
      v12 = [v6 destinationEnvironment];
      if (v12)
      {
        objc_storeWeak(&v7->_initialDestinationEnvironment, v12);
      }

      else
      {
        v13 = [v6 environment];
        objc_storeWeak(&v7->_initialDestinationEnvironment, v13);
      }
    }

    *&v7->_flags |= 4u;
    [(UIFocusUpdateContext *)v7 _cacheFocusBehavior];
  }
...
- (id)_initWithFocusMovementRequest:(id)a3 nextFocusedItem:(id)a4
{
  ...
  if (!v8)
  {
    ...
    [v18 handleFailureInMethod:a2 object:self file:@"UIFocusUpdateContext.m" lineNumber:84 description:{@"Invalid parameter not satisfying: %@", @"request"}];
  }
...
- (id)_initWithContext:(id)a3
{
  ...
  if (v5)
  {
    objc_storeStrong(&v5->_request, v4[15]);
    objc_storeStrong(&v6->_sourceItemInfo, v4[9]);
    objc_storeStrong(&v6->_destinationItemInfo, v4[3]);
    objc_storeStrong(&v6->_focusMovement, v4[16]);
    WeakRetained = objc_loadWeakRetained(v4 + 11);
