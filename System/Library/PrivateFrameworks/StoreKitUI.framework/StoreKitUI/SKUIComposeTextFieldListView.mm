...
- (SKUIComposeTextFieldListView)initWithFrame:(CGRect)frame style:(int64_t)style;
...
- (SKUIComposeTextFieldListView)initWithFrame:(CGRect)frame style:(int64_t)style
{
  ...
  result = [(SKUIComposeTextFieldListView *)&v6 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  if (result)
  {
    result->_style = style;
  }
