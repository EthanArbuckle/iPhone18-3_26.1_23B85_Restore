...
- (SKUIProductPageTableLockupsSection)initWithLockups:(id)lockups title:(id)title;
...
- (SKUIProductPageTableLockupsSection)initWithLockups:(id)lockups title:(id)title
{
  ...
  v16.receiver = self;
  v16.super_class = SKUIProductPageTableLockupsSection;
  ...
  if (v8)
  {
    ...
    if (titleCopy)
    {
      ...
      [(SKUIProductPageTableHeaderView *)v8->_headerView setContentInsets:11.0, 15.0, 5.0, 15.0];
      [(SKUIProductPageTableHeaderView *)v8->_headerView setTitle:titleCopy];
...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = SKUIProductPageTableLockupsSection;
...
- (id)footerViewForTableView:(id)view
{
  ...
  if (!footerView)
  {
    v5 = [[SKUIProductPageTableSeparatorView alloc] initWithFrame:0.0, 0.0, 0.0, 10.0];
    v6 = self->_footerView;
    self->_footerView = v5;
...
- (id)headerViewForTableView:(id)view
{
  ...
  if ([v4 count])
  {
    _missingItemLoader = [(SKUIProductPageTableLockupsSection *)self _missingItemLoader];
...
- (double)heightForCellInTableView:(id)view indexPath:(id)path
{
  v4 = [(SKUIProductPageTableSection *)self clientContext:view];
  v5 = dbl_215F40140[SKUIUserInterfaceIdiom(v4) == 1];
...
{
  pathCopy = path;
  v6 = [SKUIProductPageAction actionWithType:4];
...
- (id)tableViewCellForTableView:(id)view indexPath:(id)path
{
  ...
  if (!v7)
  {
    v7 = [[SKUILockupItemTableViewCell alloc] initWithStyle:0 reuseIdentifier:@"LOCKUP"];
    layout = [(SKUILockupItemTableViewCell *)v7 layout];
    clientContext2 = [(SKUIProductPageTableSection *)self clientContext];
    v11 = SKUIUserInterfaceIdiom(clientContext2);
    ...
    v13 = SKUIUserInterfaceIdiom(clientContext);
    v14 = 64.0;
    if (v13 == 1)
    {
      v14 = 72.0;
  ...
  if (item)
  {
    [(SKUILockupItemTableViewCell *)v7 configureForItem:item clientContext:clientContext rowIndex:v15];
    v22 = [(SKUIProductPageTableLockupsSection *)self _productImageForItem:item];
...
- (void)artworkRequest:(id)request didLoadImage:(id)image
{
  ...
  requestCopy = request;
  imageCopy = image;
  delegate = [(SKUITableViewSection *)self delegate];
  if (objc_opt_respondsToSelector())
  {
    v24 = imageCopy;
    v22 = requestCopy;
    requestIdentifier = [requestCopy requestIdentifier];
    sectionIndex = [(SKUITableViewSection *)self sectionIndex];
    v21 = delegate;
...
- (id)_missingItemLoader
{
  ...
  if (!missingItemLoader)
  {
    ...
    resourceLoader = [(SKUIProductPageTableLockupsSection *)self resourceLoader];
...
- (id)_productImageForItem:(id)item
{
  ...
  {
    v5 = +[SKUIStyledImageDataConsumer parentBundleIconConsumer];
  ...
  if (!unsignedIntegerValue || ([resourceLoader cachedResourceForRequestIdentifier:unsignedIntegerValue], (v10 = objc_claimAutoreleasedReturnValue()) == 0))
  {
    ...
    else
    {
      ...
      if (!placeholderImage)
      {
        ...
        v20 = [(SKUIStyledImageDataConsumer *)v18 imageForColor:v19];
