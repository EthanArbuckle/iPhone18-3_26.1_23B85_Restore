...
- (void)addSerializedNetworkRecord:(id)record
{
  ...
  [(NSMutableArray *)serializedNetworkRecords addObject:recordCopy];
}
...
- (id)description
{
  ...
  dictionaryRepresentation = [(PBBProtoSendWirelessCredentialsToWatch *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (serializedNetworkRecords)
  {
    [dictionary setObject:serializedNetworkRecords forKey:@"serializedNetworkRecord"];
  }
...
- (void)copyTo:(id)to
{
  ...
  {
    [toCopy clearSerializedNetworkRecords];
    serializedNetworkRecordsCount = [(PBBProtoSendWirelessCredentialsToWatch *)self serializedNetworkRecordsCount];
    if (serializedNetworkRecordsCount)
    {
      v5 = serializedNetworkRecordsCount;
      for (i = 0; i != v5; ++i)
      {
        v7 = [(PBBProtoSendWirelessCredentialsToWatch *)self serializedNetworkRecordAtIndex:i];
        [toCopy addSerializedNetworkRecord:v7];
      }
    }
  }
}
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    serializedNetworkRecords = self->_serializedNetworkRecords;
    if (serializedNetworkRecords | equalCopy[1])
    {
