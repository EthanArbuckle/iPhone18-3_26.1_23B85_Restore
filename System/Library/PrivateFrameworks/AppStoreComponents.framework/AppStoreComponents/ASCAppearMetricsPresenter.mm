...
+ (OS_os_log)log;
- (ASCAppearMetricsPresenter)initWithView:(id)view;
- (ASCAppearMetricsPresenter)initWithView:(id)view metrics:(id)metrics notificationCenter:(id)center;
- (BOOL)canAppear;
- (BOOL)canDisappear;
...
- (void)setScrollObserver:(id)observer;
- (void)viewDidAction;
...
- (ASCAppearMetricsPresenter)initWithView:(id)view metrics:(id)metrics notificationCenter:(id)center
{
  ...
  if (v11)
  {
    objc_storeWeak(&v11->_view, viewCopy);
...
- (ASCAppearMetricsPresenter)initWithView:(id)view
{
  ...
  v7 = [(ASCAppearMetricsPresenter *)self initWithView:viewCopy metrics:v5 notificationCenter:defaultCenter];
...
- (void)dealloc
{
  notificationCenter = [(ASCAppearMetricsPresenter *)self notificationCenter];
...
- (void)setScrollObserver:(id)observer
{
  ...
  if (observerCopy && v8)
  {
    v10 = [(ASCMetricsScrollObserver *)v8 isEqual:observerCopy];
  ...
  objc_storeStrong(&self->_scrollObserver, observer);
  if (observerCopy)
  {
    model = [(ASCAppearMetricsPresenter *)self model];
    if (!model || (v12 = model, [(ASCAppearMetricsPresenter *)self model], v13 = objc_claimAutoreleasedReturnValue(), v14 = [(ASCMetricsScrollObserver *)observerCopy hasModelAppeared:v13], v13, v12, !v14))
    {
      [(ASCMetricsScrollObserver *)observerCopy addDelegate:self];
    }
  }
...
- (void)setModel:(id)model
{
  ...
  if (modelCopy && v6)
  {
    v8 = [(ASCViewModel *)v6 isEqual:modelCopy];
  ...
  v10 = [(ASCViewModel *)modelCopy copyWithZone:0];
  model = self->_model;
  self->_model = v10;

  if (modelCopy)
  {
    scrollObserver = [(ASCAppearMetricsPresenter *)self scrollObserver];
    if (scrollObserver)
    {
      v13 = scrollObserver;
      isAppeared = [(ASCAppearMetricsPresenter *)self isAppeared];
...
- (NSString)description
{
  ...
  view = [(ASCAppearMetricsPresenter *)self view];
  window = [view window];
  [(ASCDescriber *)v3 addBool:window != 0 withName:@"isInWindow"];

  view2 = [(ASCAppearMetricsPresenter *)self view];
  -[ASCDescriber addBool:withName:](v3, "addBool:withName:", [view2 isHidden], @"isHidden");

  scrollObserver = [(ASCAppearMetricsPresenter *)self scrollObserver];
  [(ASCDescriber *)v3 addBool:scrollObserver != 0 withName:@"isInScrollView"];
...
- (void)viewDidMoveToWindow
{
  ...
  if (window2)
  {
    view3 = [(ASCAppearMetricsPresenter *)self view];
    v9 = [view3 asc_nearestSuperviewPassingTest:&__block_literal_global_28];
...
- (void)viewDidAction
{
  ...
  metrics2 = +[ASCAppearMetricsPresenter log];
  v6 = os_log_type_enabled(metrics2, OS_LOG_TYPE_INFO);
  if (metrics)
  {
    if (v6)
    {
      v11 = 138543362;
      selfCopy2 = self;
      _os_log_impl(&dword_21571A000, metrics2, OS_LOG_TYPE_INFO, "%{public}@ did action", &v11, 0xCu);
    }

    metrics2 = [(ASCAppearMetricsPresenter *)self metrics];
    model2 = [(ASCAppearMetricsPresenter *)self model];
    metrics3 = [model2 metrics];
    activity = [(ASCAppearMetricsPresenter *)self activity];
    v10 = [metrics2 processViewMetrics:metrics3 atInvocationPoint:@"click" withActivity:activity];
  }
  ...
  else if (v6)
  {
    v11 = 138543362;
    selfCopy2 = self;
    _os_log_impl(&dword_21571A000, metrics2, OS_LOG_TYPE_INFO, "%{public}@ has no metrics for action", &v11, 0xCu);
  }
}
...
- (void)viewDidEndRenderWithPredicate:(id)predicate
{
  ...
  if (scrollObserver)
  {
    scrollObserver2 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    model2 = [(ASCAppearMetricsPresenter *)self model];
    v9 = [scrollObserver2 hasModelRendered:model2];
    ...
    scrollObserver3 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    model3 = [(ASCAppearMetricsPresenter *)self model];
    [scrollObserver3 modelDidRender:model3];
  }

  model4 = [(ASCAppearMetricsPresenter *)self model];
  metrics = [model4 metrics];

  if (metrics)
  {
    metrics2 = [(ASCAppearMetricsPresenter *)self metrics];
    v15 = [metrics2 processViewRenderWithPredicate:predicateCopy];
  }
...
- (void)viewDidFailRequest:(id)request withError:(id)error
{
  ...
  if (![domain isEqualToString:*MEMORY[0x277CCA050]] || objc_msgSend(errorCopy, "code") < 4096)
  {
...
LABEL_7:
    ...
    metrics = [(ASCAppearMetricsPresenter *)self metrics];
    activity = [(ASCAppearMetricsPresenter *)self activity];
    v21 = [metrics processMetricsData:v18 withActivity:activity];
  ...
  {
    [(ASCAppearMetricsPresenter *)self viewDidFailRequest:errorCopy withError:v10];
  }
...
- (void)viewDidRetryFailedRequest
{
  ...
  metrics = [(ASCAppearMetricsPresenter *)self metrics];
  activity = [(ASCAppearMetricsPresenter *)self activity];
  v10 = [metrics processMetricsData:v7 withActivity:activity];
}
...
LABEL_7:
  ...
  if (scrollObserver)
  {
    scrollObserver2 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    model2 = [(ASCAppearMetricsPresenter *)self model];
    if ([scrollObserver2 hasModelAppeared:model2] && !-[ASCAppearMetricsPresenter isSettingModel](self, "isSettingModel"))
    {
      isEnteringForeground = [(ASCAppearMetricsPresenter *)self isEnteringForeground];
    ...
    scrollObserver3 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    [scrollObserver3 visibleRect];
    v18 = v17;
    v20 = v19;
    v22 = v21;
    v24 = v23;
    scrollObserver4 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    view3 = [(ASCAppearMetricsPresenter *)self view];
    [scrollObserver4 centerOfView:view3];
    v41.x = v27;
    v41.y = v28;
    v42.origin.x = v18;
    v42.origin.y = v20;
    ...
    scrollObserver5 = [(ASCAppearMetricsPresenter *)self scrollObserver];
    [scrollObserver5 contentSize];
    v32 = v31;
    v34 = v33;
...
- (void)appearIfNeeded
{
  ...
  if ([(ASCAppearMetricsPresenter *)self canAppear])
  {
    ...
    activity = [(ASCAppearMetricsPresenter *)self activity];
    v8 = [metrics processViewMetrics:metrics2 atInvocationPoint:@"appear" withActivity:activity];
    ...
    if (scrollObserver)
    {
      scrollObserver2 = [(ASCAppearMetricsPresenter *)self scrollObserver];
      model2 = [(ASCAppearMetricsPresenter *)self model];
      [scrollObserver2 modelDidAppear:model2];

      scrollObserver3 = [(ASCAppearMetricsPresenter *)self scrollObserver];
      [scrollObserver3 removeDelegate:self];
    }
...
LABEL_22:
  ...
  v7 = window;
  view2 = [(ASCAppearMetricsPresenter *)self view];
  if (([view2 isHidden] & 1) != 0 || -[ASCAppearMetricsPresenter isSettingModel](self, "isSettingModel"))
  {
...
LABEL_8:
  ...
  isEnteringBackground = [(ASCAppearMetricsPresenter *)self isEnteringBackground];
  ...
  if (!isEnteringBackground)
  {
    ...
    if (os_log_type_enabled(v10, OS_LOG_TYPE_INFO))
    {
      isSettingModel = [(ASCAppearMetricsPresenter *)self isSettingModel];
      view3 = [(ASCAppearMetricsPresenter *)self view];
      v17 = 138543874;
      selfCopy6 = self;
      v19 = 1026;
...
{
  v13 = *MEMORY[0x277D85DE8];
  canDisappear = [(ASCAppearMetricsPresenter *)self canDisappear];
  ...
  if (canDisappear)
  {
    ...
    metrics2 = [model metrics];
    activity = [(ASCAppearMetricsPresenter *)self activity];
    v10 = [metrics processViewMetrics:metrics2 atInvocationPoint:@"disappear" withActivity:activity];
