...
- (id)description
{
  ...
  v9 = [v3 stringWithFormat:@"<%@: %p, value: %@, effective: %@, expiration: %@ >", v5, self, value, effectiveDate, expirationDate];
...
- (id)debugInfo
{
  ...
  [dictionary setObject:v6 forKeyedSubscript:@"effectiveDate"];

  expirationDate = [(MTIDSecret *)self expirationDate];
  v8 = [expirationDate description];
  [dictionary setObject:v8 forKeyedSubscript:@"expirationDate"];

  v9 = [MEMORY[0x277CCABB0] numberWithBool:{-[MTIDSecret isSynchronized](self, "isSynchronized")}];
  [dictionary setObject:v9 forKeyedSubscript:@"isSynchronized"];

  syncStatusCode = [(MTIDSecret *)self syncStatusCode];
  [dictionary setObject:syncStatusCode forKeyedSubscript:@"syncStatusCode"];
