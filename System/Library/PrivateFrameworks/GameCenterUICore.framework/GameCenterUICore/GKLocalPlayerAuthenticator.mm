...
+ (BOOL)shouldDisableLoginConsideringSharePlayAvailabilityWithAuthenticateResponse:(id)response;
...
+ (void)postURL:(id)l postBody:(id)body completion:(id)completion;
...
- (void)_authenticateUsingAuthUI:(BOOL)i authenticationResults:(id)results usernameEditable:(BOOL)editable authUIDismissHandler:(id)handler completionHandler:(id)completionHandler;
- (void)_handleAuthResponse:(id)response error:(id)error handler:(id)handler;
- (void)_handleEditedAuthResponse:(id)response error:(id)error handler:(id)handler;
- (void)applicationDidEnterBackground:(id)background;
- (void)authenticateFromExternalServiceWithAuthenticationResults:(id)results withHandler:(id)handler;
- (void)authenticateUsingAuthUIAllowingAppleIDCreation:(BOOL)creation usernameEditable:(BOOL)editable dismissHandler:(id)handler completionHandler:(id)completionHandler;
- (void)authenticateWithAuthKitResults:(id)results completionHandler:(id)handler;
- (void)cancelAuthDueToAuthKitErrorWithHandler:(id)handler;
- (void)dealloc;
- (void)handleAuthKitResultsForUnder13WithContinuationData:(id)data altDSID:(id)d password:(id)password;
...
+ (id)authenticatorForExistingPlayer:(id)player
{
  ...
  [v5 setInputLocalPlayer:playerCopy];
...
+ (id)authenticatorForExistingPlayer:(id)player withPresentingViewController:(id)controller
{
  ...
  playerCopy = player;
  v7 = [objc_opt_class() authenticatorForExistingPlayer:playerCopy];
...
+ (id)authenticatorForPlayerWithUsername:(id)username password:(id)password withPresentingViewController:(id)controller
{
  ...
  passwordCopy = password;
  usernameCopy = username;
  v10 = [objc_opt_class() authenticatorForPlayerWithUsername:usernameCopy password:passwordCopy];

  [v10 setPresentingViewController:controllerCopy];
...
- (GKLocalPlayerAuthenticator)init
{
  ...
  if (v2)
  {
    ...
    [defaultCenter addObserver:v2 selector:sel_applicationDidEnterBackground_ name:didEnterBackground object:0];
  }
...
- (BOOL)shouldAuthenticateForGameCenter
{
  ...
  if ([mEMORY[0x277D0C1D8] isGameCenterDisabled])
  {
    isGameKitAvailable = 0;
  }

  else
  {
    currentDevice = [MEMORY[0x277D0C018] currentDevice];
    isGameKitAvailable = [currentDevice isGameKitAvailable];
  }

  return isGameKitAvailable;
}
...
- (void)authenticateWithAuthKitResults:(id)results completionHandler:(id)handler
{
  ...
  v9 = handlerCopy;
  v7 = handlerCopy;
  [(GKLocalPlayerAuthenticator *)self _authenticateUsingAuthUI:0 authenticationResults:results usernameEditable:1 authUIDismissHandler:0 completionHandler:v8];
}
...
- (void)authenticateUsingAuthUIAllowingAppleIDCreation:(BOOL)creation usernameEditable:(BOOL)editable dismissHandler:(id)handler completionHandler:(id)completionHandler
{
  ...
  [v18 setShouldAllowAppleIDCreation:creationCopy];
  [v18 setIsUsernameEditable:editableCopy];
...
- (void)_authenticateUsingAuthUI:(BOOL)i authenticationResults:(id)results usernameEditable:(BOOL)editable authUIDismissHandler:(id)handler completionHandler:(id)completionHandler
{
  ...
  if (!completionHandlerCopy)
  {
    ...
    v19 = [v15 stringWithFormat:@"%@ (handler != ((void*)0))\n[%s (%s:%d)]", v16, "-[GKLocalPlayerAuthenticator _authenticateUsingAuthUI:authenticationResults:usernameEditable:authUIDismissHandler:completionHandler:]", objc_msgSend(lastPathComponent, "UTF8String"), 318];
  ...
  else
  {
    if ([(GKLocalPlayerAuthenticator *)self isAuthenticated])
    {
      ...
      resultantLocalPlayer = [(GKLocalPlayerAuthenticator *)self resultantLocalPlayer];
      (*(completionHandlerCopy + 2))(completionHandlerCopy, 0, resultantLocalPlayer, 0, 0);
      goto LABEL_42;
    }
  ...
  mEMORY[0x277D0C1D8] = [MEMORY[0x277D0C1D8] shared];
  isGameCenterRestricted = [mEMORY[0x277D0C1D8] isGameCenterRestricted];
...
LABEL_41:
    ...
    (*(completionHandlerCopy + 2))(completionHandlerCopy, 1, resultantLocalPlayer, v42, 0);
...
LABEL_42:
  ...
  lockedDown = [mEMORY[0x277D0C1D8]2 lockedDown];
  ...
  if (iCopy)
  {
    username = [(GKLocalPlayerAuthenticator *)self username];
    username2 = [(GKLocalPlayerAuthenticator *)self username];
    if (username2)
    {
      v9 = v9;
    }
    ...
    v68[3] = &unk_27967EBC0;
    v68[4] = self;
    v69 = completionHandlerCopy;
    [(GKLocalPlayerAuthenticator *)self authenticateUsingAuthUIAllowingAppleIDCreation:username == 0 usernameEditable:v9 dismissHandler:handlerCopy completionHandler:v68];
  ...
  else
  {
    if (resultsCopy)
    {
      [(GKLocalPlayerAuthenticator *)self authenticateFromExternalServiceWithAuthenticationResults:resultsCopy withHandler:completionHandlerCopy];
      goto LABEL_43;
    }

    username3 = [(GKLocalPlayerAuthenticator *)self username];
    if (username3 && (v44 = username3, [(GKLocalPlayerAuthenticator *)self password], v45 = objc_claimAutoreleasedReturnValue(), v45, v44, v45))
    {
      ...
      inputLocalPlayer = [(GKLocalPlayerAuthenticator *)self inputLocalPlayer];
      v51 = [v49 proxyForPlayer:inputLocalPlayer];
      accountServicePrivate = [v51 accountServicePrivate];
      username4 = [(GKLocalPlayerAuthenticator *)self username];
      password = [(GKLocalPlayerAuthenticator *)self password];
      ...
      [accountServicePrivate authenticatePlayerWithUsername:username4 password:password handler:v62];
    ...
    else
    {
      ...
      inputLocalPlayer2 = [(GKLocalPlayerAuthenticator *)self inputLocalPlayer];
      v60 = [v58 proxyForPlayer:inputLocalPlayer2];
      accountService = [v60 accountService];
      ...
      [accountService authenticatePlayerWithExistingCredentialsWithHandler:v65];
...
- (void)applicationDidEnterBackground:(id)background
{
  ...
  [(GKLocalPlayerAuthenticator *)self setLastAuthDate:date];
}
...
- (void)_handleAuthResponse:(id)response error:(id)error handler:(id)handler
{
  ...
  if ([errorCopy gkRequiresCredentialRenewal])
  {
    if (!*MEMORY[0x277D0C2A0])
    {
      v14 = GKOSLoggers();
...
- (void)_handleEditedAuthResponse:(id)response error:(id)error handler:(id)handler
{
  ...
  if (!handlerCopy)
  {
    ...
    v15 = [v11 stringWithFormat:@"%@ (handler != ((void*)0))\n[%s (%s:%d)]", v12, "-[GKLocalPlayerAuthenticator _handleEditedAuthResponse:error:handler:]", objc_msgSend(lastPathComponent, "UTF8String"), 551];
  ...
  playerID = [(__CFString *)responseCopy playerID];
  ...
  v78 = 0u;
  v28 = v25;
  localPlayer = [v28 countByEnumeratingWithState:&v77 objects:v81 count:16];
  if (localPlayer)
  {
    v74 = errorCopy;
    v75 = responseCopy;
    ...
    while (2)
    {
      for (i = 0; i != localPlayer; i = i + 1)
      {
        ...
        v36 = [playerID2 isEqualToString:playerID];

        if (v36)
        {
          self = selfCopy;
          [(GKLocalPlayerAuthenticator *)selfCopy setAuthenticated:1];
          localPlayer = v33;
          responseCopy = v75;
          goto LABEL_25;
        }
...
LABEL_25:
  ...
  [(GKLocalPlayerAuthenticator *)self setResultantLocalPlayer:localPlayer];
  if (!errorCopy && ![(__CFString *)responseCopy loginDisabled])
  {
    v40 = 0;
    errorCopy = 0;
    goto LABEL_61;
  ...
  if ([GKLocalPlayerAuthenticator shouldDisableLoginConsideringSharePlayAvailabilityWithAuthenticateResponse:responseCopy])
  {
    if (!*v16)
    {
      v37 = GKOSLoggers();
    ...
    if (([localPlayer isAuthenticated] & 1) == 0)
    {
      ...
      if ([(__CFString *)responseCopy loginBannerDisabled])
      {
        v40 = 1;
      }
...
LABEL_37:
  ...
  v42 = [domain isEqualToString:*MEMORY[0x277D0BA78]];
...
LABEL_60:
  ...
  v47 = [userInfo objectForKey:*MEMORY[0x277CCA7E8]];
...
LABEL_91:
  ...
  _gkCurrentReachabilityStatus = [_gkReachabilityForInternetConnection _gkCurrentReachabilityStatus];
  ...
  gkNotConnectedToInternetError = [MEMORY[0x277CCA9B8] gkNotConnectedToInternetError];

  v40 = 1;
  errorCopy = gkNotConnectedToInternetError;
LABEL_92:
...
LABEL_61:
  ...
  handlerCopy[2](handlerCopy, v40, localPlayer, errorCopy, responseCopy);
}
...
+ (BOOL)shouldDisableLoginConsideringSharePlayAvailabilityWithAuthenticateResponse:(id)response
{
  ...
  if (os_log_type_enabled(*MEMORY[0x277D0C2C8], OS_LOG_TYPE_INFO))
  {
    ...
    v7 = v5;
    sharedMatchmaker = [v6 sharedMatchmaker];
    v13 = 136315650;
    v14 = "+[GKLocalPlayerAuthenticator shouldDisableLoginConsideringSharePlayAvailabilityWithAuthenticateResponse:]";
    v15 = 1024;
    isEligibleForGroupSession = [sharedMatchmaker isEligibleForGroupSession];
    v17 = 1024;
    loginDisabled = [responseCopy loginDisabled];
    _os_log_impl(&dword_24E4A8000, v7, OS_LOG_TYPE_INFO, "%s isEligibleForGroupSession=%d loginDisabled=%d", &v13, 0x18u);
  }

  mEMORY[0x277D0C158] = [MEMORY[0x277D0C158] sharedMatchmaker];
  isEligibleForGroupSession2 = [mEMORY[0x277D0C158] isEligibleForGroupSession];

  if (isEligibleForGroupSession2)
  {
    loginDisabled2 = 0;
  }
  ...
  {
    loginDisabled2 = [responseCopy loginDisabled];
  }

  return loginDisabled2;
}
...
+ (void)postURL:(id)l postBody:(id)body completion:(id)completion
{
  ...
  if (body)
  {
    body = [MEMORY[0x277CCAC58] dataWithPropertyList:body format:100 options:0 error:0];
  }
  ...
  v13 = completionCopy;
  v11 = completionCopy;
  [utilityServicePrivate loadDataForURL:lCopy postBody:body completionHandler:v12];
}
...
- (void)authenticateFromExternalServiceWithAuthenticationResults:(id)results withHandler:(id)handler
{
  ...
  v10 = [resultsCopy objectForKeyedSubscript:*MEMORY[0x277CEFFC8]];
  [(GKLocalPlayerAuthenticator *)self setPassword:v10];

  v11 = [resultsCopy objectForKeyedSubscript:*MEMORY[0x277CEFF78]];
  v12 = [resultsCopy objectForKey:*MEMORY[0x277CEFFC0]];
  ...
  if (self->_username && self->_password)
  {
    ...
    v17[4] = self;
    v18 = handlerCopy;
    [accountServicePrivate authenticatePlayerWithUsername:username password:password altDSID:v11 isGame:0 usingFastPath:1 handler:v17];
  }

  else
  {
    [(GKLocalPlayerAuthenticator *)self cancelAuthDueToAuthKitErrorWithHandler:handlerCopy];
  }
}
...
- (void)remoteUIControllerDidDismiss:(id)dismiss
{
  ...
  dismissCopy = dismiss;
  u13NavigationController = [(GKLocalPlayerAuthenticator *)self u13NavigationController];
  [u13NavigationController dismissViewControllerAnimated:1 completion:0];
  ...
  else
  {
    ...
    currentLocale = [MEMORY[0x277CBEAF8] currentLocale];
    v13 = [currentLocale objectForKey:*MEMORY[0x277CBE690]];
    ...
    if ([_gkPreferredSystemLanguage rangeOfString:@"-"] == 0x7FFFFFFFFFFFFFFFLL)
    {
      ...
      v17 = [v15 stringWithFormat:@"%@-%@", _gkPreferredSystemLanguage, lowercaseString];

      _gkPreferredSystemLanguage = v17;
    }
    ...
    if ([name length])
    {
      [v6 setObject:name forKey:*MEMORY[0x277CEC710]];
    }

    [v6 setObject:_gkPreferredSystemLanguage forKey:*MEMORY[0x277CEC708]];
    uppercaseString = [v13 uppercaseString];
    [v6 setObject:uppercaseString forKey:*MEMORY[0x277CEC700]];

    v21 = objc_alloc(MEMORY[0x277CEC848]);
    username = [(GKLocalPlayerAuthenticator *)self username];
    password = [(GKLocalPlayerAuthenticator *)self password];
    v24 = [v21 initWithAppleID:username password:password];
