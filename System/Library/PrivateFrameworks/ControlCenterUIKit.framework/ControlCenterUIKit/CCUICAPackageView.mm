...
- (CCUICAPackageView)initWithFrame:(CGRect)frame;
...
- (void)setGlyphTintStyle:(unint64_t)style;
- (void)setPackage:(id)package;
- (void)setPackageDescription:(id)description;
...
LABEL_7:
    ...
    {
      traitCollection = [(CCUICAPackageView *)self traitCollection];
      whiteColor = [v4 resolvedColorWithTraitCollection:traitCollection];
...
- (BOOL)ccui_configureForPunchOutRendering:(BOOL)rendering
{
  ...
  layer = [(CCUICAPackageView *)self layer];
  v5 = layer;
  if (renderingCopy)
  {
...
- (void)ccui_setCompensationAlpha:(double)alpha
{
  ...
  [(CCUICAPackageView *)self setAlpha:alpha];
}

- (void)ccui_applyGlyphTintColor:(id)color
{
  [(CCUICAPackageView *)self setGlyphTintColor:color];
...
- (void)setPackageDescription:(id)description
{
  ...
  {
    objc_storeStrong(&self->_packageDescription, description);
...
- (void)setStateName:(id)name
{
  ...
  v11 = [(CALayer *)self->_packageLayer stateWithName:nameCopy];

  honorsUIViewAnimationState = [(CCUICAPackageDescription *)self->_packageDescription honorsUIViewAnimationState];
  LODWORD(v9) = 1.0;
  if (honorsUIViewAnimationState)
  {
...
- (void)setGlyphTintColor:(id)color
{
  ...
  if ((BSEqualObjects() & 1) == 0)
  {
    ...
    else
    {
      ...
      v7 = [(CCUICAPackageView *)self registerForTraitChanges:systemTraitsAffectingColorAppearance withHandler:&__block_literal_global_1];
...
{
  glyphTintStyle = self->_glyphTintStyle;
  if (glyphTintStyle != style)
  {
    self->_glyphTintStyle = style;
    if (self->_glyphTintColor)
    {
      if (style - 1 >= 2 && glyphTintStyle - 3 >= 0xFFFFFFFFFFFFFFFELL)
      {
        [(CCUICAPackageView *)self _resetPackage];
      }
...
- (CCUICAPackageView)initWithFrame:(CGRect)frame
{
  ...
  v3 = [(CCUICAPackageView *)&v8 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  v4 = v3;
  if (v3)
  {
    v3->_scale = 1.0;
...
- (void)_setPackage:(id)package
{
  ...
  objc_storeStrong(&self->_packageLayer, rootLayer);
  v10 = [objc_alloc(MEMORY[0x1E69794D0]) initWithLayer:rootLayer];
  stateController = self->_stateController;
  self->_stateController = v10;

  stateUpdateHandlers = [(CCUICAPackageDescription *)self->_packageDescription stateUpdateHandlers];
  if ([stateUpdateHandlers count])
  {
    ...
    v13[3] = &unk_1E83EA798;
    v13[4] = self;
    [stateUpdateHandlers enumerateKeysAndObjectsUsingBlock:v13];
  }
...
- (void)_resetPackage
{
  ...
  v8 = a2;
  v4 = v6;
  _os_log_error_impl(&dword_1D168A000, selfCopy, OS_LOG_TYPE_ERROR, "%@:%p needs to reset its package layers but doesn't have a packageDescription set", &v5, 0x16u);
}
