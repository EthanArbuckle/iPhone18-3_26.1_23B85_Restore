...
- (CKPinnedConversationContactItemAvatarView)init
{
  ...
  if (v2)
  {
    ...
    layer = [(CKPinnedConversationContactItemAvatarView *)v3 layer];
...
- (void)traitCollectionDidChange:(id)change
{
  ...
  [(CNAvatarView *)&v4 traitCollectionDidChange:change];
  [(CKPinnedConversationContactItemAvatarView *)self _updateContactItemViewShadows];
}

- (void)_updateContactItemViewShadows
{
  traitCollection = [(CKPinnedConversationContactItemAvatarView *)self traitCollection];
  ...
  {
    v5 = dword_190DD1060[userInterfaceStyle];
    v6 = 4.0;
    v7 = 0.0;
    v8 = 3.0;
  }

  layer = [(CKPinnedConversationContactItemAvatarView *)self layer];
  LODWORD(v10) = v5;
  [layer setShadowOpacity:v10];

  layer2 = [(CKPinnedConversationContactItemAvatarView *)self layer];
  [layer2 setShadowRadius:v6];

  layer3 = [(CKPinnedConversationContactItemAvatarView *)self layer];
  [layer3 setShadowOffset:{v7, v8}];
}
