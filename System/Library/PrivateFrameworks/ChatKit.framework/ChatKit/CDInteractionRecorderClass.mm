...
Class __get_CDInteractionRecorderClass_block_invoke(uint64_t a1)
{
  ...
  *(*(*(a1 + 32) + 8) + 24) = result;
  ...
  get_CDInteractionRecorderClass_softClass = *(*(*(a1 + 32) + 8) + 24);
  return result;
}
...
void __get_CDInteractionRecorderClass_block_invoke_cold_1()
{
  ...
  v1 = [MEMORY[0x1E696AEC0] stringWithUTF8String:"Class get_CDInteractionRecorderClass(void)_block_invoke"];
  [v0 handleFailureInFunction:v1 file:@"CKSMSComposeViewServiceController.m" lineNumber:96 description:{@"Unable to find class %s", "_CDInteractionRecorder"}];
