...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    [dictionary setObject:objc_msgSend(MEMORY[0x1E696AD98] forKey:{"numberWithLongLong:", self->_participantChangeType), @"participant_change_type"}];
  }
...
- (void)copyTo:(id)to
{
  ...
  if (has)
  {
    *(to + 1) = self->_participantChangeType;
    *(to + 36) |= 1u;
  }
...
LABEL_16:
    ...
    if (*&self->_has)
    {
      if ((*(equal + 36) & 1) == 0 || self->_participantChangeType != *(equal + 1))
      {
        goto LABEL_16;
      }
    }
...
- (void)mergeFrom:(id)from
{
  ...
  if (v5)
  {
    self->_participantChangeType = *(from + 1);
    *&self->_has |= 1u;
  }
