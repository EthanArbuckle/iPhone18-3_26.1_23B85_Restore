...
+ (void)_lookupAppStoreUsing:(id)using platform:(id)platform clientApplication:(id)application withCompletionHandler:(id)handler;
+ (void)_lookupGenreIDAndCounterpartIdentifiersInPlist:(id)plist withCompletionHandler:(id)handler;
- (Categories_Service)init;
- (void)dealloc;
- (void)genreIDsAndCounterpartIdentifiersForInstalledBundleIDs:(id)ds replyHandler:(id)handler;
- (void)lookupAppStoreForBundleIDs:(id)ds platform:(id)platform replyHandler:(id)handler;
@end
...
+ (void)_lookupGenreIDAndCounterpartIdentifiersInPlist:(id)plist withCompletionHandler:(id)handler
{
  ...
  v10 = v8;
  [plistCopy enumerateObjectsUsingBlock:v9];
...
+ (void)_lookupAppStoreUsing:(id)using platform:(id)platform clientApplication:(id)application withCompletionHandler:(id)handler
{
  ...
  if (+[CTNetworkReachability networkReachable])
  {
    ...
    v17 = [[CTAppStoreSearch alloc] initWithCallingProcess:applicationCopy];
    if ([platformCopy isEqualToString:CTOSPlatformmacOS])
    {
      v18 = 1;
    }

    else if ([platformCopy isEqualToString:CTOSPlatformiOS] & 1) != 0 || (objc_msgSend(platformCopy, "isEqualToString:", CTOSPlatformwatchOS) & 1) != 0 || (objc_msgSend(platformCopy, "isEqualToString:", CTOSPlatformvisionOS))
    {
      v18 = 6;
    }
  ...
  else
  {
    ...
    (*(handlerCopy + 2))(handlerCopy, v37[5], 0);
  }
