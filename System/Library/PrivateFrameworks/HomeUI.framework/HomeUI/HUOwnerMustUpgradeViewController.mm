...
- (HUOwnerMustUpgradeViewController)initWithUpgradeRequirements:(unint64_t)requirements;
...
- (void)_continueButtonTapped:(id)tapped
{
  ...
  delegate = [(HUOwnerMustUpgradeViewController *)self delegate];
...
- (void)viewDidLoad
{
  ...
  headerView = [(HUOwnerMustUpgradeViewController *)self headerView];
  subviews = [headerView subviews];
  [HUAccessibilityIdentifierUtilities setAccessibilityIDForViews:subviews withIDDictionary:&unk_282493080];

  boldButton = [MEMORY[0x277D37618] boldButton];
  [(HUOwnerMustUpgradeViewController *)self setContinueButton:boldButton];

  continueButton = [(HUOwnerMustUpgradeViewController *)self continueButton];
  [continueButton setTranslatesAutoresizingMaskIntoConstraints:0];

  continueButton2 = [(HUOwnerMustUpgradeViewController *)self continueButton];
  ...
  continueButton3 = [(HUOwnerMustUpgradeViewController *)self continueButton];
  [continueButton3 setAccessibilityIdentifier:@"Home.OnboardingView.OwnerMustUpgrade.ContinueButton"];

  continueButton4 = [(HUOwnerMustUpgradeViewController *)self continueButton];
  [continueButton4 addTarget:self action:sel__continueButtonTapped_ forControlEvents:64];

  buttonTray = [(HUOwnerMustUpgradeViewController *)self buttonTray];
