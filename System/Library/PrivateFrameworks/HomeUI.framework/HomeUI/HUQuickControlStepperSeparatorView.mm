@interface HUQuickControlStepperSeparatorView
- (HUQuickControlStepperSeparatorView)initWithFrame:(CGRect)frame;
@end

@implementation HUQuickControlStepperSeparatorView

- (HUQuickControlStepperSeparatorView)initWithFrame:(CGRect)frame
{
  v9.receiver = self;
  v9.super_class = HUQuickControlStepperSeparatorView;
  v3 = [(HUQuickControlStepperSeparatorView *)&v9 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  if (v3)
  {
    systemGrayColor = [MEMORY[0x277D75348] systemGrayColor];
    v5 = [systemGrayColor colorWithAlphaComponent:0.3];
    [(HUQuickControlStepperSeparatorView *)v3 setBackgroundColor:v5];

    v6 = [MEMORY[0x277CD9EA0] filterWithType:*MEMORY[0x277CDA5D8]];
    layer = [(HUQuickControlStepperSeparatorView *)v3 layer];
    [layer setCompositingFilter:v6];
  }

  return v3;
}

@end