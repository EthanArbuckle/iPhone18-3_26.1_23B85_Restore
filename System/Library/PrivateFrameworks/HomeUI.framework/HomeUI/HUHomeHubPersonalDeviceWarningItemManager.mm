...
- (HUHomeHubPersonalDeviceWarningItemManager)initWithDelegate:(id)delegate devices:(id)devices;
- (HUHomeHubPersonalDeviceWarningItemManager)initWithDelegate:(id)delegate sourceItem:(id)item;
- (id)_buildItemProvidersForHome:(id)home;
- (id)_buildSectionsWithDisplayedItems:(id)items;
- (id)itemForDevice:(id)device;
@end
...
- (HUHomeHubPersonalDeviceWarningItemManager)initWithDelegate:(id)delegate sourceItem:(id)item
{
  ...
  [currentHandler handleFailureInMethod:a2 object:self file:@"HUHomeHubPersonalDeviceWarningItemManager.m" lineNumber:27 description:{@"%s is unavailable; use %@ instead", "-[HUHomeHubPersonalDeviceWarningItemManager initWithDelegate:sourceItem:]", v7}];
...
- (id)_buildItemProvidersForHome:(id)home
{
  ...
  {
    devices = [(HUHomeHubPersonalDeviceWarningItemManager *)self devices];
    *buf = 136315394;
...
- (id)itemForDevice:(id)device
{
  ...
  if (deviceCopy)
  {
    ...
    if (-[__CFString length](name, "length") || ([deviceCopy productInfo], v8 = objc_claimAutoreleasedReturnValue(), v8, !v8))
    {
      v7 = name;
    }
    ...
    productInfo3 = [deviceCopy productInfo];

    if (productInfo3)
    {
      productInfo4 = [deviceCopy productInfo];
      [productInfo4 productClass];
      ...
      v17 = [MEMORY[0x277D755B8] hu_symbolForDevice:deviceCopy];
...
- (id)_buildSectionsWithDisplayedItems:(id)items
{
  ...
  v10 = [allObjects sortedArrayUsingComparator:&__block_literal_global_189];
  [v7 setItems:v10];

  [array addObject:v7];
  v11 = [MEMORY[0x277D14778] filterSections:array toDisplayedItems:itemsCopy];
