...
- (OPTTSTextToSpeechRequestDebug)initWithFlatbuffData:(id)data root:(const TextToSpeechRequestDebug *)root verify:(BOOL)verify;
...
{
  selfCopy = self;
  force_use_tts_service = [(OPTTSTextToSpeechRequestDebug *)self force_use_tts_service];
  LODWORD(selfCopy) = [(OPTTSTextToSpeechRequestDebug *)selfCopy disable_cache];
  apple::aiml::flatbuffers2::FlatBufferBuilder::NotNested(buffer);
  *(buffer + 70) = 1;
  v6 = *(buffer + 10);
  v7 = *(buffer + 8) - *(buffer + 12);
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddElement<unsigned char>(buffer, 4, force_use_tts_service);
