...
- (ASVTextureDescription)initWithName:(id)name destinationPixelFormat:(id)format imageProperties:(__CFDictionary *)properties fileType:(__CFString *)type generateMipmaps:(BOOL)mipmaps;
- (ASVTextureDescription)initWithName:(id)name originalSize:(id)size originalPixelFormat:(id)format destinationPixelFormat:(int64_t)pixelFormat downsamplingFactor:(int64_t)factor fileType:(BOOL)type generateMipmaps:;
...
- (ASVTextureDescription)initWithName:(id)name originalSize:(id)size originalPixelFormat:(id)format destinationPixelFormat:(int64_t)pixelFormat downsamplingFactor:(int64_t)factor fileType:(BOOL)type generateMipmaps:
{
  ...
  nameCopy = name;
  v21.receiver = self;
  v21.super_class = ASVTextureDescription;
...
- (ASVTextureDescription)initWithName:(id)name destinationPixelFormat:(id)format imageProperties:(__CFDictionary *)properties fileType:(__CFString *)type generateMipmaps:(BOOL)mipmaps
{
  ...
  nameCopy = name;
  v41.receiver = self;
  v41.super_class = ASVTextureDescription;
  v15 = [(ASVTextureDescription *)&v41 init];
  v16 = v15;
  if (!v15)
  {
    goto LABEL_39;
...
LABEL_37:
        v36 = [ASVTextureDescription pixelEncodingForBitsPerComponent:v38];
...
- (id)debugDescription
{
  ...
  name = [(ASVTextureDescription *)self name];
...
+ (unint64_t)bytesPerComponentForColorModel:(int64_t)model
{
  ...
  {
    return qword_2413647A8[model - 1];
  }
}
...
+ (unint64_t)bytesPerComponentForPixelEncoding:(int64_t)encoding
{
  ...
  {
    return qword_2413647D8[encoding - 1];
  }
}
...
{
  var1 = format.var1;
  v4 = [ASVTextureDescription bytesPerComponentForColorModel:format.var0];
