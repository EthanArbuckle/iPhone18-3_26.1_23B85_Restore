...
- (TSDBitmapRenderingQualityInfo)initWithQuality:(int)quality canvasScale:(float)scale;
...
- (void)cacheProvider:(id)provider ofSize:(CGSize)size
{
  ...
  if (!v8)
  {
    ...
    -[NSMutableDictionary setObject:forKey:](self->mImageMap, "setObject:forKey:", v8, [MEMORY[0x277CCAE60] valueWithNonretainedObject:provider]);
  }
...
- (CGImage)degradedImageRefForProvider:(id)provider ofSize:(CGSize)size
{
  [(TSDBitmapRenderingQualityInfo *)self cacheProvider:size.width ofSize:size.height];
  v6 = -[NSMutableDictionary objectForKey:](self->mImageMap, "objectForKey:", [MEMORY[0x277CCAE60] valueWithNonretainedObject:provider]);
  if (!v6)
  {
    currentHandler = [MEMORY[0x277D6C290] currentHandler];
    v9 = [MEMORY[0x277CCACA8] stringWithUTF8String:"-[TSDBitmapRenderingQualityInfo degradedImageRefForProvider:ofSize:]"];
    [currentHandler handleFailureInFunction:v9 file:objc_msgSend(MEMORY[0x277CCACA8] lineNumber:"stringWithUTF8String:" description:{"/Library/Caches/com.apple.xbs/Sources/AlderShared/drawables/TSDBitmapRenderingQualityInfo.m"), 80, @"Bitmap rendering quality info encountered an unprocessed image ref"}];
  }

  mQuality = self->mQuality;
  *&v7 = self->mCanvasScale;

  return [v6 degradedImageRefForForProvider:provider quality:mQuality canvasScale:v7];
}
