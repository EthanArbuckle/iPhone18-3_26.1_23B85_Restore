...
- (id)description
{
  ...
  v8.receiver = self;
  v8.super_class = RMSNavigationCommandMessage;
  v4 = [(RMSNavigationCommandMessage *)&v8 description];
  dictionaryRepresentation = [(RMSNavigationCommandMessage *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    v5 = [MEMORY[0x277CCABB0] numberWithInt:self->_navigationCommand];
    [dictionary setObject:v5 forKey:@"navigationCommand"];
...
- (void)copyTo:(id)to
{
  ...
  if (has)
  {
    toCopy[2] = self->_navigationCommand;
    *(toCopy + 16) |= 1u;
    has = self->_has;
  }
...
- (id)copyWithZone:(_NSZone *)zone
{
  ...
  if (has)
  {
    *(result + 2) = self->_navigationCommand;
    *(result + 16) |= 1u;
    has = self->_has;
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 16) & 1) == 0 || self->_navigationCommand != *(equalCopy + 2))
    {
      goto LABEL_11;
    }
  }
...
- (void)mergeFrom:(id)from
{
  ...
  if (v5)
  {
    self->_navigationCommand = *(fromCopy + 2);
    *&self->_has |= 1u;
    v5 = *(fromCopy + 16);
  }
