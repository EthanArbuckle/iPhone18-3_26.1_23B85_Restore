...
+ (id)sharedUtils;
- (BOOL)_permissiveLocationAuthWithManager:(id)manager;
- (MCPolarisUtils)init;
...
- (BOOL)_permissiveLocationAuthWithManager:(id)manager
{
  managerCopy = manager;
  v4 = managerCopy;
  v5 = managerCopy && [managerCopy authorizationStatus] >= 3 && objc_msgSend(v4, "accuracyAuthorization") == 0;
...
- (MCPolarisUtils)init
{
  ...
  if (v2)
  {
    ...
    v8->_hasPrimaryAppleAccount = aa_primaryAppleAccount != 0;
    v8->_isManagedAppleAccount = [aa_primaryAppleAccount aa_isManagedAppleID];
    aa_personID = [aa_primaryAppleAccount aa_personID];
    appleAccountPersonId = v8->_appleAccountPersonId;
    v8->_appleAccountPersonId = aa_personID;

    aa_mapsToken = [aa_primaryAppleAccount aa_mapsToken];
    appleAccountMapsToken = v8->_appleAccountMapsToken;
    v8->_appleAccountMapsToken = aa_mapsToken;
    ...
    v8->_isMultiUserMode = isEphemeralMultiUser;
...
- (void)locationManagerDidChangeAuthorization:(id)authorization
{
  v4 = [(MCPolarisUtils *)self _permissiveLocationAuthWithManager:authorization];
