@interface STTimerShowTimerAndPerformAction
- (STTimerShowTimerAndPerformAction)initWithCoder:(id)a3;
- (id)_initWithTimer:(id)a3 action:(unint64_t)a4;
- (id)_initWithTimers:(id)a3 templateActions:(id)a4 timerAction:(unint64_t)a5;
- (void)encodeWithCoder:(id)a3;
@end

@implementation STTimerShowTimerAndPerformAction

- (STTimerShowTimerAndPerformAction)initWithCoder:(id)a3
{
  v20[2] = *MEMORY[0x277D85DE8];
  v4 = a3;
  v18.receiver = self;
  v18.super_class = STTimerShowTimerAndPerformAction;
  v5 = [(AFSiriRequest *)&v18 initWithCoder:v4];
  if (v5)
  {
    v6 = MEMORY[0x277CBEB98];
    v20[0] = objc_opt_class();
    v20[1] = objc_opt_class();
    v7 = [MEMORY[0x277CBEA60] arrayWithObjects:v20 count:2];
    v8 = [v6 setWithArray:v7];
    v9 = [v4 decodeObjectOfClasses:v8 forKey:@"_timers"];
    timers = v5->_timers;
    v5->_timers = v9;

    v11 = MEMORY[0x277CBEB98];
    v19[0] = objc_opt_class();
    v19[1] = objc_opt_class();
    v12 = [MEMORY[0x277CBEA60] arrayWithObjects:v19 count:2];
    v13 = [v11 setWithArray:v12];
    v14 = [v4 decodeObjectOfClasses:v13 forKey:@"_templateActions"];
    templateActions = v5->_templateActions;
    v5->_templateActions = v14;

    v5->_action = [v4 decodeIntegerForKey:@"_action"];
  }

  v16 = *MEMORY[0x277D85DE8];
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  v5.receiver = self;
  v5.super_class = STTimerShowTimerAndPerformAction;
  v4 = a3;
  [(AFSiriRequest *)&v5 encodeWithCoder:v4];
  [v4 encodeObject:self->_timers forKey:{@"_timers", v5.receiver, v5.super_class}];
  [v4 encodeObject:self->_templateActions forKey:@"_templateActions"];
  [v4 encodeInteger:self->_action forKey:@"_action"];
}

- (id)_initWithTimers:(id)a3 templateActions:(id)a4 timerAction:(unint64_t)a5
{
  v8 = a3;
  v9 = a4;
  v16.receiver = self;
  v16.super_class = STTimerShowTimerAndPerformAction;
  v10 = [(AFSiriRequest *)&v16 init];
  if (v10)
  {
    v11 = [v8 copy];
    timers = v10->_timers;
    v10->_timers = v11;

    v13 = [v9 copy];
    templateActions = v10->_templateActions;
    v10->_templateActions = v13;

    v10->_action = a5;
  }

  return v10;
}

- (id)_initWithTimer:(id)a3 action:(unint64_t)a4
{
  v6 = a3;
  v11.receiver = self;
  v11.super_class = STTimerShowTimerAndPerformAction;
  v7 = [(AFSiriRequest *)&v11 init];
  if (v7)
  {
    if (v6)
    {
      v8 = [MEMORY[0x277CBEA60] arrayWithObject:v6];
      timers = v7->_timers;
      v7->_timers = v8;
    }

    v7->_action = a4;
  }

  return v7;
}

@end