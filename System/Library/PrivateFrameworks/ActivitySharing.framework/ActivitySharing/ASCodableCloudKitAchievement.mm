...
- (id)description
{
  ...
  v8.receiver = self;
  v8.super_class = ASCodableCloudKitAchievement;
  v4 = [(ASCodableCloudKitAchievement *)&v8 description];
  dictionaryRepresentation = [(ASCodableCloudKitAchievement *)self dictionaryRepresentation];
...
- (void)mergeFrom:(id)from
{
  ...
  else
  {
    ...
    [(ASCodableCloudKitAchievement *)self setSample:?];
  }
...
LABEL_12:
  if (*(fromCopy + 5))
  {
    [(ASCodableCloudKitAchievement *)self setDefinitionIdentifier:?];
    fromCopy = v8;
  }

  if (*(fromCopy + 7))
  {
    [(ASCodableCloudKitAchievement *)self setTemplateUniqueName:?];
  }
