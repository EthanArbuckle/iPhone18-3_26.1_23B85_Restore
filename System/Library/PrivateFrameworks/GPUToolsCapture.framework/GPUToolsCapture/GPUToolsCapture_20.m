uint64_t GTFenum_getReceiverType(int a1)
{
  if (a1 > -12544)
  {
    if (a1 > -6657)
    {
      if (a1 <= -6654)
      {
        if (a1 == -6656 || a1 == -6655)
        {
          return 84;
        }

        v3 = -6654;
LABEL_84:
        if (a1 == v3)
        {
          return 84;
        }

        else
        {
          return 0;
        }
      }

      else
      {
        switch(a1)
        {
          case -6144:
          case -6132:
          case -6130:
          case -6105:
          case -6101:
            result = 10;
            break;
          case -6143:
          case -6141:
          case -6134:
          case -6133:
          case -6131:
          case -6129:
          case -6122:
          case -6119:
          case -6116:
          case -6111:
          case -6110:
          case -6109:
          case -6096:
          case -6095:
LABEL_3:
            result = 36;
            break;
          case -6142:
          case -6126:
          case -6121:
            result = 7;
            break;
          case -6140:
          case -6137:
            result = 11;
            break;
          case -6139:
            result = 15;
            break;
          case -6138:
          case -6135:
          case -6117:
          case -6115:
            result = 8;
            break;
          case -6136:
          case -6120:
          case -6112:
          case -6108:
          case -6106:
            result = 5;
            break;
          case -6128:
          case -6124:
          case -6104:
          case -6103:
            result = 9;
            break;
          case -6127:
          case -6114:
          case -6113:
          case -6107:
            result = 6;
            break;
          case -6125:
          case -6123:
          case -6118:
            result = 4;
            break;
          case -6102:
          case -6100:
            result = 14;
            break;
          case -6099:
          case -6098:
            result = 13;
            break;
          case -6097:
            result = 12;
            break;
          default:
            if (a1 == -6653)
            {
              return 84;
            }

            v3 = -6652;
            goto LABEL_84;
        }
      }
    }

    else
    {
      v2 = a1 + 10240;
      result = 1;
      switch(v2)
      {
        case 0:
        case 11:
        case 15:
LABEL_15:
          result = 25;
          break;
        case 1:
        case 3:
        case 7:
        case 10:
          goto LABEL_3;
        case 2:
        case 4:
        case 6:
        case 8:
        case 9:
        case 13:
        case 18:
        case 28:
        case 35:
        case 36:
        case 39:
        case 42:
        case 55:
        case 76:
LABEL_7:
          result = 80;
          break;
        case 5:
        case 12:
        case 17:
        case 19:
        case 30:
        case 37:
        case 40:
        case 53:
        case 54:
LABEL_25:
          result = 22;
          break;
        case 14:
        case 77:
LABEL_71:
          result = 75;
          break;
        case 20:
        case 41:
        case 48:
        case 56:
        case 68:
        case 75:
LABEL_21:
          result = 71;
          break;
        case 21:
        case 22:
        case 23:
        case 24:
        case 25:
        case 31:
        case 38:
        case 74:
LABEL_36:
          result = 57;
          break;
        case 26:
        case 49:
        case 69:
        case 78:
LABEL_41:
          result = 29;
          break;
        case 27:
LABEL_47:
          result = 74;
          break;
        case 29:
        case 32:
        case 33:
LABEL_40:
          result = 50;
          break;
        case 34:
          result = 77;
          break;
        case 43:
        case 44:
        case 45:
        case 46:
        case 50:
        case 58:
        case 59:
        case 67:
        case 70:
        case 71:
        case 73:
        case 81:
        case 84:
LABEL_32:
          result = 16;
          break;
        case 47:
        case 62:
        case 63:
        case 66:
        case 80:
        case 87:
LABEL_35:
          result = 83;
          break;
        case 51:
LABEL_70:
          result = 37;
          break;
        case 52:
        case 57:
LABEL_39:
          result = 62;
          break;
        case 60:
        case 61:
        case 64:
        case 65:
        case 79:
        case 88:
LABEL_27:
          result = 60;
          break;
        case 72:
          return result;
        case 82:
        case 85:
        case 86:
LABEL_18:
          result = 17;
          break;
        case 83:
LABEL_50:
          result = 43;
          break;
        case 89:
LABEL_52:
          result = 23;
          break;
        case 90:
        case 91:
LABEL_81:
          result = 34;
          break;
        case 92:
        case 93:
        case 94:
        case 95:
        case 96:
        case 97:
        case 98:
LABEL_29:
          result = 101;
          break;
        case 99:
        case 100:
LABEL_80:
          result = 44;
          break;
        case 101:
        case 102:
LABEL_14:
          result = 95;
          break;
        default:
          return 0;
      }
    }
  }

  else
  {
    switch(a1)
    {
      case -18432:
      case -18431:
      case -18429:
      case -18428:
      case -18427:
      case -18426:
      case -18425:
      case -18424:
      case -16319:
      case -16318:
      case -16317:
      case -16316:
      case -16315:
      case -16314:
      case -16313:
      case -16312:
      case -16311:
      case -16310:
      case -16309:
      case -16308:
      case -16307:
      case -16306:
      case -16305:
      case -16304:
      case -16303:
      case -16302:
      case -16301:
      case -16300:
      case -16299:
      case -16298:
      case -16297:
      case -16296:
      case -16295:
      case -16294:
      case -16206:
      case -16205:
      case -16200:
      case -16199:
      case -16198:
      case -16197:
      case -16166:
      case -16165:
      case -16163:
      case -16128:
      case -16121:
      case -16120:
      case -16112:
      case -16111:
      case -16101:
      case -16099:
      case -16096:
      case -16095:
      case -16093:
      case -16091:
      case -16090:
      case -16089:
      case -16088:
      case -16075:
      case -16066:
      case -16050:
      case -16049:
      case -16048:
      case -16039:
      case -16037:
      case -16036:
      case -16035:
      case -16031:
      case -16030:
      case -16029:
      case -16018:
      case -16016:
      case -16015:
      case -15998:
      case -15997:
      case -15996:
      case -15980:
      case -15979:
      case -15978:
      case -15972:
      case -15971:
      case -15969:
      case -15968:
      case -15912:
      case -15909:
      case -15902:
      case -15901:
      case -15900:
      case -15892:
      case -15891:
      case -15864:
      case -15863:
      case -15862:
      case -15861:
      case -15860:
      case -15859:
      case -15858:
      case -15857:
      case -15856:
      case -15855:
      case -15854:
      case -15853:
      case -15852:
      case -15851:
      case -15850:
      case -15849:
      case -15848:
      case -15847:
      case -15846:
      case -15845:
      case -15844:
      case -15843:
      case -15842:
      case -15841:
      case -15840:
      case -15839:
      case -15838:
      case -15837:
      case -15836:
      case -15835:
      case -15834:
      case -15833:
      case -15832:
      case -15831:
      case -15814:
      case -15813:
      case -15796:
      case -15793:
      case -15791:
      case -15780:
      case -15779:
      case -15774:
      case -15759:
      case -15758:
      case -15757:
      case -15751:
      case -15747:
      case -15746:
      case -15737:
      case -15731:
      case -15730:
      case -15719:
      case -15718:
      case -15717:
      case -15716:
      case -15708:
      case -15707:
      case -15706:
      case -15705:
      case -15704:
      case -15698:
      case -15697:
      case -15696:
      case -15695:
      case -15694:
      case -15693:
      case -15672:
      case -15671:
      case -15617:
      case -15616:
      case -15615:
      case -15614:
      case -15613:
      case -15612:
      case -15577:
      case -15576:
      case -15575:
      case -15535:
      case -15532:
      case -15531:
      case -15509:
      case -15506:
      case -15499:
      case -15498:
      case -15497:
      case -15496:
      case -15475:
      case -15468:
      case -15467:
      case -15464:
      case -15435:
      case -15434:
      case -15433:
      case -15432:
      case -15431:
      case -15430:
      case -15429:
      case -15428:
      case -15427:
      case -15425:
      case -15421:
      case -15420:
      case -15419:
      case -15418:
      case -15417:
      case -15406:
      case -15405:
      case -15404:
      case -15403:
      case -15402:
      case -15363:
      case -15362:
      case -15361:
      case -15360:
      case -15359:
      case -15358:
      case -15350:
      case -15349:
      case -15348:
      case -15347:
      case -15317:
      case -15315:
      case -15314:
      case -15298:
      case -15296:
      case -15270:
      case -15269:
      case -15268:
      case -15267:
      case -15261:
      case -15260:
      case -15259:
      case -15246:
      case -15245:
      case -15214:
      case -15202:
      case -15201:
      case -15200:
      case -15199:
      case -15198:
      case -15197:
      case -15196:
      case -15195:
      case -15194:
      case -15193:
      case -15192:
      case -15191:
      case -15190:
      case -15189:
      case -14961:
      case -14960:
      case -14959:
      case -14958:
      case -14957:
      case -14853:
      case -14852:
      case -14851:
      case -14838:
      case -14835:
      case -14834:
      case -14797:
      case -14796:
      case -14795:
      case -14794:
        goto LABEL_3;
      case -18430:
LABEL_43:
        result = 40;
        break;
      case -18423:
LABEL_38:
        result = 109;
        break;
      case -18422:
LABEL_24:
        result = 104;
        break;
      case -18421:
      case -18420:
      case -18419:
      case -18418:
      case -18417:
      case -18416:
      case -18415:
      case -18414:
      case -18413:
      case -18412:
      case -18411:
      case -18410:
      case -18409:
      case -18408:
      case -18407:
      case -18406:
      case -18405:
      case -18404:
      case -18403:
      case -18402:
      case -18401:
      case -18400:
      case -18399:
      case -18398:
      case -18397:
      case -18396:
      case -18395:
      case -18394:
      case -18393:
      case -18392:
      case -18391:
      case -18390:
      case -18389:
      case -18388:
      case -18387:
      case -18386:
      case -18385:
      case -18384:
      case -18383:
      case -18382:
      case -18381:
      case -18380:
      case -18379:
      case -18378:
      case -18377:
      case -18376:
      case -18375:
      case -18374:
      case -18373:
      case -18372:
      case -18371:
      case -18370:
      case -18369:
      case -18368:
      case -18367:
      case -18366:
      case -18365:
      case -18364:
      case -18363:
      case -18362:
      case -18361:
      case -18360:
      case -18359:
      case -18358:
      case -18357:
      case -18356:
      case -18355:
      case -18354:
      case -18353:
      case -18352:
      case -18351:
      case -18350:
      case -18349:
      case -18348:
      case -18347:
      case -18346:
      case -18345:
      case -18344:
      case -18343:
      case -18342:
      case -18341:
      case -18340:
      case -18339:
      case -18338:
      case -18337:
      case -18336:
      case -18335:
      case -18334:
      case -18333:
      case -18332:
      case -18331:
      case -18330:
      case -18329:
      case -18328:
      case -18327:
      case -18326:
      case -18325:
      case -18324:
      case -18323:
      case -18322:
      case -18321:
      case -18320:
      case -18319:
      case -18318:
      case -18317:
      case -18316:
      case -18315:
      case -18314:
      case -18313:
      case -18312:
      case -18311:
      case -18310:
      case -18309:
      case -18308:
      case -18307:
      case -18306:
      case -18305:
      case -18304:
      case -18303:
      case -18302:
      case -18301:
      case -18300:
      case -18299:
      case -18298:
      case -18297:
      case -18296:
      case -18295:
      case -18294:
      case -18293:
      case -18292:
      case -18291:
      case -18290:
      case -18289:
      case -18288:
      case -18287:
      case -18286:
      case -18285:
      case -18284:
      case -18283:
      case -18282:
      case -18281:
      case -18280:
      case -18279:
      case -18278:
      case -18277:
      case -18276:
      case -18275:
      case -18274:
      case -18273:
      case -18272:
      case -18271:
      case -18270:
      case -18269:
      case -18268:
      case -18267:
      case -18266:
      case -18265:
      case -18264:
      case -18263:
      case -18262:
      case -18261:
      case -18260:
      case -18259:
      case -18258:
      case -18257:
      case -18256:
      case -18255:
      case -18254:
      case -18253:
      case -18252:
      case -18251:
      case -18250:
      case -18249:
      case -18248:
      case -18247:
      case -18246:
      case -18245:
      case -18244:
      case -18243:
      case -18242:
      case -18241:
      case -18240:
      case -18239:
      case -18238:
      case -18237:
      case -18236:
      case -18235:
      case -18234:
      case -18233:
      case -18232:
      case -18231:
      case -18230:
      case -18229:
      case -18228:
      case -18227:
      case -18226:
      case -18225:
      case -18224:
      case -18223:
      case -18222:
      case -18221:
      case -18220:
      case -18219:
      case -18218:
      case -18217:
      case -18216:
      case -18215:
      case -18214:
      case -18213:
      case -18212:
      case -18211:
      case -18210:
      case -18209:
      case -18208:
      case -18207:
      case -18206:
      case -18205:
      case -18204:
      case -18203:
      case -18202:
      case -18201:
      case -18200:
      case -18199:
      case -18198:
      case -18197:
      case -18196:
      case -18195:
      case -18194:
      case -18193:
      case -18192:
      case -18191:
      case -18190:
      case -18189:
      case -18188:
      case -18187:
      case -18186:
      case -18185:
      case -18184:
      case -18183:
      case -18182:
      case -18181:
      case -18180:
      case -18179:
      case -18178:
      case -18177:
      case -18176:
      case -18175:
      case -18174:
      case -18173:
      case -18172:
      case -18171:
      case -18170:
      case -18169:
      case -18168:
      case -18167:
      case -18166:
      case -18165:
      case -18164:
      case -18163:
      case -18162:
      case -18161:
      case -18160:
      case -18159:
      case -18158:
      case -18157:
      case -18156:
      case -18155:
      case -18154:
      case -18153:
      case -18152:
      case -18151:
      case -18150:
      case -18149:
      case -18148:
      case -18147:
      case -18146:
      case -18145:
      case -18144:
      case -18143:
      case -18142:
      case -18141:
      case -18140:
      case -18139:
      case -18138:
      case -18137:
      case -18136:
      case -18135:
      case -18134:
      case -18133:
      case -18132:
      case -18131:
      case -18130:
      case -18129:
      case -18128:
      case -18127:
      case -18126:
      case -18125:
      case -18124:
      case -18123:
      case -18122:
      case -18121:
      case -18120:
      case -18119:
      case -18118:
      case -18117:
      case -18116:
      case -18115:
      case -18114:
      case -18113:
      case -18112:
      case -18111:
      case -18110:
      case -18109:
      case -18108:
      case -18107:
      case -18106:
      case -18105:
      case -18104:
      case -18103:
      case -18102:
      case -18101:
      case -18100:
      case -18099:
      case -18098:
      case -18097:
      case -18096:
      case -18095:
      case -18094:
      case -18093:
      case -18092:
      case -18091:
      case -18090:
      case -18089:
      case -18088:
      case -18087:
      case -18086:
      case -18085:
      case -18084:
      case -18083:
      case -18082:
      case -18081:
      case -18080:
      case -18079:
      case -18078:
      case -18077:
      case -18076:
      case -18075:
      case -18074:
      case -18073:
      case -18072:
      case -18071:
      case -18070:
      case -18069:
      case -18068:
      case -18067:
      case -18066:
      case -18065:
      case -18064:
      case -18063:
      case -18062:
      case -18061:
      case -18060:
      case -18059:
      case -18058:
      case -18057:
      case -18056:
      case -18055:
      case -18054:
      case -18053:
      case -18052:
      case -18051:
      case -18050:
      case -18049:
      case -18048:
      case -18047:
      case -18046:
      case -18045:
      case -18044:
      case -18043:
      case -18042:
      case -18041:
      case -18040:
      case -18039:
      case -18038:
      case -18037:
      case -18036:
      case -18035:
      case -18034:
      case -18033:
      case -18032:
      case -18031:
      case -18030:
      case -18029:
      case -18028:
      case -18027:
      case -18026:
      case -18025:
      case -18024:
      case -18023:
      case -18022:
      case -18021:
      case -18020:
      case -18019:
      case -18018:
      case -18017:
      case -18016:
      case -18015:
      case -18014:
      case -18013:
      case -18012:
      case -18011:
      case -18010:
      case -18009:
      case -18008:
      case -18007:
      case -18006:
      case -18005:
      case -18004:
      case -18003:
      case -18002:
      case -18001:
      case -18000:
      case -17999:
      case -17998:
      case -17997:
      case -17996:
      case -17995:
      case -17994:
      case -17993:
      case -17992:
      case -17991:
      case -17990:
      case -17989:
      case -17988:
      case -17987:
      case -17986:
      case -17985:
      case -17984:
      case -17983:
      case -17982:
      case -17981:
      case -17980:
      case -17979:
      case -17978:
      case -17977:
      case -17976:
      case -17975:
      case -17974:
      case -17973:
      case -17972:
      case -17971:
      case -17970:
      case -17969:
      case -17968:
      case -17967:
      case -17966:
      case -17965:
      case -17964:
      case -17963:
      case -17962:
      case -17961:
      case -17960:
      case -17959:
      case -17958:
      case -17957:
      case -17956:
      case -17955:
      case -17954:
      case -17953:
      case -17952:
      case -17951:
      case -17950:
      case -17949:
      case -17948:
      case -17947:
      case -17946:
      case -17945:
      case -17944:
      case -17943:
      case -17942:
      case -17941:
      case -17940:
      case -17939:
      case -17938:
      case -17937:
      case -17936:
      case -17935:
      case -17934:
      case -17933:
      case -17932:
      case -17931:
      case -17930:
      case -17929:
      case -17928:
      case -17927:
      case -17926:
      case -17925:
      case -17924:
      case -17923:
      case -17922:
      case -17921:
      case -17920:
      case -17919:
      case -17918:
      case -17917:
      case -17916:
      case -17915:
      case -17914:
      case -17913:
      case -17912:
      case -17911:
      case -17910:
      case -17909:
      case -17908:
      case -17907:
      case -17906:
      case -17905:
      case -17904:
      case -17903:
      case -17902:
      case -17901:
      case -17900:
      case -17899:
      case -17898:
      case -17897:
      case -17896:
      case -17895:
      case -17894:
      case -17893:
      case -17892:
      case -17891:
      case -17890:
      case -17889:
      case -17888:
      case -17887:
      case -17886:
      case -17885:
      case -17884:
      case -17883:
      case -17882:
      case -17881:
      case -17880:
      case -17879:
      case -17878:
      case -17877:
      case -17876:
      case -17875:
      case -17874:
      case -17873:
      case -17872:
      case -17871:
      case -17870:
      case -17869:
      case -17868:
      case -17867:
      case -17866:
      case -17865:
      case -17864:
      case -17863:
      case -17862:
      case -17861:
      case -17860:
      case -17859:
      case -17858:
      case -17857:
      case -17856:
      case -17855:
      case -17854:
      case -17853:
      case -17852:
      case -17851:
      case -17850:
      case -17849:
      case -17848:
      case -17847:
      case -17846:
      case -17845:
      case -17844:
      case -17843:
      case -17842:
      case -17841:
      case -17840:
      case -17839:
      case -17838:
      case -17837:
      case -17836:
      case -17835:
      case -17834:
      case -17833:
      case -17832:
      case -17831:
      case -17830:
      case -17829:
      case -17828:
      case -17827:
      case -17826:
      case -17825:
      case -17824:
      case -17823:
      case -17822:
      case -17821:
      case -17820:
      case -17819:
      case -17818:
      case -17817:
      case -17816:
      case -17815:
      case -17814:
      case -17813:
      case -17812:
      case -17811:
      case -17810:
      case -17809:
      case -17808:
      case -17807:
      case -17806:
      case -17805:
      case -17804:
      case -17803:
      case -17802:
      case -17801:
      case -17800:
      case -17799:
      case -17798:
      case -17797:
      case -17796:
      case -17795:
      case -17794:
      case -17793:
      case -17792:
      case -17791:
      case -17790:
      case -17789:
      case -17788:
      case -17787:
      case -17786:
      case -17785:
      case -17784:
      case -17783:
      case -17782:
      case -17781:
      case -17780:
      case -17779:
      case -17778:
      case -17777:
      case -17776:
      case -17775:
      case -17774:
      case -17773:
      case -17772:
      case -17771:
      case -17770:
      case -17769:
      case -17768:
      case -17767:
      case -17766:
      case -17765:
      case -17764:
      case -17763:
      case -17762:
      case -17761:
      case -17760:
      case -17759:
      case -17758:
      case -17757:
      case -17756:
      case -17755:
      case -17754:
      case -17753:
      case -17752:
      case -17751:
      case -17750:
      case -17749:
      case -17748:
      case -17747:
      case -17746:
      case -17745:
      case -17744:
      case -17743:
      case -17742:
      case -17741:
      case -17740:
      case -17739:
      case -17738:
      case -17737:
      case -17736:
      case -17735:
      case -17734:
      case -17733:
      case -17732:
      case -17731:
      case -17730:
      case -17729:
      case -17728:
      case -17727:
      case -17726:
      case -17725:
      case -17724:
      case -17723:
      case -17722:
      case -17721:
      case -17720:
      case -17719:
      case -17718:
      case -17717:
      case -17716:
      case -17715:
      case -17714:
      case -17713:
      case -17712:
      case -17711:
      case -17710:
      case -17709:
      case -17708:
      case -17707:
      case -17706:
      case -17705:
      case -17704:
      case -17703:
      case -17702:
      case -17701:
      case -17700:
      case -17699:
      case -17698:
      case -17697:
      case -17696:
      case -17695:
      case -17694:
      case -17693:
      case -17692:
      case -17691:
      case -17690:
      case -17689:
      case -17688:
      case -17687:
      case -17686:
      case -17685:
      case -17684:
      case -17683:
      case -17682:
      case -17681:
      case -17680:
      case -17679:
      case -17678:
      case -17677:
      case -17676:
      case -17675:
      case -17674:
      case -17673:
      case -17672:
      case -17671:
      case -17670:
      case -17669:
      case -17668:
      case -17667:
      case -17666:
      case -17665:
      case -17664:
      case -17663:
      case -17662:
      case -17661:
      case -17660:
      case -17659:
      case -17658:
      case -17657:
      case -17656:
      case -17655:
      case -17654:
      case -17653:
      case -17652:
      case -17651:
      case -17650:
      case -17649:
      case -17648:
      case -17647:
      case -17646:
      case -17645:
      case -17644:
      case -17643:
      case -17642:
      case -17641:
      case -17640:
      case -17639:
      case -17638:
      case -17637:
      case -17636:
      case -17635:
      case -17634:
      case -17633:
      case -17632:
      case -17631:
      case -17630:
      case -17629:
      case -17628:
      case -17627:
      case -17626:
      case -17625:
      case -17624:
      case -17623:
      case -17622:
      case -17621:
      case -17620:
      case -17619:
      case -17618:
      case -17617:
      case -17616:
      case -17615:
      case -17614:
      case -17613:
      case -17612:
      case -17611:
      case -17610:
      case -17609:
      case -17608:
      case -17607:
      case -17606:
      case -17605:
      case -17604:
      case -17603:
      case -17602:
      case -17601:
      case -17600:
      case -17599:
      case -17598:
      case -17597:
      case -17596:
      case -17595:
      case -17594:
      case -17593:
      case -17592:
      case -17591:
      case -17590:
      case -17589:
      case -17588:
      case -17587:
      case -17586:
      case -17585:
      case -17584:
      case -17583:
      case -17582:
      case -17581:
      case -17580:
      case -17579:
      case -17578:
      case -17577:
      case -17576:
      case -17575:
      case -17574:
      case -17573:
      case -17572:
      case -17571:
      case -17570:
      case -17569:
      case -17568:
      case -17567:
      case -17566:
      case -17565:
      case -17564:
      case -17563:
      case -17562:
      case -17561:
      case -17560:
      case -17559:
      case -17558:
      case -17557:
      case -17556:
      case -17555:
      case -17554:
      case -17553:
      case -17552:
      case -17551:
      case -17550:
      case -17549:
      case -17548:
      case -17547:
      case -17546:
      case -17545:
      case -17544:
      case -17543:
      case -17542:
      case -17541:
      case -17540:
      case -17539:
      case -17538:
      case -17537:
      case -17536:
      case -17535:
      case -17534:
      case -17533:
      case -17532:
      case -17531:
      case -17530:
      case -17529:
      case -17528:
      case -17527:
      case -17526:
      case -17525:
      case -17524:
      case -17523:
      case -17522:
      case -17521:
      case -17520:
      case -17519:
      case -17518:
      case -17517:
      case -17516:
      case -17515:
      case -17514:
      case -17513:
      case -17512:
      case -17511:
      case -17510:
      case -17509:
      case -17508:
      case -17507:
      case -17506:
      case -17505:
      case -17504:
      case -17503:
      case -17502:
      case -17501:
      case -17500:
      case -17499:
      case -17498:
      case -17497:
      case -17496:
      case -17495:
      case -17494:
      case -17493:
      case -17492:
      case -17491:
      case -17490:
      case -17489:
      case -17488:
      case -17487:
      case -17486:
      case -17485:
      case -17484:
      case -17483:
      case -17482:
      case -17481:
      case -17480:
      case -17479:
      case -17478:
      case -17477:
      case -17476:
      case -17475:
      case -17474:
      case -17473:
      case -17472:
      case -17471:
      case -17470:
      case -17469:
      case -17468:
      case -17467:
      case -17466:
      case -17465:
      case -17464:
      case -17463:
      case -17462:
      case -17461:
      case -17460:
      case -17459:
      case -17458:
      case -17457:
      case -17456:
      case -17455:
      case -17454:
      case -17453:
      case -17452:
      case -17451:
      case -17450:
      case -17449:
      case -17448:
      case -17447:
      case -17446:
      case -17445:
      case -17444:
      case -17443:
      case -17442:
      case -17441:
      case -17440:
      case -17439:
      case -17438:
      case -17437:
      case -17436:
      case -17435:
      case -17434:
      case -17433:
      case -17432:
      case -17431:
      case -17430:
      case -17429:
      case -17428:
      case -17427:
      case -17426:
      case -17425:
      case -17424:
      case -17423:
      case -17422:
      case -17421:
      case -17420:
      case -17419:
      case -17418:
      case -17417:
      case -17416:
      case -17415:
      case -17414:
      case -17413:
      case -17412:
      case -17411:
      case -17410:
      case -17409:
      case -17408:
      case -17407:
      case -17406:
      case -17405:
      case -17404:
      case -17403:
      case -17402:
      case -17401:
      case -17400:
      case -17399:
      case -17398:
      case -17397:
      case -17396:
      case -17395:
      case -17394:
      case -17393:
      case -17392:
      case -17391:
      case -17390:
      case -17389:
      case -17388:
      case -17387:
      case -17386:
      case -17385:
      case -17384:
      case -17383:
      case -17382:
      case -17381:
      case -17380:
      case -17379:
      case -17378:
      case -17377:
      case -17376:
      case -17375:
      case -17374:
      case -17373:
      case -17372:
      case -17371:
      case -17370:
      case -17369:
      case -17368:
      case -17367:
      case -17366:
      case -17365:
      case -17364:
      case -17363:
      case -17362:
      case -17361:
      case -17360:
      case -17359:
      case -17358:
      case -17357:
      case -17356:
      case -17355:
      case -17354:
      case -17353:
      case -17352:
      case -17351:
      case -17350:
      case -17349:
      case -17348:
      case -17347:
      case -17346:
      case -17345:
      case -17344:
      case -17343:
      case -17342:
      case -17341:
      case -17340:
      case -17339:
      case -17338:
      case -17337:
      case -17336:
      case -17335:
      case -17334:
      case -17333:
      case -17332:
      case -17331:
      case -17330:
      case -17329:
      case -17328:
      case -17327:
      case -17326:
      case -17325:
      case -17324:
      case -17323:
      case -17322:
      case -17321:
      case -17320:
      case -17319:
      case -17318:
      case -17317:
      case -17316:
      case -17315:
      case -17314:
      case -17313:
      case -17312:
      case -17311:
      case -17310:
      case -17309:
      case -17308:
      case -17307:
      case -17306:
      case -17305:
      case -17304:
      case -17303:
      case -17302:
      case -17301:
      case -17300:
      case -17299:
      case -17298:
      case -17297:
      case -17296:
      case -17295:
      case -17294:
      case -17293:
      case -17292:
      case -17291:
      case -17290:
      case -17289:
      case -17288:
      case -17287:
      case -17286:
      case -17285:
      case -17284:
      case -17283:
      case -17282:
      case -17281:
      case -17280:
      case -17279:
      case -17278:
      case -17277:
      case -17276:
      case -17275:
      case -17274:
      case -17273:
      case -17272:
      case -17271:
      case -17270:
      case -17269:
      case -17268:
      case -17267:
      case -17266:
      case -17265:
      case -17264:
      case -17263:
      case -17262:
      case -17261:
      case -17260:
      case -17259:
      case -17258:
      case -17257:
      case -17256:
      case -17255:
      case -17254:
      case -17253:
      case -17252:
      case -17251:
      case -17250:
      case -17249:
      case -17248:
      case -17247:
      case -17246:
      case -17245:
      case -17244:
      case -17243:
      case -17242:
      case -17241:
      case -17240:
      case -17239:
      case -17238:
      case -17237:
      case -17236:
      case -17235:
      case -17234:
      case -17233:
      case -17232:
      case -17231:
      case -17230:
      case -17229:
      case -17228:
      case -17227:
      case -17226:
      case -17225:
      case -17224:
      case -17223:
      case -17222:
      case -17221:
      case -17220:
      case -17219:
      case -17218:
      case -17217:
      case -17216:
      case -17215:
      case -17214:
      case -17213:
      case -17212:
      case -17211:
      case -17210:
      case -17209:
      case -17208:
      case -17207:
      case -17206:
      case -17205:
      case -17204:
      case -17203:
      case -17202:
      case -17201:
      case -17200:
      case -17199:
      case -17198:
      case -17197:
      case -17196:
      case -17195:
      case -17194:
      case -17193:
      case -17192:
      case -17191:
      case -17190:
      case -17189:
      case -17188:
      case -17187:
      case -17186:
      case -17185:
      case -17184:
      case -17183:
      case -17182:
      case -17181:
      case -17180:
      case -17179:
      case -17178:
      case -17177:
      case -17176:
      case -17175:
      case -17174:
      case -17173:
      case -17172:
      case -17171:
      case -17170:
      case -17169:
      case -17168:
      case -17167:
      case -17166:
      case -17165:
      case -17164:
      case -17163:
      case -17162:
      case -17161:
      case -17160:
      case -17159:
      case -17158:
      case -17157:
      case -17156:
      case -17155:
      case -17154:
      case -17153:
      case -17152:
      case -17151:
      case -17150:
      case -17149:
      case -17148:
      case -17147:
      case -17146:
      case -17145:
      case -17144:
      case -17143:
      case -17142:
      case -17141:
      case -17140:
      case -17139:
      case -17138:
      case -17137:
      case -17136:
      case -17135:
      case -17134:
      case -17133:
      case -17132:
      case -17131:
      case -17130:
      case -17129:
      case -17128:
      case -17127:
      case -17126:
      case -17125:
      case -17124:
      case -17123:
      case -17122:
      case -17121:
      case -17120:
      case -17119:
      case -17118:
      case -17117:
      case -17116:
      case -17115:
      case -17114:
      case -17113:
      case -17112:
      case -17111:
      case -17110:
      case -17109:
      case -17108:
      case -17107:
      case -17106:
      case -17105:
      case -17104:
      case -17103:
      case -17102:
      case -17101:
      case -17100:
      case -17099:
      case -17098:
      case -17097:
      case -17096:
      case -17095:
      case -17094:
      case -17093:
      case -17092:
      case -17091:
      case -17090:
      case -17089:
      case -17088:
      case -17087:
      case -17086:
      case -17085:
      case -17084:
      case -17083:
      case -17082:
      case -17081:
      case -17080:
      case -17079:
      case -17078:
      case -17077:
      case -17076:
      case -17075:
      case -17074:
      case -17073:
      case -17072:
      case -17071:
      case -17070:
      case -17069:
      case -17068:
      case -17067:
      case -17066:
      case -17065:
      case -17064:
      case -17063:
      case -17062:
      case -17061:
      case -17060:
      case -17059:
      case -17058:
      case -17057:
      case -17056:
      case -17055:
      case -17054:
      case -17053:
      case -17052:
      case -17051:
      case -17050:
      case -17049:
      case -17048:
      case -17047:
      case -17046:
      case -17045:
      case -17044:
      case -17043:
      case -17042:
      case -17041:
      case -17040:
      case -17039:
      case -17038:
      case -17037:
      case -17036:
      case -17035:
      case -17034:
      case -17033:
      case -17032:
      case -17031:
      case -17030:
      case -17029:
      case -17028:
      case -17027:
      case -17026:
      case -17025:
      case -17024:
      case -17023:
      case -17022:
      case -17021:
      case -17020:
      case -17019:
      case -17018:
      case -17017:
      case -17016:
      case -17015:
      case -17014:
      case -17013:
      case -17012:
      case -17011:
      case -17010:
      case -17009:
      case -17008:
      case -17007:
      case -17006:
      case -17005:
      case -17004:
      case -17003:
      case -17002:
      case -17001:
      case -17000:
      case -16999:
      case -16998:
      case -16997:
      case -16996:
      case -16995:
      case -16994:
      case -16993:
      case -16992:
      case -16991:
      case -16990:
      case -16989:
      case -16988:
      case -16987:
      case -16986:
      case -16985:
      case -16984:
      case -16983:
      case -16982:
      case -16981:
      case -16980:
      case -16979:
      case -16978:
      case -16977:
      case -16976:
      case -16975:
      case -16974:
      case -16973:
      case -16972:
      case -16971:
      case -16970:
      case -16969:
      case -16968:
      case -16967:
      case -16966:
      case -16965:
      case -16964:
      case -16963:
      case -16962:
      case -16961:
      case -16960:
      case -16959:
      case -16958:
      case -16957:
      case -16956:
      case -16955:
      case -16954:
      case -16953:
      case -16952:
      case -16951:
      case -16950:
      case -16949:
      case -16948:
      case -16947:
      case -16946:
      case -16945:
      case -16944:
      case -16943:
      case -16942:
      case -16941:
      case -16940:
      case -16939:
      case -16938:
      case -16937:
      case -16936:
      case -16935:
      case -16934:
      case -16933:
      case -16932:
      case -16931:
      case -16930:
      case -16929:
      case -16928:
      case -16927:
      case -16926:
      case -16925:
      case -16924:
      case -16923:
      case -16922:
      case -16921:
      case -16920:
      case -16919:
      case -16918:
      case -16917:
      case -16916:
      case -16915:
      case -16914:
      case -16913:
      case -16912:
      case -16911:
      case -16910:
      case -16909:
      case -16908:
      case -16907:
      case -16906:
      case -16905:
      case -16904:
      case -16903:
      case -16902:
      case -16901:
      case -16900:
      case -16899:
      case -16898:
      case -16897:
      case -16896:
      case -16895:
      case -16894:
      case -16893:
      case -16892:
      case -16891:
      case -16890:
      case -16889:
      case -16888:
      case -16887:
      case -16886:
      case -16885:
      case -16884:
      case -16883:
      case -16882:
      case -16881:
      case -16880:
      case -16879:
      case -16878:
      case -16877:
      case -16876:
      case -16875:
      case -16874:
      case -16873:
      case -16872:
      case -16871:
      case -16870:
      case -16869:
      case -16868:
      case -16867:
      case -16866:
      case -16865:
      case -16864:
      case -16863:
      case -16862:
      case -16861:
      case -16860:
      case -16859:
      case -16858:
      case -16857:
      case -16856:
      case -16855:
      case -16854:
      case -16853:
      case -16852:
      case -16851:
      case -16850:
      case -16849:
      case -16848:
      case -16847:
      case -16846:
      case -16845:
      case -16844:
      case -16843:
      case -16842:
      case -16841:
      case -16840:
      case -16839:
      case -16838:
      case -16837:
      case -16836:
      case -16835:
      case -16834:
      case -16833:
      case -16832:
      case -16831:
      case -16830:
      case -16829:
      case -16828:
      case -16827:
      case -16826:
      case -16825:
      case -16824:
      case -16823:
      case -16822:
      case -16821:
      case -16820:
      case -16819:
      case -16818:
      case -16817:
      case -16816:
      case -16815:
      case -16814:
      case -16813:
      case -16812:
      case -16811:
      case -16810:
      case -16809:
      case -16808:
      case -16807:
      case -16806:
      case -16805:
      case -16804:
      case -16803:
      case -16802:
      case -16801:
      case -16800:
      case -16799:
      case -16798:
      case -16797:
      case -16796:
      case -16795:
      case -16794:
      case -16793:
      case -16792:
      case -16791:
      case -16790:
      case -16789:
      case -16788:
      case -16787:
      case -16786:
      case -16785:
      case -16784:
      case -16783:
      case -16782:
      case -16781:
      case -16780:
      case -16779:
      case -16778:
      case -16777:
      case -16776:
      case -16775:
      case -16774:
      case -16773:
      case -16772:
      case -16771:
      case -16770:
      case -16769:
      case -16768:
      case -16767:
      case -16766:
      case -16765:
      case -16764:
      case -16763:
      case -16762:
      case -16761:
      case -16760:
      case -16759:
      case -16758:
      case -16757:
      case -16756:
      case -16755:
      case -16754:
      case -16753:
      case -16752:
      case -16751:
      case -16750:
      case -16749:
      case -16748:
      case -16747:
      case -16746:
      case -16745:
      case -16744:
      case -16743:
      case -16742:
      case -16741:
      case -16740:
      case -16739:
      case -16738:
      case -16737:
      case -16736:
      case -16735:
      case -16734:
      case -16733:
      case -16732:
      case -16731:
      case -16730:
      case -16729:
      case -16728:
      case -16727:
      case -16726:
      case -16725:
      case -16724:
      case -16723:
      case -16722:
      case -16721:
      case -16720:
      case -16719:
      case -16718:
      case -16717:
      case -16716:
      case -16715:
      case -16714:
      case -16713:
      case -16712:
      case -16711:
      case -16710:
      case -16709:
      case -16708:
      case -16707:
      case -16706:
      case -16705:
      case -16704:
      case -16703:
      case -16702:
      case -16701:
      case -16700:
      case -16699:
      case -16698:
      case -16697:
      case -16696:
      case -16695:
      case -16694:
      case -16693:
      case -16692:
      case -16691:
      case -16690:
      case -16689:
      case -16688:
      case -16687:
      case -16686:
      case -16685:
      case -16684:
      case -16683:
      case -16682:
      case -16681:
      case -16680:
      case -16679:
      case -16678:
      case -16677:
      case -16676:
      case -16675:
      case -16674:
      case -16673:
      case -16672:
      case -16671:
      case -16670:
      case -16669:
      case -16668:
      case -16667:
      case -16666:
      case -16665:
      case -16664:
      case -16663:
      case -16662:
      case -16661:
      case -16660:
      case -16659:
      case -16658:
      case -16657:
      case -16656:
      case -16655:
      case -16654:
      case -16653:
      case -16652:
      case -16651:
      case -16650:
      case -16649:
      case -16648:
      case -16647:
      case -16646:
      case -16645:
      case -16644:
      case -16643:
      case -16642:
      case -16641:
      case -16640:
      case -16639:
      case -16638:
      case -16637:
      case -16636:
      case -16635:
      case -16634:
      case -16633:
      case -16632:
      case -16631:
      case -16630:
      case -16629:
      case -16628:
      case -16627:
      case -16626:
      case -16625:
      case -16624:
      case -16623:
      case -16622:
      case -16621:
      case -16620:
      case -16619:
      case -16618:
      case -16617:
      case -16616:
      case -16615:
      case -16614:
      case -16613:
      case -16612:
      case -16611:
      case -16610:
      case -16609:
      case -16608:
      case -16607:
      case -16606:
      case -16605:
      case -16604:
      case -16603:
      case -16602:
      case -16601:
      case -16600:
      case -16599:
      case -16598:
      case -16597:
      case -16596:
      case -16595:
      case -16594:
      case -16593:
      case -16592:
      case -16591:
      case -16590:
      case -16589:
      case -16588:
      case -16587:
      case -16586:
      case -16585:
      case -16584:
      case -16583:
      case -16582:
      case -16581:
      case -16580:
      case -16579:
      case -16578:
      case -16577:
      case -16576:
      case -16575:
      case -16574:
      case -16573:
      case -16572:
      case -16571:
      case -16570:
      case -16569:
      case -16568:
      case -16567:
      case -16566:
      case -16565:
      case -16564:
      case -16563:
      case -16562:
      case -16561:
      case -16560:
      case -16559:
      case -16558:
      case -16557:
      case -16556:
      case -16555:
      case -16554:
      case -16553:
      case -16552:
      case -16551:
      case -16550:
      case -16549:
      case -16548:
      case -16547:
      case -16546:
      case -16545:
      case -16544:
      case -16543:
      case -16542:
      case -16541:
      case -16540:
      case -16539:
      case -16538:
      case -16537:
      case -16536:
      case -16535:
      case -16534:
      case -16533:
      case -16532:
      case -16531:
      case -16530:
      case -16529:
      case -16528:
      case -16527:
      case -16526:
      case -16525:
      case -16524:
      case -16523:
      case -16522:
      case -16521:
      case -16520:
      case -16519:
      case -16518:
      case -16517:
      case -16516:
      case -16515:
      case -16514:
      case -16513:
      case -16512:
      case -16511:
      case -16510:
      case -16509:
      case -16508:
      case -16507:
      case -16506:
      case -16505:
      case -16504:
      case -16503:
      case -16502:
      case -16501:
      case -16500:
      case -16499:
      case -16498:
      case -16497:
      case -16496:
      case -16495:
      case -16494:
      case -16493:
      case -16492:
      case -16491:
      case -16490:
      case -16489:
      case -16488:
      case -16487:
      case -16486:
      case -16485:
      case -16484:
      case -16483:
      case -16482:
      case -16481:
      case -16480:
      case -16479:
      case -16478:
      case -16477:
      case -16476:
      case -16475:
      case -16474:
      case -16473:
      case -16472:
      case -16471:
      case -16470:
      case -16469:
      case -16468:
      case -16467:
      case -16466:
      case -16465:
      case -16464:
      case -16463:
      case -16462:
      case -16461:
      case -16460:
      case -16459:
      case -16458:
      case -16457:
      case -16456:
      case -16455:
      case -16454:
      case -16453:
      case -16452:
      case -16451:
      case -16450:
      case -16449:
      case -16448:
      case -16447:
      case -16446:
      case -16445:
      case -16444:
      case -16443:
      case -16442:
      case -16441:
      case -16440:
      case -16439:
      case -16438:
      case -16437:
      case -16436:
      case -16435:
      case -16434:
      case -16433:
      case -16432:
      case -16431:
      case -16430:
      case -16429:
      case -16428:
      case -16427:
      case -16426:
      case -16425:
      case -16424:
      case -16423:
      case -16422:
      case -16421:
      case -16420:
      case -16419:
      case -16418:
      case -16417:
      case -16416:
      case -16415:
      case -16414:
      case -16413:
      case -16412:
      case -16411:
      case -16410:
      case -16409:
      case -16408:
      case -16407:
      case -16406:
      case -16405:
      case -16404:
      case -16403:
      case -16402:
      case -16401:
      case -16400:
      case -16399:
      case -16398:
      case -16397:
      case -16396:
      case -16395:
      case -16394:
      case -16393:
      case -16392:
      case -16391:
      case -16390:
      case -16389:
      case -16388:
      case -16387:
      case -16386:
      case -16385:
        return 0;
      case -16384:
      case -16383:
      case -16382:
      case -16381:
      case -16380:
      case -16379:
      case -16378:
      case -16377:
      case -16376:
      case -16375:
      case -16374:
      case -16373:
      case -16230:
      case -16229:
      case -16213:
      case -16212:
      case -16211:
      case -16210:
      case -16209:
      case -16131:
      case -16124:
      case -15995:
      case -15994:
      case -15993:
      case -15992:
      case -15977:
      case -15976:
      case -15960:
      case -15959:
      case -15958:
      case -15878:
      case -15877:
      case -15876:
      case -15875:
      case -15802:
      case -15800:
      case -15798:
      case -15756:
      case -15755:
      case -15474:
      case -15473:
      case -15472:
      case -15471:
      case -15470:
      case -15469:
      case -14966:
      case -14965:
      case -14964:
      case -14804:
        result = 21;
        break;
      case -16372:
      case -16371:
      case -16370:
      case -16369:
      case -16368:
      case -16367:
      case -16366:
      case -16228:
      case -16208:
      case -16122:
      case -16113:
      case -16104:
      case -16103:
      case -16102:
      case -16098:
      case -15914:
      case -15904:
      case -15874:
      case -15768:
      case -15767:
      case -15505:
      case -15504:
      case -15357:
      case -14840:
        goto LABEL_25;
      case -16365:
      case -16364:
      case -16363:
      case -16362:
      case -16361:
      case -16360:
      case -16359:
      case -16358:
      case -16357:
      case -16356:
      case -16355:
      case -16354:
      case -16353:
      case -16352:
      case -16351:
      case -16350:
      case -16202:
      case -16195:
      case -16164:
      case -16151:
      case -16133:
      case -16065:
      case -16064:
      case -16014:
      case -16013:
      case -16012:
      case -15991:
      case -15990:
      case -15967:
      case -15966:
      case -15908:
      case -15899:
      case -15898:
      case -15897:
      case -15873:
      case -15872:
      case -15871:
      case -15870:
      case -15749:
      case -15712:
      case -15703:
      case -15673:
      case -15587:
      case -15586:
      case -15585:
      case -15584:
      case -15533:
      case -15501:
      case -15500:
      case -15410:
      case -15364:
      case -15326:
      case -15318:
      case -15316:
      case -15286:
      case -15285:
      case -15256:
      case -15255:
      case -15254:
      case -15253:
      case -15217:
      case -15216:
      case -15215:
      case -14800:
      case -14799:
        goto LABEL_15;
      case -16349:
      case -16348:
      case -16347:
      case -16346:
      case -16345:
      case -16344:
      case -16343:
      case -16342:
      case -16341:
      case -16340:
      case -16134:
      case -15811:
      case -15810:
      case -15809:
      case -15808:
      case -15807:
      case -15806:
      case -15805:
      case -15804:
      case -15701:
      case -15624:
      case -15252:
      case -15251:
      case -15250:
      case -15249:
LABEL_95:
        result = 27;
        break;
      case -16339:
      case -16338:
      case -16337:
      case -16336:
      case -16335:
      case -16334:
      case -16333:
      case -16332:
      case -16331:
      case -16330:
      case -16329:
      case -16328:
      case -16327:
      case -16326:
      case -16325:
      case -16324:
      case -16323:
      case -16322:
      case -16227:
      case -16226:
      case -16220:
      case -16207:
      case -16150:
      case -16149:
      case -16130:
      case -16129:
      case -16100:
      case -16079:
      case -16078:
      case -16047:
      case -16046:
      case -16045:
      case -16044:
      case -16017:
      case -16011:
      case -16010:
      case -16009:
      case -16008:
      case -15965:
      case -15964:
      case -15963:
      case -15869:
      case -15754:
      case -15732:
      case -15711:
      case -15710:
      case -15709:
      case -15623:
      case -15622:
      case -15621:
      case -15583:
      case -15582:
      case -15581:
      case -15580:
      case -15518:
      case -15517:
      case -15516:
      case -15515:
      case -15514:
      case -15513:
      case -15512:
      case -15511:
      case -15510:
      case -15503:
      case -15284:
      case -15283:
      case -15282:
      case -15281:
      case -15280:
      case -15279:
      case -15278:
      case -15248:
      case -15247:
      case -14963:
      case -14962:
      case -14854:
        result = 28;
        break;
      case -16321:
      case -16092:
      case -15989:
      case -15868:
      case -15700:
      case -15699:
      case -15620:
      case -15619:
      case -15618:
      case -15579:
      case -15578:
      case -15541:
      case -15540:
      case -14883:
      case -14882:
      case -14839:
        goto LABEL_41;
      case -16320:
        goto LABEL_81;
      case -16293:
      case -16135:
      case -16123:
      case -16052:
      case -16051:
      case -15830:
      case -15715:
      case -15714:
      case -15713:
      case -15670:
      case -15669:
        goto LABEL_50;
      case -16292:
      case -16291:
      case -16290:
      case -16107:
      case -16106:
      case -16083:
      case -16082:
      case -16081:
      case -16038:
      case -15772:
      case -15609:
      case -15608:
      case -15551:
      case -15550:
      case -15487:
      case -15423:
      case -15323:
      case -15322:
      case -14850:
        goto LABEL_39;
      case -16289:
      case -16288:
      case -16287:
      case -16286:
      case -16285:
      case -16284:
      case -16283:
      case -16282:
      case -16162:
      case -16161:
      case -16160:
      case -16028:
      case -16027:
      case -16026:
      case -14956:
      case -14955:
        result = 65;
        break;
      case -16281:
      case -16280:
      case -16279:
      case -16278:
      case -16277:
      case -16276:
      case -16275:
      case -16274:
      case -16273:
      case -16272:
      case -16271:
      case -16270:
      case -16269:
      case -16268:
      case -16267:
      case -16266:
      case -16265:
      case -16264:
      case -16263:
      case -16262:
      case -16261:
      case -16260:
      case -16259:
      case -16258:
      case -16257:
      case -16256:
      case -16255:
      case -16254:
      case -16253:
      case -16252:
      case -16251:
      case -16250:
      case -16249:
      case -16248:
      case -16247:
      case -16246:
      case -16245:
      case -16244:
      case -16243:
      case -16225:
      case -16224:
      case -16223:
      case -16222:
      case -16219:
      case -16218:
      case -16217:
      case -16216:
      case -16215:
      case -16214:
      case -16204:
      case -16203:
      case -16201:
      case -16159:
      case -16158:
      case -16157:
      case -16156:
      case -16155:
      case -16148:
      case -16147:
      case -16146:
      case -16145:
      case -16144:
      case -16143:
      case -16142:
      case -16141:
      case -16140:
      case -16139:
      case -16138:
      case -16137:
      case -16132:
      case -16126:
      case -16125:
      case -16087:
      case -16086:
      case -16085:
      case -16084:
      case -16080:
      case -16077:
      case -16076:
      case -16074:
      case -16043:
      case -16042:
      case -16041:
      case -16040:
      case -16022:
      case -16021:
      case -16020:
      case -16007:
      case -16006:
      case -16005:
      case -16004:
      case -16003:
      case -15988:
      case -15987:
      case -15986:
      case -15985:
      case -15984:
      case -15970:
      case -15962:
      case -15961:
      case -15823:
      case -15822:
      case -15821:
      case -15820:
      case -15819:
      case -15818:
      case -15812:
      case -15750:
      case -15729:
      case -15728:
      case -15727:
      case -15726:
      case -15725:
      case -15724:
      case -15723:
      case -15722:
      case -15721:
      case -15720:
      case -15507:
      case -15486:
      case -15485:
      case -15484:
      case -15483:
      case -15482:
      case -15481:
      case -15480:
      case -15479:
      case -15478:
      case -15477:
      case -15476:
      case -15459:
      case -15458:
      case -15457:
      case -15456:
      case -15455:
      case -15454:
      case -15453:
      case -15452:
      case -15451:
      case -15450:
      case -15449:
      case -15448:
      case -15447:
      case -15446:
      case -15445:
      case -15416:
      case -15415:
      case -15414:
      case -15401:
      case -15400:
      case -15399:
      case -15398:
      case -15397:
      case -15396:
      case -15395:
      case -15394:
      case -15393:
      case -15392:
      case -15391:
      case -15390:
      case -15389:
      case -15388:
      case -15387:
      case -15386:
      case -15385:
      case -15384:
      case -15383:
      case -15382:
      case -15381:
      case -15297:
      case -15275:
      case -15274:
      case -15273:
      case -15272:
      case -15244:
      case -15243:
      case -15203:
      case -14954:
      case -14953:
      case -14914:
      case -14849:
        result = 70;
        break;
      case -16242:
      case -15817:
      case -15463:
      case -15462:
      case -15461:
      case -15460:
      case -15444:
      case -15443:
      case -15442:
      case -15441:
      case -15440:
      case -15439:
      case -15438:
      case -15437:
      case -15436:
      case -15413:
      case -15380:
      case -15379:
      case -15378:
      case -15377:
      case -15376:
      case -15375:
      case -15374:
      case -15373:
      case -15372:
      case -15371:
      case -15370:
      case -15369:
      case -15368:
      case -15367:
      case -15319:
      case -15288:
      case -14881:
      case -14880:
        goto LABEL_21;
      case -16241:
      case -15983:
      case -15816:
        goto LABEL_71;
      case -16240:
      case -16239:
      case -16238:
      case -16237:
      case -16236:
      case -16235:
      case -16234:
      case -16233:
      case -16232:
      case -16231:
      case -16221:
      case -16196:
      case -16114:
      case -16108:
      case -16094:
      case -15981:
      case -15913:
      case -15910:
      case -15903:
      case -15894:
      case -15815:
      case -15775:
      case -15761:
      case -15760:
      case -15690:
      case -15689:
      case -15688:
      case -15534:
      case -14801:
        goto LABEL_7;
      case -16194:
      case -16193:
      case -16192:
      case -16191:
      case -16190:
      case -16189:
      case -16188:
      case -16187:
      case -16186:
      case -16185:
      case -16184:
      case -16183:
      case -16182:
      case -16181:
      case -16180:
      case -16179:
      case -16178:
      case -16177:
      case -16176:
      case -16175:
      case -16174:
      case -16173:
      case -16172:
      case -16171:
      case -16170:
      case -16169:
      case -16168:
      case -16167:
      case -16154:
      case -16153:
      case -16152:
      case -16025:
      case -16024:
      case -16023:
        result = 42;
        break;
      case -16136:
      case -16127:
        result = 41;
        break;
      case -16119:
      case -16118:
      case -16117:
      case -16116:
      case -16115:
      case -16110:
      case -16109:
      case -16105:
      case -15829:
      case -15828:
      case -15778:
      case -15777:
      case -15356:
      case -15355:
      case -15354:
      case -15353:
      case -15352:
      case -15351:
        goto LABEL_40;
      case -16097:
      case -16032:
        result = 81;
        break;
      case -16073:
      case -16072:
      case -16071:
      case -16070:
        result = 24;
        break;
      case -16069:
      case -16068:
      case -16067:
      case -16034:
      case -16019:
      case -15736:
      case -15735:
      case -15734:
      case -15733:
        goto LABEL_52;
      case -16063:
      case -16062:
      case -16061:
      case -16060:
      case -16059:
      case -16058:
      case -16057:
      case -16056:
      case -16055:
      case -16054:
      case -16053:
      case -16033:
      case -16002:
      case -16001:
      case -16000:
      case -15999:
      case -15916:
      case -15915:
      case -15627:
      case -15626:
      case -15625:
      case -15549:
      case -15548:
      case -15547:
      case -15546:
      case -15539:
      case -15538:
      case -15537:
      case -15536:
      case -15205:
      case -15204:
        result = 18;
        break;
      case -15982:
        result = 78;
        break;
      case -15975:
      case -15974:
      case -15973:
      case -15911:
      case -15907:
      case -15422:
      case -15321:
        result = 76;
        break;
      case -15957:
      case -15956:
      case -15955:
      case -15954:
      case -15953:
      case -15952:
      case -15951:
      case -15950:
      case -15949:
      case -15948:
      case -15947:
      case -15946:
      case -15945:
      case -15944:
      case -15943:
      case -15942:
      case -15941:
      case -15940:
      case -15939:
      case -15938:
      case -15937:
      case -15936:
      case -15906:
      case -15276:
      case -15266:
      case -15265:
      case -15264:
      case -15263:
      case -15262:
      case -14928:
      case -14927:
      case -14926:
      case -14925:
      case -14924:
      case -14923:
      case -14922:
      case -14921:
      case -14920:
      case -14919:
      case -14918:
      case -14917:
      case -14916:
      case -14915:
      case -14837:
        result = 59;
        break;
      case -15935:
      case -15934:
      case -15933:
      case -15932:
      case -15931:
      case -15930:
      case -15929:
      case -15928:
      case -15927:
      case -15926:
      case -15905:
      case -15827:
      case -15826:
      case -15825:
      case -15824:
      case -15773:
      case -15277:
        result = 58;
        break;
      case -15925:
      case -15924:
      case -15923:
      case -15922:
      case -15921:
      case -15920:
      case -15919:
      case -15918:
      case -15917:
      case -15896:
      case -15895:
      case -15766:
      case -15765:
      case -15764:
      case -15763:
      case -15762:
        goto LABEL_36;
      case -15893:
        result = 64;
        break;
      case -15890:
      case -15889:
      case -15888:
      case -15887:
      case -15886:
      case -15885:
      case -15884:
      case -15883:
      case -15882:
      case -15881:
      case -14930:
      case -14929:
        result = 82;
        break;
      case -15880:
      case -15879:
      case -15325:
        result = 38;
        break;
      case -15867:
      case -15753:
        result = 31;
        break;
      case -15866:
      case -15865:
        result = 35;
        break;
      case -15803:
      case -15801:
      case -15799:
      case -15797:
      case -15795:
      case -15776:
      case -15771:
      case -15770:
      case -15412:
      case -15411:
        result = 68;
        break;
      case -15794:
      case -15792:
      case -15790:
      case -15789:
      case -15788:
      case -15787:
      case -15786:
      case -15785:
      case -15784:
      case -15783:
      case -15782:
      case -15781:
      case -15271:
      case -14952:
      case -14951:
        goto LABEL_47;
      case -15769:
        result = 73;
        break;
      case -15752:
        result = 32;
        break;
      case -15748:
        result = 30;
        break;
      case -15745:
        result = 66;
        break;
      case -15744:
      case -15743:
      case -15742:
      case -15741:
      case -15740:
      case -15739:
      case -15738:
        result = 67;
        break;
      case -15702:
        result = 26;
        break;
      case -15692:
        result = 45;
        break;
      case -15691:
        result = 79;
        break;
      case -15687:
      case -15686:
      case -15685:
      case -15684:
      case -15668:
      case -15667:
      case -15666:
      case -15665:
      case -15664:
      case -15663:
      case -15662:
      case -15555:
      case -15554:
      case -15553:
      case -15426:
      case -15409:
      case -15408:
      case -15407:
      case -15327:
        result = 19;
        break;
      case -15683:
      case -15682:
      case -15681:
      case -15680:
      case -15679:
      case -15678:
      case -15677:
        result = 20;
        break;
      case -15676:
      case -15675:
      case -15674:
      case -15611:
        goto LABEL_70;
      case -15661:
      case -15660:
      case -15659:
      case -15658:
      case -15657:
      case -15656:
      case -15655:
      case -15654:
      case -15653:
      case -15652:
      case -15651:
      case -15650:
      case -15649:
      case -15552:
        goto LABEL_32;
      case -15648:
      case -15647:
      case -15646:
      case -15645:
      case -15644:
      case -15643:
      case -15642:
      case -15641:
      case -15640:
      case -15639:
      case -15638:
      case -15637:
      case -15636:
      case -15635:
      case -15634:
      case -15633:
      case -15632:
      case -15631:
      case -15630:
      case -15629:
      case -15628:
      case -15595:
      case -15594:
      case -15593:
      case -15592:
      case -15591:
      case -15590:
      case -15589:
      case -15588:
      case -15524:
      case -15523:
      case -15522:
      case -15521:
      case -15520:
      case -15519:
      case -15502:
      case -15466:
      case -15465:
      case -15366:
      case -15365:
      case -15287:
      case -14968:
      case -14967:
        goto LABEL_18;
      case -15610:
        goto LABEL_80;
      case -15607:
      case -15606:
      case -15605:
      case -15604:
      case -15603:
      case -15602:
      case -15601:
      case -15600:
      case -15599:
      case -15598:
      case -15597:
      case -15596:
      case -15574:
      case -15573:
      case -15543:
      case -15542:
      case -15528:
      case -15527:
        goto LABEL_35;
      case -15572:
      case -15571:
      case -15570:
      case -15569:
      case -15568:
      case -15567:
      case -15566:
      case -15565:
      case -15564:
      case -15563:
      case -15562:
      case -15561:
      case -15560:
      case -15559:
      case -15558:
      case -15557:
      case -15556:
      case -15545:
      case -15544:
      case -15530:
      case -15529:
      case -15526:
      case -15525:
      case -15258:
      case -15257:
        goto LABEL_27;
      case -15508:
        result = 69;
        break;
      case -15495:
        result = 46;
        break;
      case -15494:
        result = 47;
        break;
      case -15493:
        result = 48;
        break;
      case -15492:
        result = 49;
        break;
      case -15491:
      case -15490:
      case -15489:
      case -15488:
      case -15424:
      case -15324:
        result = 61;
        break;
      case -15346:
      case -15345:
      case -15344:
      case -15343:
      case -15342:
      case -15341:
      case -15340:
      case -15339:
      case -15338:
      case -15337:
      case -15336:
      case -15335:
      case -15313:
      case -15312:
      case -15311:
      case -15310:
      case -15309:
      case -15308:
      case -15295:
      case -15294:
      case -15293:
      case -15292:
        result = 51;
        break;
      case -15334:
      case -15333:
      case -15332:
      case -15307:
      case -15306:
      case -15291:
        result = 52;
        break;
      case -15331:
        result = 55;
        break;
      case -15330:
      case -15329:
      case -15299:
        result = 56;
        break;
      case -15328:
      case -15305:
      case -15304:
      case -15303:
      case -15302:
      case -15301:
      case -15300:
        result = 54;
        break;
      case -15320:
        result = 33;
        break;
      case -15290:
      case -15289:
        result = 53;
        break;
      case -15242:
      case -15241:
      case -15240:
      case -15239:
      case -15238:
      case -15237:
      case -15236:
      case -15235:
      case -15234:
      case -15233:
      case -15232:
      case -15231:
      case -15230:
      case -15229:
      case -15228:
      case -15227:
      case -15226:
      case -15225:
      case -15224:
      case -15223:
      case -15222:
      case -15221:
      case -15220:
      case -15219:
      case -15218:
      case -15212:
      case -15211:
      case -15210:
      case -15209:
      case -15208:
      case -15207:
      case -15206:
        result = 72;
        break;
      case -15213:
        result = 63;
        break;
      case -15188:
      case -15187:
      case -15186:
      case -15185:
      case -15184:
      case -15183:
      case -14879:
      case -14878:
      case -14877:
      case -14876:
      case -14824:
      case -14817:
        result = 87;
        break;
      case -15182:
      case -15181:
      case -15180:
      case -15179:
      case -15178:
      case -14848:
        result = 88;
        break;
      case -15177:
        result = 89;
        break;
      case -15176:
      case -15175:
      case -15174:
        result = 90;
        break;
      case -15173:
      case -15172:
      case -15171:
      case -15170:
      case -15169:
      case -15168:
      case -15167:
      case -15166:
      case -15165:
      case -15164:
      case -15163:
      case -15162:
      case -14907:
      case -14884:
      case -14875:
      case -14874:
      case -14798:
        result = 91;
        break;
      case -15161:
      case -15160:
      case -15159:
      case -15158:
      case -15157:
      case -15156:
      case -15155:
      case -15154:
      case -15153:
      case -15152:
      case -15151:
      case -15150:
      case -15149:
      case -15148:
      case -15147:
      case -15146:
      case -14906:
      case -14873:
      case -14872:
      case -14847:
      case -14846:
      case -14845:
      case -14844:
      case -14843:
      case -14842:
      case -14833:
      case -14823:
      case -14822:
      case -14821:
      case -14820:
        result = 92;
        break;
      case -15145:
      case -15144:
      case -15143:
      case -15142:
      case -15141:
      case -15140:
      case -15139:
      case -15138:
      case -15137:
      case -15136:
      case -15135:
      case -15134:
      case -15133:
      case -15132:
      case -15131:
      case -15130:
      case -14871:
      case -14870:
      case -14869:
      case -14868:
      case -14867:
      case -14866:
      case -14836:
      case -14819:
      case -14818:
        result = 93;
        break;
      case -15129:
      case -15128:
      case -14816:
        result = 94;
        break;
      case -15127:
      case -15126:
      case -15125:
      case -15124:
      case -15123:
      case -15122:
      case -15121:
      case -15120:
      case -15119:
      case -15118:
      case -15117:
      case -15116:
      case -15115:
      case -15114:
      case -15113:
      case -15112:
      case -15111:
      case -15110:
      case -15109:
      case -15108:
      case -15107:
      case -15106:
      case -15105:
      case -15104:
      case -15103:
      case -15102:
      case -15101:
      case -15100:
      case -15099:
      case -15098:
      case -15097:
      case -15096:
      case -15095:
      case -15094:
      case -15093:
      case -15092:
      case -15091:
      case -15090:
      case -15089:
      case -15088:
      case -15087:
      case -15086:
      case -15085:
      case -15084:
      case -15083:
      case -15082:
      case -15081:
      case -14913:
      case -14912:
      case -14911:
      case -14865:
      case -14864:
      case -14863:
      case -14862:
      case -14841:
      case -14832:
      case -14831:
      case -14830:
      case -14829:
      case -14828:
      case -14827:
      case -14826:
      case -14815:
      case -14814:
      case -14813:
      case -14812:
        goto LABEL_14;
      case -15080:
      case -15079:
      case -15078:
      case -15077:
      case -15076:
      case -15075:
      case -15074:
      case -15073:
      case -15072:
        result = 96;
        break;
      case -15071:
      case -15070:
      case -15069:
        result = 97;
        break;
      case -15068:
      case -15067:
      case -15066:
      case -15065:
      case -15064:
      case -15063:
      case -15062:
      case -15061:
      case -15060:
      case -15059:
      case -15058:
      case -15057:
      case -15056:
      case -15055:
      case -15054:
      case -15053:
      case -15052:
      case -15051:
      case -15050:
      case -15049:
      case -15048:
      case -15047:
      case -15046:
      case -15045:
      case -15044:
      case -15043:
      case -15042:
      case -15041:
      case -15040:
      case -15039:
      case -15038:
      case -15037:
      case -15036:
      case -15035:
      case -15034:
      case -15033:
      case -15032:
      case -15031:
      case -15030:
      case -15029:
      case -15028:
      case -15027:
      case -15026:
      case -15025:
      case -15024:
      case -15023:
      case -15022:
      case -15021:
      case -15020:
      case -15019:
      case -15018:
      case -15017:
      case -15016:
      case -14910:
      case -14909:
      case -14908:
      case -14859:
      case -14858:
      case -14857:
      case -14856:
      case -14808:
      case -14807:
      case -14806:
      case -14805:
        result = 98;
        break;
      case -15015:
      case -15014:
      case -15013:
      case -15012:
      case -15011:
      case -15010:
      case -15009:
      case -15008:
      case -15007:
      case -15006:
      case -15005:
      case -15004:
      case -15003:
      case -15002:
      case -15001:
      case -15000:
      case -14999:
      case -14998:
      case -14997:
      case -14996:
      case -14995:
      case -14994:
      case -14993:
      case -14992:
      case -14991:
      case -14990:
      case -14989:
      case -14988:
      case -14987:
      case -14986:
      case -14985:
      case -14984:
      case -14983:
      case -14982:
      case -14981:
      case -14980:
      case -14979:
      case -14978:
      case -14977:
      case -14976:
      case -14975:
      case -14974:
        result = 99;
        break;
      case -14973:
      case -14972:
      case -14971:
      case -14970:
      case -14969:
      case -14855:
      case -14802:
        result = 100;
        break;
      case -14950:
      case -14949:
      case -14948:
      case -14947:
      case -14946:
      case -14945:
      case -14944:
      case -14943:
      case -14942:
      case -14941:
      case -14940:
      case -14939:
      case -14938:
      case -14937:
      case -14936:
      case -14935:
      case -14934:
      case -14933:
      case -14932:
      case -14803:
        goto LABEL_29;
      case -14931:
        result = 102;
        break;
      case -14905:
      case -14904:
      case -14903:
      case -14902:
      case -14901:
      case -14900:
      case -14899:
      case -14898:
      case -14897:
      case -14896:
      case -14895:
      case -14894:
      case -14893:
      case -14892:
      case -14891:
      case -14890:
      case -14889:
      case -14888:
      case -14887:
      case -14886:
      case -14885:
      case -14861:
      case -14860:
      case -14811:
      case -14810:
      case -14809:
        result = 105;
        break;
      case -14825:
        result = 106;
        break;
      default:
        switch(a1)
        {
          case -20480:
          case -20479:
          case -20478:
          case -20477:
          case -20476:
          case -20475:
          case -20474:
            result = 39;
            break;
          case -20473:
          case -20472:
          case -20471:
          case -20470:
          case -20469:
          case -20468:
          case -20467:
          case -20466:
          case -20465:
          case -20464:
          case -20463:
          case -20462:
          case -20461:
          case -20460:
          case -20459:
          case -20458:
          case -20457:
          case -20456:
            goto LABEL_43;
          case -20455:
          case -20454:
          case -20453:
          case -20452:
          case -20451:
          case -20450:
          case -20449:
          case -20448:
          case -20447:
          case -20446:
          case -20445:
          case -20350:
          case -20349:
          case -20336:
          case -20335:
          case -20334:
          case -20333:
          case -20332:
          case -20331:
          case -20330:
          case -20329:
          case -20328:
          case -20321:
          case -20316:
            result = 103;
            break;
          case -20444:
          case -20443:
          case -20442:
          case -20441:
          case -20440:
          case -20439:
          case -20438:
          case -20437:
          case -20436:
          case -20435:
          case -20434:
          case -20433:
          case -20432:
          case -20431:
          case -20430:
          case -20429:
          case -20428:
          case -20427:
          case -20426:
          case -20425:
          case -20424:
          case -20423:
          case -20422:
          case -20421:
          case -20420:
          case -20348:
          case -20347:
          case -20346:
          case -20320:
          case -20315:
          case -20314:
            goto LABEL_24;
          case -20419:
          case -20418:
          case -20417:
          case -20416:
          case -20415:
          case -20414:
          case -20413:
          case -20412:
          case -20411:
          case -20410:
          case -20409:
          case -20408:
          case -20407:
          case -20406:
          case -20345:
          case -20344:
          case -20343:
          case -20342:
          case -20341:
          case -20340:
          case -20339:
          case -20338:
          case -20337:
          case -20327:
          case -20323:
          case -20319:
            result = 107;
            break;
          case -20405:
          case -20404:
          case -20403:
          case -20402:
          case -20401:
          case -20400:
          case -20399:
          case -20398:
          case -20326:
            result = 108;
            break;
          case -20397:
          case -20396:
          case -20395:
          case -20394:
          case -20393:
          case -20392:
          case -20391:
          case -20390:
          case -20389:
          case -20388:
          case -20387:
          case -20386:
          case -20385:
          case -20384:
          case -20383:
          case -20382:
          case -20381:
          case -20380:
          case -20379:
          case -20378:
          case -20377:
          case -20376:
          case -20375:
          case -20374:
          case -20373:
          case -20372:
          case -20371:
          case -20370:
          case -20325:
          case -20322:
          case -20318:
          case -20317:
            result = 110;
            break;
          case -20369:
          case -20368:
          case -20367:
          case -20366:
          case -20365:
          case -20364:
          case -20363:
          case -20362:
          case -20361:
          case -20360:
          case -20359:
          case -20358:
          case -20357:
          case -20356:
          case -20355:
          case -20354:
          case -20353:
          case -20352:
          case -20351:
          case -20324:
            goto LABEL_38;
          default:
            if (a1 == -12544)
            {
              goto LABEL_95;
            }

            return 0;
        }

        break;
    }
  }

  return result;
}

uint64_t GTFenum_isMTLResource(unsigned int a1)
{
  result = 1;
  if (a1 > 0x3C || ((1 << a1) & 0x1200000000410000) == 0)
  {
    v3 = a1 - 80;
    if (v3 > 0x15 || ((1 << v3) & 0x200009) == 0)
    {
      return 0;
    }
  }

  return result;
}

BOOL GTFenum_isCreateMTLCommandEncoder(int a1)
{
  ConstructorType = GTFenum_getConstructorType(a1);

  return GTFenum_isMTLCommandEncoder(ConstructorType);
}

uint64_t GTFenum_isEncodeWaitForEvent(int a1)
{
  result = 1;
  if (a1 <= -15340)
  {
    if (a1 == -15991)
    {
      return result;
    }

    v3 = -15749;
    goto LABEL_7;
  }

  if (a1 != -15339 && a1 != -15308)
  {
    v3 = -14842;
LABEL_7:
    if (a1 != v3)
    {
      return 0;
    }
  }

  return result;
}

uint64_t GTFenum_isEncodeSignalEvent(int a1)
{
  result = 1;
  if (a1 > -15310)
  {
    if (a1 == -15309)
    {
      return result;
    }

    v3 = -14844;
  }

  else
  {
    if (a1 == -15990)
    {
      return result;
    }

    v3 = -15340;
  }

  if (a1 != v3)
  {
    return 0;
  }

  return result;
}

uint64_t GTFenum_isEncodeWaitForFence(int a1)
{
  result = 1;
  if (a1 <= -15887)
  {
    v3 = a1 + 16131;
    if (v3 <= 6 && ((1 << v3) & 0x45) != 0)
    {
      return result;
    }

    return 0;
  }

  if (a1 != -15886 && a1 != -15781 && a1 != -15630)
  {
    return 0;
  }

  return result;
}

uint64_t GTFenum_isEncodeUpdateFence(int a1)
{
  result = 1;
  if (a1 <= -15886)
  {
    v3 = a1 + 16130;
    if (v3 <= 6 && ((1 << v3) & 0x51) != 0)
    {
      return result;
    }

    return 0;
  }

  if (a1 != -15885 && a1 != -15785 && a1 != -15635)
  {
    return 0;
  }

  return result;
}

uint64_t GTFenum_isBarrier(int a1)
{
  result = 1;
  if ((a1 + 16009) > 6 || ((1 << (a1 - 119)) & 0x63) == 0)
  {
    v4 = a1 + 15344;
    if (v4 > 0x35 || ((1 << v4) & 0x22000000000401) == 0)
    {
      return 0;
    }
  }

  return result;
}

uint64_t GTFenum_isCreateResource(int a1)
{
  ConstructorType = GTFenum_getConstructorType(a1);
  v2 = ConstructorType;
  LOBYTE(v3) = 0;
  if ((ConstructorType > 0x33 || ((1 << ConstructorType) & 0x8000002000001) == 0) && ConstructorType - 91 >= 2)
  {
    isMTLCommandEncoder = GTFenum_isMTLCommandEncoder(ConstructorType);
    v3 = 0x3F6u >> (v2 - 95);
    if (v2 - 95 > 0xA)
    {
      LOBYTE(v3) = 1;
    }

    if (isMTLCommandEncoder)
    {
      LOBYTE(v3) = 0;
    }
  }

  return v3 & 1;
}

uint64_t GTFenum_isCommandEncoder(int a1)
{
  isMTLCommandEncoder = GTFenum_isMTLCommandEncoder(a1);
  v3 = 0x409u >> (a1 - 95);
  if ((a1 - 95) > 0xA)
  {
    LOBYTE(v3) = 0;
  }

  if (isMTLCommandEncoder)
  {
    v4 = 1;
  }

  else
  {
    v4 = v3;
  }

  return v4 & 1;
}

uint64_t GTFenum_isCommandBufferCommit(int a1)
{
  result = 1;
  if (a1 <= -15909)
  {
    if (a1 == -16361)
    {
      return result;
    }

    v3 = -16202;
    goto LABEL_7;
  }

  if (a1 != -15908 && a1 != -15318)
  {
    v3 = -15316;
LABEL_7:
    if (a1 != v3)
    {
      return 0;
    }
  }

  return result;
}

uint64_t GTFenum_isSharedResourceConstructor(int a1)
{
  result = 1;
  if (a1 <= -16097)
  {
    if (a1 > -16164)
    {
      if (a1 == -16163)
      {
        return result;
      }

      v3 = -16104;
    }

    else
    {
      if (a1 == -16312)
      {
        return result;
      }

      v3 = -16294;
    }

    goto LABEL_11;
  }

  if (((a1 + 16031) > 0x34 || ((1 << (a1 - 97)) & 0x18000000000003) == 0) && (a1 + 15914) >= 2)
  {
    v3 = -16096;
LABEL_11:
    if (a1 != v3)
    {
      return 0;
    }
  }

  return result;
}

uint64_t GTFenum_isParallelCommandEncoderCall(int a1)
{
  result = 1;
  if (a1 > -16163)
  {
    if ((a1 + 16162) >= 3 && (a1 + 16028) >= 3)
    {
      return 0;
    }
  }

  else if (((a1 + 16289) > 7 || a1 == -16286) && a1 != -16351)
  {
    return 0;
  }

  return result;
}

unint64_t GTFenum_isCommandBufferRelated(int a1)
{
  ReceiverType = GTFenum_getReceiverType(a1);
  if (ReceiverType == 25 || (v3 = ReceiverType, GTFenum_isMTLCommandEncoder(ReceiverType)) || v3 == 27 && GTFenum_getConstructorType(a1))
  {
    LOBYTE(v4) = 1;
  }

  else
  {
    v4 = 0x40000020020001uLL >> (a1 - 6);
    if ((a1 + 20474) > 0x36)
    {
      LOBYTE(v4) = 0;
    }
  }

  return v4 & 1;
}

BOOL GTFenum_isMTL4CommandBufferRelated(int a1)
{
  ReceiverType = GTFenum_getReceiverType(a1);
  if (ReceiverType - 91) < 0xF && ((0x4091u >> (ReceiverType - 91)))
  {
    return 1;
  }

  v4 = GTFenum_getConstructorType(a1) - 91;
  return ReceiverType == 92 || v4 < 2;
}

BOOL GTFenum_isMIOCommandBufferRelated(int a1)
{
  ReceiverType = GTFenum_getReceiverType(a1);
  if (ReceiverType == 51)
  {
    return 1;
  }

  if (ReceiverType == 52)
  {
    return GTFenum_getConstructorType(a1) != 0;
  }

  return 0;
}

uint64_t GTFenum_isFrameBoundary(int a1)
{
  isPresent = GTFenum_isPresent(a1);
  v4 = a1 == -10168 || a1 == -16341;
  return isPresent || v4;
}

uint64_t GTFenum_isBeginCommandBuffer(int a1)
{
  result = 1;
  if (a1 > -15174)
  {
    if ((a1 + 15173) < 2)
    {
      return result;
    }

    v3 = -12544;
  }

  else
  {
    if ((a1 + 16343) < 2)
    {
      return result;
    }

    v3 = -15701;
  }

  if (a1 != v3)
  {
    return 0;
  }

  return result;
}

uint64_t GTFenum_isStreamConstructor(int a1)
{
  if (GTFenum_getConstructorType(a1))
  {
    return 1;
  }

  return GTFenum_isBeginCommandBuffer(a1);
}

uint64_t GTFenum_isEndCommandBuffer(int a1)
{
  result = 1;
  if (a1 > -15319)
  {
    if (a1 != -15318 && a1 != -15316)
    {
      v3 = -15169;
LABEL_8:
      if (a1 != v3)
      {
        return 0;
      }
    }
  }

  else if (a1 != -16361 && a1 != -16202)
  {
    v3 = -15908;
    goto LABEL_8;
  }

  return result;
}

uint64_t GTFenum_isMTL4CommandEncoderCall(int a1)
{
  LOBYTE(v1) = 1;
  if (((a1 + 15171) > 6 || ((1 << (a1 + 67)) & 0x61) == 0) && a1 != -14884)
  {
    ReceiverType = GTFenum_getReceiverType(a1);
    v1 = 0x409u >> (ReceiverType - 95);
    if ((ReceiverType - 95) > 0xA)
    {
      LOBYTE(v1) = 0;
    }
  }

  return v1 & 1;
}

uint64_t GTFenum_getTargetType(int a1)
{
  result = GTFenum_getConstructorType(a1);
  if (!result)
  {

    return GTFenum_getReceiverType(a1);
  }

  return result;
}

uint64_t GTFenum_isArgumentTableSnapshotCall(int a1)
{
  if (GTFenum_isDrawCall(a1) & 1) != 0 || (GTFenum_isComputeCall(a1))
  {
    return 1;
  }

  result = 1;
  if (a1 > -15298)
  {
    if (a1 == -15297)
    {
      return result;
    }

    v3 = -15059;
  }

  else
  {
    if (a1 == -16137)
    {
      return result;
    }

    v3 = -15486;
  }

  if (a1 != v3)
  {
    v4 = GTFenum_isMeshCall(a1);
    v5 = a1 >> 2 == 1073737833;
    if (v4)
    {
      v5 = 1;
    }

    return a1 == -14893 || v5;
  }

  return result;
}

uint64_t GetFreeIndex(unint64_t a1)
{
  if (a1 < 0x101)
  {
    return 0;
  }

  LODWORD(v1) = 0;
  for (i = 256; i < a1; i *= 2)
  {
    v1 = (v1 + 1);
  }

  return v1;
}

uint64_t GetFreeIndexCapacity(int a1)
{
  if (a1 < 1)
  {
    return 256;
  }

  v1 = a1;
  result = 256;
  do
  {
    result *= 2;
    --v1;
  }

  while (v1);
  return result;
}

vm_address_t GTTraceStoreCreate(uint64_t a1)
{
  v1 = GTCoreAlloc(a1, 0x148uLL);
  pthread_mutex_init((v1 + 264), 0);
  return v1;
}

uint64_t GTTraceStoreDestroy(int *a1)
{
  if (*a1 >= 1)
  {
    v2 = 0;
    v3 = 0;
    do
    {
      vm_deallocate(mach_task_self_, *(*(a1 + 1) + v2), *(*(a1 + 1) + v2 + 8));
      ++v3;
      v2 += 16;
    }

    while (v3 < *a1);
  }

  pthread_mutex_destroy((a1 + 66));
  free(*(a1 + 1));
  v4 = mach_task_self_;

  return vm_deallocate(v4, a1, 0x148uLL);
}

uint64_t GTTraceStoreAllocNode(int *a1, uint64_t a2, char a3, uint64_t a4)
{
  v7 = a2 + 64;
  if ((a2 + 64) < 0x101)
  {
    v12 = (a1 + 66);
    v13 = pthread_mutex_lock((a1 + 66));
    v11 = 0;
    goto LABEL_8;
  }

  v8 = -1;
  v9 = 256;
  do
  {
    v10 = v8;
    v9 *= 2;
    ++v8;
  }

  while (v9 < v7);
  v11 = v10 + 2;
  v12 = (a1 + 66);
  v13 = pthread_mutex_lock((a1 + 66));
  if (v8 <= 0x1D)
  {
LABEL_8:
    v14 = v11;
    while (!*&a1[2 * v14 + 4])
    {
      if (++v14 == 31)
      {
        goto LABEL_11;
      }
    }

    goto LABEL_26;
  }

  if (v11 != 31)
  {
LABEL_30:
    v27 = v11;
    goto LABEL_31;
  }

  v11 = 31;
LABEL_11:
  v15 = 0;
  v16 = 0x800000;
  if (v7 > 0x800000)
  {
    v16 = v7;
  }

  for (i = 256; i < v16; i *= 2)
  {
    ++v15;
  }

  v18 = 256;
  do
  {
    v18 *= 2;
    --v15;
  }

  while (v15);
  v19 = GTCoreAlloc(v13, v18);
  v20 = *a1;
  v21 = a1[1];
  if (v21 == *a1)
  {
    v22 = 2 * v21;
    if (!v21)
    {
      v22 = 8;
    }

    a1[1] = v22;
    v23 = malloc_type_realloc(*(a1 + 1), 16 * v22, 0x10C004028F705ECuLL);
    *(a1 + 1) = v23;
    v20 = *a1;
  }

  else
  {
    v23 = *(a1 + 1);
  }

  *a1 = v20 + 1;
  v24 = &v23[16 * v20];
  *v24 = v19;
  v24[1] = v18;
  *v19 = v18;
  *(v19 + 4) = 64;
  *(v19 + 8) = 0;
  *(v19 + 16) = 0;
  *(v19 + 24) = 0;
  *(v19 + 32) = v19;
  *(v19 + 48) = 0;
  *(v19 + 56) = 0;
  *(v19 + 40) = 0;
  LODWORD(v14) = 0;
  if (v18)
  {
    v25 = 256;
    do
    {
      v25 *= 2;
      LODWORD(v14) = v14 + 1;
    }

    while (v25 < v18);
  }

  *&a1[2 * v14 + 4] = v19;
LABEL_26:
  if (v11 >= v14)
  {
    goto LABEL_30;
  }

  v26 = v14;
  v27 = v11;
  v28 = &a1[2 * v14];
  v31 = *(v28 + 2);
  v29 = v28 + 4;
  v30 = v31;
  v32 = v26 + 2;
  do
  {
    *v29 = *(v30 + 5);
    v33 = *v30 >> 1;
    *v30 >>= 1;
    v30[1] = 64;
    *(v30 + 1) = 0;
    *(v30 + 2) = 0;
    *(v30 + 3) = 0;
    *(v30 + 4) = v30;
    *(v30 + 6) = 0;
    *(v30 + 7) = 0;
    v34 = v30 + v33;
    *v34 = v33;
    *(v34 + 1) = 64;
    *(v34 + 1) = 0;
    *(v34 + 2) = 0;
    *(v34 + 3) = 0;
    *(v34 + 4) = v34;
    *(v34 + 6) = 0;
    *(v34 + 7) = 0;
    *(v34 + 5) = 0;
    *(v30 + 5) = v30 + v33;
    *--v29 = v30;
    v35 = v32 - 3;
    --v32;
  }

  while (v35 > v11);
LABEL_31:
  v36 = &a1[2 * v27];
  v37 = *(v36 + 2);
  *(v36 + 2) = *(v37 + 40);
  *(v37 + 8) = a3;
  *(v37 + 16) = a4;
  *(v37 + 24) = a1;
  *(v37 + 40) = 0;
  pthread_mutex_unlock(v12);
  return v37;
}

uint64_t GTTraceStoreFreeNodes(uint64_t result, unsigned int *a2)
{
  if (a2)
  {
    v2 = a2;
    v3 = result;
    pthread_mutex_lock((result + 264));
    v4 = v3 + 16;
    do
    {
      v5 = *v2;
      if (v5 < 0x101)
      {
        v6 = 0;
      }

      else
      {
        v6 = 0;
        for (i = 256; i < v5; i *= 2)
        {
          ++v6;
        }
      }

      v8 = *(v2 + 5);
      v2[1] = 64;
      *(v2 + 1) = 0;
      *(v2 + 2) = 0;
      *(v2 + 3) = 0;
      *(v2 + 4) = v2;
      *(v2 + 6) = 0;
      *(v2 + 7) = 0;
      *(v2 + 5) = 0;
      *(v2 + 5) = *(v4 + 8 * v6);
      *(v4 + 8 * v6) = v2;
      v2 = v8;
    }

    while (v8);

    return pthread_mutex_unlock((v3 + 264));
  }

  return result;
}

char *GTTraceStoreNodeSubAlloc(unsigned int *a1, uint64_t a2)
{
  v2 = atomic_load(a1 + 1);
  while (1)
  {
    v3 = v2;
    v4 = a2 + v2;
    v5 = *a1;
    if (v4 > v5)
    {
      break;
    }

    v2 = v3;
    atomic_compare_exchange_strong(a1 + 1, &v2, v4);
    if (v2 == v3)
    {
      v5 = *a1;
      break;
    }
  }

  v6 = a1 + v3;
  if (v4 <= v5)
  {
    return v6;
  }

  else
  {
    return 0;
  }
}

uint64_t GTTraceStoreListLock(uint64_t a1)
{
  add = atomic_fetch_add((a1 + 16), 1u);
  do
  {
    v2 = atomic_load((a1 + 20));
  }

  while (v2 != add);
  return add;
}

void *GTTraceStoreListPrependNode(void *result, uint64_t a2)
{
  *(a2 + 40) = *result;
  *result = a2;
  if (!result[1])
  {
    result[1] = a2;
  }

  return result;
}

uint64_t GTTraceStoreListConcat(uint64_t result, unsigned int *a2)
{
  if (*(result + 8))
  {
    v2 = atomic_load(a2 + 4);
    atomic_fetch_add((result + 16), v2);
    v3 = atomic_load(a2 + 5);
    atomic_fetch_add((result + 20), v3);
    *(*(result + 8) + 40) = *a2;
    v4 = *(a2 + 1);
    if (v4)
    {
      *(result + 8) = v4;
    }
  }

  else
  {
    v5 = *a2;
    *(result + 16) = *(a2 + 2);
    *result = v5;
  }

  *a2 = 0;
  *(a2 + 1) = 0;
  *(a2 + 2) = 0;
  return result;
}

void *GTTraceStoreListAppendNode(void *result, uint64_t a2)
{
  v2 = result[1];
  if (v2)
  {
    v3 = (v2 + 40);
  }

  else
  {
    v3 = result;
  }

  *v3 = a2;
  result[1] = a2;
  *(a2 + 40) = 0;
  return result;
}

char *GTTraceMemPool_allocateBytes(uint64_t a1, uint64_t a2, unint64_t a3)
{
  v6 = (HIDWORD(a3) + 23) & 0x1FFFFFFF8;
  add = atomic_fetch_add((a1 + 24), 1u);
  do
  {
    v8 = atomic_load((a1 + 28));
  }

  while (v8 != add);
  v9 = *(a1 + 8);
  if (!v9)
  {
    goto LABEL_12;
  }

  v10 = atomic_load(v9 + 1);
  while (1)
  {
    v11 = v10;
    v12 = v6 + v10;
    v13 = *v9;
    if (v12 > v13)
    {
      break;
    }

    v10 = v11;
    atomic_compare_exchange_strong(v9 + 1, &v10, v12);
    if (v10 == v11)
    {
      v13 = *v9;
      break;
    }
  }

  v14 = v9 + v11;
  result = v12 <= v13 ? v14 : 0;
  if (v12 > v13)
  {
LABEL_12:
    v16 = GTTraceStoreAllocNode(*a1, (HIDWORD(a3) + 23) & 0x1FFFFFFF8, 2, *(a1 + 32));
    v17 = *(a1 + 16);
    if (v17)
    {
      v18 = (v17 + 40);
    }

    else
    {
      v18 = (a1 + 8);
    }

    *v18 = v16;
    *(a1 + 16) = v16;
    *(v16 + 5) = 0;
    v19 = atomic_load(v16 + 1);
    while (1)
    {
      v20 = v19;
      v21 = v6 + v19;
      v22 = *v16;
      if (v21 > v22)
      {
        break;
      }

      v19 = v20;
      atomic_compare_exchange_strong(v16 + 1, &v19, v21);
      if (v19 == v20)
      {
        v22 = *v16;
        break;
      }
    }

    v23 = v16 + v20;
    if (v21 <= v22)
    {
      result = v23;
    }

    else
    {
      result = 0;
    }
  }

  atomic_store(add + 1, (a1 + 28));
  *result = a2;
  *(result + 1) = a3;
  return result;
}

void *GTTraceMemPool_storeBytes(void *result, uint64_t a2, unint64_t a3, const void *a4)
{
  if (a4)
  {
    v4 = HIDWORD(a3);
    if (HIDWORD(a3))
    {
      v6 = GTTraceMemPool_allocateBytes(result, a2, a3) + 16;

      return memcpy(v6, a4, v4);
    }
  }

  return result;
}

uint64_t GTTraceMemPool_findHeader(uint64_t a1, uint64_t a2, unsigned __int8 a3)
{
  v3 = *(a1 + 8);
  if (!v3)
  {
    return 0;
  }

  while (1)
  {
    v4 = atomic_load((v3 + 4));
    v5 = v4 - 64;
    if (v5 >= 1)
    {
      break;
    }

LABEL_8:
    v3 = *(v3 + 40);
    if (!v3)
    {
      return 0;
    }
  }

  result = v3 + 64;
  while (a2 != *result || *(result + 8) != a3)
  {
    v7 = (*(result + 12) + 23) & 0xFFFFFFF8;
    result += v7;
    v8 = __OFSUB__(v5, v7);
    v5 -= v7;
    if ((v5 < 0) ^ v8 | (v5 == 0))
    {
      goto LABEL_8;
    }
  }

  return result;
}

uint64_t GTTraceMemHeader_data(uint64_t result)
{
  if (result)
  {
    v1 = result;
    result += 16;
    v2 = *(v1 + 12);
  }

  return result;
}

uint64_t GTTraceMemHeader_bytes(uint64_t a1)
{
  if (a1)
  {
    return a1 + 16;
  }

  else
  {
    return 0;
  }
}

void GTTraceMemPool_buildMemoryMap(uint64_t a1, apr_hash_t *ht)
{
  for (i = *(a1 + 8); i; i = *(i + 40))
  {
    v4 = atomic_load((i + 4));
    v5 = v4 - 64;
    if ((v4 - 64) >= 1)
    {
      v6 = (i + 64);
      do
      {
        apr_hash_set(ht, v6, 9, v6);
        v7 = (v6[3] + 23) & 0xFFFFFFF8;
        v6 = (v6 + v7);
        v8 = __OFSUB__(v5, v7);
        v5 -= v7;
      }

      while (!((v5 < 0) ^ v8 | (v5 == 0)));
    }
  }
}

uint64_t GTTraceMemPool_headerFromMap(uint64_t a1, uint64_t a2, uint64_t a3)
{
  v5[0] = a2;
  v5[1] = a3;
  entry = find_entry(a1, v5, 9uLL, 0);
  if (*entry)
  {
    return *(*entry + 32);
  }

  else
  {
    return 0;
  }
}

uint64_t fbuf_header_length(_BYTE *a1)
{
  v1 = *a1;
  if ((a1[33] & 0x10) == 0)
  {
    return *a1;
  }

  v2 = *a1;
  do
  {
    a1 += v1;
    v1 = *a1;
    v2 += v1;
  }

  while ((a1[33] & 0x20) == 0);
  return v2;
}

uint64_t fbuf_header_fits(unsigned int *a1, unint64_t a2)
{
  if (a2 < 0x24)
  {
    return 0;
  }

  v2 = *a1;
  if (v2 > a2)
  {
    return 0;
  }

  if ((*(a1 + 33) & 0x10) != 0)
  {
    while (1)
    {
      a2 -= v2;
      if (a2 < 0x24)
      {
        break;
      }

      a1 = (a1 + v2);
      v2 = *a1;
      if (a2 < v2)
      {
        break;
      }

      if ((*(a1 + 33) & 0x20) != 0)
      {
        return 1;
      }
    }

    return 0;
  }

  return 1;
}

uint64_t FBDecoder_DecodeArguments(unsigned int *a1, uint64_t a2, uint64_t a3, uint64_t a4, unint64_t a5)
{
  v10 = *a1;
  v11 = (a2 + v10);
  v12 = memchr((a2 + v10), 0, a3 - v10);
  if (v12)
  {
    v13 = v12;
    v31 = a2;
    v32 = a1;
    v14 = a1[1];
    v15 = v12 + 1;
    if (v11 == v12)
    {
LABEL_37:
      result = 0;
      *v32 = ((v15 + 3) & 0xFFFFFFFC) - v31;
      v32[1] = v14;
      return result;
    }

    v16 = a2 + a3;
    while (1)
    {
      v17 = v14;
      if (v14 >= a5)
      {
        return 2;
      }

      v18 = a4 + 16 * v14;
      *(v18 + 11) = 0;
      *(v18 + 8) = 1;
      v19 = *v11;
      if (v19 == 64)
      {
        v20 = 0;
        v21 = (v11 + 1);
        if (v11 + 1 != v13)
        {
          v22 = v11[1] - 48;
          if (v22 <= 9)
          {
            LOWORD(v20) = 0;
            v23 = (v11 + 2);
            do
            {
              v21 = v23;
              LOWORD(v20) = v22 + 10 * v20;
              if (v23 == v13)
              {
                break;
              }

              ++v23;
              v22 = *v21 - 48;
            }

            while (v22 < 0xA);
            v20 = v20;
          }
        }

        *(v18 + 11) = 1;
        *(v18 + 8) = v20;
        v19 = *v21;
        v24 = 3;
        v11 = v21;
      }

      else
      {
        v24 = 2;
        v20 = 1;
      }

      if (v19 == 117)
      {
        *(v18 + 11) = v24;
        v25 = *++v11;
        v19 = v25;
      }

      *(v18 + 10) = v19;
      result = 0xFFFFFFFFLL;
      if (v19 <= 97)
      {
        if (v19 != 67)
        {
          if (v19 != 83 && v19 != 85)
          {
            return result;
          }

          if (v16 - v15 >= 8 && *v15 == 0xFFFFFFFFLL)
          {
            *v18 = 0;
            *(v18 + 8) = 0;
            ++v15;
          }

          else
          {
            *v18 = v15;
            v29 = memchr(v15, 0, v16 - v15);
            if (!v29)
            {
              return 0xFFFFFFFFLL;
            }

            *(v18 + 8) = v29 - v15;
            v15 = v29 + 1;
          }

          goto LABEL_32;
        }
      }

      else
      {
        v27 = v19 - 98;
        if ((v19 - 98) > 0x15)
        {
          return result;
        }

        if (((1 << v27) & 0x244404) == 0)
        {
          if (((1 << v27) & 0x90) != 0)
          {
            v30 = (v15 + 3) & 0xFFFFFFFFFFFFFFFCLL;
            *v18 = v30;
            v15 = (v30 + 4 * v20);
          }

          else
          {
            if (v19 != 98)
            {
              return result;
            }

            *v18 = v15;
            v15 = (v15 + v20);
          }

          goto LABEL_32;
        }
      }

      v28 = (v15 + 3) & 0xFFFFFFFFFFFFFFFCLL;
      *v18 = v28;
      v15 = (v28 + 8 * v20);
LABEL_32:
      if (v16 < v15)
      {
        return 0xFFFFFFFFLL;
      }

      if ((v11 + 1) < v16 && v11[1] == 60)
      {
        v11 = memchr(v11 + 2, 62, v13 - (v11 + 2));
      }

      v14 = v17 + 1;
      if (++v11 == v13)
      {
        goto LABEL_37;
      }
    }
  }

  return 0xFFFFFFFFLL;
}

uint64_t Fbuf_ReturnValue(_DWORD *a1)
{
  v1 = (a1 + 9);
  v2 = (*a1 - 36);
  v4 = 0;
  FBDecoder_DecodeArguments(&v4, (a1 + 9), v2, v5, 0x10uLL);
  HIDWORD(v4) = 0;
  FBDecoder_DecodeArguments(&v4, v1, v2, v5, 0x10uLL);
  return *v5[0];
}

uint64_t GTTraceFuncArgs_getMtl4Commits(uint64_t a1, uint64_t a2, char **a3)
{
  *a3 = 0;
  v6 = *(a1 + 8);
  if (v6 == -14847)
  {
    v8 = 16;
  }

  else
  {
    result = 0;
    if (v6 != -14846)
    {
      return result;
    }

    v8 = 24;
  }

  v9 = GTTraceFunc_argumentBytesWithMap(a1, *(a1 + 13), a2);
  *a3 = GTTraceFunc_argumentBytesWithMap(a1, v9[v8], a2);
  return *(v9 + 1);
}

void ***MakeDYMTLRenderPassSampleBufferAttachmentDescriptor(void ***result, void *a2, uint64_t a3, uint64_t a4, int a5, uint64_t a6)
{
  v7 = *result;
  v8 = *(*result)++;
  v9 = v7[1];
  *result = v7 + 2;
  v10 = v7[2];
  *result = v7 + 3;
  v11 = v7[3];
  *result = v7 + 4;
  v12 = v7[4];
  *result = v7 + 5;
  if (a5)
  {
    if (a6)
    {
      result = GetStream_19685(a6, v8);
      if (result)
      {
        v8 = *result;
      }

      else
      {
        v8 = 0;
      }
    }

    *a2 = v12;
    a2[1] = v10;
    a2[2] = v8;
    a2[3] = v11;
    a2[4] = v9;
  }

  return result;
}

uint64_t GetStream_19685(uint64_t a1, uint64_t a2)
{
  v7 = a2;
  entry = find_entry(*(a1 + 24), &v7, 8uLL, 0);
  if (*entry && (v4 = *(*entry + 32)) != 0 && (v5 = atomic_load((*(a1 + 16) + 80)), v5 < *(v4 + 24)))
  {
    return *(v4 + 8);
  }

  else
  {
    return 0;
  }
}

void *SaveDYMTLRenderPassSampleBufferAttachmentDescriptor(void *a1, void *a2, uint64_t a3)
{
  v16 = a2[2];
  v5 = &dword_8;
  result = find_entry(a3, &v16, 8uLL, 0);
  if (*result)
  {
    v7 = *(*result + 32);
    if (v7)
    {
      v8 = atomic_load((v7 + 56));
      v5 = (v7 + (~(v8 >> 2) & 8));
    }

    else
    {
      v5 = &dword_8;
    }
  }

  v10 = a1[1];
  v9 = a1[2];
  v11 = v10 + 8;
  if (v10 + 8 <= v9)
  {
    *(*a1 + v10) = *v5;
    v9 = a1[2];
    v11 = a1[1] + 8;
  }

  a1[1] = v11;
  v12 = v11 + 8;
  if (v11 + 8 <= v9)
  {
    *(*a1 + v11) = a2[4];
    v9 = a1[2];
    v12 = a1[1] + 8;
  }

  a1[1] = v12;
  v13 = v12 + 8;
  if (v12 + 8 <= v9)
  {
    *(*a1 + v12) = a2[1];
    v9 = a1[2];
    v13 = a1[1] + 8;
  }

  a1[1] = v13;
  v14 = v13 + 8;
  if (v13 + 8 <= v9)
  {
    *(*a1 + v13) = a2[3];
    v9 = a1[2];
    v14 = a1[1] + 8;
  }

  a1[1] = v14;
  v15 = v14 + 8;
  if (v14 + 8 <= v9)
  {
    *(*a1 + v14) = *a2;
    v15 = a1[1] + 8;
  }

  a1[1] = v15;
  return result;
}

uint64_t DecodeDYMTLTextureDescriptor(uint64_t result, uint64_t a2, uint64_t *a3, int a4)
{
  v4 = *a3;
  *a3 += 56;
  if (a4)
  {
    *v4 = GTMTLTextureDescriptorDefaults;
    *(v4 + 16) = unk_29CCD8;
    *(v4 + 32) = xmmword_29CCE8;
    *(v4 + 48) = 512;
    v5 = *result;
    *(v4 + 49) = *(result + 8);
    *(v4 + 34) = *(result + 16);
    *(v4 + 24) = *(result + 24);
    *(v4 + 32) = *(result + 32);
    *(v4 + 30) = *(result + 40);
    *(v4 + 45) = *(result + 48);
    *(v4 + 47) = *(result + 56);
    *(v4 + 28) = *(result + 64);
    v6 = 72;
    if (v5 < 3)
    {
      v6 = 80;
    }

    v7 = result + v6;
    *(v4 + 43) = *v7;
    *(v4 + 44) = *(v7 + 8);
    if (v5)
    {
      *(v4 + 20) = *(v7 + 16);
      if (v5 <= 3)
      {
        if (v5 == 3)
        {
          *(v4 + 36) = *(v7 + 32) | (16 * *(v7 + 24));
        }
      }

      else
      {
        v8 = *(v7 + 24);
        *(v4 + 46) = v8;
        v9 = *(v7 + 40);
        v10 = v9 | (16 * *(v7 + 32));
        *(v4 + 36) = v9 | (16 * *(v7 + 32));
        if (v5 >= 0x15)
        {
          *(v4 + 38) = *(v7 + 48);
          *(v4 + 40) = *(v7 + 56);
          if (v5 != 21)
          {
            *(v4 + 8) = *(v7 + 64);
            *(v4 + 42) = *(v7 + 72);
            if (v5 >= 0x23)
            {
              v11 = *(v7 + 88);
              v12 = *(v7 + 96);
              *(v4 + 16) = *(v7 + 80) | (*(v7 + 88) << 8) | (*(v7 + 96) << 16) | (*(v7 + 104) << 24);
              if (v5 >= 0x27)
              {
                *(v4 + 16) = *(v7 + 112);
                if (v5 >= 0x2A)
                {
                  *(v4 + 36) = v10 | (*(v7 + 120) << 8);
                  *v4 = *(v7 + 128);
                  *(v4 + 48) = *(v7 + 136);
                  *(v4 + 50) = *(v7 + 144);
                  if (v5 >= 0x31)
                  {
                    *(v4 + 41) = *(v7 + 152);
                    *(v4 + 39) = *(v7 + 160);
                  }
                }
              }
            }
          }
        }

        if (v8 == 255)
        {
          *(v4 + 46) = 0;
        }
      }
    }
  }

  return result;
}

uint64_t EncodeDYMTLTextureDescriptor(uint64_t a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *(a1 + 49);
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 34);
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 24);
          if (a3 >= 0x28)
          {
            a2[4] = *(a1 + 32);
            if (a3 >= 0x30)
            {
              a2[5] = *(a1 + 30);
              if (a3 >= 0x38)
              {
                a2[6] = *(a1 + 45);
                if (a3 >= 0x40)
                {
                  a2[7] = *(a1 + 47);
                  if (a3 >= 0x48)
                  {
                    a2[8] = *(a1 + 28);
                    if (a3 >= 0x50)
                    {
                      a2[9] = *(a1 + 43);
                      if (a3 >= 0x58)
                      {
                        a2[10] = *(a1 + 44);
                        if (a3 >= 0x60)
                        {
                          a2[11] = *(a1 + 20);
                          if (a3 >= 0x68)
                          {
                            a2[12] = *(a1 + 46);
                            if (a3 >= 0x70)
                            {
                              a2[13] = *(a1 + 36) >> 4;
                              if (a3 >= 0x78)
                              {
                                a2[14] = *(a1 + 36) & 0xF;
                                if (a3 >= 0x80)
                                {
                                  a2[15] = *(a1 + 38);
                                  if (a3 >= 0x88)
                                  {
                                    a2[16] = *(a1 + 40);
                                    if (a3 >= 0x90)
                                    {
                                      a2[17] = *(a1 + 8);
                                      if (a3 >= 0x98)
                                      {
                                        a2[18] = *(a1 + 42);
                                        if (a3 >= 0xA0)
                                        {
                                          a2[19] = *(a1 + 16);
                                          if (a3 >= 0xA8)
                                          {
                                            a2[20] = BYTE1(*(a1 + 16));
                                            if (a3 >= 0xB0)
                                            {
                                              a2[21] = BYTE2(*(a1 + 16));
                                              if (a3 >= 0xB8)
                                              {
                                                a2[22] = *(a1 + 16) >> 24;
                                                if (a3 >= 0xC0)
                                                {
                                                  a2[23] = *(a1 + 16);
                                                  if (a3 >= 0xC8)
                                                  {
                                                    a2[24] = *(a1 + 37) & 3;
                                                    if (a3 >= 0xD0)
                                                    {
                                                      a2[25] = *a1;
                                                      if (a3 >= 0xD8)
                                                      {
                                                        a2[26] = *(a1 + 48);
                                                        if (a3 >= 0xE0)
                                                        {
                                                          a2[27] = *(a1 + 50);
                                                          if (a3 >= 0xE8)
                                                          {
                                                            a2[28] = *(a1 + 41);
                                                            if (a3 >= 0xF0)
                                                            {
                                                              a2[29] = *(a1 + 39);
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }

  return 240;
}

double DecodeDYMTLBufferDescriptor(void *a1, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 8);
  *(a3 + 8) = v4 + 40;
  if (a4)
  {
    *(v4 + 32) = 0;
    result = 0.0;
    *v4 = 0u;
    *(v4 + 16) = 0u;
    *v4 = a1[1];
    *(v4 + 32) = a1[2];
    *(v4 + 34) = a1[6];
    *(v4 + 24) = a1[8];
    *(v4 + 8) = a1[9];
    *(v4 + 16) = a1[10];
  }

  return result;
}

uint64_t EncodeDYMTLBufferDescriptor(uint64_t a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *a1;
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 32);
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 32) & 0xF;
          if (a3 >= 0x28)
          {
            a2[4] = *(a1 + 32) >> 4;
            if (a3 >= 0x30)
            {
              a2[5] = *(a1 + 32) & 0xF;
              if (a3 >= 0x38)
              {
                a2[6] = *(a1 + 34);
                if (a3 >= 0x40)
                {
                  a2[7] = 0;
                  if (a3 >= 0x48)
                  {
                    a2[8] = *(a1 + 24);
                    if (a3 >= 0x50)
                    {
                      a2[9] = *(a1 + 8);
                      if (a3 >= 0x58)
                      {
                        a2[10] = *(a1 + 16);
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }

  return 88;
}

void **MakeDYMTLVertexBufferLayoutDescriptor(void **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = *(*result)++;
  v7 = v5[1];
  *result = v5 + 2;
  v8 = v5[2];
  *result = v5 + 3;
  if (a5)
  {
    if (v6 == 0xFFFFFFFFLL)
    {
      v6 = -1;
    }

    *a2 = v6;
    *(a2 + 8) = v7;
    *(a2 + 10) = v8;
    *(a2 + 11) = 0;
    *(a2 + 15) = 0;
  }

  return result;
}

void *SaveDYMTLVertexBufferLayoutDescriptor(void *result, uint64_t a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = *a2;
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = *(a2 + 8);
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = *(a2 + 10);
    v6 = result[1] + 8;
  }

  result[1] = v6;
  return result;
}

void **MakeDYMTLVertexAttributeDescriptor(void **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = *(*result)++;
  v7 = v5[1];
  *result = v5 + 2;
  v8 = v5[2];
  *result = v5 + 3;
  if (a5)
  {
    *a2 = v7;
    *(a2 + 4) = v8;
    *(a2 + 5) = v6;
    *(a2 + 6) = 0;
  }

  return result;
}

void *SaveDYMTLVertexAttributeDescriptor(void *result, unsigned int *a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = *(a2 + 5);
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = *a2;
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = *(a2 + 4);
    v6 = result[1] + 8;
  }

  result[1] = v6;
  return result;
}

void **MakeDYMTLRenderPipelineColorAttachmentDescriptor(void **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = *(*result)++;
  v7 = v5[1];
  *result = v5 + 2;
  v8 = v5[2];
  *result = v5 + 3;
  v9 = v5[3];
  *result = v5 + 4;
  v10 = v5[4];
  *result = v5 + 5;
  v11 = v5[5];
  *result = v5 + 6;
  v12 = v5[6];
  *result = v5 + 7;
  v13 = v5[7];
  *result = v5 + 8;
  v14 = v5[8];
  *result = v5 + 9;
  if (a5)
  {
    *a2 = v14;
    *(a2 + 2) = v12;
    *(a2 + 3) = v6;
    *(a2 + 4) = v11;
    *(a2 + 5) = v8;
    *(a2 + 6) = v9;
    *(a2 + 7) = v10;
    *(a2 + 8) = v7;
    *(a2 + 9) = v13;
    *(a2 + 10) = 0;
    *(a2 + 14) = 0;
  }

  return result;
}

void *SaveDYMTLRenderPipelineColorAttachmentDescriptor(void *result, unsigned __int8 *a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = a2[3];
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = a2[8];
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = a2[5];
    v2 = result[2];
    v6 = result[1] + 8;
  }

  result[1] = v6;
  v7 = v6 + 8;
  if (v6 + 8 <= v2)
  {
    *(*result + v6) = a2[6];
    v2 = result[2];
    v7 = result[1] + 8;
  }

  result[1] = v7;
  v8 = v7 + 8;
  if (v7 + 8 <= v2)
  {
    *(*result + v7) = a2[7];
    v2 = result[2];
    v8 = result[1] + 8;
  }

  result[1] = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v2)
  {
    *(*result + v8) = a2[4];
    v2 = result[2];
    v9 = result[1] + 8;
  }

  result[1] = v9;
  v10 = v9 + 8;
  if (v9 + 8 <= v2)
  {
    *(*result + v9) = a2[2];
    v2 = result[2];
    v10 = result[1] + 8;
  }

  result[1] = v10;
  v11 = v10 + 8;
  if (v10 + 8 <= v2)
  {
    *(*result + v10) = a2[9];
    v2 = result[2];
    v11 = result[1] + 8;
  }

  result[1] = v11;
  v12 = v11 + 8;
  if (v11 + 8 <= v2)
  {
    *(*result + v11) = *a2;
    v12 = result[1] + 8;
  }

  result[1] = v12;
  return result;
}

size_t MakeDYMTLLinkedFunctions(size_t result, void *a2, void *a3, unsigned int a4, int a5, uint64_t a6)
{
  v10 = result;
  v11 = (*result + 8);
  v12 = **result;
  *result = v11;
  if (v12)
  {
    v51 = a3[72];
    v13 = v12;
    do
    {
      v14 = **v10;
      *v10 += 8;
      if (a5)
      {
        if (a6)
        {
          result = GetStream_19685(a6, v14);
          if (result)
          {
            v14 = *result;
          }

          else
          {
            v14 = 0;
          }
        }

        v15 = a3[72];
        *v15 = v14;
      }

      else
      {
        v15 = a3[72];
      }

      a3[72] = v15 + 1;
      --v13;
    }

    while (v13);
    v11 = *v10;
  }

  else
  {
    v51 = 0;
  }

  v18 = *v11;
  v16 = v11 + 8;
  v17 = v18;
  *v10 = v16;
  if (v18)
  {
    v50 = a3[72];
    v19 = v17;
    do
    {
      v20 = **v10;
      *v10 += 8;
      if (a5)
      {
        if (a6)
        {
          result = GetStream_19685(a6, v20);
          if (result)
          {
            v20 = *result;
          }

          else
          {
            v20 = 0;
          }
        }

        v21 = a3[72];
        *v21 = v20;
      }

      else
      {
        v21 = a3[72];
      }

      a3[72] = v21 + 1;
      --v19;
    }

    while (v19);
    v16 = *v10;
  }

  else
  {
    v50 = 0;
  }

  v22 = *v16;
  *v10 = v16 + 8;
  if (v22)
  {
    v49 = a2;
    v23 = *(v16 + 1);
    v24 = v16 + 16;
    *v10 = v16 + 16;
    if (v23)
    {
      v47 = v17;
      v48 = v12;
      v25 = 0;
      v53 = a3[63];
      v55 = v16 + 16;
      *v10 = &v24[v23];
      a3[63] += 24 * v23;
      while (1)
      {
        v26 = *v10;
        result = strlen(*v10);
        *v10 += (result & 0xFFFFFFFFFFFFFFF8) + 8;
        if (*v26)
        {
          v27 = v26;
        }

        else
        {
          v27 = 0;
        }

        if (*v26)
        {
          result = strlen(v27);
          v28 = a3[74];
          v29 = result + 1;
          if (result != -1)
          {
            v30 = a3[74];
            goto LABEL_36;
          }
        }

        else
        {
          v28 = a3[74];
        }

        v29 = 0;
        v30 = 0;
LABEL_36:
        a3[74] = v28 + v29;
        if (a5)
        {
          result = memcpy(v30, v27, v29);
        }

        v31 = v24[v25];
        if (v31)
        {
          v32 = v23;
          v33 = a3[72];
          v34 = v31;
          do
          {
            v35 = **v10;
            *v10 += 8;
            if (a5)
            {
              if (a6)
              {
                result = GetStream_19685(a6, v35);
                if (result)
                {
                  v35 = *result;
                }

                else
                {
                  v35 = 0;
                }
              }

              v36 = a3[72];
              *v36 = v35;
            }

            else
            {
              v36 = a3[72];
            }

            a3[72] = v36 + 1;
            --v34;
          }

          while (v34);
          v23 = v32;
          v24 = v55;
          if ((a5 & 1) == 0)
          {
            goto LABEL_50;
          }
        }

        else
        {
          v33 = 0;
          if ((a5 & 1) == 0)
          {
            goto LABEL_50;
          }
        }

        v37 = v53 + 24 * v25;
        *v37 = v30;
        *(v37 + 8) = v33;
        *(v37 + 16) = v31;
        *(v37 + 20) = 0;
LABEL_50:
        if (++v25 == v23)
        {
          v17 = v47;
          v12 = v48;
          v38 = a4;
          v39 = v53;
          goto LABEL_56;
        }
      }
    }

    v39 = 0;
    v38 = a4;
LABEL_56:
    if (a5)
    {
      v22 = v23;
    }

    else
    {
      v39 = 0;
      v22 = 0;
    }

    a2 = v49;
  }

  else
  {
    v39 = 0;
    v38 = a4;
  }

  v40 = v17;
  v41 = v12;
  if (v38 <= 0x2C)
  {
    v43 = 0;
    v42 = 0;
    if (!a5)
    {
      return result;
    }

    goto LABEL_74;
  }

  v42 = **v10;
  *v10 += 8;
  if (!v42)
  {
    v43 = 0;
    if (!a5)
    {
      return result;
    }

    goto LABEL_74;
  }

  v54 = v22;
  v56 = v39;
  v43 = a3[72];
  v44 = v42;
  do
  {
    v45 = **v10;
    *v10 += 8;
    if (a5)
    {
      if (a6)
      {
        result = GetStream_19685(a6, v45);
        if (result)
        {
          v45 = *result;
        }

        else
        {
          v45 = 0;
        }
      }

      v46 = a3[72];
      *v46 = v45;
    }

    else
    {
      v46 = a3[72];
    }

    a3[72] = v46 + 1;
    --v44;
  }

  while (v44);
  v22 = v54;
  v39 = v56;
  if (a5)
  {
LABEL_74:
    *a2 = v50;
    a2[1] = v40;
    a2[2] = v51;
    a2[3] = v41;
    a2[4] = v39;
    a2[5] = v22;
    a2[6] = v43;
    a2[7] = v42;
  }

  return result;
}

uint64_t SaveDYMTLLinkedFunctions(uint64_t *a1, uint64_t a2, uint64_t a3)
{
  v6 = *(a2 + 24);
  v7 = a1[1];
  v8 = v7 + 8;
  if (v7 + 8 <= a1[2])
  {
    *(*a1 + v7) = v6;
    v8 = a1[1] + 8;
    v6 = *(a2 + 24);
  }

  a1[1] = v8;
  if (v6)
  {
    v9 = 0;
    do
    {
      v36 = *(*(a2 + 16) + 8 * v9);
      entry = find_entry(a3, &v36, 8uLL, 0);
      v11 = &dword_8;
      if (*entry)
      {
        v12 = *(*entry + 32);
        if (v12)
        {
          v13 = atomic_load((v12 + 56));
          v11 = (v12 + (~(v13 >> 2) & 8));
        }

        else
        {
          v11 = &dword_8;
        }
      }

      v14 = a1[1];
      v8 = v14 + 8;
      if (v14 + 8 <= a1[2])
      {
        *(*a1 + v14) = *v11;
        v8 = a1[1] + 8;
      }

      a1[1] = v8;
      ++v9;
    }

    while (*(a2 + 24) > v9);
  }

  v15 = *(a2 + 8);
  v16 = v8 + 8;
  if (v8 + 8 <= a1[2])
  {
    *(*a1 + v8) = v15;
    v16 = a1[1] + 8;
    v15 = *(a2 + 8);
  }

  a1[1] = v16;
  if (v15)
  {
    v17 = 0;
    do
    {
      v35 = *(*a2 + 8 * v17);
      v18 = find_entry(a3, &v35, 8uLL, 0);
      v19 = &dword_8;
      if (*v18)
      {
        v20 = *(*v18 + 32);
        if (v20)
        {
          v21 = atomic_load((v20 + 56));
          v19 = (v20 + (~(v21 >> 2) & 8));
        }

        else
        {
          v19 = &dword_8;
        }
      }

      v22 = a1[1];
      v23 = v22 + 8;
      if (v22 + 8 <= a1[2])
      {
        *(*a1 + v22) = *v19;
        v23 = a1[1] + 8;
      }

      a1[1] = v23;
      ++v17;
    }

    while (*(a2 + 8) > v17);
  }

  result = SaveDYMTLLinkedFunctionsGroup(*(a2 + 32), *(a2 + 40), a1, a3);
  v25 = *(a2 + 56);
  v26 = a1[1];
  v27 = v26 + 8;
  if (v26 + 8 <= a1[2])
  {
    *(*a1 + v26) = v25;
    v27 = a1[1] + 8;
    v25 = *(a2 + 56);
  }

  a1[1] = v27;
  if (v25)
  {
    v28 = 0;
    do
    {
      v34 = *(*(a2 + 48) + 8 * v28);
      result = find_entry(a3, &v34, 8uLL, 0);
      v29 = &dword_8;
      if (*result)
      {
        v30 = *(*result + 32);
        if (v30)
        {
          v31 = atomic_load((v30 + 56));
          v29 = (v30 + (~(v31 >> 2) & 8));
        }

        else
        {
          v29 = &dword_8;
        }
      }

      v32 = a1[1];
      v33 = v32 + 8;
      if (v32 + 8 <= a1[2])
      {
        *(*a1 + v32) = *v29;
        v33 = a1[1] + 8;
      }

      a1[1] = v33;
      ++v28;
    }

    while (*(a2 + 56) > v28);
  }

  return result;
}

uint64_t SaveDYMTLLinkedFunctionsGroup(uint64_t a1, uint64_t a2, uint64_t *a3, uint64_t a4)
{
  v5 = a2;
  v27 = 0;
  v28 = 0;
  v29 = 0;
  if (a3)
  {
    v7 = a3;
    v8 = SaveDYMTLLinkedFunctionsGroup(a1, a2, 0, a4);
    v10 = (v7 + 2);
    v9 = v7[2];
    v12 = v7 + 1;
    v11 = v7[1];
  }

  else
  {
    v9 = 0;
    v11 = 0;
    v8 = 0;
    v7 = &v27;
    v10 = &v29;
    v12 = &v28;
  }

  v13 = v11 + 8;
  if (v11 + 8 <= v9)
  {
    *(*v7 + v11) = v8;
    v13 = *v12 + 8;
    v9 = *v10;
  }

  *v12 = v13;
  result = v13 + 8;
  if (v13 + 8 <= v9)
  {
    *(*v7 + v13) = v5;
    result = *v12 + 8;
  }

  *v12 = result;
  if (v5 >= 1)
  {
    v15 = (a1 + 16);
    v16 = v5;
    do
    {
      v17 = result + 8;
      if (result + 8 <= *v10)
      {
        *(*v7 + result) = *v15;
        v17 = *v12 + 8;
      }

      *v12 = v17;
      v15 += 6;
      result = v17;
      --v16;
    }

    while (v16);
    for (i = 0; i != v5; ++i)
    {
      v19 = a1 + 24 * i;
      WriteDynamicBufferString(v7, *v19);
      if (*(v19 + 16))
      {
        v20 = 0;
        do
        {
          v26 = *(*(v19 + 8) + 8 * v20);
          entry = find_entry(a4, &v26, 8uLL, 0);
          v22 = &dword_8;
          if (*entry)
          {
            v23 = *(*entry + 32);
            if (v23)
            {
              v24 = atomic_load((v23 + 56));
              v22 = (v23 + (~(v24 >> 2) & 8));
            }

            else
            {
              v22 = &dword_8;
            }
          }

          v25 = *v12 + 8;
          if (v25 <= *v10)
          {
            *(*v7 + *v12) = *v22;
            v25 = *v12 + 8;
          }

          *v12 = v25;
          ++v20;
        }

        while (v20 < *(v19 + 16));
      }
    }

    return *v12;
  }

  return result;
}

void WriteDynamicBufferString(void *a1, const char *a2)
{
  if (a2)
  {
    v3 = a2;
  }

  else
  {
    v3 = "";
  }

  v4 = strlen(v3);
  v5 = v4 + 1;
  v7 = a1[1];
  v6 = a1[2];
  v8 = v7 + v4 + 1;
  if (v8 <= v6)
  {
    memcpy((*a1 + v7), v3, v5);
    v6 = a1[2];
    v8 = a1[1] + v5;
  }

  a1[1] = v8;
  v9 = (v4 & 0xFFFFFFFFFFFFFFF8) + ~v4;
  v10 = v9 + 8;
  v11 = v6 > v8;
  v12 = v6 - v8;
  if (v11)
  {
    if (v12 >= v10)
    {
      v13 = v9 + 8;
    }

    else
    {
      v13 = v12;
    }

    bzero((*a1 + v8), v13);
    v8 = a1[1];
  }

  a1[1] = v8 + v10;
}

void **MakeDYMTLPipelineBufferDescriptor(void **result, _BYTE *a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *(*result)++;
  if (a5)
  {
    *a2 = v5;
  }

  return result;
}

void *SaveDYMTLPipelineBufferDescriptor(void *result, unsigned __int8 *a2)
{
  v2 = result[1];
  v3 = v2 + 8;
  if ((v2 + 8) <= result[2])
  {
    *(*result + v2) = *a2;
    v3 = result[1] + 8;
  }

  result[1] = v3;
  return result;
}

void *MakeDYMTLStencilDescriptor(void *result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  *result += 8;
  v6 = v5[1];
  *result = v5 + 2;
  v7 = v5[2];
  *result = v5 + 3;
  v8 = v5[3];
  *result = v5 + 4;
  v9 = v5[4];
  *result = v5 + 5;
  v10 = v5[5];
  *result = v5 + 6;
  v11 = v5[6];
  *result = v5 + 7;
  if (a5)
  {
    *a2 = v10;
    *(a2 + 4) = v11;
    *(a2 + 8) = v8;
    *(a2 + 9) = v9;
    *(a2 + 10) = v6;
    *(a2 + 11) = v7;
    *(a2 + 12) = 0;
  }

  return result;
}

void *SaveDYMTLStencilDescriptor(void *result, unsigned int *a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = 0;
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = *(a2 + 10);
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = *(a2 + 11);
    v2 = result[2];
    v6 = result[1] + 8;
  }

  result[1] = v6;
  v7 = v6 + 8;
  if (v6 + 8 <= v2)
  {
    *(*result + v6) = *(a2 + 8);
    v2 = result[2];
    v7 = result[1] + 8;
  }

  result[1] = v7;
  v8 = v7 + 8;
  if (v7 + 8 <= v2)
  {
    *(*result + v7) = *(a2 + 9);
    v2 = result[2];
    v8 = result[1] + 8;
  }

  result[1] = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v2)
  {
    *(*result + v8) = *a2;
    v2 = result[2];
    v9 = result[1] + 8;
  }

  result[1] = v9;
  v10 = v9 + 8;
  if (v9 + 8 <= v2)
  {
    *(*result + v9) = a2[1];
    v10 = result[1] + 8;
  }

  result[1] = v10;
  return result;
}

uint64_t DecodeDYMTLSamplerDescriptor(uint64_t a1, uint64_t a2, uint64_t a3, int a4)
{
  v7 = *a1;
  v35 = *(a1 + 16);
  v36 = *(a1 + 24);
  v37 = *(a1 + 8);
  v8 = *(a1 + 32);
  v38 = *(a1 + 40);
  v39 = *(a1 + 48);
  v9 = *(a1 + 56);
  v10 = *(a1 + 64);
  v11 = *(a1 + 72);
  v12 = (a1 + 88);
  v13 = *(a1 + 80);
  v14 = strlen((a1 + 88));
  if (*(a1 + 88))
  {
    v15 = v12;
  }

  else
  {
    v15 = 0;
  }

  if (*(a1 + 88))
  {
    v16 = strlen(v15);
    v17 = (a3 + 592);
    v18 = *(a3 + 592);
    v19 = v16 + 1;
    if (v16 != -1)
    {
      v20 = *(a3 + 592);
      goto LABEL_9;
    }
  }

  else
  {
    v17 = (a3 + 592);
    v18 = *(a3 + 592);
  }

  v19 = 0;
  v20 = 0;
LABEL_9:
  *v17 = v18 + v19;
  if (a4)
  {
    memcpy(v20, v15, v19);
  }

  v21 = 0uLL;
  v22 = 0;
  if (v7 < 2)
  {
    v32 = 0;
    v30 = 0;
    v27 = 0;
    v25 = 0;
    v24 = 0;
LABEL_24:
    v29 = 0;
    v28 = 0;
    v26 = 0;
LABEL_25:
    v31 = 0;
    goto LABEL_26;
  }

  v23 = &v12[v14 & 0xFFFFFFFFFFFFFFF8];
  v24 = *(v23 + 8);
  if (v7 < 8)
  {
    v32 = 0;
    v30 = 0;
    v27 = 0;
    v25 = 0;
    goto LABEL_24;
  }

  v25 = v23[16];
  if (v7 < 0xA)
  {
    v32 = 0;
    v30 = 0;
    v27 = 0;
    goto LABEL_24;
  }

  v26 = *(v23 + 24);
  if (v7 < 0x11)
  {
    v32 = 0;
    v30 = 0;
    v27 = 0;
LABEL_41:
    v29 = 0;
    v28 = 0;
    goto LABEL_25;
  }

  v27 = *(v23 + 32);
  if (v7 < 0x16)
  {
    v32 = 0;
    v30 = 0;
    goto LABEL_41;
  }

  v28 = *(v23 + 5);
  v29 = *(v23 + 48);
  if (v7 < 0x1C)
  {
    v32 = 0;
    v30 = 0;
    goto LABEL_25;
  }

  v30 = v23[56];
  if (v7 < 0x27)
  {
    v32 = 0;
    goto LABEL_25;
  }

  v26 = *(v23 + 64);
  v21 = vuzp1q_s32(*(v23 + 72), *(v23 + 88));
  v31 = v23[104];
  if (v7 < 0x2A)
  {
    v32 = 0;
  }

  else
  {
    v32 = *(v23 + 56);
    if (v7 >= 0x53)
    {
      v22 = *(v23 + 30);
    }
  }

LABEL_26:
  v33 = *(a3 + 16);
  result = v33 + 72;
  *(a3 + 16) = v33 + 72;
  if (a4)
  {
    if (v29 == 255)
    {
      LOBYTE(v29) = 0;
    }

    *v33 = v20;
    *(v33 + 8) = v8;
    if (v28 == -1)
    {
      v28 = 0;
    }

    *(v33 + 16) = v28;
    if (v27 == 255)
    {
      LOBYTE(v27) = 0;
    }

    *(v33 + 24) = v21;
    if (v26 == 255)
    {
      LOBYTE(v26) = 0;
    }

    if (v24 == 255)
    {
      LOBYTE(v24) = 0;
    }

    *(v33 + 40) = v22;
    *(v33 + 44) = v13;
    *(v33 + 48) = v11;
    *(v33 + 52) = v32;
    *(v33 + 54) = v26;
    *(v33 + 55) = v25;
    *(v33 + 56) = v29;
    *(v33 + 57) = v31;
    *(v33 + 58) = v24;
    *(v33 + 59) = v35;
    *(v33 + 60) = v37;
    *(v33 + 61) = v36;
    *(v33 + 62) = v10;
    *(v33 + 63) = v9;
    *(v33 + 64) = v30;
    *(v33 + 65) = v38;
    *(v33 + 66) = v27;
    *(v33 + 67) = v39;
    *(v33 + 68) = 0;
  }

  return result;
}

unint64_t EncodeDYMTLSamplerDescriptor(uint64_t a1, uint64_t a2, unint64_t a3)
{
  v28 = a2;
  v30 = a3;
  if (a3 <= 7)
  {
    v29 = 8;
LABEL_9:
    v29 = 56;
    goto LABEL_10;
  }

  *a2 = 115;
  v29 = 8;
  if (a3 < 0x10)
  {
    goto LABEL_9;
  }

  *(a2 + 8) = *(a1 + 60);
  if (a3 < 0x18)
  {
    goto LABEL_9;
  }

  *(a2 + 16) = *(a1 + 59);
  if (a3 < 0x20)
  {
    goto LABEL_9;
  }

  *(a2 + 24) = *(a1 + 61);
  if (a3 < 0x28)
  {
    goto LABEL_9;
  }

  *(a2 + 32) = *(a1 + 8);
  if (a3 < 0x30)
  {
    goto LABEL_9;
  }

  *(a2 + 40) = *(a1 + 65);
  if (a3 <= 0x37)
  {
    goto LABEL_9;
  }

  *(a2 + 48) = *(a1 + 67);
  v29 = 56;
  if (a3 >= 0x40)
  {
    *(a2 + 56) = *(a1 + 63);
    if (a3 >= 0x48)
    {
      *(a2 + 64) = *(a1 + 62);
      if (a3 >= 0x4C)
      {
        *(a2 + 72) = *(a1 + 48);
        if (a3 >= 0x4D)
        {
          v26 = a3 - 76 >= 4 ? 4 : a3 - 76;
          bzero((a2 + 76), v26);
          if (a3 >= 0x54)
          {
            *(a2 + 80) = *(a1 + 44);
            if (a3 >= 0x55)
            {
              if (a3 - 84 >= 4)
              {
                v27 = 4;
              }

              else
              {
                v27 = a3 - 84;
              }

              bzero((a2 + 84), v27);
            }
          }
        }
      }
    }
  }

LABEL_10:
  v29 = 88;
  WriteDynamicBufferString(&v28, *a1);
  v6 = v30;
  v7 = v29 + 8;
  if (v29 + 8 <= v30)
  {
    *(v28 + v29) = *(a1 + 58);
    v6 = v30;
    v7 = v29 + 8;
  }

  v29 = v7;
  v8 = v7 + 8;
  if (v7 + 8 <= v6)
  {
    *(v28 + v7) = *(a1 + 55);
    v6 = v30;
    v8 = v29 + 8;
  }

  v29 = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v6)
  {
    *(v28 + v8) = *(a1 + 54);
    v6 = v30;
    v9 = v29 + 8;
  }

  v29 = v9;
  v10 = v9 + 8;
  if (v9 + 8 <= v6)
  {
    *(v28 + v9) = *(a1 + 66);
    v6 = v30;
    v10 = v29 + 8;
  }

  v29 = v10;
  v11 = v10 + 8;
  if (v10 + 8 <= v6)
  {
    *(v28 + v10) = *(a1 + 16);
    v6 = v30;
    v11 = v29 + 8;
  }

  v29 = v11;
  v12 = v11 + 8;
  if (v11 + 8 <= v6)
  {
    *(v28 + v11) = *(a1 + 56);
    v6 = v30;
    v12 = v29 + 8;
  }

  v29 = v12;
  v13 = v12 + 8;
  if (v12 + 8 <= v6)
  {
    *(v28 + v12) = *(a1 + 64);
    v6 = v30;
    v13 = v29 + 8;
  }

  v29 = v13;
  v14 = v13 + 8;
  if (v13 + 8 <= v6)
  {
    *(v28 + v13) = *(a1 + 54);
    v6 = v30;
    v14 = v29 + 8;
  }

  v29 = v14;
  v15 = v14 + 8;
  if (v14 + 8 <= v6)
  {
    *(v28 + v14) = *(a1 + 24);
    v6 = v30;
    v15 = v29 + 8;
  }

  v29 = v15;
  v16 = v15 + 8;
  if (v15 + 8 <= v6)
  {
    *(v28 + v15) = *(a1 + 28);
    v6 = v30;
    v16 = v29 + 8;
  }

  v29 = v16;
  v17 = v16 + 8;
  if (v16 + 8 <= v6)
  {
    *(v28 + v16) = *(a1 + 32);
    v6 = v30;
    v17 = v29 + 8;
  }

  v29 = v17;
  v18 = v17 + 8;
  if (v17 + 8 <= v6)
  {
    *(v28 + v17) = *(a1 + 36);
    v6 = v30;
    v18 = v29 + 8;
  }

  v29 = v18;
  v19 = v18 + 8;
  if (v18 + 8 <= v6)
  {
    *(v28 + v18) = *(a1 + 57);
    v6 = v30;
    v19 = v29 + 8;
  }

  v29 = v19;
  v20 = v19 + 8;
  if (v19 + 8 <= v6)
  {
    *(v28 + v19) = *(a1 + 52);
    v6 = v30;
    v20 = v29 + 8;
  }

  v29 = v20;
  v21 = v20 + 4;
  if (v20 + 4 <= v6)
  {
    *(v28 + v20) = *(a1 + 40);
    v6 = v30;
    v21 = v29 + 4;
  }

  v22 = v6 > v21;
  v23 = v6 - v21;
  if (v22)
  {
    if (v23 >= 4)
    {
      v24 = 4;
    }

    else
    {
      v24 = v23;
    }

    bzero((v28 + v21), v24);
  }

  return v21 + 4;
}

uint64_t DecodeDYMTLHeapDescriptor(uint64_t result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 24);
  *(a3 + 24) = v4 + 24;
  if (a4)
  {
    *v4 = GTMTLHeapDescriptorDefaults;
    *(v4 + 16) = 32;
    v5 = *result;
    *(v4 + 8) = *(result + 8);
    *(v4 + 20) = *(result + 32);
    *(v4 + 16) = *(result + 56);
    if (v5 >= 0x2A)
    {
      *(v4 + 19) = *(result + 64);
      if (v5 >= 0x34)
      {
        *v4 = *(result + 72);
        if (v5 >= 0x66)
        {
          *(v4 + 18) = *(result + 80);
        }
      }
    }
  }

  return result;
}

uint64_t EncodeDYMTLHeapDescriptor(uint64_t a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *(a1 + 8);
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 16) & 0xF;
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 16) >> 4;
          if (a3 >= 0x28)
          {
            a2[4] = *(a1 + 20);
            if (a3 >= 0x30)
            {
              a2[5] = (*(a1 + 16) & 0x300) == 512;
              if (a3 >= 0x38)
              {
                a2[6] = (*(a1 + 16) >> 8) & 3;
                if (a3 >= 0x40)
                {
                  a2[7] = *(a1 + 16);
                  if (a3 >= 0x48)
                  {
                    a2[8] = *(a1 + 19);
                    if (a3 >= 0x50)
                    {
                      a2[9] = *a1;
                      if (a3 >= 0x58)
                      {
                        a2[10] = *(a1 + 18);
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }

  return 88;
}

uint64_t DecodeDYMTLIndirectCommandBufferDescriptor(uint64_t result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 32);
  *(a3 + 32) = v4 + 48;
  if (a4)
  {
    *v4 = GTMTLIndirectCommandBufferDescriptorDefaults;
    *(v4 + 16) = unk_29BF78;
    *(v4 + 32) = xmmword_29BF88;
    v5 = *result;
    *(v4 + 16) = *(result + 8);
    *(v4 + 26) = *(result + 16);
    *(v4 + 19) = *(result + 24);
    *(v4 + 38) = *(result + 32);
    *(v4 + 31) = *(result + 40);
    if (v5 >= 0x1B)
    {
      *(v4 + 32) = *(result + 48);
      *(v4 + 33) = *(result + 56);
      if (v5 >= 0x29)
      {
        *(v4 + 8) = *(result + 64);
        if (v5 >= 0x3C)
        {
          *(v4 + 41) = *(result + 72);
          if (v5 >= 0x48)
          {
            *(v4 + 35) = *(result + 80);
            *(v4 + 34) = *(result + 88);
            *(v4 + 36) = *(result + 96);
            if (v5 != 72)
            {
              *(v4 + 40) = *(result + 104);
              if (v5 >= 0x4A)
              {
                *(v4 + 42) = *(result + 112);
                if (v5 >= 0x65)
                {
                  *(v4 + 37) = *(result + 120);
                  *(v4 + 39) = *(result + 128);
                  *(v4 + 23) = *(result + 136);
                  *(v4 + 21) = *(result + 144);
                  *(v4 + 28) = *(result + 152);
                  *(v4 + 22) = *(result + 160);
                  *(v4 + 20) = *(result + 168);
                  *(v4 + 25) = *(result + 176);
                  *(v4 + 29) = *(result + 184);
                  *(v4 + 24) = *(result + 192);
                  *(v4 + 27) = *(result + 200);
                  *(v4 + 30) = *(result + 208);
                  *(v4 + 18) = *(result + 216);
                  if (v5 >= 0x69)
                  {
                    *v4 = *(result + 224);
                  }
                }
              }
            }
          }
        }
      }
    }
  }

  return result;
}

uint64_t EncodeDYMTLIndirectCommandBufferDescriptor(uint64_t a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *(a1 + 16);
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 26);
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 19);
          if (a3 >= 0x28)
          {
            a2[4] = *(a1 + 38);
            if (a3 >= 0x30)
            {
              a2[5] = *(a1 + 31);
              if (a3 >= 0x38)
              {
                a2[6] = *(a1 + 32);
                if (a3 >= 0x40)
                {
                  a2[7] = *(a1 + 33);
                  if (a3 >= 0x48)
                  {
                    a2[8] = *(a1 + 8);
                    if (a3 >= 0x50)
                    {
                      a2[9] = *(a1 + 41);
                      if (a3 >= 0x58)
                      {
                        a2[10] = *(a1 + 35);
                        if (a3 >= 0x60)
                        {
                          a2[11] = *(a1 + 34);
                          if (a3 >= 0x68)
                          {
                            a2[12] = *(a1 + 36);
                            if (a3 >= 0x70)
                            {
                              a2[13] = *(a1 + 40);
                              if (a3 >= 0x78)
                              {
                                a2[14] = *(a1 + 42);
                                if (a3 >= 0x80)
                                {
                                  a2[15] = *(a1 + 37);
                                  if (a3 >= 0x88)
                                  {
                                    a2[16] = *(a1 + 39);
                                    if (a3 >= 0x90)
                                    {
                                      a2[17] = *(a1 + 23);
                                      if (a3 >= 0x98)
                                      {
                                        a2[18] = *(a1 + 21);
                                        if (a3 >= 0xA0)
                                        {
                                          a2[19] = *(a1 + 28);
                                          if (a3 >= 0xA8)
                                          {
                                            a2[20] = *(a1 + 22);
                                            if (a3 >= 0xB0)
                                            {
                                              a2[21] = *(a1 + 20);
                                              if (a3 >= 0xB8)
                                              {
                                                a2[22] = *(a1 + 25);
                                                if (a3 >= 0xC0)
                                                {
                                                  a2[23] = *(a1 + 29);
                                                  if (a3 >= 0xC8)
                                                  {
                                                    a2[24] = *(a1 + 24);
                                                    if (a3 >= 0xD0)
                                                    {
                                                      a2[25] = *(a1 + 27);
                                                      if (a3 >= 0xD8)
                                                      {
                                                        a2[26] = *(a1 + 30);
                                                        if (a3 >= 0xE0)
                                                        {
                                                          a2[27] = *(a1 + 18);
                                                          if (a3 >= 0xE8)
                                                          {
                                                            a2[28] = *a1;
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }

  return 232;
}

void **MakeDYMTLAttributeDescriptor(void **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = *(*result)++;
  v7 = v5[1];
  *result = v5 + 2;
  v8 = v5[2];
  *result = v5 + 3;
  if (a5)
  {
    *a2 = v7;
    *(a2 + 4) = v8;
    *(a2 + 5) = v6;
    *(a2 + 6) = 0;
  }

  return result;
}

void *SaveDYMTLAttributeDescriptor(void *result, unsigned int *a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = *(a2 + 5);
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = *a2;
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = *(a2 + 4);
    v6 = result[1] + 8;
  }

  result[1] = v6;
  return result;
}

void **MakeDYMTLBufferLayoutDescriptor(void **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = *(*result)++;
  v7 = v5[1];
  *result = v5 + 2;
  v8 = v5[2];
  *result = v5 + 3;
  if (a5)
  {
    if (v6 == 0xFFFFFFFFLL)
    {
      v6 = -1;
    }

    *a2 = v6;
    *(a2 + 8) = v7;
    *(a2 + 10) = v8;
    *(a2 + 11) = 0;
    *(a2 + 15) = 0;
  }

  return result;
}

void *SaveDYMTLBufferLayoutDescriptor(void *result, uint64_t a2)
{
  v3 = result[1];
  v2 = result[2];
  v4 = v3 + 8;
  if (v3 + 8 <= v2)
  {
    *(*result + v3) = *a2;
    v2 = result[2];
    v4 = result[1] + 8;
  }

  result[1] = v4;
  v5 = v4 + 8;
  if (v4 + 8 <= v2)
  {
    *(*result + v4) = *(a2 + 8);
    v2 = result[2];
    v5 = result[1] + 8;
  }

  result[1] = v5;
  v6 = v5 + 8;
  if (v5 + 8 <= v2)
  {
    *(*result + v5) = *(a2 + 10);
    v6 = result[1] + 8;
  }

  result[1] = v6;
  return result;
}

void **MakeDYMTLTileRenderPipelineColorAttachmentDescriptor(void **result, _WORD *a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *(*result)++;
  if (a5)
  {
    *a2 = v5;
  }

  return result;
}

void *SaveDYMTLTileRenderPipelineColorAttachmentDescriptor(void *result, unsigned __int16 *a2)
{
  v2 = result[1];
  v3 = v2 + 8;
  if ((v2 + 8) <= result[2])
  {
    *(*result + v2) = *a2;
    v3 = result[1] + 8;
  }

  result[1] = v3;
  return result;
}

uint64_t DecodeDYMTLMotionEstimationPipelineDescriptor(uint64_t result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 40);
  *(a3 + 40) = v4 + 4;
  if (a4)
  {
    *v4 = 0;
    *(v4 + 2) = *(result + 8);
    *v4 = *(result + 16);
  }

  return result;
}

uint64_t EncodeDYMTLMotionEstimationPipelineDescriptor(unsigned __int16 *a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = a1[1];
      if (a3 >= 0x18)
      {
        a2[2] = *a1;
      }
    }
  }

  return 24;
}

uint64_t DecodeDYMTLCommandQueueDescriptor(uint64_t result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 48);
  *(a3 + 48) = v4 + 16;
  if (a4)
  {
    *v4 = GTMTLCommandQueueDescriptorDefaults;
    v5 = *result;
    *v4 = *(result + 8);
    *(v4 + 8) = *(result + 16);
    *(v4 + 4) = *(result + 24);
    *(v4 + 7) = *(result + 32);
    if (v5 >= 0x2A)
    {
      *(v4 + 6) = *(result + 40);
      if (v5 >= 0x51)
      {
        *(v4 + 5) = *(result + 48);
      }
    }
  }

  return result;
}

uint64_t EncodeDYMTLCommandQueueDescriptor(unsigned __int8 *a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *a1;
      if (a3 >= 0x18)
      {
        a2[2] = a1[8];
        if (a3 >= 0x20)
        {
          a2[3] = a1[4];
          if (a3 >= 0x28)
          {
            a2[4] = a1[7];
            if (a3 >= 0x30)
            {
              a2[5] = a1[6];
              if (a3 >= 0x38)
              {
                a2[6] = a1[5];
              }
            }
          }
        }
      }
    }
  }

  return 56;
}

void *DecodeDYMTLIOCommandQueueDescriptor(void *result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 56);
  *(a3 + 56) = v4 + 8;
  if (a4)
  {
    *v4 = 64;
    *v4 = result[1];
    *(v4 + 4) = result[2];
    *(v4 + 5) = result[3];
  }

  return result;
}

uint64_t EncodeDYMTLIOCommandQueueDescriptor(unsigned int *a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *a1;
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 4);
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 5);
        }
      }
    }
  }

  return 32;
}

uint64_t *DecodeDYMTLCommandBufferDescriptor(uint64_t *result, uint64_t a2, uint64_t a3, int a4, uint64_t a5)
{
  v5 = *(a3 + 64);
  *(a3 + 64) = v5 + 16;
  if (a4)
  {
    *v5 = GTMTLCommandBufferDescriptorDefaults;
    *(v5 + 8) = result[1];
    *(v5 + 9) = result[2];
    v6 = result[3];
    *v5 = v6;
    if (a5)
    {
      result = GetStream_19685(a5, v6);
      if (result)
      {
        v6 = *result;
      }

      else
      {
        v6 = 0;
      }
    }

    *v5 = v6;
  }

  return result;
}

uint64_t EncodeDYMTLCommandBufferDescriptor(uint64_t *a1, void *a2, unint64_t a3, uint64_t a4)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *(a1 + 8);
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 9);
      }
    }
  }

  v11 = *a1;
  v6 = &dword_8;
  entry = find_entry(a4, &v11, 8uLL, 0);
  if (*entry)
  {
    v8 = *(*entry + 32);
    if (v8)
    {
      v9 = atomic_load((v8 + 56));
      v6 = (v8 + (~(v9 >> 2) & 8));
    }

    else
    {
      v6 = &dword_8;
    }
  }

  if (a3 >= 0x20)
  {
    a2[3] = *v6;
  }

  return 32;
}

size_t MakeDYMTLFunctionConstant(const char **a1, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v9 = *a1;
  v10 = **a1;
  v11 = *a1 + 8;
  *a1 = v11;
  result = strlen(v11);
  *a1 = &v11[(result & 0xFFFFFFFFFFFFFFF8) + 8];
  if (v9[8])
  {
    result = strlen(v11);
    v15 = (a3 + 592);
    v16 = *(a3 + 592);
    v13 = result + 1;
    if (result != -1)
    {
      v14 = *(a3 + 592);
      goto LABEL_6;
    }
  }

  else
  {
    v11 = 0;
    v15 = (a3 + 592);
    v16 = *(a3 + 592);
  }

  v13 = 0;
  v14 = 0;
LABEL_6:
  *v15 = v16 + v13;
  if (a5)
  {
    result = memcpy(v14, v11, v13);
  }

  v17 = *a1;
  v18 = **a1;
  *a1 += 8;
  v19 = *(v17 + 1);
  *a1 = v17 + 16;
  v20 = *(v17 + 2);
  v21 = v17 + 24;
  *a1 = v21 + ((v20 + 7) & 0xFFFFFFFFFFFFFFF8);
  v22 = *(a3 + 592);
  *(a3 + 592) = &v22[v20];
  if (a5)
  {
    if (v20)
    {
      v23 = v22;
    }

    else
    {
      v23 = 0;
    }

    if (v20)
    {
      v24 = v21;
    }

    else
    {
      v24 = 0;
    }

    result = memcpy(v23, v24, v20);
    *a2 = v23;
    *(a2 + 8) = v20;
    *(a2 + 16) = v14;
    *(a2 + 24) = v10;
    *(a2 + 26) = v19;
    *(a2 + 27) = v18;
    *(a2 + 28) = 0;
  }

  return result;
}

void SaveDYMTLFunctionConstant(void *a1, uint64_t a2)
{
  v4 = a1[1];
  v5 = v4 + 8;
  if ((v4 + 8) <= a1[2])
  {
    *(*a1 + v4) = *(a2 + 24);
    v5 = a1[1] + 8;
  }

  a1[1] = v5;
  WriteDynamicBufferString(a1, *(a2 + 16));
  v7 = a1[1];
  v6 = a1[2];
  v8 = v7 + 8;
  if (v7 + 8 <= v6)
  {
    *(*a1 + v7) = *(a2 + 27);
    v6 = a1[2];
    v8 = a1[1] + 8;
  }

  a1[1] = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v6)
  {
    *(*a1 + v8) = *(a2 + 26);
    v9 = a1[1] + 8;
  }

  a1[1] = v9;
  v10 = *a2;
  v11 = *(a2 + 8);

  WriteGTData(a1, v10, v11);
}

void WriteGTData(void *a1, const void *a2, size_t __n)
{
  v5 = a1[1];
  v6 = v5 + 8;
  if ((v5 + 8) <= a1[2])
  {
    *(*a1 + v5) = __n;
    v6 = a1[1] + 8;
  }

  a1[1] = v6;
  if (a2)
  {
    v7 = v6 + __n;
    v8 = a1[2];
    if (v6 + __n <= v8)
    {
      memcpy((*a1 + v6), a2, __n);
      v8 = a1[2];
      v7 = a1[1] + __n;
    }

    a1[1] = v7;
    v9 = ((__n + 7) & 0xFFFFFFFFFFFFFFF8) - __n;
    if (v8 > v7)
    {
      if (v8 - v7 >= v9)
      {
        v10 = v9;
      }

      else
      {
        v10 = v8 - v7;
      }

      bzero((*a1 + v7), v10);
      v7 = a1[1];
    }

    a1[1] = v7 + v9;
  }
}

uint64_t DecodeDYMTLVisibleFunctionTableDescriptor(uint64_t result, uint64_t a2, uint64_t a3, int a4)
{
  v4 = *(a3 + 72);
  *(a3 + 72) = v4 + 24;
  if (a4)
  {
    v5 = *result;
    *v4 = *(result + 8);
    if (v5 >= 0x2F)
    {
      *(v4 + 8) = *(result + 16);
      if (v5 >= 0x36)
      {
        *(v4 + 16) = *(result + 24);
      }
    }
  }

  return result;
}

uint64_t EncodeDYMTLVisibleFunctionTableDescriptor(uint64_t a1, void *a2, unint64_t a3)
{
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *a1;
      if (a3 >= 0x18)
      {
        a2[2] = *(a1 + 8);
        if (a3 >= 0x20)
        {
          a2[3] = *(a1 + 16);
        }
      }
    }
  }

  return 32;
}

int **MakeDYMTLRasterizationRateLayerDescriptor(int **result, uint64_t a2, uint64_t a3, uint64_t a4, int a5)
{
  v5 = *result;
  v6 = **result;
  *result += 2;
  v7 = *(v5 + 1);
  *result = v5 + 4;
  v8 = v5 + 6;
  v9 = *(v5 + 8);
  *result = v8;
  v10 = v6;
  if (v6)
  {
    v11 = *(a3 + 584);
    do
    {
      v12 = **result;
      *result += 2;
      v13 = *(a3 + 584);
      if (a5)
      {
        *v13 = v12;
      }

      *(a3 + 584) = v13 + 1;
      --v10;
    }

    while (v10);
  }

  else
  {
    v11 = 0;
  }

  v14 = v7;
  if (!v7)
  {
    v15 = 0;
    if ((a5 & 1) == 0)
    {
      return result;
    }

    goto LABEL_14;
  }

  v15 = *(a3 + 584);
  do
  {
    v16 = **result;
    *result += 2;
    v17 = *(a3 + 584);
    if (a5)
    {
      *v17 = v16;
    }

    *(a3 + 584) = v17 + 1;
    --v14;
  }

  while (v14);
  if (a5)
  {
LABEL_14:
    *a2 = v11;
    *(a2 + 8) = v15;
    *(a2 + 16) = v6;
    *(a2 + 18) = v9;
    *(a2 + 20) = v7;
    *(a2 + 22) = 0;
  }

  return result;
}

void SaveDYMTLRasterizationRateLayerDescriptor(void *a1, uint64_t a2)
{
  v5 = a1[1];
  v4 = a1[2];
  v6 = v5 + 8;
  if (v5 + 8 <= v4)
  {
    *(*a1 + v5) = *(a2 + 16);
    v4 = a1[2];
    v6 = a1[1] + 8;
  }

  a1[1] = v6;
  v7 = v6 + 8;
  if (v6 + 8 <= v4)
  {
    *(*a1 + v6) = *(a2 + 20);
    v4 = a1[2];
    v7 = a1[1] + 8;
  }

  a1[1] = v7;
  v8 = v7 + 8;
  if (v7 + 8 <= v4)
  {
    *(*a1 + v7) = *(a2 + 18);
    v8 = a1[1] + 8;
  }

  a1[1] = v8;
  if (*(a2 + 16))
  {
    v9 = 0;
    do
    {
      v10 = v8 + 4;
      v11 = a1[2];
      if (v8 + 4 <= v11)
      {
        *(*a1 + v8) = *(*a2 + 4 * v9);
        v11 = a1[2];
        v10 = a1[1] + 4;
      }

      a1[1] = v10;
      if (v11 > v10)
      {
        if (v11 - v10 >= 4)
        {
          v12 = 4;
        }

        else
        {
          v12 = v11 - v10;
        }

        bzero((*a1 + v10), v12);
        v10 = a1[1];
      }

      v8 = v10 + 4;
      a1[1] = v10 + 4;
      ++v9;
    }

    while (v9 < *(a2 + 16));
  }

  if (*(a2 + 20))
  {
    v13 = 0;
    do
    {
      v14 = v8 + 4;
      v15 = a1[2];
      if (v8 + 4 <= v15)
      {
        *(*a1 + v8) = *(*(a2 + 8) + 4 * v13);
        v15 = a1[2];
        v14 = a1[1] + 4;
      }

      a1[1] = v14;
      if (v15 > v14)
      {
        if (v15 - v14 >= 4)
        {
          v16 = 4;
        }

        else
        {
          v16 = v15 - v14;
        }

        bzero((*a1 + v14), v16);
        v14 = a1[1];
      }

      v8 = v14 + 4;
      a1[1] = v14 + 4;
      ++v13;
    }

    while (v13 < *(a2 + 20));
  }
}

size_t DecodeDYMTLFunctionDescriptor(uint64_t a1, uint64_t a2, void *a3, uint64_t a4, uint64_t a5)
{
  v9 = (a1 + 16);
  v38 = *(a1 + 8);
  v39 = *a1;
  v10 = strlen((a1 + 16)) & 0xFFFFFFFFFFFFFFF8;
  v11 = &v9[v10 + 8];
  if (*(a1 + 16))
  {
    v12 = v9;
  }

  else
  {
    v12 = 0;
  }

  if (*(a1 + 16))
  {
    v13 = strlen(v12);
    v14 = a3 + 74;
    v15 = a3[74];
    v16 = v13 + 1;
    if (v13 != -1)
    {
      v17 = a3[74];
      goto LABEL_9;
    }
  }

  else
  {
    v14 = a3 + 74;
    v15 = a3[74];
  }

  v16 = 0;
  v17 = 0;
LABEL_9:
  *v14 = v15 + v16;
  if (a4)
  {
    memcpy(v17, v12, v16);
  }

  result = strlen(v11);
  v19 = result & 0xFFFFFFFFFFFFFFF8;
  v20 = &v11[result & 0xFFFFFFFFFFFFFFF8];
  if (*v11)
  {
    v21 = v11;
  }

  else
  {
    v21 = 0;
  }

  if (*v11)
  {
    result = strlen(v21);
    v22 = a3 + 74;
    v23 = a3[74];
    v24 = result + 1;
    if (result != -1)
    {
      v25 = a3[74];
      goto LABEL_19;
    }
  }

  else
  {
    v22 = a3 + 74;
    v23 = a3[74];
  }

  v24 = 0;
  v25 = 0;
LABEL_19:
  v37 = v17;
  *v22 = v23 + v24;
  if (a4)
  {
    result = memcpy(v25, v21, v24);
  }

  v26 = *(v20 + 1);
  v27 = (v20 + 16);
  if (v26)
  {
    v28 = a3[59];
    result = DecodeDYMTLFunctionConstantValues(v27, v26, a3, a4);
  }

  else
  {
    v28 = 0;
  }

  if (v39 < 0x3E)
  {
    v30 = 0;
    goto LABEL_36;
  }

  v29 = (v26 + 7) & 0xFFFFFFFFFFFFFFF8;
  v30 = *(v27 + v29);
  if (!v30)
  {
LABEL_36:
    v32 = 0;
    goto LABEL_37;
  }

  v40 = v28;
  v31 = 0;
  v32 = a3[72];
  v33 = v10 + v19 + v29 + a1 + 48;
  v34 = v32;
  do
  {
    if (a4)
    {
      v35 = *(v33 + 8 * v31);
      if (a5)
      {
        result = GetStream_19685(a5, v35);
        if (result)
        {
          v35 = *result;
        }

        else
        {
          v35 = 0;
        }
      }

      v34 = a3[72];
      *v34 = v35;
    }

    a3[72] = ++v34;
    ++v31;
  }

  while (v30 != v31);
  v28 = v40;
LABEL_37:
  v36 = a3[10];
  a3[10] = v36 + 48;
  if (a4)
  {
    *v36 = v28;
    *(v36 + 8) = v37;
    *(v36 + 16) = v32;
    *(v36 + 24) = v30;
    *(v36 + 32) = v25;
    *(v36 + 40) = v38;
    *(v36 + 41) = 0;
    *(v36 + 44) = 0;
  }

  return result;
}

const char *DecodeDYMTLFunctionConstantValues(uint64_t a1, const char *a2, uint64_t a3, uint64_t a4)
{
  v4 = a4;
  v7 = *(a1 + 8);
  result = (a1 + 8);
  v12[0] = result;
  v12[1] = a2;
  v8 = *(a3 + 488);
  if (v7 == -1)
  {
    v9 = 0;
  }

  else
  {
    v9 = 0;
    do
    {
      ++v9;
      v10 = *(a3 + 488);
      *(a3 + 488) = v10 + 32;
      result = MakeDYMTLFunctionConstant(v12, v10, a3, a4, v4);
    }

    while (*v12[0] != -1);
  }

  v11 = *(a3 + 472);
  *(a3 + 472) = v11 + 2;
  if (v4)
  {
    *v11 = v8;
    v11[1] = v9;
  }

  return result;
}

unint64_t EncodeDYMTLFunctionDescriptor(void **a1, void *a2, unint64_t a3, uint64_t a4)
{
  v21 = a2;
  v23 = a3;
  if (a3 >= 8)
  {
    *a2 = 115;
    if (a3 >= 0x10)
    {
      a2[1] = *(a1 + 40);
    }
  }

  v22 = 16;
  WriteDynamicBufferString(&v21, a1[1]);
  WriteDynamicBufferString(&v21, a1[4]);
  if (*a1)
  {
    v6 = EncodeDYMTLFunctionConstantValues(*a1, 0, 0);
    v7 = v6;
    v8 = v23;
    v9 = v22 + 8;
    v10 = v23 - (v22 + 8);
    if (v23 >= v22 + 8)
    {
      *(v21 + v22) = v6;
    }

    if (v6 && v9 + v6 <= v8)
    {
      EncodeDYMTLFunctionConstantValues(*a1, (v21 + v9), v10);
    }
  }

  else
  {
    v8 = v23;
    v9 = v22 + 8;
    v7 = 0;
    if (v22 + 8 <= v23)
    {
      *(v21 + v22) = 0;
    }
  }

  v11 = v9 + v7;
  v12 = a1[3];
  v13 = v9 + v7 + 8;
  if (v13 <= v8)
  {
    *(v21 + v11) = v12;
    v12 = a1[3];
  }

  v22 = v13;
  if (!v12)
  {
    return v13;
  }

  v14 = 0;
  do
  {
    v20 = a1[2][v14];
    entry = find_entry(a4, &v20, 8uLL, 0);
    if (*entry && (v16 = *(*entry + 32)) != 0)
    {
      v17 = atomic_load((v16 + 56));
      v18 = (v16 + (~(v17 >> 2) & 8));
      v13 = v22;
      v8 = v23;
    }

    else
    {
      v18 = &dword_8;
    }

    result = v13 + 8;
    if (v13 + 8 <= v8)
    {
      *(v21 + v13) = *v18;
    }

    v22 = v13 + 8;
    ++v14;
    v13 += 8;
  }

  while (a1[3] > v14);
  return result;
}

uint64_t EncodeDYMTLFunctionConstantValues(void *a1, void *a2, unint64_t a3)
{
  v8 = a2;
  v10 = a3;
  if (a3 >= 8)
  {
    *a2 = 115;
  }

  v4 = 8;
  v9 = 8;
  if (a1[1])
  {
    v5 = 0;
    v6 = 0;
    do
    {
      SaveDYMTLFunctionConstant(&v8, *a1 + v5);
      ++v6;
      v5 += 32;
    }

    while (a1[1] > v6);
    v4 = v9;
    a3 = v10;
  }

  result = v4 + 8;
  if (v4 + 8 <= a3)
  {
    *(v8 + v4) = -1;
  }

  return result;
}

void *MakeDYMTLCompileOptions(size_t **a1, uint64_t a2, uint64_t a3, unsigned int a4, int a5, uint64_t a6)
{
  v12 = (*a1 + 1);
  v13 = **a1;
  *a1 = (v12 + ((v13 + 7) & 0xFFFFFFFFFFFFFFF8));
  if (v13)
  {
    v14 = *(a3 + 592);
  }

  else
  {
    v14 = 0;
  }

  *(a3 + 592) += v13;
  if (a5)
  {
    if (v13)
    {
      v15 = v12;
    }

    else
    {
      v15 = 0;
    }

    memcpy(v14, v15, v13);
  }

  v16 = *a1;
  v17 = *(*a1)++;
  v18 = v17 == 0;
  v19 = 2 * (v17 != 0);
  v20 = *(v16 + 8);
  *a1 = v16 + 2;
  if (a4 <= 0xF)
  {
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    if (a4 < 6)
    {
      v32 = 0;
      result = 0;
      v27 = 0;
      v28 = 0;
      v29 = 0;
      v30 = 0;
      v31 = 0;
    }

    else
    {
      result = 0;
      v27 = 0;
      v28 = 0;
      v29 = 0;
      v30 = 0;
      v31 = 0;
      *a1 = v16 + 5;
      v32 = *(v16 + 10);
      *a1 = v16 + 6;
    }

    goto LABEL_35;
  }

  v59 = *(v16 + 4);
  v33 = (v16 + 3);
  *a1 = v16 + 3;
  if (a4 < 0x28)
  {
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    result = 0;
LABEL_34:
    v27 = 0;
    v28 = 0;
    v29 = 0;
    v30 = 0;
    v31 = 0;
    v32 = v59;
    goto LABEL_35;
  }

  v55 = v20;
  v56 = 2 * (v17 != 0);
  v57 = v17 == 0;
  v58 = v17;
  *a1 = &v33[(strlen(v16 + 24) & 0xFFFFFFFFFFFFFFF8) + 8];
  if (*v33)
  {
    v34 = v33;
  }

  else
  {
    v34 = 0;
  }

  if (*v33)
  {
    v35 = strlen(v34);
    v36 = *(a3 + 592);
    v37 = v35 + 1;
    if (v35 != -1)
    {
      result = *(a3 + 592);
      v18 = v57;
      LOBYTE(v17) = v58;
      v20 = v55;
      v19 = v56;
      goto LABEL_23;
    }
  }

  else
  {
    v36 = *(a3 + 592);
  }

  v18 = v57;
  LOBYTE(v17) = v58;
  v20 = v55;
  v19 = v56;
  v37 = 0;
  result = 0;
LABEL_23:
  *(a3 + 592) = v36 + v37;
  if (a5)
  {
    v38 = result;
    memcpy(result, v34, v37);
    result = v38;
    v20 = v55;
    v19 = v56;
    v18 = v57;
    LOBYTE(v17) = v58;
  }

  if (a4 < 0x2B)
  {
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    goto LABEL_34;
  }

  v39 = (*a1 + 1);
  v54 = **a1;
  *a1 = v39;
  if (a4 < 0x30)
  {
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
    v27 = 0;
    v28 = 0;
    v29 = 0;
    v30 = 0;
    v31 = 0;
    v32 = v59;
    v25 = v54;
    goto LABEL_35;
  }

  v53 = result;
  *a1 = &v39[(strlen(v39) & 0xFFFFFFFFFFFFFFF8) + 8];
  if (*v39)
  {
    v40 = v39;
  }

  else
  {
    v40 = 0;
  }

  if (*v39)
  {
    v41 = strlen(v40);
    v42 = *(a3 + 592);
    v43 = v41 + 1;
    if (v41 != -1)
    {
      __dst = *(a3 + 592);
      v18 = v57;
      LOBYTE(v17) = v58;
      v20 = v55;
      v19 = v56;
      result = v53;
      goto LABEL_41;
    }
  }

  else
  {
    v42 = *(a3 + 592);
  }

  v18 = v57;
  LOBYTE(v17) = v58;
  v20 = v55;
  v19 = v56;
  result = v53;
  v43 = 0;
  __dst = 0;
LABEL_41:
  *(a3 + 592) = v42 + v43;
  if (a5)
  {
    memcpy(__dst, v40, v43);
    result = v53;
    v20 = v55;
    v19 = v56;
    v18 = v57;
    LOBYTE(v17) = v58;
  }

  v44 = *a1;
  v51 = *(*a1)++;
  v27 = v44[1];
  *a1 = v44 + 2;
  if (v27)
  {
    v50 = v14;
    v29 = *(a3 + 576);
    v45 = v27;
    do
    {
      v46 = *(*a1)++;
      if (a5)
      {
        if (a6)
        {
          Stream_19685 = GetStream_19685(a6, v46);
          if (Stream_19685)
          {
            v46 = *Stream_19685;
          }

          else
          {
            v46 = 0;
          }
        }

        v48 = *(a3 + 576);
        *v48 = v46;
      }

      else
      {
        v48 = *(a3 + 576);
      }

      *(a3 + 576) = v48 + 1;
      --v45;
    }

    while (v45);
    v14 = v50;
    v18 = v57;
    LOBYTE(v17) = v58;
    v20 = v55;
    v19 = v56;
    result = v53;
  }

  else
  {
    v29 = 0;
  }

  if (a4 < 0x3B)
  {
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
  }

  else
  {
    v49 = *a1;
    v24 = *(*a1)++;
    if (a4 >= 0x40)
    {
      v31 = *(v49 + 8);
      *a1 = v49 + 2;
      if (a4 < 0x47)
      {
        v21 = 0;
        v22 = 0;
        v23 = 0;
      }

      else
      {
        v21 = *(v49 + 16);
        *a1 = v49 + 3;
        v22 = *(v49 + 24);
        *a1 = v49 + 4;
        v23 = *(v49 + 16);
        *a1 = v49 + 5;
        if (a4 >= 0x4C)
        {
          v18 = *(v49 + 40);
          *a1 = v49 + 6;
          v19 = *(v49 + 48);
          *a1 = v49 + 7;
        }
      }

      goto LABEL_63;
    }

    v21 = 0;
    v22 = 0;
    v23 = 0;
  }

  v31 = 0;
LABEL_63:
  v32 = v59;
  v25 = v54;
  v30 = __dst;
  v28 = v51;
LABEL_35:
  if (a5)
  {
    *a2 = v14;
    *(a2 + 8) = v13;
    *(a2 + 16) = result;
    *(a2 + 24) = v30;
    *(a2 + 32) = v29;
    *(a2 + 40) = v27;
    *(a2 + 48) = v32;
    *(a2 + 52) = v23;
    *(a2 + 54) = v22;
    *(a2 + 55) = v21;
    *(a2 + 56) = v20;
    *(a2 + 57) = v17;
    *(a2 + 58) = v28;
    *(a2 + 59) = v18;
    *(a2 + 60) = v19;
    *(a2 + 61) = v31;
    *(a2 + 62) = v25;
    *(a2 + 63) = v24;
  }

  return result;
}

void SaveDYMTLCompileOptions(void *a1, uint64_t a2, uint64_t a3)
{
  WriteGTData(a1, *a2, *(a2 + 8));
  v7 = a1[1];
  v6 = a1[2];
  v8 = v7 + 8;
  if (v7 + 8 <= v6)
  {
    *(*a1 + v7) = *(a2 + 57);
    v6 = a1[2];
    v8 = a1[1] + 8;
  }

  a1[1] = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v6)
  {
    *(*a1 + v8) = *(a2 + 56);
    v6 = a1[2];
    v9 = a1[1] + 8;
  }

  a1[1] = v9;
  v10 = v9 + 8;
  if (v9 + 8 <= v6)
  {
    *(*a1 + v9) = *(a2 + 48);
    v10 = a1[1] + 8;
  }

  a1[1] = v10;
  WriteDynamicBufferString(a1, *(a2 + 16));
  v11 = a1[1];
  v12 = v11 + 8;
  if ((v11 + 8) <= a1[2])
  {
    *(*a1 + v11) = *(a2 + 62);
    v12 = a1[1] + 8;
  }

  a1[1] = v12;
  WriteDynamicBufferString(a1, *(a2 + 24));
  v13 = a1[1];
  v14 = a1[2];
  v15 = v13 + 8;
  if (v13 + 8 <= v14)
  {
    *(*a1 + v13) = *(a2 + 58);
    v14 = a1[2];
    v15 = a1[1] + 8;
  }

  a1[1] = v15;
  v16 = *(a2 + 40);
  v17 = v15 + 8;
  if (v15 + 8 <= v14)
  {
    *(*a1 + v15) = v16;
    v17 = a1[1] + 8;
    v16 = *(a2 + 40);
  }

  a1[1] = v17;
  if (v16)
  {
    v18 = 0;
    do
    {
      v32 = *(*(a2 + 32) + 8 * v18);
      entry = find_entry(a3, &v32, 8uLL, 0);
      v20 = &dword_8;
      if (*entry)
      {
        v21 = *(*entry + 32);
        if (v21)
        {
          v22 = atomic_load((v21 + 56));
          v20 = (v21 + (~(v22 >> 2) & 8));
        }

        else
        {
          v20 = &dword_8;
        }
      }

      v23 = a1[1];
      v17 = v23 + 8;
      if ((v23 + 8) <= a1[2])
      {
        *(*a1 + v23) = *v20;
        v17 = a1[1] + 8;
      }

      a1[1] = v17;
      ++v18;
    }

    while (*(a2 + 40) > v18);
  }

  v24 = v17 + 8;
  v25 = a1[2];
  if (v17 + 8 <= v25)
  {
    *(*a1 + v17) = *(a2 + 63);
    v25 = a1[2];
    v24 = a1[1] + 8;
  }

  a1[1] = v24;
  v26 = v24 + 8;
  if (v24 + 8 <= v25)
  {
    *(*a1 + v24) = *(a2 + 61);
    v25 = a1[2];
    v26 = a1[1] + 8;
  }

  a1[1] = v26;
  v27 = v26 + 8;
  if (v26 + 8 <= v25)
  {
    *(*a1 + v26) = *(a2 + 55);
    v25 = a1[2];
    v27 = a1[1] + 8;
  }

  a1[1] = v27;
  v28 = v27 + 8;
  if (v27 + 8 <= v25)
  {
    *(*a1 + v27) = *(a2 + 54);
    v25 = a1[2];
    v28 = a1[1] + 8;
  }

  a1[1] = v28;
  v29 = v28 + 8;
  if (v28 + 8 <= v25)
  {
    *(*a1 + v28) = *(a2 + 52);
    v25 = a1[2];
    v29 = a1[1] + 8;
  }

  a1[1] = v29;
  v30 = v29 + 8;
  if (v29 + 8 <= v25)
  {
    *(*a1 + v29) = *(a2 + 59);
    v25 = a1[2];
    v30 = a1[1] + 8;
  }

  a1[1] = v30;
  v31 = v30 + 8;
  if (v30 + 8 <= v25)
  {
    *(*a1 + v30) = *(a2 + 60);
    v31 = a1[1] + 8;
  }

  a1[1] = v31;
}

uint64_t DecodeDYMTLCompileOptions(uint64_t a1, uint64_t a2, void *a3, int a4, uint64_t a5)
{
  v8 = *a1;
  v9 = *(a1 + 8);
  v10 = a1 + 16 + ((v9 + 7) & 0xFFFFFFFFFFFFFFF8);
  if (v9)
  {
    v11 = a3[74];
  }

  else
  {
    v11 = 0;
  }

  a3[74] += v9;
  if (a4)
  {
    if (v9)
    {
      v12 = (a1 + 16);
    }

    else
    {
      v12 = 0;
    }

    memcpy(v11, v12, v9);
  }

  v13 = *v10;
  v15 = *v10 == 0;
  v14 = *v10 != 0;
  v16 = 2 * v14;
  result = *(v10 + 8);
  if (v8 <= 0xF)
  {
    v18 = 0;
    v19 = 0;
    v20 = 0;
    v21 = 0;
    v22 = 0;
    if (v8 < 6)
    {
      v28 = 0;
      v29 = 0;
      v23 = 0;
      v24 = 0;
      v25 = 0;
      v26 = 0;
      v27 = 0;
    }

    else
    {
      v23 = 0;
      v24 = 0;
      v25 = 0;
      v26 = 0;
      v27 = 0;
      v28 = *(v10 + 40);
      v29 = 0;
    }

    goto LABEL_35;
  }

  v58 = *(v10 + 16);
  if (v8 < 0x28)
  {
    v18 = 0;
    v19 = 0;
    v20 = 0;
    v21 = 0;
    v22 = 0;
    v29 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    v26 = 0;
    v27 = 0;
LABEL_34:
    v28 = v58;
    goto LABEL_35;
  }

  v54 = *(v10 + 8);
  v55 = 2 * v14;
  v56 = *v10 == 0;
  v57 = *v10;
  v53 = v11;
  v30 = v10 + 24;
  v31 = strlen((v10 + 24));
  if (*(v10 + 24))
  {
    v32 = (v10 + 24);
  }

  else
  {
    v32 = 0;
  }

  if (*(v10 + 24))
  {
    v33 = strlen(v32);
    v34 = a3[74];
    v35 = v33 + 1;
    if (v33 != -1)
    {
      v29 = a3[74];
      v15 = v56;
      v13 = v57;
      result = v54;
      v16 = v55;
      goto LABEL_23;
    }
  }

  else
  {
    v34 = a3[74];
  }

  v15 = *v10 == 0;
  v13 = *v10;
  result = *(v10 + 8);
  v16 = v55;
  v35 = 0;
  v29 = 0;
LABEL_23:
  a3[74] = v34 + v35;
  if (a4)
  {
    memcpy(v29, v32, v35);
    result = v54;
    v16 = v55;
    v15 = v56;
    v13 = v57;
  }

  if (v8 < 0x2B)
  {
    v18 = 0;
    v19 = 0;
    v20 = 0;
    v21 = 0;
    v22 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    v26 = 0;
    v27 = 0;
    v11 = v53;
    goto LABEL_34;
  }

  v36 = v30 + (v31 & 0xFFFFFFFFFFFFFFF8);
  v52 = *(v36 + 8);
  v11 = v53;
  if (v8 < 0x30)
  {
    v18 = 0;
    v19 = 0;
    v20 = 0;
    v21 = 0;
    v23 = 0;
    v24 = 0;
    v25 = 0;
    v26 = 0;
    v27 = 0;
    v28 = v58;
    v22 = v52;
    goto LABEL_35;
  }

  v37 = v36 + 16 + (strlen((v36 + 16)) & 0xFFFFFFFFFFFFFFF8);
  v38 = *(v36 + 16);
  if (*(v36 + 16))
  {
    v39 = (v36 + 16);
  }

  else
  {
    v39 = 0;
  }

  if (v38)
  {
    v40 = strlen(v39);
    v41 = a3[74];
    v42 = v40 + 1;
    if (v40 != -1)
    {
      v26 = a3[74];
      v15 = v56;
      v13 = v57;
      result = v54;
      v16 = v55;
      goto LABEL_41;
    }
  }

  else
  {
    v41 = a3[74];
  }

  v15 = v56;
  v13 = v57;
  result = v54;
  v16 = v55;
  v42 = 0;
  v26 = 0;
LABEL_41:
  v51 = v29;
  a3[74] = v41 + v42;
  if (a4)
  {
    memcpy(v26, v39, v42);
    result = v54;
    v16 = v55;
    v15 = v56;
    v13 = v57;
  }

  v50 = *(v37 + 8);
  v24 = *(v37 + 16);
  v44 = v37 + 24;
  if (v24)
  {
    v25 = a3[72];
    v45 = v24;
    v46 = v25;
    do
    {
      v48 = *v44;
      v44 += 8;
      v47 = v48;
      if (a4)
      {
        if (a5)
        {
          Stream_19685 = GetStream_19685(a5, v47);
          if (Stream_19685)
          {
            v47 = *Stream_19685;
          }

          else
          {
            v47 = 0;
          }
        }

        v46 = a3[72];
        *v46 = v47;
      }

      a3[72] = ++v46;
      --v45;
    }

    while (v45);
    v11 = v53;
    v15 = v56;
    v13 = v57;
    result = v54;
    v16 = v55;
  }

  else
  {
    v25 = 0;
  }

  if (v8 < 0x3B)
  {
    v18 = 0;
    v19 = 0;
    v20 = 0;
    v21 = 0;
  }

  else
  {
    v21 = *v44;
    if (v8 >= 0x40)
    {
      v27 = *(v44 + 8);
      if (v8 < 0x47)
      {
        v18 = 0;
        v19 = 0;
        v20 = 0;
      }

      else
      {
        v19 = *(v44 + 16);
        v20 = *(v44 + 24);
        v18 = *(v44 + 32);
        if (v8 >= 0x4C)
        {
          v15 = *(v44 + 40);
          v16 = *(v44 + 48);
        }
      }

      goto LABEL_62;
    }

    v18 = 0;
    v19 = 0;
    v20 = 0;
  }

  v27 = 0;
LABEL_62:
  v28 = v58;
  v29 = v51;
  v22 = v52;
  v23 = v50;
LABEL_35:
  v43 = a3[57];
  a3[57] = v43 + 64;
  if (a4)
  {
    *v43 = v11;
    *(v43 + 8) = v9;
    *(v43 + 16) = v29;
    *(v43 + 24) = v26;
    *(v43 + 32) = v25;
    *(v43 + 40) = v24;
    *(v43 + 48) = v28;
    *(v43 + 52) = v18;
    *(v43 + 54) = v20;
    *(v43 + 55) = v19;
    *(v43 + 56) = result;
    *(v43 + 57) = v13;
    *(v43 + 58) = v23;
    *(v43 + 59) = v15;
    *(v43 + 60) = v16;
    *(v43 + 61) = v27;
    *(v43 + 62) = v22;
    *(v43 + 63) = v21;
  }

  return result;
}

uint64_t EncodeDYMTLCompileOptions(uint64_t a1, void *a2, unint64_t a3, uint64_t a4)
{
  v29 = a2;
  v31 = a3;
  if (a3 >= 8)
  {
    *a2 = 115;
  }

  v30 = 8;
  WriteGTData(&v29, *a1, *(a1 + 8));
  v6 = v31;
  v7 = v30 + 8;
  if (v30 + 8 <= v31)
  {
    *(v29 + v30) = *(a1 + 57);
    v6 = v31;
    v7 = v30 + 8;
  }

  v30 = v7;
  v8 = v7 + 8;
  if (v7 + 8 <= v6)
  {
    *(v29 + v7) = *(a1 + 56);
    v6 = v31;
    v8 = v30 + 8;
  }

  v30 = v8;
  v9 = v8 + 8;
  if (v8 + 8 <= v6)
  {
    *(v29 + v8) = *(a1 + 48);
    v9 = v30 + 8;
  }

  v30 = v9;
  WriteDynamicBufferString(&v29, *(a1 + 16));
  v10 = v30 + 8;
  if (v30 + 8 <= v31)
  {
    *(v29 + v30) = *(a1 + 62);
    v10 = v30 + 8;
  }

  v30 = v10;
  WriteDynamicBufferString(&v29, *(a1 + 24));
  v11 = v31;
  v12 = v30 + 8;
  if (v30 + 8 <= v31)
  {
    *(v29 + v30) = *(a1 + 58);
    v11 = v31;
    v12 = v30 + 8;
  }

  v30 = v12;
  v13 = *(a1 + 40);
  v14 = v12 + 8;
  if (v12 + 8 <= v11)
  {
    *(v29 + v12) = v13;
    v14 = v30 + 8;
    v13 = *(a1 + 40);
  }

  v30 = v14;
  if (v13)
  {
    v15 = 0;
    do
    {
      v28 = *(*(a1 + 32) + 8 * v15);
      entry = find_entry(a4, &v28, 8uLL, 0);
      v17 = &dword_8;
      if (*entry)
      {
        v18 = *(*entry + 32);
        if (v18)
        {
          v19 = atomic_load((v18 + 56));
          v17 = (v18 + (~(v19 >> 2) & 8));
        }

        else
        {
          v17 = &dword_8;
        }
      }

      v14 = v30 + 8;
      if (v30 + 8 <= v31)
      {
        *(v29 + v30) = *v17;
        v14 = v30 + 8;
      }

      v30 = v14;
      ++v15;
    }

    while (*(a1 + 40) > v15);
  }

  v20 = v14 + 8;
  v21 = v31;
  if (v14 + 8 <= v31)
  {
    *(v29 + v14) = *(a1 + 63);
    v21 = v31;
    v20 = v30 + 8;
  }

  v30 = v20;
  v22 = v20 + 8;
  if (v20 + 8 <= v21)
  {
    *(v29 + v20) = *(a1 + 61);
    v21 = v31;
    v22 = v30 + 8;
  }

  v30 = v22;
  v23 = v22 + 8;
  if (v22 + 8 <= v21)
  {
    *(v29 + v22) = *(a1 + 55);
    v21 = v31;
    v23 = v30 + 8;
  }

  v30 = v23;
  v24 = v23 + 8;
  if (v23 + 8 <= v21)
  {
    *(v29 + v23) = *(a1 + 54);
    v21 = v31;
    v24 = v30 + 8;
  }

  v30 = v24;
  v25 = v24 + 8;
  if (v24 + 8 <= v21)
  {
    *(v29 + v24) = *(a1 + 52);
    v21 = v31;
    v25 = v30 + 8;
  }

  v30 = v25;
  v26 = v25 + 8;
  if (v25 + 8 <= v21)
  {
    *(v29 + v25) = *(a1 + 59);
    v21 = v31;
    v26 = v30 + 8;
  }

  v30 = v26;
  result = v26 + 8;
  if (v26 + 8 <= v21)
  {
    *(v29 + v26) = *(a1 + 60);
    return v30 + 8;
  }

  return result;
}