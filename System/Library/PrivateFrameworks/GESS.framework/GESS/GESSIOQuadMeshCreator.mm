@interface GESSIOQuadMeshCreator
- (BOOL)create:(id)create;
- (BOOL)setFaceGroupIDs:(unsigned __int16 *)ds size:(unint64_t)size;
- (BOOL)setFaceVertexCounts:(char *)counts size:(unint64_t)size;
...
@implementation GESSIOQuadMeshCreator
...
{
  v7 = objc_alloc(MEMORY[0x277CBEA90]);
  v9 = objc_msgSend_initWithBytesNoCopy_length_freeWhenDone_(v7, v8, counts, size, 0);
  LOBYTE(counts) = objc_msgSend_setFaceVertexCountData_(self, v10, v9, v11);
...
{
  v4.receiver = self;
  v4.super_class = GESSIOQuadMeshCreator;
