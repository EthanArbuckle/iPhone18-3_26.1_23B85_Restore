...
- (PKCloudRecordPendingRequest)initWithCoder:(id)a3;
...
- (void)encodeWithCoder:(id)a3
{
  ...
  [v4 encodeObject:self->_pendingRequest forKey:{@"pendingRequest", v5.receiver, v5.super_class}];
}
...
- (int64_t)compare:(id)a3
{
  ...
  v4 = a3;
  v5 = [(PKPeerPaymentPendingRequest *)pendingRequest requestToken];
  v6 = [v4 pendingRequest];
...
- (id)_descriptionWithIncludeItem:(BOOL)a3
{
  ...
  if (self->_pendingRequest)
  {
    if (v3)
    {
      [v5 appendFormat:@"pendingRequest: %@\n", self->_pendingRequest];
    }
  }

  else
  {
    [v5 appendFormat:@"No associated pendingRequest in database\n", v8];
  }
...
- (void)applyCloudRecordObject:(id)a3
{
  ...
  if (objc_opt_isKindOfClass())
  {
    ...
    if (!self->_pendingRequest)
    {
      v7 = [v5 pendingRequest];

      if (v7)
      {
        v8 = [v6 pendingRequest];
        pendingRequest = self->_pendingRequest;
        self->_pendingRequest = v8;
      }
    }
  }
}
