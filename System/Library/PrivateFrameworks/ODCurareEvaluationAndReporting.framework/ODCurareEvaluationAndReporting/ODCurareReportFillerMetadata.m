@interface ODCurareReportFillerMetadata
- (BOOL)isEqual:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation ODCurareReportFillerMetadata

- (id)description
{
  v3 = MEMORY[0x277CCACA8];
  v8.receiver = self;
  v8.super_class = ODCurareReportFillerMetadata;
  v4 = [(ODCurareReportFillerMetadata *)&v8 description];
  v5 = [(ODCurareReportFillerMetadata *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x277CBEB38] dictionary];
  v4 = v3;
  date = self->_date;
  if (date)
  {
    [v3 setObject:date forKey:@"date"];
  }

  selectedModelName = self->_selectedModelName;
  if (selectedModelName)
  {
    [v4 setObject:selectedModelName forKey:@"selectedModelName"];
  }

  return v4;
}

- (void)writeTo:(id)a3
{
  v4 = a3;
  if (!self->_date)
  {
    [ODCurareReportFillerMetadata writeTo:];
  }

  v5 = v4;
  PBDataWriterWriteStringField();
  if (!self->_selectedModelName)
  {
    [ODCurareReportFillerMetadata writeTo:];
  }

  PBDataWriterWriteStringField();
}

- (void)copyTo:(id)a3
{
  date = self->_date;
  v5 = a3;
  [v5 setDate:date];
  [v5 setSelectedModelName:self->_selectedModelName];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [objc_msgSend(objc_opt_class() allocWithZone:{a3), "init"}];
  v6 = [(NSString *)self->_date copyWithZone:a3];
  v7 = v5[1];
  v5[1] = v6;

  v8 = [(NSString *)self->_selectedModelName copyWithZone:a3];
  v9 = v5[2];
  v5[2] = v8;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()] && ((date = self->_date, !(date | v4[1])) || -[NSString isEqual:](date, "isEqual:")))
  {
    selectedModelName = self->_selectedModelName;
    if (selectedModelName | v4[2])
    {
      v7 = [(NSString *)selectedModelName isEqual:?];
    }

    else
    {
      v7 = 1;
    }
  }

  else
  {
    v7 = 0;
  }

  return v7;
}

- (void)mergeFrom:(id)a3
{
  v4 = a3;
  v5 = v4;
  if (v4[1])
  {
    [(ODCurareReportFillerMetadata *)self setDate:?];
    v4 = v5;
  }

  if (v4[2])
  {
    [(ODCurareReportFillerMetadata *)self setSelectedModelName:?];
    v4 = v5;
  }
}

@end