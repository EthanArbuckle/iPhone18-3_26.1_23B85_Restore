...
- (OISFUZipArchiveMemoryDataRepresentation)initWithData:(id)data;
...
- (id)inputStream
{
  dataLength = [(OISFUZipArchiveMemoryDataRepresentation *)self dataLength];

  return [(OISFUZipArchiveMemoryDataRepresentation *)self bufferedInputStreamWithOffset:0 length:dataLength];
}
...
- (BOOL)hasSameLocationAs:(id)as
{
  ...
  data = [(OISFUZipArchiveMemoryDataRepresentation *)self data];
  return data == [as data];
}
...
- (id)bufferedInputStreamWithOffset:(int64_t)offset length:(int64_t)length
{
  ...
  v9 = [[OISFUMemoryInputStream alloc] initWithData:self->mData offset:offset length:length];
