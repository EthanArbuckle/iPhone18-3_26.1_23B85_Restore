...
- (OITSUDateFormatCategory)initWithInitialPattern:(id)pattern locale:(id)locale;
...
- (OITSUDateFormatCategory)initWithInitialPattern:(id)pattern locale:(id)locale
{
  ...
  if (v8)
  {
    ...
    CFDateFormatterSetFormat(ADateFormatter, patternCopy);
...
- (void)addSeparator:(unsigned __int16)separator format:(id)format locale:(id)locale
{
  ...
  formatCopy = format;
  localeCopy = locale;
  v9 = [(OITSUDateFormatCategory *)self entryForSeparator:separatorCopy];
  if (!v9)
  {
    v9 = [[OITSUDateFormatCategoryEntry alloc] initWithSeparator:separatorCopy];
    [(NSMutableArray *)self->_entries addObject:v9];
  }

  [(OITSUDateFormatCategoryEntry *)v9 addFormat:formatCopy locale:localeCopy];
}
