...
- (OISFUZipArchiveOutputStream)initWithOutputStream:(id)stream cryptoKey:(id)key passphraseHint:(id)hint;
- (OISFUZipArchiveOutputStream)initWithPath:(id)path cryptoKey:(id)key passphraseHint:(id)hint;
...
- (OISFUZipArchiveOutputStream)initWithOutputStream:(id)stream cryptoKey:(id)key passphraseHint:(id)hint
{
  ...
  if (v8)
  {
    ...
    v8->mCryptoKey = key;
    v8->mPassphraseHint = [OISFUCryptoUtils encodePassphraseHint:hint];
...
- (OISFUZipArchiveOutputStream)initWithPath:(id)path cryptoKey:(id)key passphraseHint:(id)hint
{
  v8 = [[OISFUMoveableFileOutputStream alloc] initWithPath:path];

  return [(OISFUZipArchiveOutputStream *)self initWithOutputStream:v8 cryptoKey:key passphraseHint:hint];
}
...
+ (BOOL)createZipWithItemsAtPath:(id)path zippedPath:(id)zippedPath rootPathComponentName:(id)name
{
  ...
  v7 = [[OISFUZipArchiveOutputStream alloc] initWithPath:zippedPath];
      ...
      for (i = 0; i != v12; ++i)
      {
        ...
        if ([defaultManager fileExistsAtPath:v33 isDirectory:&v31])
        {
          if ((v31 & 1) == 0)
          {
            ...
            bufferedInputStream = [(OISFUDataRepresentation *)v20 bufferedInputStream];
            while (1)
            {
              v30 = 0;
...
- (void)beginEntryWithName:(id)name isCompressed:(BOOL)compressed uncompressedSize:(unint64_t)size
{
  ...
  if (self->mCryptoKey)
  {
    size = [OISFUCryptoOutputStream encodedLengthForDataLength:size key:?];
  }
  ...
  if (!compressedCopy && [(OISFUMoveableFileOutputStream *)self->mOutputStream canSeek])
  {
...
- (id)beginUncompressedUnknownSizeEntryWithName:(id)name
{
  [(OISFUZipArchiveOutputStream *)self beginUnknownSizeEntryWithName:name isCompressed:0];
...
LABEL_14:
  ...
  +[OITSUAssertionHandler handleFailureInFunction:file:lineNumber:isFatal:description:](OITSUAssertionHandler, "handleFailureInFunction:file:lineNumber:isFatal:description:", v12, [MEMORY[0x277CCACA8] stringWithUTF8String:"/Library/Caches/com.apple.xbs/Sources/OfficeImport/OfficeParser/shared/utility/sf/SFUZipArchiveOutputStream.m"], 382, 0, "Can't find entry named: %@", name);
  +[OITSUAssertionHandler logBacktraceThrottled];
  return 0;
}
...
LABEL_19:
      ...
      +[OITSUAssertionHandler handleFailureInFunction:file:lineNumber:isFatal:description:](OITSUAssertionHandler, "handleFailureInFunction:file:lineNumber:isFatal:description:", v17, [MEMORY[0x277CCACA8] stringWithUTF8String:"/Library/Caches/com.apple.xbs/Sources/OfficeImport/OfficeParser/shared/utility/sf/SFUZipArchiveOutputStream.m"], 407, 0, "Can't find entry named: %@", name);
  ...
  {
    offset = [(OISFUMoveableFileOutputStream *)self->mOutputStream offset];
    [(OISFUMoveableFileOutputStream *)self->mOutputStream truncateToLength:v7->offset];
    if (offset < v7->offset)
    {
      [(OISFUMoveableFileOutputStream *)self->mOutputStream seekToOffset:offset whence:0];
    }
  ...
  else
  {
    ...
    else
    {
      ...
      [OISFUZipException raise:@"SFUZipOutputError" format:@"Removing entry named %@ produced free space that is too small.", name];
    }
  }
}
...
- (void)close
{
  ...
  if (nextObject)
  {
    [(OISFUZipArchiveOutputStream *)self writeCentralFileHeaderUsingEntry:nextObject isFirstEntry:1];
  }
  ...
  if (nextObject2)
  {
    ...
    do
    {
      [(OISFUZipArchiveOutputStream *)self writeCentralFileHeaderUsingEntry:nextObject3 isFirstEntry:0];
  ...
  [(OISFUZipArchiveOutputStream *)self writeEndOfCentralDirectoryWithOffset:offset];
...
- (void)moveToPath:(id)path
{
  ...
  [(OISFUZipArchiveOutputStream *)self finishEntry];
  path = [(OISFUMoveableFileOutputStream *)self->mOutputStream path];
  if (![(OISFUMoveableFileOutputStream *)self->mOutputStream moveToPath:path])
  {
    [OISFUZipException raise:@"SFUZipOutputError" format:@"Could not move output stream from %@ to %@", path, path];
  }
}
...
- (id)entriesAtPath:(id)path
{
  ...
  v6 = [[OISFUZipArchiveFileDataRepresentation alloc] initWithPath:path];
  objectEnumerator = [(NSMutableArray *)self->mEntries objectEnumerator];
  nextObject = [objectEnumerator nextObject];
  if (nextObject)
  {
    ...
    do
    {
      v10 = [[OISFUZipEntry alloc] initWithDataRepresentation:v6 compressionMethod:*(nextObject2 + 24) compressedSize:*(nextObject2 + 32) uncompressedSize:*(nextObject2 + 40) offset:*(nextObject2 + 48) crc:*(nextObject2 + 64)];
      v11 = v10;
      if (*(nextObject2 + 25) == 1)
      {
...
- (unint64_t)writeLocalFileHeaderForEntry:(id)entry
{
  ...
  *(mBuffer + 22) = 0;
  ...
  if (v9 >= 0x10000)
  {
    [OISFUZipException raise:@"SFUZipOutputError" format:@"File name %@ is too long", *(entry + 1)];
    v9 = *(entry + 2);
  }
  ...
  *(mBuffer + 14) = v10;
  *(entry + 6) = [(OISFUMoveableFileOutputStream *)self->mOutputStream offset];
  [(OISFUMoveableFileOutputStream *)self->mOutputStream writeBuffer:self->mBuffer size:30];
  -[OISFUMoveableFileOutputStream writeBuffer:size:](self->mOutputStream, "writeBuffer:size:", [*(entry + 1) UTF8String], *(entry + 2));
  if (*(entry + 69) == 1)
  {
    v11 = self->mBuffer;
    *v11 = 1048577;
    ...
    *(v32 + 1) = 1987082089;
    ...
    if (v28)
    {
      ...
      *(v34 + 1) = *"iwuu";
      ...
      [(OISFUMoveableFileOutputStream *)mOutputStream writeBuffer:bytes size:v37];
    }
  }
}
...
- (void)writeEndOfCentralDirectoryWithOffset:(int64_t)offset
{
  v5 = [(OISFUMoveableFileOutputStream *)self->mOutputStream offset]- offset;
  v6 = [(NSMutableArray *)self->mEntries count];
  v7 = v6;
  if (offset > 0xFFFFFFFFLL || (!(v6 >> 16) ? (v8 = HIDWORD(v5) == 0) : (v8 = 0), !v8))
  {
    offset = [(OISFUMoveableFileOutputStream *)self->mOutputStream offset];
    [(OISFUZipArchiveOutputStream *)self writeZip64EndOfCentralDirectoryWithOffset:offset];
    [(OISFUZipArchiveOutputStream *)self writeZip64EndOfCentralDirectoryLocatorWithOffset:offset];
  }
...
- (void)writeZip64EndOfCentralDirectoryWithOffset:(int64_t)offset
{
  ...
  *(mBuffer + 3) = v6;
  *(mBuffer + 4) = v6;
  *(mBuffer + 5) = [(OISFUMoveableFileOutputStream *)self->mOutputStream offset]- offset;
  *(mBuffer + 6) = offset;
  mOutputStream = self->mOutputStream;
  v8 = self->mBuffer;
