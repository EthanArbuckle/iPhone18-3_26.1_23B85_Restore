@interface EMFPlayer
+ (CGRect)boundsInLogicalUnits:(id)units;
+ (CGRect)boundsInPoints:(id)points;
- (EMFPlayer)initWithGraphicsDevice:(id)device;
- (int)play:(unsigned int)play in_pRecord:(const char *)record;
- (int)playAlphaBlend:(unsigned int)blend in_pRecord:(const char *)record;
- (int)playAngleArc:(unsigned int)arc in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playArc:(unsigned int)arc in_pRecord:(const char *)record;
- (int)playArcTo:(unsigned int)to in_pRecord:(const char *)record;
- (int)playBitBlt:(unsigned int)blt in_pRecord:(const char *)record;
- (int)playChord:(unsigned int)chord in_pRecord:(const char *)record;
- (int)playCreateBrushIndirect:(unsigned int)indirect in_pRecord:(const char *)record;
- (int)playCreateDibPatternBrushPT:(unsigned int)t in_pRecord:(const char *)record;
- (int)playCreateMonoBrush:(unsigned int)brush in_pRecord:(const char *)record;
- (int)playCreatePalette:(unsigned int)palette in_pRecord:(const char *)record;
- (int)playCreatePen:(unsigned int)pen in_pRecord:(const char *)record;
- (int)playDeleteObject:(unsigned int)object in_pRecord:(const char *)record;
- (int)playEllipse:(unsigned int)ellipse in_pRecord:(const char *)record;
- (int)playExcludeClipRect:(unsigned int)rect in_pRecord:(const char *)record;
- (int)playExtCreateFontIndirectW:(unsigned int)w in_pRecord:(const char *)record;
- (int)playExtCreatePen:(unsigned int)pen in_pRecord:(const char *)record;
- (int)playExtTextOut:(unsigned int)out in_pRecord:(const char *)record wideChars:(BOOL)chars;
- (int)playFillRgn:(unsigned int)rgn in_pRecord:(const char *)record;
- (int)playHeader:(unsigned int)header in_pRecord:(const char *)record;
- (int)playIntersectClipRect:(unsigned int)rect in_pRecord:(const char *)record;
- (int)playInvertRgn:(unsigned int)rgn in_pRecord:(const char *)record;
- (int)playLineTo:(unsigned int)to in_pRecord:(const char *)record;
- (int)playMaskBlt:(unsigned int)blt in_pRecord:(const char *)record;
- (int)playModifyWorldTransform:(unsigned int)transform in_pRecord:(const char *)record;
- (int)playMoveToEx:(unsigned int)ex in_pRecord:(const char *)record;
- (int)playOffsetClipRegion:(unsigned int)region in_pRecord:(const char *)record;
- (int)playPaintRgn:(unsigned int)rgn in_pRecord:(const char *)record;
- (int)playPie:(unsigned int)pie in_pRecord:(const char *)record;
- (int)playPlgBlt:(unsigned int)blt in_pRecord:(const char *)record;
- (int)playPolyBezier:(unsigned int)bezier in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyBezierTo:(unsigned int)to in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyDraw:(unsigned int)draw in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyLine:(unsigned int)line in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyLineTo:(unsigned int)to in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyPolyLine:(unsigned int)line in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolyPolygon:(unsigned int)polygon in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playPolygon:(unsigned int)polygon in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit;
- (int)playRealizePalette:(unsigned int)palette in_pRecord:(const char *)record;
- (int)playRectangle:(unsigned int)rectangle in_pRecord:(const char *)record;
- (int)playResizePalette:(unsigned int)palette in_pRecord:(const char *)record;
- (int)playRestoreDC:(unsigned int)c in_pRecord:(const char *)record;
- (int)playRoundRect:(unsigned int)rect in_pRecord:(const char *)record;
- (int)playSaveDC:(unsigned int)c in_pRecord:(const char *)record;
- (int)playScaleViewportExt:(unsigned int)ext in_pRecord:(const char *)record;
- (int)playScaleWindowExt:(unsigned int)ext in_pRecord:(const char *)record;
- (int)playSelectClipPath:(unsigned int)path in_pRecord:(const char *)record;
- (int)playSelectClipRegion:(unsigned int)region in_pRecord:(const char *)record;
- (int)playSelectObject:(unsigned int)object in_pRecord:(const char *)record;
- (int)playSelectPalette:(unsigned int)palette in_pRecord:(const char *)record;
- (int)playSetArcDirection:(unsigned int)direction in_pRecord:(const char *)record;
- (int)playSetBkColour:(unsigned int)colour in_pRecord:(const char *)record;
- (int)playSetBkMode:(unsigned int)mode in_pRecord:(const char *)record;
- (int)playSetBrushOrg:(unsigned int)org in_pRecord:(const char *)record;
- (int)playSetDIBitsToDevice:(unsigned int)device in_pRecord:(const char *)record;
- (int)playSetMapMode:(unsigned int)mode in_pRecord:(const char *)record;
- (int)playSetMiterLimit:(unsigned int)limit in_pRecord:(const char *)record;
- (int)playSetPaletteEntries:(unsigned int)entries in_pRecord:(const char *)record;
- (int)playSetPolyFillMode:(unsigned int)mode in_pRecord:(const char *)record;
- (int)playSetRop2:(unsigned int)rop2 in_pRecord:(const char *)record;
- (int)playSetStretchBltMode:(unsigned int)mode in_pRecord:(const char *)record;
- (int)playSetTextAlign:(unsigned int)align in_pRecord:(const char *)record;
- (int)playSetTextColour:(unsigned int)colour in_pRecord:(const char *)record;
- (int)playSetTextJustification:(unsigned int)justification in_pRecord:(const char *)record;
- (int)playSetViewportExt:(unsigned int)ext in_pRecord:(const char *)record;
- (int)playSetViewportOrg:(unsigned int)org in_pRecord:(const char *)record;
- (int)playSetWindowExt:(unsigned int)ext in_pRecord:(const char *)record;
- (int)playSetWindowOrg:(unsigned int)org in_pRecord:(const char *)record;
- (int)playSetWorldTransform:(unsigned int)transform in_pRecord:(const char *)record;
- (int)playStretchBlt:(unsigned int)blt in_pRecord:(const char *)record;
- (int)playStretchDIBits:(unsigned int)bits in_pRecord:(const char *)record;
- (void)done;
@end

@implementation EMFPlayer

- (void)done
{
  [(MFGraphicsDevice *)self->m_gdi done];
  m_emfPlusPlayer = self->m_emfPlusPlayer;
  self->m_emfPlusPlayer = 0;
}

- (EMFPlayer)initWithGraphicsDevice:(id)device
{
  deviceCopy = device;
  v12.receiver = self;
  v12.super_class = EMFPlayer;
  v6 = [(EMFPlayer *)&v12 init];
  v7 = v6;
  if (v6)
  {
    objc_storeStrong(&v6->m_gdi, device);
    v7->m_headerDone = 0;
    v8 = [EMFPlusPlayer alloc];
    [deviceCopy getCanvas];
    v9 = [(EMFPlusPlayer *)v8 initWithOwner:v7 canvas:?];
    m_emfPlusPlayer = v7->m_emfPlusPlayer;
    v7->m_emfPlusPlayer = v9;
  }

  return v7;
}

- (int)play:(unsigned int)play in_pRecord:(const char *)record
{
  if (play < 4)
  {
    return -7;
  }

  v7 = *record;
  if (self->m_headerDone)
  {
    if (!self->m_ignoreEMFPlusRecords && v7 == 70)
    {
      if (play < 0x10)
      {
        return -7;
      }

      if (*(record + 3) == 726027589)
      {
        v8 = *(record + 2);
        self->m_ignoreEMFRecords = 1;
        if (v8 - 4 <= play - 16)
        {
          return [(EMFPlusPlayer *)self->m_emfPlusPlayer play:record + 16 in_size:?];
        }

        return -7;
      }
    }

    if (self->m_ignoreEMFRecords)
    {
      return 0;
    }

    v4 = -2;
    switch(v7)
    {
      case 1:
      case 15:
      case 16:
      case 23:
      case 53:
      case 65:
      case 70:
      case 72:
      case 96:
      case 97:
      case 98:
      case 99:
      case 100:
      case 101:
      case 102:
      case 103:
      case 104:
      case 105:
      case 106:
      case 107:
      case 108:
      case 109:
      case 110:
      case 111:
      case 112:
      case 113:
      case 115:
      case 116:
      case 117:
      case 118:
      case 119:
        result = [EMFPlayer playUnsupported:"playUnsupported:in_recordName:in_pRecord:" in_recordName:? in_pRecord:?];
        break;
      case 2:
      case 85:
        result = [EMFPlayer playPolyBezier:"playPolyBezier:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 3:
      case 86:
        result = [EMFPlayer playPolygon:"playPolygon:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 4:
      case 87:
        result = [EMFPlayer playPolyLine:"playPolyLine:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 5:
      case 88:
        result = [EMFPlayer playPolyBezierTo:"playPolyBezierTo:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 6:
      case 89:
        result = [EMFPlayer playPolyLineTo:"playPolyLineTo:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 7:
      case 90:
        result = [EMFPlayer playPolyPolyLine:"playPolyPolyLine:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 8:
      case 91:
        result = [EMFPlayer playPolyPolygon:"playPolyPolygon:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 9:
        result = [EMFPlayer playSetWindowExt:"playSetWindowExt:in_pRecord:" in_pRecord:?];
        break;
      case 10:
        result = [EMFPlayer playSetWindowOrg:"playSetWindowOrg:in_pRecord:" in_pRecord:?];
        break;
      case 11:
        result = [EMFPlayer playSetViewportExt:"playSetViewportExt:in_pRecord:" in_pRecord:?];
        break;
      case 12:
        result = [EMFPlayer playSetViewportOrg:"playSetViewportOrg:in_pRecord:" in_pRecord:?];
        break;
      case 13:
        result = [EMFPlayer playSetBrushOrg:"playSetBrushOrg:in_pRecord:" in_pRecord:?];
        break;
      case 14:
        return v4;
      case 17:
        result = [EMFPlayer playSetMapMode:"playSetMapMode:in_pRecord:" in_pRecord:?];
        break;
      case 18:
        result = [EMFPlayer playSetBkMode:"playSetBkMode:in_pRecord:" in_pRecord:?];
        break;
      case 19:
        result = [EMFPlayer playSetPolyFillMode:"playSetPolyFillMode:in_pRecord:" in_pRecord:?];
        break;
      case 20:
        result = [EMFPlayer playSetRop2:"playSetRop2:in_pRecord:" in_pRecord:?];
        break;
      case 21:
        result = [EMFPlayer playSetStretchBltMode:"playSetStretchBltMode:in_pRecord:" in_pRecord:?];
        break;
      case 22:
        result = [EMFPlayer playSetTextAlign:"playSetTextAlign:in_pRecord:" in_pRecord:?];
        break;
      case 24:
        result = [EMFPlayer playSetTextColour:"playSetTextColour:in_pRecord:" in_pRecord:?];
        break;
      case 25:
        result = [EMFPlayer playSetBkColour:"playSetBkColour:in_pRecord:" in_pRecord:?];
        break;
      case 26:
        result = [EMFPlayer playOffsetClipRegion:"playOffsetClipRegion:in_pRecord:" in_pRecord:?];
        break;
      case 27:
        result = [EMFPlayer playMoveToEx:"playMoveToEx:in_pRecord:" in_pRecord:?];
        break;
      case 28:
        result = [EMFPlayer playSetMetaRgn:"playSetMetaRgn:in_pRecord:" in_pRecord:?];
        break;
      case 29:
        result = [EMFPlayer playExcludeClipRect:"playExcludeClipRect:in_pRecord:" in_pRecord:?];
        break;
      case 30:
        result = [EMFPlayer playIntersectClipRect:"playIntersectClipRect:in_pRecord:" in_pRecord:?];
        break;
      case 31:
        result = [EMFPlayer playScaleViewportExt:"playScaleViewportExt:in_pRecord:" in_pRecord:?];
        break;
      case 32:
        result = [EMFPlayer playScaleWindowExt:"playScaleWindowExt:in_pRecord:" in_pRecord:?];
        break;
      case 33:
        result = [EMFPlayer playSaveDC:"playSaveDC:in_pRecord:" in_pRecord:?];
        break;
      case 34:
        result = [EMFPlayer playRestoreDC:"playRestoreDC:in_pRecord:" in_pRecord:?];
        break;
      case 35:
        result = [EMFPlayer playSetWorldTransform:"playSetWorldTransform:in_pRecord:" in_pRecord:?];
        break;
      case 36:
        result = [EMFPlayer playModifyWorldTransform:"playModifyWorldTransform:in_pRecord:" in_pRecord:?];
        break;
      case 37:
        result = [EMFPlayer playSelectObject:"playSelectObject:in_pRecord:" in_pRecord:?];
        break;
      case 38:
        result = [EMFPlayer playCreatePen:"playCreatePen:in_pRecord:" in_pRecord:?];
        break;
      case 39:
        result = [EMFPlayer playCreateBrushIndirect:"playCreateBrushIndirect:in_pRecord:" in_pRecord:?];
        break;
      case 40:
        result = [EMFPlayer playDeleteObject:"playDeleteObject:in_pRecord:" in_pRecord:?];
        break;
      case 41:
        result = [EMFPlayer playAngleArc:"playAngleArc:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 42:
        result = [EMFPlayer playEllipse:"playEllipse:in_pRecord:" in_pRecord:?];
        break;
      case 43:
        result = [EMFPlayer playRectangle:"playRectangle:in_pRecord:" in_pRecord:?];
        break;
      case 44:
        result = [EMFPlayer playRoundRect:"playRoundRect:in_pRecord:" in_pRecord:?];
        break;
      case 45:
        result = [EMFPlayer playArc:"playArc:in_pRecord:" in_pRecord:?];
        break;
      case 46:
        result = [EMFPlayer playChord:"playChord:in_pRecord:" in_pRecord:?];
        break;
      case 47:
        result = [EMFPlayer playPie:"playPie:in_pRecord:" in_pRecord:?];
        break;
      case 48:
        result = [EMFPlayer playSelectPalette:"playSelectPalette:in_pRecord:" in_pRecord:?];
        break;
      case 49:
        result = [EMFPlayer playCreatePalette:"playCreatePalette:in_pRecord:" in_pRecord:?];
        break;
      case 50:
        result = [EMFPlayer playSetPaletteEntries:"playSetPaletteEntries:in_pRecord:" in_pRecord:?];
        break;
      case 51:
        result = [EMFPlayer playResizePalette:"playResizePalette:in_pRecord:" in_pRecord:?];
        break;
      case 52:
        result = [EMFPlayer playRealizePalette:"playRealizePalette:in_pRecord:" in_pRecord:?];
        break;
      case 54:
        result = [EMFPlayer playLineTo:"playLineTo:in_pRecord:" in_pRecord:?];
        break;
      case 55:
        result = [EMFPlayer playArcTo:"playArcTo:in_pRecord:" in_pRecord:?];
        break;
      case 56:
      case 92:
        result = [EMFPlayer playPolyDraw:"playPolyDraw:in_pRecord:in_is16bit:" in_pRecord:? in_is16bit:?];
        break;
      case 57:
        result = [EMFPlayer playSetArcDirection:"playSetArcDirection:in_pRecord:" in_pRecord:?];
        break;
      case 58:
        result = [EMFPlayer playSetMiterLimit:"playSetMiterLimit:in_pRecord:" in_pRecord:?];
        break;
      case 59:
        result = [EMFPlayer playBeginPath:"playBeginPath:in_pRecord:" in_pRecord:?];
        break;
      case 60:
        result = [EMFPlayer playEndPath:"playEndPath:in_pRecord:" in_pRecord:?];
        break;
      case 61:
        result = [EMFPlayer playCloseFigure:"playCloseFigure:in_pRecord:" in_pRecord:?];
        break;
      case 62:
      case 63:
      case 64:
        result = [EMFPlayer playStrokeAndFillPath:"playStrokeAndFillPath:in_pRecord:in_stroke:in_fill:" in_pRecord:? in_stroke:? in_fill:?];
        break;
      case 66:
        result = [EMFPlayer playWidenPath:"playWidenPath:in_pRecord:" in_pRecord:?];
        break;
      case 67:
        result = [EMFPlayer playSelectClipPath:"playSelectClipPath:in_pRecord:" in_pRecord:?];
        break;
      case 68:
        result = [EMFPlayer playAbortPath:"playAbortPath:in_pRecord:" in_pRecord:?];
        break;
      case 71:
        result = [EMFPlayer playFillRgn:"playFillRgn:in_pRecord:" in_pRecord:?];
        break;
      case 73:
        result = [EMFPlayer playInvertRgn:"playInvertRgn:in_pRecord:" in_pRecord:?];
        break;
      case 74:
        result = [EMFPlayer playPaintRgn:"playPaintRgn:in_pRecord:" in_pRecord:?];
        break;
      case 75:
        result = [EMFPlayer playSelectClipRegion:"playSelectClipRegion:in_pRecord:" in_pRecord:?];
        break;
      case 76:
        result = [EMFPlayer playBitBlt:"playBitBlt:in_pRecord:" in_pRecord:?];
        break;
      case 77:
        result = [EMFPlayer playStretchBlt:"playStretchBlt:in_pRecord:" in_pRecord:?];
        break;
      case 78:
        result = [EMFPlayer playMaskBlt:"playMaskBlt:in_pRecord:" in_pRecord:?];
        break;
      case 79:
        result = [EMFPlayer playPlgBlt:"playPlgBlt:in_pRecord:" in_pRecord:?];
        break;
      case 80:
        result = [EMFPlayer playSetDIBitsToDevice:"playSetDIBitsToDevice:in_pRecord:" in_pRecord:?];
        break;
      case 81:
        result = [EMFPlayer playStretchDIBits:"playStretchDIBits:in_pRecord:" in_pRecord:?];
        break;
      case 82:
        result = [EMFPlayer playExtCreateFontIndirectW:"playExtCreateFontIndirectW:in_pRecord:" in_pRecord:?];
        break;
      case 83:
        result = [EMFPlayer playExtTextOutA:"playExtTextOutA:in_pRecord:" in_pRecord:?];
        break;
      case 84:
        result = [EMFPlayer playExtTextOutW:"playExtTextOutW:in_pRecord:" in_pRecord:?];
        break;
      case 93:
        result = [EMFPlayer playCreateMonoBrush:"playCreateMonoBrush:in_pRecord:" in_pRecord:?];
        break;
      case 94:
        result = [EMFPlayer playCreateDibPatternBrushPT:"playCreateDibPatternBrushPT:in_pRecord:" in_pRecord:?];
        break;
      case 95:
        result = [EMFPlayer playExtCreatePen:"playExtCreatePen:in_pRecord:" in_pRecord:?];
        break;
      case 114:
        result = [EMFPlayer playAlphaBlend:"playAlphaBlend:in_pRecord:" in_pRecord:?];
        break;
      case 120:
        result = [EMFPlayer playSetTextJustification:"playSetTextJustification:in_pRecord:" in_pRecord:?];
        break;
      default:
        result = [EMFPlayer playUnknown:"playUnknown:in_recordType:in_pRecord:" in_recordType:? in_pRecord:?];
        break;
    }
  }

  else
  {
    if (v7 != 1)
    {
      return -7;
    }

    return [EMFPlayer playHeader:"playHeader:in_pRecord:" in_pRecord:?];
  }

  return result;
}

- (int)playHeader:(unsigned int)header in_pRecord:(const char *)record
{
  if (header < 0x58)
  {
    return -7;
  }

  v6 = *(record + 20);
  v5 = *(record + 21);
  if (!v6 || v5 == 0)
  {
    return -7;
  }

  v11 = *(record + 15);
  v12 = *(record + 16);
  v13 = *(record + 28);
  v14 = *(record + 18) / (v6 * 100.0);
  v15 = *(record + 19) / (v5 * 100.0);
  [(MFGraphicsDevice *)self->m_gdi setViewBox:v14 * *(record + 6) top:v15 * *(record + 7) width:v14 * (*(record + 8) - *(record + 6)) height:v15 * (*(record + 9) - *(record + 7))];
  if (v13)
  {
    v16 = (v13 + 1);
  }

  else
  {
    v16 = 0xFFFFLL;
  }

  [(MFGraphicsDevice *)self->m_gdi setMaximumObjectTableSize:v16];
  v8 = 0;
  if (v11)
  {
    if (v12)
    {
      v17 = v12 + v11;
      v8 = -7;
      if (!HIDWORD(v17) && v17 <= header)
      {
        v18 = [objc_alloc(MEMORY[0x277CCACA8]) initWithBytes:&record[v12] length:v11 encoding:10];
        v8 = [(MFGraphicsDevice *)self->m_gdi description:v18];
      }
    }
  }

  self->m_headerDone = 1;
  return v8;
}

- (int)playSetWindowExt:(unsigned int)ext in_pRecord:(const char *)record
{
  if (ext >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setWindowExt:*(record + 2) in_y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetWindowOrg:(unsigned int)org in_pRecord:(const char *)record
{
  if (org >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setWindowOrg:*(record + 2) in_y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playScaleWindowExt:(unsigned int)ext in_pRecord:(const char *)record
{
  if (ext >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi scaleWindowExt:*(record + 2) in_xDenom:*(record + 3) in_yNum:*(record + 4) in_yDenom:*(record + 5)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetWorldTransform:(unsigned int)transform in_pRecord:(const char *)record
{
  if (transform < 0x20)
  {
    return -7;
  }

  v15[10] = v4;
  v15[11] = v5;
  [MFByteUtils readFloat32FromByteArray:record in_offset:8];
  [MFByteUtils readFloat32FromByteArray:record in_offset:12, v9];
  *&v15[1] = v10;
  [MFByteUtils readFloat32FromByteArray:record in_offset:16];
  *&v15[2] = v11;
  [MFByteUtils readFloat32FromByteArray:record in_offset:20];
  *&v15[3] = v12;
  [MFByteUtils readFloat32FromByteArray:record in_offset:24];
  *&v15[4] = v13;
  [MFByteUtils readFloat32FromByteArray:record in_offset:28];
  *&v15[5] = v14;
  return [(MFGraphicsDevice *)self->m_gdi setWorldTransform:v15];
}

- (int)playModifyWorldTransform:(unsigned int)transform in_pRecord:(const char *)record
{
  if (transform < 0x24)
  {
    return -7;
  }

  v15[10] = v4;
  v15[11] = v5;
  [MFByteUtils readFloat32FromByteArray:record in_offset:8];
  [MFByteUtils readFloat32FromByteArray:record in_offset:12, v9];
  *&v15[1] = v10;
  [MFByteUtils readFloat32FromByteArray:record in_offset:16];
  *&v15[2] = v11;
  [MFByteUtils readFloat32FromByteArray:record in_offset:20];
  *&v15[3] = v12;
  [MFByteUtils readFloat32FromByteArray:record in_offset:24];
  *&v15[4] = v13;
  [MFByteUtils readFloat32FromByteArray:record in_offset:28];
  *&v15[5] = v14;
  return [(MFGraphicsDevice *)self->m_gdi modifyWorldTransform:v15 in_command:*(record + 8)];
}

- (int)playSetViewportExt:(unsigned int)ext in_pRecord:(const char *)record
{
  if (ext >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setViewportExt:*(record + 2) in_y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetViewportOrg:(unsigned int)org in_pRecord:(const char *)record
{
  if (org >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setViewportOrg:*(record + 2) in_y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playScaleViewportExt:(unsigned int)ext in_pRecord:(const char *)record
{
  if (ext >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi scaleViewportExt:*(record + 2) in_xDenom:*(record + 3) in_yNum:*(record + 4) in_yDenom:*(record + 5)];
  }

  else
  {
    return -7;
  }
}

- (int)playSelectPalette:(unsigned int)palette in_pRecord:(const char *)record
{
  if (palette < 0xC)
  {
    return -7;
  }

  v5 = *(record + 11);
  m_gdi = self->m_gdi;
  if (v5 << 24 < 0)
  {
    return [(MFGraphicsDevice *)m_gdi getStockObject:(*(record + 4) | (*(record + 10) << 16)) & 0xFFFFFF | (v5 << 24) & 0x7FFFFFFFu];
  }

  else
  {
    return [(MFGraphicsDevice *)m_gdi selectObject:(*(record + 4) | (*(record + 10) << 16)) & 0xFFFFFF | (v5 << 24)];
  }
}

- (int)playSelectObject:(unsigned int)object in_pRecord:(const char *)record
{
  if (object < 0xC)
  {
    return -7;
  }

  v5 = *(record + 11);
  m_gdi = self->m_gdi;
  if (v5 << 24 < 0)
  {
    return [(MFGraphicsDevice *)m_gdi getStockObject:(*(record + 4) | (*(record + 10) << 16)) & 0xFFFFFF | (v5 << 24) & 0x7FFFFFFFu];
  }

  else
  {
    return [(MFGraphicsDevice *)m_gdi selectObject:(*(record + 4) | (*(record + 10) << 16)) & 0xFFFFFF | (v5 << 24)];
  }
}

- (int)playDeleteObject:(unsigned int)object in_pRecord:(const char *)record
{
  if (object >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi deleteObject:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playCreatePen:(unsigned int)pen in_pRecord:(const char *)record
{
  if (pen < 0x1C)
  {
    return -7;
  }

  v7 = *(record + 3);
  v6 = *(record + 4);
  v8 = *(record + 2);
  v9 = [OITSUColor colorWithRGBBytes:*(record + 24) green:*(record + 25) blue:*(record + 26)];
  v4 = [(MFGraphicsDevice *)self->m_gdi createPen:v7 width:v6 in_colour:v9 in_userStyleArray:0 index:v8];

  return v4;
}

- (int)playExtCreatePen:(unsigned int)pen in_pRecord:(const char *)record
{
  if (pen < 0x34)
  {
    return -7;
  }

  v5 = *(record + 51);
  v6 = *(record + 24) | (*(record + 50) << 16) | (v5 << 24);
  if ((4 * v6) > 0xFFFFFFCB)
  {
    return -7;
  }

  v7 = -7;
  if (v5 <= 0x3F && 4 * v6 + 52 <= pen)
  {
    v9 = *(record + 2);
    v11 = *(record + 7);
    v10 = *(record + 8);
    if (v6)
    {
      v12 = smalloc_typed(v6, 8uLL, 0x100004000313F17uLL);
      if (!v12)
      {
        return -4;
      }
    }

    else
    {
      v12 = 0;
    }

    v14 = [OITSUColor colorWithRGBBytes:*(record + 40) green:*(record + 41) blue:*(record + 42)];
    if (v6)
    {
      v15 = 55;
      v16 = v12;
      do
      {
        *v16++ = (record[v15 - 3] | (record[v15 - 2] << 8) | (record[v15 - 1] << 16) | (record[v15] << 24));
        v15 += 4;
        --v6;
      }

      while (v6);
    }

    v7 = [(MFGraphicsDevice *)self->m_gdi createPen:v11 width:v10 in_colour:v14 in_userStyleArray:v12 index:v9];
    if (v12)
    {
      free(v12);
    }
  }

  return v7;
}

- (int)playCreatePalette:(unsigned int)palette in_pRecord:(const char *)record
{
  if (palette < 0xC)
  {
    return -7;
  }

  v5 = *(record + 7);
  if (4 * v5 + 16 > palette)
  {
    return -7;
  }

  v8 = *(record + 2);
  v9 = [MEMORY[0x277CBEB18] arrayWithCapacity:*(record + 7)];
  if (v5)
  {
    v10 = (record + 18);
    do
    {
      v11 = [OITSUColor colorWithRGBBytes:*(v10 - 2) green:*(v10 - 1) blue:*v10];
      [v9 addObject:v11];
      v10 += 4;

      --v5;
    }

    while (v5);
  }

  v6 = [(MFGraphicsDevice *)self->m_gdi createPalette:v9 in_index:v8];

  return v6;
}

- (int)playSetPaletteEntries:(unsigned int)entries in_pRecord:(const char *)record
{
  if (entries < 0x14)
  {
    return -7;
  }

  v5 = *(record + 19);
  v6 = *(record + 8) | (*(record + 18) << 16) | (v5 << 24);
  if ((4 * v6) > 0xFFFFFFEB)
  {
    return -7;
  }

  v7 = -7;
  if (v5 <= 0x3F && 4 * v6 + 20 <= entries)
  {
    v9 = *(record + 2);
    v10 = *(record + 3);
    v11 = [MEMORY[0x277CBEB18] arrayWithCapacity:v6];
    if (v6)
    {
      v12 = 20;
      do
      {
        v13 = [OITSUColor colorWithRGBBytes:record[v12] green:record[v12 + 1] blue:record[v12 + 2]];
        [v11 addObject:v13];

        v12 += 4;
        LODWORD(v6) = v6 - 1;
      }

      while (v6);
    }

    v7 = [(MFGraphicsDevice *)self->m_gdi setPaletteEntries:v10 in_colours:v11 in_index:v9];
  }

  return v7;
}

- (int)playResizePalette:(unsigned int)palette in_pRecord:(const char *)record
{
  if (palette >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi resizePalette:*(record + 3) in_index:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playRealizePalette:(unsigned int)palette in_pRecord:(const char *)record
{
  if (palette >= 8)
  {
    return [(MFGraphicsDevice *)self->m_gdi realizePalette];
  }

  else
  {
    return -7;
  }
}

- (int)playExtCreateFontIndirectW:(unsigned int)w in_pRecord:(const char *)record
{
  v49 = *MEMORY[0x277D85DE8];
  if (w < 0x149)
  {
    return -7;
  }

  v6 = 0;
  v44 = *(record + 4);
  v45 = *(record + 3);
  v43 = *(record + 5);
  v42 = *(record + 6);
  v40 = *(record + 2);
  v41 = *(record + 7);
  v35 = *(record + 32);
  v36 = *(record + 33);
  v37 = *(record + 34);
  v38 = record[37];
  v34 = *(record + 38);
  v7 = record + 41;
  v8 = *(record + 39);
  v46 = *(record + 36);
  v47 = *(record + 35);
  do
  {
    v9 = *(v7 - 1);
    v48[v6] = v9;
    if (!v9)
    {
      break;
    }

    v7 += 2;
    ++v6;
  }

  while (v6 != 32);
  v10 = [MEMORY[0x277CCACA8] stringWithCharacters:v48 length:?];
  v11 = 0;
  v12 = v10;
  v13 = record + 105;
  do
  {
    v14 = *(v13 - 1);
    v48[v11] = v14;
    if (!v14)
    {
      break;
    }

    v13 += 2;
    ++v11;
  }

  while (v11 != 64);
  v15 = [MEMORY[0x277CCACA8] stringWithCharacters:v48 length:?];
  v16 = 0;
  v17 = v15;
  v18 = record + 233;
  do
  {
    v19 = *(v18 - 1);
    v48[v16] = v19;
    if (!v19)
    {
      break;
    }

    v18 += 2;
    ++v16;
  }

  while (v16 != 32);
  v20 = [MEMORY[0x277CCACA8] stringWithCharacters:v48 length:?];
  v21 = *(record + 74);
  v22 = *(record + 75);
  v23 = *(record + 76);
  v24 = *(record + 78);
  v25 = *(record + 79);
  IsValidCharSet = ChValueIsValidCharSet(v47);
  v28 = v8 < 0x60 && v46 < 0xBu;
  if (v28 & IsValidCharSet)
  {
    v29 = v34;
    if (v34 >= 5)
    {
      v29 = 0;
    }

    LODWORD(v33) = v8 & 0x70;
    BYTE4(v32) = v38;
    HIDWORD(v31) = v47;
    LODWORD(v32) = v46;
    BYTE1(v31) = v37 != 0;
    LOBYTE(v31) = v36 != 0;
    v4 = [MFGraphicsDevice createFontIndirectW:"createFontIndirectW:lfWidth:lfEscapement:lfOrientation:lfWeight:lfItalic:lfUnderline:lfStrikeOut:lfCharSet:lfOutPrecision:lfClipPrecision:lfQuality:pitch:family:lfFaceName:elfFullName:elfStyle:elfVersion:elfStyleSize:elfMatch:elfVendorId:elfCulture:index:" lfWidth:v45 lfEscapement:v44 lfOrientation:v43 lfWeight:v42 lfItalic:v41 lfUnderline:v35 != 0 lfStrikeOut:v31 lfCharSet:v32 lfOutPrecision:__PAIR64__(v8 & 3 lfClipPrecision:v29) lfQuality:v33 pitch:v12 family:v17 lfFaceName:v20 elfFullName:__PAIR64__(v22 elfStyle:v21) elfVersion:__PAIR64__(v24 elfStyleSize:v23) elfMatch:__PAIR64__(v40 elfVendorId:v25) elfCulture:? index:?];
  }

  else
  {
    v4 = -7;
  }

  return v4;
}

- (int)playCreateBrushIndirect:(unsigned int)indirect in_pRecord:(const char *)record
{
  if (indirect < 0x18)
  {
    return -7;
  }

  v7 = *(record + 8);
  v8 = *(record + 9);
  v9 = *(record + 10);
  v10 = *(record + 11);
  v11 = *(record + 3);
  v12 = [OITSUColor colorWithRGBBytes:*(record + 16) green:*(record + 17) blue:*(record + 18)];
  if (v11 > 9)
  {
    v4 = -7;
  }

  else
  {
    v13 = *(record + 5);
    if (v13 >= 6)
    {
      v14 = 0xFFFFFFLL;
    }

    else
    {
      v14 = v13;
    }

    v4 = [(MFGraphicsDevice *)self->m_gdi createBrush:dword_25D6FD6E8[v11] in_colour:v12 hatchstyle:v14 index:v7 | (v8 << 8) | (v9 << 16) | (v10 << 24)];
  }

  return v4;
}

- (int)playCreateMonoBrush:(unsigned int)brush in_pRecord:(const char *)record
{
  if (brush < 0x20)
  {
    return -7;
  }

  v5 = *(record + 4);
  v4 = *(record + 5);
  if (v5 + v4 > brush)
  {
    return -7;
  }

  v6 = *(record + 6);
  v7 = v6 + *(record + 7);
  v8 = !HIDWORD(v7) && (v5 + v4) >> 32 == 0;
  if (!v8 || v7 > brush)
  {
    return -7;
  }

  v13 = *(record + 2);
  v14 = [MFGraphicsDevice createDIBitmap:"createDIBitmap:in_headerSize:in_bitmap:in_bitmapSize:in_usage:" in_headerSize:&record[v5] in_bitmap:*(record + 5) in_bitmapSize:&record[v6] in_usage:?];
  v15 = v14;
  if (v14)
  {
    v10 = [(MFGraphicsDevice *)self->m_gdi createPatternBrush:v14 index:v13 usePaletteForBilevel:0];
  }

  else
  {
    v10 = -7;
  }

  return v10;
}

- (int)playCreateDibPatternBrushPT:(unsigned int)t in_pRecord:(const char *)record
{
  if (t < 0x20)
  {
    return -7;
  }

  v5 = *(record + 4);
  v4 = *(record + 5);
  if (v5 + v4 > t)
  {
    return -7;
  }

  v6 = *(record + 6);
  v7 = v6 + *(record + 7);
  v8 = !HIDWORD(v7) && (v5 + v4) >> 32 == 0;
  if (!v8 || v7 > t)
  {
    return -7;
  }

  v13 = *(record + 2);
  v14 = [MFGraphicsDevice createDIBitmap:"createDIBitmap:in_headerSize:in_bitmap:in_bitmapSize:in_usage:" in_headerSize:&record[v5] in_bitmap:*(record + 5) in_bitmapSize:&record[v6] in_usage:?];
  v15 = v14;
  if (v14)
  {
    v10 = [(MFGraphicsDevice *)self->m_gdi createPatternBrush:v14 index:v13 usePaletteForBilevel:1];
  }

  else
  {
    v10 = -7;
  }

  return v10;
}

- (int)playExtTextOut:(unsigned int)out in_pRecord:(const char *)record wideChars:(BOOL)chars
{
  v64 = *MEMORY[0x277D85DE8];
  if (out < 0x38)
  {
    return -7;
  }

  if (chars)
  {
    v9 = 2;
  }

  else
  {
    v9 = 1;
  }

  v10 = *(record + 44) | (*(record + 45) << 8) | (*(record + 46) << 16) & 0xFFFFFF | (*(record + 47) << 24);
  v11 = *(record + 50);
  v12 = *(record + 48) | (*(record + 49) << 8) | (*(record + 50) << 16) & 0xFFFFFF | (*(record + 51) << 24);
  v13 = *(record + 52);
  if ((v13 & 6) != 0)
  {
    v14 = *(record + 14);
    v15 = *(record + 15);
    v17 = *(record + 16);
    v16 = *(record + 17);
  }

  else
  {
    v14 = 0;
    v15 = 0;
    v17 = 0;
    v16 = 0;
  }

  v18 = v10 * v9;
  v19 = (v18 & 0xFFFFFFFF00000000) != 0;
  v20 = v12 + v18;
  v5 = -7;
  if (HIDWORD(v20))
  {
    v21 = 1;
  }

  else
  {
    v21 = v20 > out;
  }

  if (!v21 && !v19)
  {
    v22 = *(record + 53);
    v54 = *(record + 54);
    v55 = v22;
    v23 = *(record + 9);
    v56 = *(record + 10);
    v57 = v23;
    v53 = *(record + 55);
    v24 = *(record + 72);
    v25 = *(record + 73);
    v62 = *(record + 74);
    v63 = v24;
    v60 = *(record + 75);
    v61 = v25;
    v58 = v16;
    v59 = v17;
    if (chars)
    {
      v26 = MEMORY[0x28223BE20](v11);
      v35 = &v52 - v34;
      if (v10)
      {
        v36 = (v33 << 24) | (v31 << 16) | (v32 << 8) | v29;
        v37 = (v27 << 24) + (v26 << 16) + (v28 << 8) + v30;
        v38 = v35;
        do
        {
          *v38++ = record[v37] | (record[v37 + 1] << 8);
          v37 += 2;
          --v36;
        }

        while (v36);
      }

      if ((v13 & 0x10) != 0)
      {
        [MEMORY[0x277CBEA90] dataWithBytes:? length:?];
      }

      else
      {
        [MEMORY[0x277CCACA8] stringWithCharacters:? length:?];
      }
      v40 = ;
    }

    else
    {
      getCurrentFont = [(MFGraphicsDevice *)self->m_gdi getCurrentFont];
      v40 = [getCurrentFont stringWithBytes:&record[v12] length:v10];
    }

    v41 = 0;
    LODWORD(v42) = 0;
    v43 = v63 | (v61 << 8) | (v62 << 16) & 0xFFFFFF | (v60 << 24);
    if (v43)
    {
      v44 = out >= v43;
      v45 = out - v43;
      if (v45 != 0 && v44)
      {
        v46 = v45 >> 2;
        if (v46 >= v10)
        {
          v42 = v10;
        }

        else
        {
          v42 = v46;
        }

        v47 = smalloc_typed(v42, 4uLL, 0x100004052888210uLL);
        v41 = v47;
        if (!v47)
        {
          v5 = -4;
          goto LABEL_34;
        }

        if (v42)
        {
          v48 = 0;
          v49 = (v60 << 24) + (v62 << 16) + (v61 << 8) + v63;
          do
          {
            *&v47[v48] = record[(v49 + v48)] | (record[(v49 + v48 + 1)] << 8) | (record[(v49 + v48 + 2)] << 16) | (record[(v49 + v48 + 3)] << 24);
            v48 += 4;
          }

          while (4 * v42 != v48);
        }
      }
    }

    LODWORD(v51) = v42;
    v5 = [(MFGraphicsDevice *)self->m_gdi extTextOut:v57 y:v56 in_text:v40 options:(v54 << 16) | (v55 << 8) | (v53 << 24) | v13 left:v14 top:v15 right:__PAIR64__(v58 bottom:v59) spacingValues:v41 numValues:v51];
    free(v41);
LABEL_34:
  }

  return v5;
}

- (int)playSetTextAlign:(unsigned int)align in_pRecord:(const char *)record
{
  if (align >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi setTextAlign:record[8] & 1 textHorizontalAlign:record[8] & 6 textVerticalAlign:record[8] & 0x18 textDirection:(record[9] & 1) << 8];
  }

  else
  {
    return -7;
  }
}

- (int)playSetTextColour:(unsigned int)colour in_pRecord:(const char *)record
{
  if (colour < 0xC)
  {
    return -7;
  }

  v6 = [OITSUColor colorWithRGBBytes:*(record + 8) green:*(record + 9) blue:*(record + 10)];
  v4 = [(MFGraphicsDevice *)self->m_gdi setTextColour:v6];

  return v4;
}

- (int)playSetTextJustification:(unsigned int)justification in_pRecord:(const char *)record
{
  if (justification >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setTextJustification:*(record + 3) in_breakCount:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetBkColour:(unsigned int)colour in_pRecord:(const char *)record
{
  if (colour < 0xC)
  {
    return -7;
  }

  v6 = [OITSUColor colorWithRGBBytes:*(record + 8) green:*(record + 9) blue:*(record + 10)];
  v4 = [(MFGraphicsDevice *)self->m_gdi setBkColour:v6];

  return v4;
}

- (int)playOffsetClipRegion:(unsigned int)region in_pRecord:(const char *)record
{
  if (region >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi offsetClipRegionByX:*(record + 2) y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetBkMode:(unsigned int)mode in_pRecord:(const char *)record
{
  if (mode < 0xC || (*(record + 2) - 1) > 1)
  {
    return -7;
  }

  else
  {
    return [(MFGraphicsDevice *)self->m_gdi setBkMode:?];
  }
}

- (int)playSetBrushOrg:(unsigned int)org in_pRecord:(const char *)record
{
  if (org >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi setBrushOrg:*(record + 2) y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetMapMode:(unsigned int)mode in_pRecord:(const char *)record
{
  if (mode < 0xC || (*(record + 2) - 1) > 7)
  {
    return -7;
  }

  else
  {
    return [(MFGraphicsDevice *)self->m_gdi setMapMode:?];
  }
}

- (int)playPolyDraw:(unsigned int)draw in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (draw < 0x1C)
  {
    return -7;
  }

  v23 = v5;
  v24 = v6;
  in_is16bitCopy = in_is16bit;
  v10 = *(record + 6);
  if (in_is16bit)
  {
    v11 = 5;
  }

  else
  {
    v11 = 9;
  }

  v12 = v10 * v11;
  if (v12 > 0xFFFFFFE3)
  {
    return -7;
  }

  result = -7;
  if (v12 + 28 <= draw && (v12 & 0xFFFFFFFF00000000) == 0)
  {
    if (in_is16bit)
    {
      v14 = 2;
    }

    else
    {
      v14 = 3;
    }

    if (v10)
    {
      v15 = 0;
      v16 = (v10 << v14) + 28;
      while (1)
      {
        *v22 = 0;
        v17 = v15 + v16;
        v18 = record[v15 + v16];
        parsePoint((record + 28), v15, &v22[1], v22, in_is16bitCopy);
        v19 = v18 & 0xFE;
        switch(v19)
        {
          case 6:
            result = [(MFGraphicsDevice *)self->m_gdi moveTo:v22[1] y:v22[0]];
            if (result)
            {
              return result;
            }

            break;
          case 4:
            *v20 = 0;
            *v21 = 0;
            if (v10 - v15 > 2 || (record[v17 + 1] & 0xFE) != 6 || (record[v17 + 2] & 0xFE) != 6)
            {
              return -7;
            }

            parsePoint((record + 28), v15 + 1, &v21[1], v21, in_is16bitCopy);
            v15 += 2;
            parsePoint((record + 28), v15, &v20[1], v20, in_is16bitCopy);
            result = [(MFGraphicsDevice *)self->m_gdi bezierTo:v22[1] y:v22[0] x2:v21[1] y2:v21[0] x3:v20[1] y3:v20[0]];
            v18 = record[v15 + v16];
            if (result)
            {
              return result;
            }

            break;
          case 2:
            result = [(MFGraphicsDevice *)self->m_gdi lineTo:v22[1] y:v22[0]];
            if (result)
            {
              return result;
            }

            break;
          default:
            return -7;
        }

        if ((v18 & 1) == 0 || (result = [(MFGraphicsDevice *)self->m_gdi closeCurrentPath:1]) == 0)
        {
          result = 0;
          if (++v15 < v10)
          {
            continue;
          }
        }

        return result;
      }
    }

    return 0;
  }

  return result;
}

- (int)playPolyBezier:(unsigned int)bezier in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (bezier < 0x1C)
  {
    return -7;
  }

  v24 = v5;
  v25 = v6;
  in_is16bitCopy = in_is16bit;
  if (in_is16bit)
  {
    v10 = 4;
  }

  else
  {
    v10 = 8;
  }

  v11 = *(record + 6);
  v12 = v11 * v10;
  if (v12 > 0xFFFFFFE3)
  {
    return -7;
  }

  result = -7;
  if (v11 && v12 + 28 <= bezier && (v12 & 0xFFFFFFFF00000000) == 0 && -1431655765 * v11 + 1431655765 <= 0x55555555)
  {
    *v22 = 0;
    *v23 = 0;
    *v20 = 0;
    *v21 = 0;
    parsePoint((record + 28), 0, &v23[1], v23, in_is16bit);
    if (v11 < 2)
    {
      return 0;
    }

    else
    {
      v14 = v23[0];
      v15 = v23[1];
      v16 = 1;
      while (1)
      {
        parsePoint((record + 28), v16, &v22[1], v22, in_is16bitCopy);
        parsePoint((record + 28), v16 + 1, &v21[1], v21, in_is16bitCopy);
        v17 = v16 + 2;
        parsePoint((record + 28), v17, &v20[1], v20, in_is16bitCopy);
        v19 = v20[0];
        v18 = v20[1];
        result = [(MFGraphicsDevice *)self->m_gdi bezier:v15 y:v14 x2:v22[1] y2:v22[0] x3:v21[1] y3:v21[0] x4:__PAIR64__(v20[0] y4:v20[1])];
        if (result)
        {
          break;
        }

        v16 = v17 + 1;
        v15 = v18;
        v14 = v19;
        if (v16 >= v11)
        {
          return 0;
        }
      }
    }
  }

  return result;
}

- (int)playPolyBezierTo:(unsigned int)to in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (to < 0x1C)
  {
    return -7;
  }

  v20 = v5;
  v21 = v6;
  in_is16bitCopy = in_is16bit;
  if (in_is16bit)
  {
    v11 = 4;
  }

  else
  {
    v11 = 8;
  }

  v12 = *(record + 6);
  v13 = v12 * v11;
  result = -7;
  v15 = -1431655765 * v12 > 0x55555555 || v12 == 0 || v13 + 28 > to;
  if (!v15 && (v13 & 0xFFFFFFFF00000000) == 0 && v13 <= 0xFFFFFFE3)
  {
    v16 = 0;
    *v18 = 0;
    *v19 = 0;
    *v17 = 0;
    while (v16 < v12)
    {
      parsePoint((record + 28), v16, &v19[1], v19, in_is16bitCopy);
      parsePoint((record + 28), v16 + 1, &v18[1], v18, in_is16bitCopy);
      parsePoint((record + 28), v16 + 2, &v17[1], v17, in_is16bitCopy);
      result = [(MFGraphicsDevice *)self->m_gdi bezierTo:v19[1] y:v19[0] x2:v18[1] y2:v18[0] x3:v17[1] y3:v17[0]];
      v16 += 3;
      if (result)
      {
        return result;
      }
    }

    return 0;
  }

  return result;
}

- (int)playPolygon:(unsigned int)polygon in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  result = -7;
  if (polygon >= 0x1C && record)
  {
    in_is16bitCopy = in_is16bit;
    if (in_is16bit)
    {
      v9 = 4;
    }

    else
    {
      v9 = 8;
    }

    v10 = *(record + 24);
    v11 = *(record + 25);
    v12 = *(record + 26);
    v13 = *(record + 27);
    v14 = v10 | (*(record + 25) << 8) | (v12 << 16) & 0xFFFFFF | (v13 << 24);
    v15 = v14 * v9;
    if (v15 <= 0xFFFFFFE3 && v15 + 28 <= polygon && (v15 & 0xFFFFFFFF00000000) == 0)
    {
      if (v14 < 2)
      {
        return 0;
      }

      else
      {
        v16 = smalloc_typed(*(record + 24) | (*(record + 25) << 8) | (*(record + 26) << 16) & 0xFFFFFF | (v13 << 24), 0x10uLL, 0x1000040451B5BE8uLL);
        if (v16)
        {
          v17 = v16;
          v18 = 0;
          *v23 = 0;
          v19 = (v13 << 24) | (v12 << 16) | (v11 << 8) | v10;
          v20 = v16 + 1;
          do
          {
            parsePoint((record + 28), v18, &v23[1], v23, in_is16bitCopy);
            v21 = v23[0];
            *(v20 - 1) = v23[1];
            *v20 = v21;
            ++v18;
            v20 += 2;
          }

          while (v19 != v18);
          v22 = [(MFGraphicsDevice *)self->m_gdi polygon:v17 in_count:v14];
          free(v17);
          return v22;
        }

        else
        {
          return -4;
        }
      }
    }
  }

  return result;
}

- (int)playPolyLine:(unsigned int)line in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (in_is16bit)
  {
    v6 = 4;
  }

  else
  {
    v6 = 8;
  }

  result = -7;
  if (line >= 0x1C)
  {
    if (record)
    {
      v9 = *(record + 6);
      if (v9 >= 2)
      {
        v10 = v9 * v6;
        if (v10 <= 0xFFFFFFE3 && (v10 & 0xFFFFFFFF00000000) == 0 && v10 + 28 <= line)
        {
          in_is16bitCopy = in_is16bit;
          v12 = smalloc_typed(*(record + 6), 0x10uLL, 0x1000040451B5BE8uLL);
          if (v12)
          {
            v13 = v12;
            v14 = 0;
            *v18 = 0;
            v15 = v12 + 1;
            do
            {
              parsePoint((record + 28), v14, &v18[1], v18, in_is16bitCopy);
              v16 = v18[0];
              *(v15 - 1) = v18[1];
              *v15 = v16;
              ++v14;
              v15 += 2;
            }

            while (v9 != v14);
            v17 = [(MFGraphicsDevice *)self->m_gdi polyline:v13 in_count:v9];
            free(v13);
            return v17;
          }

          else
          {
            return -4;
          }
        }
      }
    }
  }

  return result;
}

- (int)playPolyLineTo:(unsigned int)to in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (to < 2)
  {
    return -7;
  }

  v21 = v5;
  v22 = v6;
  in_is16bitCopy = in_is16bit;
  if (in_is16bit)
  {
    v10 = 4;
  }

  else
  {
    v10 = 8;
  }

  v11 = *(record + 24);
  v12 = *(record + 25);
  v13 = *(record + 26);
  v14 = *(record + 27);
  v15 = v11 | (*(record + 25) << 8) | (v13 << 16) & 0xFFFFFF | (v14 << 24);
  v16 = v15 * v10;
  if (v16 > 0xFFFFFFE3)
  {
    return -7;
  }

  result = -7;
  if (v16 + 28 <= to && (v16 & 0xFFFFFFFF00000000) == 0)
  {
    if (v15)
    {
      v18 = 0;
      *v20 = 0;
      v19 = (v14 << 24) + (v13 << 16) + (v12 << 8) + v11;
      while (1)
      {
        parsePoint((record + 28), v18, &v20[1], v20, in_is16bitCopy);
        result = [(MFGraphicsDevice *)self->m_gdi lineTo:v20[1] y:v20[0]];
        if (result)
        {
          break;
        }

        if (v19 == ++v18)
        {
          return 0;
        }
      }
    }

    else
    {
      return 0;
    }
  }

  return result;
}

- (int)playRoundRect:(unsigned int)rect in_pRecord:(const char *)record
{
  if (rect >= 0x20)
  {
    return [(MFGraphicsDevice *)self->m_gdi roundRect:*(record + 2) top:*(record + 3) right:*(record + 4) bottom:*(record + 5) rx:vcvtd_n_f64_s32(*(record + 6) ry:1uLL), vcvtd_n_f64_s32(*(record + 7), 1uLL)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetStretchBltMode:(unsigned int)mode in_pRecord:(const char *)record
{
  if (mode < 0xC || (*(record + 2) - 1) > 3)
  {
    return -7;
  }

  else
  {
    return [(MFGraphicsDevice *)self->m_gdi setStretchBltMode:?];
  }
}

- (int)playSaveDC:(unsigned int)c in_pRecord:(const char *)record
{
  if (c >= 8)
  {
    return [(MFGraphicsDevice *)self->m_gdi saveDC];
  }

  else
  {
    return -7;
  }
}

- (int)playRestoreDC:(unsigned int)c in_pRecord:(const char *)record
{
  if (c >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi restoreDC:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playLineTo:(unsigned int)to in_pRecord:(const char *)record
{
  if (to >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi lineTo:*(record + 2) y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playMoveToEx:(unsigned int)ex in_pRecord:(const char *)record
{
  if (ex >= 0x10)
  {
    return [(MFGraphicsDevice *)self->m_gdi moveTo:*(record + 2) y:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playSetArcDirection:(unsigned int)direction in_pRecord:(const char *)record
{
  if (direction >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi setArcDirection:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playArc:(unsigned int)arc in_pRecord:(const char *)record
{
  if (arc >= 0x28)
  {
    return [(MFGraphicsDevice *)self->m_gdi arc:*(record + 2) nTopRect:*(record + 3) nRightRect:*(record + 4) nBottomRect:*(record + 5) nXStartArc:*(record + 6) nYStartArc:*(record + 7) nXEndArc:*(record + 4) nYEndArc:?];
  }

  else
  {
    return -7;
  }
}

- (int)playArcTo:(unsigned int)to in_pRecord:(const char *)record
{
  if (to >= 0x28)
  {
    return [(MFGraphicsDevice *)self->m_gdi arcTo:*(record + 2) nTopRect:*(record + 3) nRightRect:*(record + 4) nBottomRect:*(record + 5) nXStartArc:*(record + 6) nYStartArc:*(record + 7) nXEndArc:*(record + 4) nYEndArc:?];
  }

  else
  {
    return -7;
  }
}

- (int)playChord:(unsigned int)chord in_pRecord:(const char *)record
{
  if (chord >= 0x28)
  {
    return [(MFGraphicsDevice *)self->m_gdi chord:*(record + 2) nTopRect:*(record + 3) nRightRect:*(record + 4) nBottomRect:*(record + 5) nXStartArc:*(record + 6) nYStartArc:*(record + 7) nXEndArc:*(record + 4) nYEndArc:?];
  }

  else
  {
    return -7;
  }
}

- (int)playPie:(unsigned int)pie in_pRecord:(const char *)record
{
  if (pie >= 0x28)
  {
    return [(MFGraphicsDevice *)self->m_gdi pie:*(record + 2) nTopRect:*(record + 3) nRightRect:*(record + 4) nBottomRect:*(record + 5) nXStartArc:*(record + 6) nYStartArc:*(record + 7) nXEndArc:*(record + 4) nYEndArc:?];
  }

  else
  {
    return -7;
  }
}

- (int)playEllipse:(unsigned int)ellipse in_pRecord:(const char *)record
{
  if (ellipse >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi ellipse:*(record + 4) y1:*(record + 5) x2:*(record + 2) y2:*(record + 3)];
  }

  else
  {
    return -7;
  }
}

- (int)playRectangle:(unsigned int)rectangle in_pRecord:(const char *)record
{
  if (rectangle >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi rectangle:*(record + 2) y1:*(record + 3) x2:*(record + 4) y2:*(record + 5)];
  }

  else
  {
    return -7;
  }
}

- (int)playPolyPolygon:(unsigned int)polygon in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (polygon < 0x20)
  {
    return -7;
  }

  in_is16bitCopy = in_is16bit;
  v7 = in_is16bit ? 4 : 8;
  v8 = *(record + 27);
  v9 = *(record + 12) | (*(record + 26) << 16) | (v8 << 24);
  v10 = *(record + 28);
  v11 = *(record + 29);
  v12 = *(record + 30);
  v13 = *(record + 31);
  v14 = v10 | (*(record + 29) << 8) | (v12 << 16) & 0xFFFFFF | (v13 << 24);
  v15 = 4 * v9;
  v16 = v14 * v7;
  v17 = (v16 & 0xFFFFFFFF00000000) != 0;
  v18 = v16 + (4 * v9 + 32);
  if (HIDWORD(v18))
  {
    return -7;
  }

  v19 = -7;
  if (v15 <= 0xFFFFFFDF && !v17 && v8 <= 0x3F && v18 <= polygon)
  {
    v20 = smalloc_typed(v14, 0x10uLL, 0x1000040451B5BE8uLL);
    v21 = smalloc_typed(v9, 4uLL, 0x100004052888210uLL);
    if (v20 && v21)
    {
      v35 = v21;
      v36 = v20;
      if (v9)
      {
        v22 = 0;
        v23 = 35;
        v24 = v9;
        v25 = v21;
        while (1)
        {
          v26 = record[v23 - 3] | (record[v23 - 2] << 8) | (record[v23 - 1] << 16) | (record[v23] << 24);
          *v25 = v26;
          v22 += v26;
          if (v26 <= 1)
          {
            break;
          }

          ++v25;
          v23 += 4;
          if (!--v24)
          {
            v27 = 0;
            goto LABEL_26;
          }
        }

        v27 = -7;
      }

      else
      {
        v27 = 0;
        v22 = 0;
      }

LABEL_26:
      if (v22 <= v14)
      {
        v19 = v27;
      }

      else
      {
        v19 = -7;
      }

      if (!v19)
      {
        if (v14)
        {
          v30 = 0;
          v31 = &record[v15];
          v32 = (v13 << 24) | (v12 << 16) | (v11 << 8) | v10;
          v33 = v36 + 1;
          do
          {
            *v38 = 0;
            parsePoint((v31 + 32), v30, &v38[1], v38, in_is16bitCopy);
            v34 = v38[0];
            *(v33 - 1) = v38[1];
            *v33 = v34;
            ++v30;
            v33 += 2;
          }

          while (v32 != v30);
        }

        v19 = [(MFGraphicsDevice *)self->m_gdi polyPolygon:v36 in_polyCounts:v35 in_count:v9, v35];
      }

      free(v36);
      free(v35);
    }

    else
    {
      if (v20)
      {
        v29 = v21;
        free(v20);
        v21 = v29;
      }

      if (v21)
      {
        free(v21);
      }

      return -4;
    }
  }

  return v19;
}

- (int)playPolyPolyLine:(unsigned int)line in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (line < 0x20)
  {
    return -7;
  }

  in_is16bitCopy = in_is16bit;
  v7 = in_is16bit ? 4 : 8;
  v8 = *(record + 27);
  v9 = *(record + 12) | (*(record + 26) << 16) | (v8 << 24);
  v10 = *(record + 28);
  v11 = *(record + 29);
  v12 = *(record + 30);
  v13 = *(record + 31);
  v14 = v10 | (*(record + 29) << 8) | (v12 << 16) & 0xFFFFFF | (v13 << 24);
  v15 = 4 * v9;
  v16 = v14 * v7;
  v17 = (v16 & 0xFFFFFFFF00000000) != 0;
  v18 = v16 + (4 * v9 + 32);
  if (HIDWORD(v18))
  {
    return -7;
  }

  v19 = -7;
  if (v15 <= 0xFFFFFFDF && !v17 && v8 <= 0x3F && v18 <= line)
  {
    v20 = smalloc_typed(v14, 0x10uLL, 0x1000040451B5BE8uLL);
    v21 = smalloc_typed(v9, 4uLL, 0x100004052888210uLL);
    if (v20 && v21)
    {
      v35 = v21;
      v36 = v20;
      if (v9)
      {
        v22 = 0;
        v23 = 35;
        v24 = v9;
        v25 = v21;
        while (1)
        {
          v26 = record[v23 - 3] | (record[v23 - 2] << 8) | (record[v23 - 1] << 16) | (record[v23] << 24);
          *v25 = v26;
          v22 += v26;
          if (v26 <= 1)
          {
            break;
          }

          ++v25;
          v23 += 4;
          if (!--v24)
          {
            v27 = 0;
            goto LABEL_26;
          }
        }

        v27 = -7;
      }

      else
      {
        v27 = 0;
        v22 = 0;
      }

LABEL_26:
      if (v22 <= v14)
      {
        v19 = v27;
      }

      else
      {
        v19 = -7;
      }

      if (!v19)
      {
        if (v14)
        {
          v30 = 0;
          v31 = &record[v15];
          v32 = (v13 << 24) | (v12 << 16) | (v11 << 8) | v10;
          v33 = v36 + 1;
          do
          {
            *v38 = 0;
            parsePoint((v31 + 32), v30, &v38[1], v38, in_is16bitCopy);
            v34 = v38[0];
            *(v33 - 1) = v38[1];
            *v33 = v34;
            ++v30;
            v33 += 2;
          }

          while (v32 != v30);
        }

        v19 = [(MFGraphicsDevice *)self->m_gdi polyPolyline:v36 in_polyCounts:v35 in_count:v9, v35];
      }

      free(v36);
      free(v35);
    }

    else
    {
      if (v20)
      {
        v29 = v21;
        free(v20);
        v21 = v29;
      }

      if (v21)
      {
        free(v21);
      }

      return -4;
    }
  }

  return v19;
}

- (int)playAngleArc:(unsigned int)arc in_pRecord:(const char *)record in_is16bit:(BOOL)in_is16bit
{
  if (arc < 0x1C)
  {
    return -7;
  }

  v9 = *(record + 2);
  v10 = *(record + 3);
  v11 = *(record + 4);
  [MFByteUtils readFloat32FromByteArray:record in_offset:20, in_is16bit];
  v13 = v12;
  [MFByteUtils readFloat32FromByteArray:record in_offset:24];
  m_gdi = self->m_gdi;

  return [(MFGraphicsDevice *)m_gdi angleArc:v9 y:v10 radius:v11 startAngle:v13 sweepAngle:v14];
}

- (int)playBitBlt:(unsigned int)blt in_pRecord:(const char *)record
{
  if (blt < 0x64)
  {
    return -7;
  }

  v6 = *(record + 21);
  v5 = *(record + 22);
  if (v6 + v5 > blt)
  {
    return -7;
  }

  v8 = *(record + 23);
  v7 = *(record + 24);
  v9 = v8 + v7;
  v10 = !HIDWORD(v9) && (v6 + v5) >> 32 == 0;
  if (!v10 || v9 > blt)
  {
    return -7;
  }

  v32 = *(record + 20);
  [MFByteUtils readFloat32FromByteArray:record in_offset:52];
  v33[0] = v15;
  [MFByteUtils readFloat32FromByteArray:record in_offset:56];
  v33[1] = v16;
  [MFByteUtils readFloat32FromByteArray:record in_offset:60];
  v33[2] = v17;
  [MFByteUtils readFloat32FromByteArray:record in_offset:64];
  v33[3] = v18;
  [MFByteUtils readFloat32FromByteArray:record in_offset:68];
  v33[4] = v19;
  [MFByteUtils readFloat32FromByteArray:record in_offset:72];
  v33[5] = v20;
  v30 = *(record + 7);
  v31 = *(record + 6);
  v28 = *(record + 9);
  v29 = *(record + 8);
  v21 = *(record + 12);
  v27 = *(record + 11);
  v22 = *(record + 10);
  v23 = *(record + 19);
  v24 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v6] in_headerSize:v5 in_bitmap:&record[v8] in_bitmapSize:v7 in_usage:v32];
  v25 = v24;
  if (v24)
  {
    LODWORD(v26) = v23;
    v12 = [(MFGraphicsDevice *)self->m_gdi bitBlt:v31 in_yDest:v30 in_widthDest:v29 in_heightDest:v28 in_sourceImage:v24 in_xSrc:v27 in_ySrc:__PAIR64__(v22 in_rop:v21) in_xform:v33 in_colour:v26];
  }

  else
  {
    v12 = -7;
  }

  return v12;
}

- (int)playStretchBlt:(unsigned int)blt in_pRecord:(const char *)record
{
  if (blt < 0x6C)
  {
    return -7;
  }

  v6 = *(record + 21);
  v5 = *(record + 22);
  if (v6 + v5 > blt)
  {
    return -7;
  }

  v8 = *(record + 23);
  v7 = *(record + 24);
  v9 = v8 + v7;
  v10 = !HIDWORD(v9) && (v6 + v5) >> 32 == 0;
  if (!v10 || v9 > blt)
  {
    return -7;
  }

  v34 = *(record + 20);
  [MFByteUtils readFloat32FromByteArray:record in_offset:52];
  v35[0] = v15;
  [MFByteUtils readFloat32FromByteArray:record in_offset:56];
  v35[1] = v16;
  [MFByteUtils readFloat32FromByteArray:record in_offset:60];
  v35[2] = v17;
  [MFByteUtils readFloat32FromByteArray:record in_offset:64];
  v35[3] = v18;
  [MFByteUtils readFloat32FromByteArray:record in_offset:68];
  v35[4] = v19;
  [MFByteUtils readFloat32FromByteArray:record in_offset:72];
  v35[5] = v20;
  v32 = *(record + 7);
  v33 = *(record + 6);
  v30 = *(record + 9);
  v31 = *(record + 8);
  v21 = *(record + 26);
  v27 = *(record + 25);
  v28 = *(record + 12);
  v22 = *(record + 10);
  v29 = *(record + 11);
  v23 = *(record + 19);
  v24 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v6] in_headerSize:v5 in_bitmap:&record[v8] in_bitmapSize:v7 in_usage:v34];
  v25 = v24;
  if (v24)
  {
    LODWORD(v26) = v23;
    v12 = [(MFGraphicsDevice *)self->m_gdi stretchBlt:v33 in_yDest:v32 in_widthDest:v31 in_heightDest:v30 in_sourceImage:v24 in_xSrc:v29 in_ySrc:__PAIR64__(v27 in_widthSrc:v28) in_heightSrc:__PAIR64__(v22 in_rop:v21) in_xform:v35 in_colour:v26];
  }

  else
  {
    v12 = -7;
  }

  return v12;
}

- (int)playAlphaBlend:(unsigned int)blend in_pRecord:(const char *)record
{
  if (blend < 0x6C)
  {
    return -7;
  }

  v6 = *(record + 21);
  v5 = *(record + 22);
  if (v6 + v5 > blend)
  {
    return -7;
  }

  v8 = *(record + 23);
  v7 = *(record + 24);
  v9 = v8 + v7;
  v10 = !HIDWORD(v9) && (v6 + v5) >> 32 == 0;
  if (!v10 || v9 > blend)
  {
    return -7;
  }

  v35 = *(record + 20);
  [MFByteUtils readFloat32FromByteArray:record in_offset:52];
  v36[0] = v15;
  [MFByteUtils readFloat32FromByteArray:record in_offset:56];
  v36[1] = v16;
  [MFByteUtils readFloat32FromByteArray:record in_offset:60];
  v36[2] = v17;
  [MFByteUtils readFloat32FromByteArray:record in_offset:64];
  v36[3] = v18;
  [MFByteUtils readFloat32FromByteArray:record in_offset:68];
  v36[4] = v19;
  [MFByteUtils readFloat32FromByteArray:record in_offset:72];
  v36[5] = v20;
  v32 = *(record + 7);
  v33 = *(record + 6);
  v30 = *(record + 9);
  v31 = *(record + 8);
  v34 = *(record + 11);
  v28 = *(record + 25);
  v29 = *(record + 12);
  HIDWORD(v27) = *(record + 26);
  v21 = record[42];
  v22 = *(record + 43);
  v23 = *(record + 19);
  v24 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v6] in_headerSize:v5 in_bitmap:&record[v8] in_bitmapSize:v7 in_usage:v35];
  v25 = v24;
  if (v24)
  {
    LODWORD(v27) = v23;
    BYTE5(v26) = v22 == 1;
    BYTE4(v26) = v21;
    LODWORD(v26) = HIDWORD(v27);
    v12 = [MFGraphicsDevice alphaBlend:"alphaBlend:in_yDest:in_widthDest:in_heightDest:in_sourceImage:in_xSrc:in_ySrc:in_widthSrc:in_heightSrc:in_sourceConstantOpacity:in_useSourceAlphaChannel:in_xform:in_colour:" in_yDest:v33 in_widthDest:v32 in_heightDest:v31 in_sourceImage:v30 in_xSrc:v24 in_ySrc:v34 in_widthSrc:__PAIR64__(v28 in_heightSrc:v29) in_sourceConstantOpacity:v26 in_useSourceAlphaChannel:v36 in_xform:v27 in_colour:?];
  }

  else
  {
    v12 = -7;
  }

  return v12;
}

- (int)playMaskBlt:(unsigned int)blt in_pRecord:(const char *)record
{
  if (blt < 0x80)
  {
    return -7;
  }

  v6 = *(record + 21);
  v5 = *(record + 22);
  if (v6 + v5 > blt)
  {
    return -7;
  }

  v7 = *(record + 23);
  v8 = *(record + 24);
  v9 = v7 + v8;
  if (v9 > blt)
  {
    return -7;
  }

  v10 = *(record + 28);
  v11 = v10 + *(record + 29);
  if (v11 > blt)
  {
    return -7;
  }

  v14 = *(record + 30) + *(record + 31);
  v12 = -7;
  if (!HIDWORD(v14) && v14 <= blt && !((v11 | (v6 + v5) | v9) >> 32))
  {
    v38 = *(record + 30);
    v39 = *(record + 31);
    v40 = *(record + 29);
    v41 = *(record + 20);
    [MFByteUtils readFloat32FromByteArray:record in_offset:52];
    v42[0] = v16;
    [MFByteUtils readFloat32FromByteArray:record in_offset:56];
    v42[1] = v17;
    [MFByteUtils readFloat32FromByteArray:record in_offset:60];
    v42[2] = v18;
    [MFByteUtils readFloat32FromByteArray:record in_offset:64];
    v42[3] = v19;
    [MFByteUtils readFloat32FromByteArray:record in_offset:68];
    v42[4] = v20;
    [MFByteUtils readFloat32FromByteArray:record in_offset:72];
    v42[5] = v21;
    v36 = *(record + 7);
    v37 = *(record + 6);
    v34 = *(record + 9);
    v35 = *(record + 8);
    v31 = *(record + 25);
    v32 = *(record + 12);
    v33 = *(record + 11);
    v29 = *(record + 10);
    v30 = *(record + 26);
    v28 = *(record + 19);
    v22 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v6] in_headerSize:v5 in_bitmap:&record[v7] in_bitmapSize:v8 in_usage:v41];
    v23 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v10] in_headerSize:v40 in_bitmap:&record[v38] in_bitmapSize:v39 in_usage:0];
    v24 = v23;
    v12 = -7;
    if (v22 && v23)
    {
      LODWORD(v27) = v28;
      LODWORD(v26) = v29;
      LODWORD(v25) = v32;
      v12 = [(MFGraphicsDevice *)self->m_gdi maskBlt:v37 in_yDest:v36 in_widthDest:v35 in_heightDest:v34 in_sourceImage:v22 in_xSrc:v33 in_ySrc:v25 in_maskImage:v23 in_xMask:__PAIR64__(v30 in_yMask:v31) in_rop:v26 in_xform:v42 in_colour:v27];
    }
  }

  return v12;
}

- (int)playPlgBlt:(unsigned int)blt in_pRecord:(const char *)record
{
  v43[5] = *MEMORY[0x277D85DE8];
  if (blt < 0x8C)
  {
    return -7;
  }

  v6 = *(record + 24);
  v5 = *(record + 25);
  if (v6 + v5 > blt)
  {
    return -7;
  }

  v7 = *(record + 26);
  v8 = *(record + 27);
  v9 = v7 + v8;
  if (v9 > blt)
  {
    return -7;
  }

  v10 = *(record + 31);
  v11 = v10 + *(record + 32);
  if (v11 > blt)
  {
    return -7;
  }

  v14 = *(record + 33) + *(record + 34);
  v12 = -7;
  if (!HIDWORD(v14) && v14 <= blt && !((v11 | (v6 + v5) | v9) >> 32))
  {
    v36 = *(record + 33);
    v37 = *(record + 34);
    v38 = *(record + 32);
    v39 = *(record + 23);
    *v41 = 0;
    [MFByteUtils readFloat32FromByteArray:record in_offset:64];
    v40[0] = v16;
    [MFByteUtils readFloat32FromByteArray:record in_offset:68];
    v40[1] = v17;
    [MFByteUtils readFloat32FromByteArray:record in_offset:72];
    v40[2] = v18;
    [MFByteUtils readFloat32FromByteArray:record in_offset:76];
    v40[3] = v19;
    [MFByteUtils readFloat32FromByteArray:record in_offset:80];
    v40[4] = v20;
    [MFByteUtils readFloat32FromByteArray:record in_offset:82];
    v21 = 0;
    v40[5] = v22;
    v23 = v43;
    do
    {
      parsePoint32((record + 24), v21, &v41[1], v41);
      v24 = v41[0];
      *(v23 - 1) = v41[1];
      *v23 = v24;
      ++v21;
      v23 += 2;
    }

    while (v21 != 3);
    v30 = *(record + 29);
    v31 = *(record + 28);
    v29 = *(record + 22);
    v34 = *(record + 13);
    v35 = *(record + 12);
    v32 = *(record + 15);
    v33 = *(record + 14);
    v25 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v6] in_headerSize:v5 in_bitmap:&record[v7] in_bitmapSize:v8 in_usage:v39];
    v26 = [(MFGraphicsDevice *)self->m_gdi createDIBitmap:&record[v10] in_headerSize:v38 in_bitmap:&record[v36] in_bitmapSize:v37 in_usage:0];
    v27 = v26;
    v12 = -7;
    if (v25 && v26)
    {
      LODWORD(v28) = v29;
      v12 = [(MFGraphicsDevice *)self->m_gdi plgBlt:v42 in_sourceImage:v25 in_xSrc:v35 in_ySrc:v34 in_widthSrc:v33 in_heightSrc:v32 in_maskImage:v26 in_xMask:__PAIR64__(v30 in_yMask:v31) in_xform:v40 in_colour:v28];
    }
  }

  return v12;
}

- (int)playSetDIBitsToDevice:(unsigned int)device in_pRecord:(const char *)record
{
  if (device < 0x4C)
  {
    return -7;
  }

  v6 = *(record + 12);
  v5 = *(record + 13);
  if (v6 + v5 > device)
  {
    return -1;
  }

  v7 = *(record + 14);
  v8 = v7 + *(record + 15);
  v9 = !HIDWORD(v8) && (v6 + v5) >> 32 == 0;
  if (!v9 || v8 > device)
  {
    return -1;
  }

  v12 = *(record + 9);
  v13 = *(record + 11);
  v14 = *(record + 17);
  if (v12 >= v14)
  {
    v15 = 0;
  }

  else
  {
    v15 = v12 - v14;
  }

  v16 = v13 - v15;
  v17 = *(record + 18) + v14;
  v18 = v13 + v12;
  v19 = __OFSUB__(v17, v18);
  v20 = v17 - v18;
  if (v20 < 0 == v19)
  {
    v20 = 0;
  }

  v21 = (v20 + v16);
  if (v21 < 1)
  {
    return 0;
  }

  v23 = *(record + 6);
  v24 = *(record + 7);
  v25 = *(record + 8);
  v26 = *(record + 10);
  v27 = [MFGraphicsDevice createDIBitmap:"createDIBitmap:in_headerSize:in_bitmap:in_bitmapSize:in_usage:" in_headerSize:&record[v6] in_bitmap:*(record + 13) in_bitmapSize:&record[v7] in_usage:?];
  if (v27)
  {
    LODWORD(v29) = 0;
    LODWORD(v28) = v15 + v12;
    HIDWORD(v28) = 13369344;
    v4 = [(MFGraphicsDevice *)self->m_gdi bitBlt:v23 in_yDest:(v15 + v24) in_widthDest:v26 in_heightDest:v21 in_sourceImage:v27 in_xSrc:v25 in_ySrc:v28 in_rop:0 in_xform:v29 in_colour:?];
  }

  else
  {
    v4 = -7;
  }

  return v4;
}

- (int)playStretchDIBits:(unsigned int)bits in_pRecord:(const char *)record
{
  if (bits < 0x50)
  {
    return -7;
  }

  v5 = *(record + 12);
  v4 = *(record + 13);
  if (v5 + v4 > bits)
  {
    return -7;
  }

  v6 = *(record + 14);
  v7 = v6 + *(record + 15);
  v8 = !HIDWORD(v7) && (v5 + v4) >> 32 == 0;
  if (!v8 || v7 > bits)
  {
    return -7;
  }

  v13 = *(record + 7);
  v23 = *(record + 6);
  v14 = *(record + 18);
  v15 = *(record + 19);
  v16 = *(record + 9);
  v24 = *(record + 8);
  v17 = *(record + 10);
  v18 = *(record + 11);
  v19 = *(record + 17);
  v20 = [MFGraphicsDevice createDIBitmap:"createDIBitmap:in_headerSize:in_bitmap:in_bitmapSize:in_usage:" in_headerSize:&record[v5] in_bitmap:v4 in_bitmapSize:&record[v6] in_usage:?];
  v21 = v20;
  if (v20)
  {
    BYTE4(v22) = 1;
    LODWORD(v22) = 0;
    v10 = [(MFGraphicsDevice *)self->m_gdi stretchBlt:v23 in_yDest:v13 in_widthDest:v14 in_heightDest:v15 in_sourceImage:v20 in_xSrc:v24 in_ySrc:__PAIR64__(v17 in_widthSrc:v16) in_heightSrc:__PAIR64__(v19 in_rop:v18) in_xform:0 in_colour:v22 usePaletteForBilevel:?];
  }

  else
  {
    v10 = -7;
  }

  return v10;
}

- (int)playSetPolyFillMode:(unsigned int)mode in_pRecord:(const char *)record
{
  if (mode < 0xC)
  {
    return -7;
  }

  m_gdi = self->m_gdi;
  if (*(record + 2) == 1)
  {
    v6 = 1;
  }

  else
  {
    v6 = 2;
  }

  return [(MFGraphicsDevice *)m_gdi setPolyFillMode:v6];
}

- (int)playSetMiterLimit:(unsigned int)limit in_pRecord:(const char *)record
{
  if (limit < 0xC)
  {
    return -7;
  }

  [MFByteUtils readFloat32FromByteArray:record in_offset:8];
  v8 = v7;
  m_gdi = self->m_gdi;

  return [(MFGraphicsDevice *)m_gdi setMiterLimit:v8];
}

- (int)playSetRop2:(unsigned int)rop2 in_pRecord:(const char *)record
{
  if (rop2 >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi setRop2:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playSelectClipPath:(unsigned int)path in_pRecord:(const char *)record
{
  if (path >= 0xC)
  {
    return [(MFGraphicsDevice *)self->m_gdi selectClipPath:*(record + 2)];
  }

  else
  {
    return -7;
  }
}

- (int)playSelectClipRegion:(unsigned int)region in_pRecord:(const char *)record
{
  if (region < 0x10)
  {
    return -7;
  }

  v5 = *(record + 2);
  if (v5 > 0xFFFFFFEF || v5 + 16 > region)
  {
    return -7;
  }

  v8 = *(record + 3);
  v9 = parseRegion(v5, record + 16);
  v10 = v9;
  if (v9)
  {
    v7 = [(MFGraphicsDevice *)self->m_gdi selectClipRegionWithRects:v9 in_mode:v8];
  }

  else
  {
    v7 = -7;
  }

  return v7;
}

- (int)playFillRgn:(unsigned int)rgn in_pRecord:(const char *)record
{
  if (rgn < 0x20)
  {
    return -7;
  }

  v5 = *(record + 6);
  if (v5 > 0xFFFFFFDF || v5 + 32 > rgn)
  {
    return -7;
  }

  v8 = *(record + 7);
  v9 = parseRegion(v5, record + 32);
  v10 = v9;
  if (v9)
  {
    v7 = [(MFGraphicsDevice *)self->m_gdi fillRgnWithRects:v9 in_brushID:v8];
  }

  else
  {
    v7 = -7;
  }

  return v7;
}

- (int)playPaintRgn:(unsigned int)rgn in_pRecord:(const char *)record
{
  if (rgn < 0x1C)
  {
    return -7;
  }

  v5 = *(record + 6);
  if (v5 > 0xFFFFFFE3 || v5 + 28 > rgn)
  {
    return -7;
  }

  v8 = parseRegion(v5, record + 28);
  v9 = v8;
  if (v8)
  {
    v7 = [(MFGraphicsDevice *)self->m_gdi paintRgnWithRects:v8];
  }

  else
  {
    v7 = -7;
  }

  return v7;
}

- (int)playInvertRgn:(unsigned int)rgn in_pRecord:(const char *)record
{
  if (rgn < 0x1C)
  {
    return -7;
  }

  v5 = *(record + 6);
  if (v5 > 0xFFFFFFE3 || v5 + 28 > rgn)
  {
    return -7;
  }

  v8 = parseRegion(v5, record + 28);
  v9 = v8;
  if (v8)
  {
    v7 = [(MFGraphicsDevice *)self->m_gdi invertRgnWithRects:v8];
  }

  else
  {
    v7 = -7;
  }

  return v7;
}

- (int)playExcludeClipRect:(unsigned int)rect in_pRecord:(const char *)record
{
  if (rect >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi excludeClipRect:*(record + 2), *(record + 3), *(record + 4), *(record + 5)];
  }

  else
  {
    return -7;
  }
}

- (int)playIntersectClipRect:(unsigned int)rect in_pRecord:(const char *)record
{
  if (rect >= 0x18)
  {
    return [(MFGraphicsDevice *)self->m_gdi intersectClipRect:*(record + 2), *(record + 3), (*(record + 4) - *(record + 2)), (*(record + 5) - *(record + 3))];
  }

  else
  {
    return -7;
  }
}

+ (CGRect)boundsInLogicalUnits:(id)units
{
  unitsCopy = units;
  bytes = [unitsCopy bytes];
  if ([unitsCopy length] > 0x4F)
  {
    v9 = bytes[18] / (bytes[20] * 100.0);
    v10 = bytes[19] / (bytes[21] * 100.0);
    v5 = v9 * bytes[6];
    v6 = v10 * bytes[7];
    v7 = v9 * bytes[8] - v5;
    v8 = v10 * bytes[9] - v6;
  }

  else
  {
    v5 = *MEMORY[0x277CBF3A0];
    v6 = *(MEMORY[0x277CBF3A0] + 8);
    v7 = *(MEMORY[0x277CBF3A0] + 16);
    v8 = *(MEMORY[0x277CBF3A0] + 24);
  }

  v11 = v5;
  v12 = v6;
  v13 = v7;
  v14 = v8;
  result.size.height = v14;
  result.size.width = v13;
  result.origin.y = v12;
  result.origin.x = v11;
  return result;
}

+ (CGRect)boundsInPoints:(id)points
{
  pointsCopy = points;
  bytes = [pointsCopy bytes];
  if ([pointsCopy length] > 0x4F)
  {
    v5 = bytes[6] * 0.0283464569;
    v6 = bytes[7] * 0.0283464569;
    v7 = bytes[8] * 0.0283464569 - v5;
    v8 = bytes[9] * 0.0283464569 - v6;
  }

  else
  {
    v5 = *MEMORY[0x277CBF3A0];
    v6 = *(MEMORY[0x277CBF3A0] + 8);
    v7 = *(MEMORY[0x277CBF3A0] + 16);
    v8 = *(MEMORY[0x277CBF3A0] + 24);
  }

  v9 = v5;
  v10 = v6;
  v11 = v7;
  v12 = v8;
  result.size.height = v12;
  result.size.width = v11;
  result.origin.y = v10;
  result.origin.x = v9;
  return result;
}

@end