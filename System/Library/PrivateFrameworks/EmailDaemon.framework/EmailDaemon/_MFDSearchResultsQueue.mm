...
- (_MFDSearchResultsQueue)initWithKeys:(id)keys resultsProxy:(id)proxy;
...
- (_MFDSearchResultsQueue)initWithKeys:(id)keys resultsProxy:(id)proxy
{
  ...
  if (v8)
  {
    ...
    v11 = [[MFMessageResultsGenerator alloc] initWithKeys:keysCopy downloadIfNecessary:0];
    generator = v8->_generator;
    v8->_generator = v11;

    v13 = v8->_generator;
    v14 = sub_100027C70();
    defaultAttachmentManager = [v14 defaultAttachmentManager];
    [(MFMessageResultsGenerator *)v13 setAttachmentManager:defaultAttachmentManager];

    objc_storeStrong(&v8->_resultsProxy, proxy);
    v16 = v8;
  }
