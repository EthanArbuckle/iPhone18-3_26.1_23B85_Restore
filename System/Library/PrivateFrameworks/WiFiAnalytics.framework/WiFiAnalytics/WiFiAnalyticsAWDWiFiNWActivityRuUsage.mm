...
- (void)copyTo:(id)a3
{
  ...
  {
    [v12 clearRxs];
    v4 = [(WiFiAnalyticsAWDWiFiNWActivityRuUsage *)self rxsCount];
    if (v4)
    {
      v5 = v4;
      for (i = 0; i != v5; ++i)
  ...
  {
    [v12 clearTxs];
    v8 = [(WiFiAnalyticsAWDWiFiNWActivityRuUsage *)self txsCount];
    if (v8)
    {
      v9 = v8;
      for (j = 0; j != v9; ++j)
...
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()] && ((rxs = self->_rxs, !(rxs | v4[1])) || -[NSMutableArray isEqual:](rxs, "isEqual:")))
  {
    txs = self->_txs;
    if (txs | v4[2])
    {
