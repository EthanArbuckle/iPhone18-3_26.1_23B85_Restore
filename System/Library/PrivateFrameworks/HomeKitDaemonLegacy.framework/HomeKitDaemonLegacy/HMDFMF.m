@interface HMDFMF
+ (id)fmfStatusWithCoder:(id)a3;
+ (id)fmfStatusWithDict:(id)a3;
+ (id)fmfStatusWithMessage:(id)a3;
+ (id)fmfStatusWithNumber:(id)a3;
+ (id)fmfStatusWithValue:(unint64_t)a3;
- (BOOL)isEqual:(id)a3;
- (HMDFMF)initWithNumber:(id)a3;
- (NSNumber)number;
- (id)description;
- (void)addToCoder:(id)a3;
- (void)addToPayload:(id)a3;
@end

@implementation HMDFMF

- (id)description
{
  v2 = MEMORY[0x277CCACA8];
  v3 = HMDFMFStatusAsString([(HMDFMF *)self value]);
  v4 = [v2 stringWithFormat:@"%@", v3];

  return v4;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if (self == v4)
  {
    v8 = 1;
  }

  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v5 = v4;
    }

    else
    {
      v5 = 0;
    }

    v6 = v5;
    if (v6)
    {
      v7 = [(HMDFMF *)self value];
      v8 = v7 == [(HMDFMF *)v6 value];
    }

    else
    {
      v8 = 0;
    }
  }

  return v8;
}

- (NSNumber)number
{
  v2 = MEMORY[0x277CCABB0];
  v3 = [(HMDFMF *)self value];

  return [v2 numberWithUnsignedInteger:v3];
}

- (void)addToPayload:(id)a3
{
  v4 = a3;
  v5 = [(HMDFMF *)self number];
  [v4 setObject:v5 forKeyedSubscript:@"FMFStatus"];
}

- (void)addToCoder:(id)a3
{
  v4 = a3;
  v5 = [(HMDFMF *)self number];
  [v4 encodeObject:v5 forKey:@"FMFStatus"];
}

- (HMDFMF)initWithNumber:(id)a3
{
  v4 = a3;
  if (v4)
  {
    v8.receiver = self;
    v8.super_class = HMDFMF;
    v5 = [(HMDFMF *)&v8 init];
    if (v5)
    {
      v5->_value = [v4 unsignedIntegerValue];
    }

    self = v5;
    v6 = self;
  }

  else
  {
    v6 = 0;
  }

  return v6;
}

+ (id)fmfStatusWithCoder:(id)a3
{
  v3 = a3;
  v4 = [v3 decodeObjectOfClass:objc_opt_class() forKey:@"FMFStatus"];

  v5 = [HMDFMF fmfStatusWithNumber:v4];

  return v5;
}

+ (id)fmfStatusWithDict:(id)a3
{
  v3 = [a3 hmf_numberForKey:@"FMFStatus"];
  v4 = [HMDFMF fmfStatusWithNumber:v3];

  return v4;
}

+ (id)fmfStatusWithMessage:(id)a3
{
  v3 = [a3 numberForKey:@"FMFStatus"];
  v4 = [HMDFMF fmfStatusWithNumber:v3];

  return v4;
}

+ (id)fmfStatusWithNumber:(id)a3
{
  v3 = a3;
  v4 = [[HMDFMF alloc] initWithNumber:v3];

  return v4;
}

+ (id)fmfStatusWithValue:(unint64_t)a3
{
  v4 = [HMDFMF alloc];
  v5 = [MEMORY[0x277CCABB0] numberWithUnsignedInteger:a3];
  v6 = [(HMDFMF *)v4 initWithNumber:v5];

  return v6;
}

@end