...
+ (id)_unzipWatchfaceFromURL:(id)l;
- (void)removeFileAtPath:(id)path withSandboxExtension:(char *)extension completionBlock:(id)block;
- (void)unzipWatchfaceFromURL:(id)l withSandboxExtension:(char *)extension writeSandboxExtension:(char *)sandboxExtension completionBlock:(id)block;
@end
...
LABEL_13:
  ...
  v12 = [objc_opt_class() _unzipWatchfaceFromURL:lCopy];
  if (!v12)
  {
    v16 = MEMORY[0x277CCACA8];
    absoluteURL = [lCopy absoluteURL];
    v13 = [v16 stringWithFormat:@"GreenfieldService failed to extract from url: %@", absoluteURL];
...
+ (id)_unzipWatchfaceFromURL:(id)l
{
  ...
  v6 = v5;
  v14 = v6;
  [v4 coordinateReadingItemAtURL:lCopy options:1 error:&v16 byAccessor:v13];
  v7 = v16;
...
- (void)removeFileAtPath:(id)path withSandboxExtension:(char *)extension completionBlock:(id)block
{
  ...
  if (v9 == -1)
  {
    v12 = [MEMORY[0x277CCACA8] stringWithFormat:@"NTKGreenfieldService remove files at path failed to consume sandbox extension:%s, handle is:%lld.", extension, -1];
    v13 = [MEMORY[0x277CCA9B8] greenfield_decodeWatchFaceErrorWithCode:2 message:v12];
  ...
  else
  {
    ...
    v15 = 0;
    [defaultManager removeItemAtPath:pathCopy error:&v15];
    v12 = v15;

    if (v12)
    {
      [MEMORY[0x277CCACA8] stringWithFormat:@"NTKGreenfieldService remove files at path failed to remove item at path:%@.", pathCopy];
    }
