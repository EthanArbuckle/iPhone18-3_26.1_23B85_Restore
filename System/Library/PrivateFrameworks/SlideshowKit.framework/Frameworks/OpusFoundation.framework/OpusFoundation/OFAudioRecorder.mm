...
- (BOOL)recordsAudio;
- (OFAudioRecorder)initWithSession:(id)session outputFileURL:(id)l;
...
- (OFAudioRecorder)initWithSession:(id)session outputFileURL:(id)l
{
  ...
  if (v6)
  {
    ...
    if ([session canAddOutput:v7])
    {
      [session addOutput:v7];
    }

    [(OFAudioRecorder *)v6 setSession:session];
    [(OFAudioRecorder *)v6 setMovieFileOutput:v7];

    [(OFAudioRecorder *)v6 setOutputFileURL:l];
    v6->_delegate = 0;
  }
...
- (void)captureOutput:(id)output didStartRecordingToOutputFileAtURL:(id)l fromConnections:(id)connections
{
  if ([(OFAudioRecorder *)self delegate:output])
  {
    [(OFAudioRecorder *)self delegate];
    if (objc_opt_respondsToSelector())
    {
      delegate = [(OFAudioRecorder *)self delegate];

      [(OFAudioRecorderDelegate *)delegate recorderRecordingDidBegin:self];
    }
  }
}
...
- (void)captureOutput:(id)output didFinishRecordingToOutputFileAtURL:(id)l fromConnections:(id)connections error:(id)error
{
  if ([(OFAudioRecorder *)self delegate:output])
  {
    ...
    if (objc_opt_respondsToSelector())
    {
      ...
      [(OFAudioRecorderDelegate *)delegate recorder:self recordingDidFinishToOutputFileURL:l error:error];
    }
  }
}
...
- (id)_connectionWithMediaType:(id)type
{
  ...
  connections = [(AVCaptureMovieFileOutput *)self->_movieFileOutput connections];
  v20 = 0u;
  v21 = 0u;
  v22 = 0u;
  v23 = 0u;
  v5 = [connections countByEnumeratingWithState:&v20 objects:v25 count:16];
  if (!v5)
  {
    return 0;
  }
  ...
  while (2)
  {
    for (i = 0; i != v6; ++i)
    {
      ...
      v11 = [inputPorts countByEnumeratingWithState:&v16 objects:v24 count:16];
      if (v11)
      {
        v12 = v11;
        v13 = *v17;
LABEL_8:
        ...
        while (1)
        {
          ...
          if (v12 == ++v14)
          {
            v12 = [inputPorts countByEnumeratingWithState:&v16 objects:v24 count:16];
            if (v12)
            {
              goto LABEL_8;
            }
    ...
    v6 = [connections countByEnumeratingWithState:&v20 objects:v25 count:16];
    v9 = 0;
    if (v6)
    {
      continue;
