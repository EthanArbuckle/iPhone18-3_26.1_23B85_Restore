...
+ (id)loadWordsForLanguage:(unint64_t)language;
+ (id)sentencePunctuationForLanguage:(unint64_t)language endOfParagraph:(BOOL)paragraph;
+ (id)wordsForLanguage:(unint64_t)language;
- (ICRandomTextGenerator)initWithRandomNumberGenerator:(id)generator;
- (id)generateMinSentences:(unint64_t)sentences maxSentences:(unint64_t)maxSentences minWords:(unint64_t)words maxWords:(unint64_t)maxWords;
- (id)generateMinWords:(unint64_t)words maxWords:(unint64_t)maxWords;
- (id)generateWords:(unint64_t)words minLength:(unint64_t)length;
- (id)paragraph;
@end
...
- (ICRandomTextGenerator)initWithRandomNumberGenerator:(id)generator
{
  ...
  if (v5)
  {
    [(ICRandomTextGenerator *)v5 setRandomNumberGenerator:generatorCopy];
  }
...
+ (id)loadWordsForLanguage:(unint64_t)language
{
  ...
  v5 = [loadWordsForLanguage__wordFileNames objectAtIndexedSubscript:language];
  ...
  if (v7)
  {
    if ((language & 0xFFFFFFFFFFFFFFFELL) == 2)
    {
      ...
      v17 = &unk_278195110;
      v18 = v9;
      whitespaceAndNewlineCharacterSet = v9;
      [v7 enumerateSubstringsInRange:0 options:v10 usingBlock:{2, &v14}];
      v12 = [whitespaceAndNewlineCharacterSet copy:v14];
    }

    else
    {
      whitespaceAndNewlineCharacterSet = [MEMORY[0x277CCA900] whitespaceAndNewlineCharacterSet];
      v12 = [v7 componentsSeparatedByCharactersInSet:whitespaceAndNewlineCharacterSet];
    }
  }

  else
  {
    whitespaceAndNewlineCharacterSet = os_log_create("com.apple.notes", "Internal");
    if (os_log_type_enabled(whitespaceAndNewlineCharacterSet, OS_LOG_TYPE_ERROR))
    {
      [(ICRandomTextGenerator *)v6 loadWordsForLanguage:v8, whitespaceAndNewlineCharacterSet];
    }
...
+ (id)wordsForLanguage:(unint64_t)language
{
  ...
  v6 = [MEMORY[0x277CCABB0] numberWithUnsignedInteger:language];
  v7 = [v5 objectForKey:v6];

  if (!v7)
  {
    v7 = [self loadWordsForLanguage:language];
    if (v7)
    {
      v8 = wordsForLanguage__wordsByLanguage;
      v9 = [MEMORY[0x277CCABB0] numberWithUnsignedInteger:language];
...
- (id)generateWords:(unint64_t)words minLength:(unint64_t)length
{
  ...
  v8 = [v7 count] - 1;
  randomNumberGenerator = [(ICRandomTextGenerator *)self randomNumberGenerator];
  v10 = [randomNumberGenerator randomIndexMin:0 max:v8];
  ...
  else
  {
    ...
    localizedCapitalizedString = [v12 localizedCapitalizedString];
    v14 = [v11 initWithString:localizedCapitalizedString];
    ...
    if (words != 1)
    {
      ...
      do
      {
        if (length && [v14 length] >= length)
        {
          break;
        }

        randomNumberGenerator2 = [(ICRandomTextGenerator *)self randomNumberGenerator];
        v20 = [randomNumberGenerator2 randomIndexMin:0 max:v8];
...
{
  randomNumberGenerator = [(ICRandomTextGenerator *)self randomNumberGenerator];
  v8 = -[ICRandomTextGenerator generateWords:minLength:](self, "generateWords:minLength:", [randomNumberGenerator randomIndexMin:words max:maxWords], 0);
...
- (id)generateMinSentences:(unint64_t)sentences maxSentences:(unint64_t)maxSentences minWords:(unint64_t)words maxWords:(unint64_t)maxWords
{
  ...
  randomNumberGenerator = [(ICRandomTextGenerator *)self randomNumberGenerator];
  v13 = [randomNumberGenerator randomIndexMin:sentences max:maxSentences];

  if (v13)
  {
    for (i = 0; i != v13; ++i)
    {
      v15 = [(ICRandomTextGenerator *)self generateMinWords:words maxWords:maxWords];
