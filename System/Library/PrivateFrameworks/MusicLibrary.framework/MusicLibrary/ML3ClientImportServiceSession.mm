...
- (BOOL)begin;
- (BOOL)finish;
- (ML3ClientImportServiceSession)initWithLibrary:(id)library connection:(id)connection configuration:(id)configuration;
- (void)dealloc;
@end
...
{
  importSession = self->_importSession;
  operationCount = [(ML3ClientImportSessionConfiguration *)self->_configuration operationCount];

  return ML3ImportSession::begin(importSession, operationCount, 0, 0);
}
...
- (ML3ClientImportServiceSession)initWithLibrary:(id)library connection:(id)connection configuration:(id)configuration
{
  ...
  if (v12)
  {
    ...
    objc_storeStrong(&v13->_databaseConnection, connection);
