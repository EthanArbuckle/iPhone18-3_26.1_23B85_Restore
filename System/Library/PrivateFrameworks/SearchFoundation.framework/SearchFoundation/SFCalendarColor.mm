@interface SFCalendarColor
- (BOOL)isEqual:(id)a3;
- (NSData)jsonData;
- (NSDictionary)dictionaryRepresentation;
- (SFCalendarColor)initWithCoder:(id)a3;
- (SFCalendarColor)initWithProtobuf:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
@end

@implementation SFCalendarColor

- (unint64_t)hash
{
  v7.receiver = self;
  v7.super_class = SFCalendarColor;
  v3 = [(SFColor *)&v7 hash];
  v4 = [(SFCalendarColor *)self eventIdentifier];
  v5 = [v4 hash];

  return v5 ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if (self == v4)
  {
    v11 = 1;
  }

  else if ([(SFCalendarColor *)v4 isMemberOfClass:objc_opt_class()]&& (v13.receiver = self, v13.super_class = SFCalendarColor, [(SFColor *)&v13 isEqual:v4]))
  {
    v5 = v4;
    v6 = [(SFCalendarColor *)self eventIdentifier];
    v7 = [(SFCalendarColor *)v5 eventIdentifier];
    if ((v6 != 0) == (v7 == 0))
    {
      v11 = 0;
    }

    else
    {
      v8 = [(SFCalendarColor *)self eventIdentifier];
      if (v8)
      {
        v9 = [(SFCalendarColor *)self eventIdentifier];
        v10 = [(SFCalendarColor *)v5 eventIdentifier];
        v11 = [v9 isEqual:v10];
      }

      else
      {
        v11 = 1;
      }
    }
  }

  else
  {
    v11 = 0;
  }

  return v11;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v8.receiver = self;
  v8.super_class = SFCalendarColor;
  v4 = [(SFColor *)&v8 copyWithZone:a3];
  v5 = [(SFCalendarColor *)self eventIdentifier];
  v6 = [v5 copy];
  [v4 setEventIdentifier:v6];

  return v4;
}

- (NSData)jsonData
{
  v2 = [[_SFPBCalendarColor alloc] initWithFacade:self];
  v3 = [(_SFPBCalendarColor *)v2 jsonData];

  return v3;
}

- (NSDictionary)dictionaryRepresentation
{
  v2 = [[_SFPBCalendarColor alloc] initWithFacade:self];
  v3 = [(_SFPBCalendarColor *)v2 dictionaryRepresentation];

  return v3;
}

- (void)encodeWithCoder:(id)a3
{
  v3.receiver = self;
  v3.super_class = SFCalendarColor;
  [(SFColor *)&v3 encodeWithCoder:a3];
}

- (SFCalendarColor)initWithCoder:(id)a3
{
  v4 = a3;
  v5 = [(SFCalendarColor *)self init];
  v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"_backingStore"];

  v7 = [[_SFPBColor alloc] initWithData:v6];
  v8 = [[SFColor alloc] initWithProtobuf:v7];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v9 = [(SFColor *)v8 eventIdentifier];
    [(SFCalendarColor *)v5 setEventIdentifier:v9];

    [(SFColor *)v8 redComponent];
    [(SFColor *)v5 setRedComponent:?];
    [(SFColor *)v8 greenComponent];
    [(SFColor *)v5 setGreenComponent:?];
    [(SFColor *)v8 blueComponent];
    [(SFColor *)v5 setBlueComponent:?];
    [(SFColor *)v8 alphaComponent];
    [(SFColor *)v5 setAlphaComponent:?];
    [(SFColor *)v5 setColorTintStyle:[(SFColor *)v8 colorTintStyle]];
    v10 = [(SFColor *)v8 darkModeColor];
    [(SFColor *)v5 setDarkModeColor:v10];
  }

  return v5;
}

- (SFCalendarColor)initWithProtobuf:(id)a3
{
  v4 = a3;
  v10.receiver = self;
  v10.super_class = SFCalendarColor;
  v5 = [(SFCalendarColor *)&v10 init];
  if (v5)
  {
    v6 = [v4 eventIdentifier];

    if (v6)
    {
      v7 = [v4 eventIdentifier];
      [(SFCalendarColor *)v5 setEventIdentifier:v7];
    }

    v8 = v5;
  }

  return v5;
}

@end