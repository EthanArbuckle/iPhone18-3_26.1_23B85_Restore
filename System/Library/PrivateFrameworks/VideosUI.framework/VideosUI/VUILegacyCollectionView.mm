...
- (VUILegacyCollectionView)initWithFrame:(CGRect)frame collectionViewLayout:(id)layout;
...
- (VUILegacyCollectionView)initWithFrame:(CGRect)frame collectionViewLayout:(id)layout
{
  ...
  v4 = [(VUILegacyCollectionView *)&v8 initWithFrame:layout collectionViewLayout:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  if (v4)
  {
...
- (BOOL)gestureRecognizerShouldBegin:(id)begin
{
  ...
  if (self->_longPressRecognizer == beginCopy)
  {
    if ((*&self->_delegateFlags & 2) != 0)
    {
      ...
      v13 = 0x2020000000;
      v14 = 0;
      delegate = [(VUILegacyCollectionView *)self delegate];
  ...
  else
  {
    ...
    v5 = [(VUILegacyCollectionView *)&v9 gestureRecognizerShouldBegin:beginCopy];
  }
...
- (void)didUpdateFocusInContext:(id)context withAnimationCoordinator:(id)coordinator
{
  ...
  [(VUILegacyCollectionView *)&v22 didUpdateFocusInContext:contextCopy withAnimationCoordinator:coordinator];
  ...
  if ([nextFocusedView isDescendantOfView:self])
  {
    ...
    v18 = 0u;
    v19 = 0u;
    indexPathsForVisibleItems = [(VUILegacyCollectionView *)self indexPathsForVisibleItems];
...
- (void)setScrollEnabled:(BOOL)enabled
{
  ...
  [(VUILegacyCollectionView *)&v3 setScrollEnabled:enabled];
}
...
- (void)_notifyDidScroll
{
  ...
  [defaultCenter postNotificationName:*MEMORY[0x1E69D59C0] object:self];
}
...
- (void)setDelegate:(id)delegate
{
  ...
  [(VUILegacyCollectionView *)&v8 setDelegate:delegateCopy];
...
- (CGRect)_visibleBounds
{
  window = [(VUILegacyCollectionView *)self window];
...
- (void)_getGradientMaskBounds:(CGRect *)bounds startInsets:(UIEdgeInsets *)insets endInsets:(UIEdgeInsets *)endInsets intensities:(UIEdgeInsets *)intensities
{
  ...
  [(VUILegacyCollectionView *)&v12 _getGradientMaskBounds:bounds startInsets:insets endInsets:endInsets intensities:intensities];
  if (bounds)
  {
    v8 = *&self->_gradientBoundsInsets.top;
    v9 = vaddq_f64(vextq_s8(v8, v8, 8uLL), bounds->origin);
    v10 = vaddq_f64(*&self->_gradientBoundsInsets.bottom, v8);
    v11 = vsubq_f64(bounds->size, vextq_s8(v10, v10, 8uLL));
    bounds->origin = v9;
    bounds->size = v11;
  }
}
