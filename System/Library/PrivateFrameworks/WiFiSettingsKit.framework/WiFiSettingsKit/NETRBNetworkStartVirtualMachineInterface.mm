...
void ___NETRBNetworkStartVirtualMachineInterface_block_invoke(uint64_t a1)
{
  ...
  else
  {
    ...
    xpc_dictionary_set_uint64(v3, netrbXPCKey[0], 0x3F6uLL);
    xpc_dictionary_set_string(v3, netrbXPCClientID[0], (_NETRBNetworkClient + 32));
    xpc_dictionary_set_BOOL(v3, netrbXPCAllocateMACAddress[0], *(a1 + 80));
    xpc_dictionary_set_value(v3, netrbXPCNetworkObject[0], *(a1 + 56));
    if (*(a1 + 81) == 1)
    {
      xpc_dictionary_set_BOOL(v3, netrbXPCEnableTSO[0], 1);
    }

    if (*(a1 + 82) == 1)
    {
      xpc_dictionary_set_BOOL(v3, netrbXPCEnableVirtIOHeader[0], 1);
    }

    if (*(a1 + 83) == 1)
    {
      xpc_dictionary_set_BOOL(v3, netrbXPCEnableMACNAT[0], 1);
    }

    if (*(a1 + 84) == 1)
    {
      xpc_dictionary_set_BOOL(v3, netrbXPCEnableIsolation[0], 1);
    }

    v4 = v2 + 16;
    if (*(a1 + 85) == 1)
    {
      xpc_dictionary_set_BOOL(v3, netrbXPCEnableChecksumOffload[0], 1);
    }

    if (xpc_dictionary_get_value(*(a1 + 64), netrbClientRelayAuditToken))
    {
      v5 = netrbXPCRelayAuditToken[0];
...
LABEL_8:
  ...
  *src = *uuid;
  ...
  v11 = xpc_dictionary_get_uuid(xdict, netrbXPCMACUUID[0]);
  if (v11)
  {
    *(a1[7] + 27) = *v11;
    *(a1[7] + 26) = 1;
  ...
  *(a1[7] + 16) = xpc_dictionary_dup_fd(xdict, netrbXPCInterfaceSocket[0]);
  result = xpc_dictionary_get_uint64(xdict, netrbXPCInterfaceId[0]);
  v12 = a1[7];
  v13 = a1[8];
  *(v12 + 48) = result;
  *(v12 + 56) = v13;
