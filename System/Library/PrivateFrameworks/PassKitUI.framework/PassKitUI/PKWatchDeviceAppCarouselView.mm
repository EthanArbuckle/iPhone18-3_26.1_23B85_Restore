...
- (PKWatchDeviceAppCarouselView)initWithFrame:(CGRect)frame;
...
- (PKWatchDeviceAppCarouselView)initWithFrame:(CGRect)frame
{
  ...
  if (v7)
  {
    height = [[PKWatchHeroImageView alloc] initWithFrame:x, y, width, height];
    watchDeviceImageView = v7->_watchDeviceImageView;
    v7->_watchDeviceImageView = height;
    ...
    _carouselViewImage = [(PKWatchDeviceAppCarouselView *)v7 _carouselViewImage];
    v12 = [v10 initWithImage:_carouselViewImage];
...
- (CGSize)sizeThatFits:(CGSize)fits
{
  [(PKWatchHeroImageView *)self->_watchDeviceImageView sizeThatFits:fits.width, fits.height];
  result.height = v4;
  result.width = v3;
  return result;
}
