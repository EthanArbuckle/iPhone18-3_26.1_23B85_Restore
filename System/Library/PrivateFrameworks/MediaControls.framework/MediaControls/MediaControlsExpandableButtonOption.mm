...
- (id)description
{
  ...
  v4 = objc_opt_class();
  identifier = [(MediaControlsExpandableButtonOption *)self identifier];
  v6 = [v3 stringWithFormat:@"<%@: %p identifer: %@ | asset: %@ | title: %@ | state: %@>", v4, self, identifier, self->_asset, self->_title, self->_selectedState];
