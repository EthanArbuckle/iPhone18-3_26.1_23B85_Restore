...
- (IDSBlockRetryHandler)initWithQueue:(id)queue backoffProvider:(id)provider block:(id)block;
- (id)_isFinishedBlock;
- (void)_deferToQueue:(id)queue after:(double)after block:(id)block;
- (void)_run;
- (void)dealloc;
- (void)start;
- (void)stop;
...
- (void)_run
{
  attempts = [(IDSBlockRetryHandler *)self attempts];
  ...
  if (attempts)
  {
    ...
    backoffProvider = [(IDSBlockRetryHandler *)self backoffProvider];
    v7 = backoffProvider[2](backoffProvider, [(IDSBlockRetryHandler *)self attempts]);
    queue = [(IDSBlockRetryHandler *)self queue];
    [(IDSBlockRetryHandler *)self _deferToQueue:queue after:v5 block:v7];
  }
