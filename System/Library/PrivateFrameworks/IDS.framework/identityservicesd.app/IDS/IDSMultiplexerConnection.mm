...
- (void)_logIncomingPacket:(int64_t)packet;
- (void)_logOutgoingPacket:(int64_t)packet;
- (void)invalidate;
...
- (NSString)description
{
  ...
  v7 = [NSString stringWithFormat:@"%p MUXConnection localID %u remoteID %u localEndpoint %@ remoteEndpoint %@", self, localPort, remotePort, localEndpoint, remoteEndpoint];
...
LABEL_13:
  ...
  if ([(IDSMultiplexerConnection *)self isQUIC])
  {
    ...
    udp6checksum(0, 0, buffer->var0, var2);
  }

  os_unfair_lock_unlock(&self->_lock);
  if (sub_100510178(self->_osChannel, self->_txRing, buffer, self->_flowIDBytes))
  {
    v10 = buffer->var2;
  ...
  else
  {
    ...
    if (v8 - self->_lastIncomingReportTime >= 5.0)
    {
      ...
      if (os_log_type_enabled(v11, OS_LOG_TYPE_DEFAULT))
      {
        ...
        v16 = v10;
        v17 = 2048;
        packetCopy = incomingPacketCount;
        v19 = 2048;
        v20 = incomingBytes;
  ...
  else
  {
    ...
    if (v8 - self->_lastOutgoingReportTime >= 5.0)
    {
      ...
      if (os_log_type_enabled(v11, OS_LOG_TYPE_DEFAULT))
      {
        ...
        v16 = v10;
        v17 = 2048;
        packetCopy = outgoingPacketCount;
        v19 = 2048;
        v20 = outgoingBytes;
...
LABEL_13:
  ...
  clientUUID = [(IDSMultiplexerConnection *)self clientUUID];
  [clientUUID getUUIDBytes:self->_flowIDBytes];

  transport = [(IDSMultiplexerConnection *)self transport];
  LODWORD(clientUUID) = [transport packetizationHeadroom];
...
LABEL_12:
  ...
  if (buffer->var2 > 3)
  {
    v10 = __rev16(*buffer->var0);
    v11 = __rev16(*(buffer->var0 + 1));
    ...
    if (self->_localPort == v11)
    {
      ...
      if (!v16 || v16 == v10)
      {
        [(IDSMultiplexerConnection *)self writePacketBuffer:buffer];
        return 1;
      }
    }
