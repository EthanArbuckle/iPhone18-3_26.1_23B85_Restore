@interface HMDAudioStreamInterfaceDataSource
+ (id)logCategory;
- (BOOL)isMuted;
- (void)createPHASEEngineAndControllerForStreamToken:(int64_t)a3 completion:(id)a4;
@end

@implementation HMDAudioStreamInterfaceDataSource

+ (id)logCategory
{
  if (logCategory__hmf_once_t5 != -1)
  {
    dispatch_once(&logCategory__hmf_once_t5, &__block_literal_global_3679);
  }

  v3 = logCategory__hmf_once_v6;

  return v3;
}

void __48__HMDAudioStreamInterfaceDataSource_logCategory__block_invoke()
{
  v0 = *MEMORY[0x277D0F1A8];
  v1 = HMFCreateOSLogHandle();
  v2 = logCategory__hmf_once_v6;
  logCategory__hmf_once_v6 = v1;
}

- (void)createPHASEEngineAndControllerForStreamToken:(int64_t)a3 completion:(id)a4
{
  v14 = *MEMORY[0x277D85DE8];
  v5 = a4;
  v6 = objc_autoreleasePoolPush();
  v7 = self;
  v8 = HMFGetOSLogHandle();
  if (os_log_type_enabled(v8, OS_LOG_TYPE_ERROR))
  {
    v9 = HMFGetLogIdentifier();
    v12 = 138543362;
    v13 = v9;
    _os_log_impl(&dword_229538000, v8, OS_LOG_TYPE_ERROR, "%{public}@Cannot create PHASE engine and controller on unsupported platform", &v12, 0xCu);
  }

  objc_autoreleasePoolPop(v6);
  v10 = [MEMORY[0x277CCA9B8] hmErrorWithCode:48];
  (*(v5 + 2))(v5, 0, 0, v10);

  v11 = *MEMORY[0x277D85DE8];
}

- (BOOL)isMuted
{
  if (_os_feature_enabled_impl() && _os_feature_enabled_impl())
  {
    v2 = MEMORY[0x277CE56E0];

    return [v2 isMicrophoneMuted];
  }

  else
  {
    v4 = [MEMORY[0x277D26E58] sharedAVSystemController];
    v5 = [v4 attributeForKey:*MEMORY[0x277D26DF0]];
    v6 = [v5 BOOLValue];

    return v6;
  }
}

@end