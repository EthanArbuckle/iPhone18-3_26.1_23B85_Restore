...
- (BMIndexRowEnumerator)initWithDatabase:(sqlite3 *)database statement:(sqlite3_stmt *)statement index:(id)index;
- (id)allRows;
- (id)next;
- (id)nextBookmark;
- (void)dealloc;
...
- (BMIndexRowEnumerator)initWithDatabase:(sqlite3 *)database statement:(sqlite3_stmt *)statement index:(id)index
{
  ...
  if (v10)
  {
    v10->_db = database;
    v10->_stmt = statement;
    objc_storeStrong(&v10->_index, index);
  }
      ...
      if (v6 >= 1)
      {
        for (i = 0; v6 != i; ++i)
        {
          ...
          if (v9 <= 2)
          {
            if (v9 == 1)
            {
              null = [MEMORY[0x1E696AD98] numberWithLongLong:{sqlite3_column_int64(self->_stmt, i)}];
            }
...
LABEL_21:
                ...
                [currentHandler handleFailureInMethod:a2 object:self file:@"BMIndexRow.m" lineNumber:96 description:{@"Invalid sqlite type: %d", v10}];

                v13 = 0;
                goto LABEL_19;
              }

              null = [MEMORY[0x1E696AD98] numberWithDouble:{sqlite3_column_double(self->_stmt, i)}];
            }
          }
          ...
          else
          {
            ...
            {
              case 3:
                null = [objc_alloc(MEMORY[0x1E696AEC0]) initWithUTF8String:{sqlite3_column_text(self->_stmt, i)}];
                break;
              case 4:
                v12 = sqlite3_column_blob(self->_stmt, i);
                null = [objc_alloc(MEMORY[0x1E695DEF0]) initWithBytes:v12 length:{sqlite3_column_bytes(self->_stmt, i)}];
                break;
              case 5:
...
LABEL_19:
    ...
    else
    {
      ...
      name = [(BMIndex *)self->_index name];
      v7 = [v26 stringWithFormat:@"Error stepping index search result on %@", name];
...
{
  v3 = objc_opt_new();
  next = [(BMIndexRowEnumerator *)self next];
  if (next)
  {
    next2 = next;
    do
    {
      ...
      next2 = [(BMIndexRowEnumerator *)self next];
    }
...
{
  next = [(BMIndexRowEnumerator *)self next];
  storeBookmark = [next storeBookmark];
