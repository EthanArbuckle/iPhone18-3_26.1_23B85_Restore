...
- (void)_callDelegate:(id)delegate;
- (void)_cleanupCurrentHandleNotifyDelegate:(BOOL)delegate;
- (void)activate;
- (void)dealloc;
- (void)invalidate;
- (void)preheat;
...
- (SSRemoteAlertHandleProvider)init
{
  ...
  if (v2)
  {
    screenshotServicesAlertHandle = [(SSRemoteAlertHandleProvider *)v2 screenshotServicesAlertHandle];
    handle = v3->_handle;
    v3->_handle = screenshotServicesAlertHandle;
...
- (void)prepare
{
  ...
  screenshotServicesAlertHandle = [(SSRemoteAlertHandleProvider *)self screenshotServicesAlertHandle];
  v6 = self->_handle;
  self->_handle = screenshotServicesAlertHandle;
}
...
- (void)_cleanupCurrentHandleNotifyDelegate:(BOOL)delegate
{
  ...
  if (v7)
  {
    ...
    if (delegateCopy)
    {
      delegate = [(SSRemoteAlertHandleProvider *)self delegate];
      [delegate remoteAlertHandleProvider:self didInvalidateWithError:0];
    }
  }
}
...
- (void)remoteAlertHandle:(id)handle didInvalidateWithError:(id)error
{
  ...
  {
    [(SSRemoteAlertHandleProvider *)errorCopy remoteAlertHandle:v8 didInvalidateWithError:?];
  }

  handle = self->_handle;
  if (handle == handleCopy)
  {
...
- (id)screenshotServicesAlertHandle
{
  ...
  _screenshotServicesServiceAlertDefinition = [(SSRemoteAlertHandleProvider *)self _screenshotServicesServiceAlertDefinition];
  v5 = [v3 lookupHandlesForDefinition:_screenshotServicesServiceAlertDefinition creatingIfNone:1];
  firstObject = [v5 firstObject];

  [firstObject registerObserver:self];
