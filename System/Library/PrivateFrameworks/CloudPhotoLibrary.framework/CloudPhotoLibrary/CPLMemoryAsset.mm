...
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((assetIdentifier = self->_assetIdentifier, !(assetIdentifier | equalCopy[2])) || -[NSString isEqual:](assetIdentifier, "isEqual:")) && ((assetFlag = self->_assetFlag, !(assetFlag | equalCopy[1])) || -[CPLMemoryAssetFlag isEqual:](assetFlag, "isEqual:")) && ((assetMovieData = self->_assetMovieData, !(assetMovieData | equalCopy[3])) || -[NSData isEqual:](assetMovieData, "isEqual:")))
  {
...
- (id)copyWithZone:(_NSZone *)zone
{
  ...
  v8 = [(CPLMemoryAssetFlag *)self->_assetFlag copyWithZone:zone];
  v9 = v5[1];
  v5[1] = v8;

  v10 = [(NSData *)self->_assetMovieData copyWithZone:zone];
  v11 = v5[3];
  v5[3] = v10;
...
- (id)dictionaryRepresentation
{
  ...
  if (assetFlag)
  {
    dictionaryRepresentation = [(CPLMemoryAssetFlag *)assetFlag dictionaryRepresentation];
...
- (id)description
{
  ...
  dictionaryRepresentation = [(CPLMemoryAsset *)self dictionaryRepresentation];
