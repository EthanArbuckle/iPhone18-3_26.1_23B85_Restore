...
- (id)description
{
  ...
  dictionaryRepresentation = [(ChannelActivityParticipantPayload *)self dictionaryRepresentation];
...
- (void)copyTo:(id)to
{
  ...
  {
    v5 = toCopy;
    [toCopy setEncryptedParticipantPayload:?];
    toCopy = v5;
  }

  if (*&self->_has)
  {
    *(toCopy + 1) = self->_serverUpdateTimestampSeconds;
    *(toCopy + 24) |= 1u;
  }
}
...
- (id)copyWithZone:(_NSZone *)zone
{
  ...
  v6 = [(NSData *)self->_encryptedParticipantPayload copyWithZone:zone];
  v7 = *(v5 + 16);
  *(v5 + 16) = v6;
...
- (BOOL)isEqual:(id)equal
{
  ...
  encryptedParticipantPayload = self->_encryptedParticipantPayload;
  if (encryptedParticipantPayload | *(equalCopy + 2))
  {
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 24) & 1) != 0 && self->_serverUpdateTimestampSeconds == *(equalCopy + 1))
    {
      v6 = 1;
      goto LABEL_9;
    }
...
- (void)mergeFrom:(id)from
{
  ...
  if (fromCopy[3])
  {
    self->_serverUpdateTimestampSeconds = fromCopy[1];
    *&self->_has |= 1u;
  }
}
