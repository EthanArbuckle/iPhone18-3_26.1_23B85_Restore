@interface SharedOwnershipAuth
- (BOOL)isEqual:(id)equal;
- (id)copyWithZone:(_NSZone *)zone;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)to;
- (void)mergeFrom:(id)from;
- (void)writeTo:(id)to;
@end

@implementation SharedOwnershipAuth

- (id)description
{
  v3 = MEMORY[0x277CCACA8];
  v8.receiver = self;
  v8.super_class = SharedOwnershipAuth;
  v4 = [(SharedOwnershipAuth *)&v8 description];
  dictionaryRepresentation = [(SharedOwnershipAuth *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, dictionaryRepresentation];

  return v6;
}

- (id)dictionaryRepresentation
{
  dictionary = [MEMORY[0x277CBEB38] dictionary];
  v4 = dictionary;
  simpleJwt = self->_simpleJwt;
  if (simpleJwt)
  {
    [dictionary setObject:simpleJwt forKey:@"simple_jwt"];
  }

  channelPublicKey = self->_channelPublicKey;
  if (channelPublicKey)
  {
    [v4 setObject:channelPublicKey forKey:@"channel_public_key"];
  }

  nonce = self->_nonce;
  if (nonce)
  {
    [v4 setObject:nonce forKey:@"nonce"];
  }

  signNonce = self->_signNonce;
  if (signNonce)
  {
    [v4 setObject:signNonce forKey:@"sign_nonce"];
  }

  serverEncryptionKey = self->_serverEncryptionKey;
  if (serverEncryptionKey)
  {
    [v4 setObject:serverEncryptionKey forKey:@"server_encryption_key"];
  }

  return v4;
}

- (void)writeTo:(id)to
{
  toCopy = to;
  v5 = toCopy;
  if (self->_simpleJwt)
  {
    PBDataWriterWriteStringField();
    toCopy = v5;
  }

  if (self->_channelPublicKey)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }

  if (self->_nonce)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }

  if (self->_signNonce)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }

  if (self->_serverEncryptionKey)
  {
    PBDataWriterWriteDataField();
    toCopy = v5;
  }
}

- (void)copyTo:(id)to
{
  toCopy = to;
  v5 = toCopy;
  if (self->_simpleJwt)
  {
    [toCopy setSimpleJwt:?];
    toCopy = v5;
  }

  if (self->_channelPublicKey)
  {
    [v5 setChannelPublicKey:?];
    toCopy = v5;
  }

  if (self->_nonce)
  {
    [v5 setNonce:?];
    toCopy = v5;
  }

  if (self->_signNonce)
  {
    [v5 setSignNonce:?];
    toCopy = v5;
  }

  if (self->_serverEncryptionKey)
  {
    [v5 setServerEncryptionKey:?];
    toCopy = v5;
  }
}

- (id)copyWithZone:(_NSZone *)zone
{
  v5 = [objc_msgSend(objc_opt_class() allocWithZone:{zone), "init"}];
  v6 = [(NSString *)self->_simpleJwt copyWithZone:zone];
  v7 = v5[5];
  v5[5] = v6;

  v8 = [(NSData *)self->_channelPublicKey copyWithZone:zone];
  v9 = v5[1];
  v5[1] = v8;

  v10 = [(NSData *)self->_nonce copyWithZone:zone];
  v11 = v5[2];
  v5[2] = v10;

  v12 = [(NSData *)self->_signNonce copyWithZone:zone];
  v13 = v5[4];
  v5[4] = v12;

  v14 = [(NSData *)self->_serverEncryptionKey copyWithZone:zone];
  v15 = v5[3];
  v5[3] = v14;

  return v5;
}

- (BOOL)isEqual:(id)equal
{
  equalCopy = equal;
  if ([equalCopy isMemberOfClass:objc_opt_class()] && ((simpleJwt = self->_simpleJwt, !(simpleJwt | equalCopy[5])) || -[NSString isEqual:](simpleJwt, "isEqual:")) && ((channelPublicKey = self->_channelPublicKey, !(channelPublicKey | equalCopy[1])) || -[NSData isEqual:](channelPublicKey, "isEqual:")) && ((nonce = self->_nonce, !(nonce | equalCopy[2])) || -[NSData isEqual:](nonce, "isEqual:")) && ((signNonce = self->_signNonce, !(signNonce | equalCopy[4])) || -[NSData isEqual:](signNonce, "isEqual:")))
  {
    serverEncryptionKey = self->_serverEncryptionKey;
    if (serverEncryptionKey | equalCopy[3])
    {
      v10 = [(NSData *)serverEncryptionKey isEqual:?];
    }

    else
    {
      v10 = 1;
    }
  }

  else
  {
    v10 = 0;
  }

  return v10;
}

- (unint64_t)hash
{
  v3 = [(NSString *)self->_simpleJwt hash];
  v4 = [(NSData *)self->_channelPublicKey hash]^ v3;
  v5 = [(NSData *)self->_nonce hash];
  v6 = v4 ^ v5 ^ [(NSData *)self->_signNonce hash];
  return v6 ^ [(NSData *)self->_serverEncryptionKey hash];
}

- (void)mergeFrom:(id)from
{
  fromCopy = from;
  if (fromCopy[5])
  {
    [(SharedOwnershipAuth *)self setSimpleJwt:?];
  }

  if (fromCopy[1])
  {
    [(SharedOwnershipAuth *)self setChannelPublicKey:?];
  }

  if (fromCopy[2])
  {
    [(SharedOwnershipAuth *)self setNonce:?];
  }

  if (fromCopy[4])
  {
    [(SharedOwnershipAuth *)self setSignNonce:?];
  }

  if (fromCopy[3])
  {
    [(SharedOwnershipAuth *)self setServerEncryptionKey:?];
  }
}

@end