...
- (MDMBackupManager)initWithPersonaIdentifier:(id)identifier;
- (void)isCloudBackupInProgressWithCompletion:(id)completion;
@end
...
- (MDMBackupManager)initWithPersonaIdentifier:(id)identifier
{
  ...
  if (v6)
  {
    ...
    v10 = [objc_alloc(MEMORY[0x277D28A40]) initWithDelegate:v7 eventQueue:v7->_queue personaIdentifier:identifierCopy];
    manager = v7->_manager;
    v7->_manager = v10;
  }
...
- (void)isCloudBackupInProgressWithCompletion:(id)completion
{
  ...
  manager = [(MDMBackupManager *)self manager];
  ...
  v8 = completionCopy;
  v6 = completionCopy;
  [manager fetchiCloudRestoreIsCompleteWithCompletion:v7];
}
