...
- (WFCarPlayConnectionTriggerConfigurationViewController)initWithTrigger:(id)trigger mode:(unint64_t)mode;
- (id)customSections;
...
{
  pathCopy = path;
  v7 = -[WFCarPlayConnectionTriggerConfigurationViewController infoForSection:](self, "infoForSection:", [pathCopy section]);
...
- (id)tableView:(id)view titleForHeaderInSection:(int64_t)section
{
  v4 = [(WFCarPlayConnectionTriggerConfigurationViewController *)self infoForSection:section];
...
- (id)tableView:(id)view cellForRowAtIndexPath:(id)path
{
  pathCopy = path;
  viewCopy = view;
  v8 = -[WFCarPlayConnectionTriggerConfigurationViewController infoForSection:](self, "infoForSection:", [pathCopy section]);
  ...
  else
  {
    ...
    [v34 setLeftViewSelected:{objc_msgSend(trigger7, "onConnect")}];

    trigger8 = [(WFTriggerConfigurationViewController *)self trigger];
    [v34 setRightViewSelected:{objc_msgSend(trigger8, "onDisconnect")}];
...
- (int64_t)tableView:(id)view numberOfRowsInSection:(int64_t)section
{
  v5 = [(WFCarPlayConnectionTriggerConfigurationViewController *)self infoForSection:section];
...
{
  v5.receiver = self;
  v5.super_class = WFCarPlayConnectionTriggerConfigurationViewController;
  [(WFCarPlayConnectionTriggerConfigurationViewController *)&v5 viewWillAppear:appear];
  tableView = [(WFTriggerConfigurationViewController *)self tableView];
  [tableView reloadData];
}
...
- (WFCarPlayConnectionTriggerConfigurationViewController)initWithTrigger:(id)trigger mode:(unint64_t)mode
{
  ...
  if ((objc_opt_isKindOfClass() & 1) == 0)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"WFCarPlayConnectionTriggerConfigurationViewController.m" lineNumber:31 description:{@"Invalid parameter not satisfying: %@", @"[trigger isKindOfClass:[WFCarPlayConnectionTrigger class]]"}];
  }

  v13.receiver = self;
  v13.super_class = WFCarPlayConnectionTriggerConfigurationViewController;
