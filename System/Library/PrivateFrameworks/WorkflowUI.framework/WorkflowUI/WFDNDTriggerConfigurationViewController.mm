...
- (WFDNDTriggerConfigurationViewController)initWithTrigger:(id)trigger mode:(unint64_t)mode;
- (id)customSections;
...
- (id)tableView:(id)view titleForHeaderInSection:(int64_t)section
{
  v4 = [(WFDNDTriggerConfigurationViewController *)self infoForSection:section];
...
{
  pathCopy = path;
  v7 = -[WFDNDTriggerConfigurationViewController infoForSection:](self, "infoForSection:", [pathCopy section]);
...
- (id)tableView:(id)view cellForRowAtIndexPath:(id)path
{
  pathCopy = path;
  viewCopy = view;
  v8 = -[WFDNDTriggerConfigurationViewController infoForSection:](self, "infoForSection:", [pathCopy section]);
...
- (int64_t)tableView:(id)view numberOfRowsInSection:(int64_t)section
{
  v5 = [(WFDNDTriggerConfigurationViewController *)self infoForSection:section];
...
- (void)viewWillAppear:(BOOL)appear
{
  ...
  [(WFDNDTriggerConfigurationViewController *)&v5 viewWillAppear:appear];
  tableView = [(WFTriggerConfigurationViewController *)self tableView];
  [tableView reloadData];
}
...
- (WFDNDTriggerConfigurationViewController)initWithTrigger:(id)trigger mode:(unint64_t)mode
{
  ...
  if ((objc_opt_isKindOfClass() & 1) == 0)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"WFDNDTriggerConfigurationViewController.m" lineNumber:32 description:{@"Invalid parameter not satisfying: %@", @"[trigger isKindOfClass:[WFDNDTrigger class]]"}];
  }
