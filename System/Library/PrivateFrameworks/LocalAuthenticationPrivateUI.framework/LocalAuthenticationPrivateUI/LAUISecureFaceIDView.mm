...
- (BOOL)_canTransitionViewToState:(int64_t)state;
- (BOOL)_transitionViewToState:(int64_t)state;
- (LAUISecureFaceIDView)initWithType:(int64_t)type;
- (NSNumber)remainingMinDisplayTimeInterval;
- (id)_caStateNameForState:(int64_t)state;
- (id)_descriptionOfState:(int64_t)state;
- (id)_flipbookFromLAUISecureFaceIDViewType:(int64_t)type;
- (id)_msSinceDisplayTime;
- (id)_shortNameForState:(int64_t)state;
- (id)_transitionDict;
- (id)description;
- (int64_t)_nextStateForUnwantedTransitionFrom:(int64_t)from to:(int64_t)to;
- (int64_t)_nextStateOnShortestPathFrom:(int64_t)from to:(int64_t)to;
...
- (void)_setupView;
- (void)_tick;
- (void)_tickWithInterval:(double)interval frequency:;
- (void)dealloc;
...
- (LAUISecureFaceIDView)initWithType:(int64_t)type
{
  ...
  typeCopy = type;
  v5 = CGSizeForLAUISecureFaceIDViewType(type);
  v7 = v6;
  v19.receiver = self;
  ...
  if (v8)
  {
    v10 = [(LAUISecureFaceIDView *)v8 _flipbookFromLAUISecureFaceIDViewType:type];
    ...
    {
      states = [(CASecureFlipBookLayer *)v9->_secureFlipBookLayer states];
      *buf = 138412290;
      v26 = states;
  ...
  {
    currentState = [(CASecureFlipBookLayer *)v9->_secureFlipBookLayer currentState];
    *buf = 138412546;
    v26 = v9;
    v27 = 2112;
    v28 = currentState;
...
- (void)didMoveToWindow
{
  window = [(LAUISecureFaceIDView *)self window];
...
- (void)didMoveToSuperview
{
  superview = [(LAUISecureFaceIDView *)self superview];
...
- (id)description
{
  ...
  v14 = [v11 stringWithFormat:@"<%@ %p flipBookState: '%@', state: '%@'>", v12, self, currentState, v3];;
...
- (id)_descriptionOfState:(int64_t)state
{
  ...
  else
  {
    ...
    state = [v5 stringWithFormat:@"%@(%d)", v6, state];
  }
...
- (BOOL)_canTransitionViewToState:(int64_t)state
{
  ...
  v4 = [(LAUISecureFaceIDView *)self _caStateNameForState:state];
...
- (BOOL)_transitionViewToState:(int64_t)state
{
  ...
  currentState = [(CASecureFlipBookLayer *)self->_secureFlipBookLayer currentState];
  v6 = [(LAUISecureFaceIDView *)self _caStateNameForState:state];
  ...
  if (v7)
  {
    ...
    else
    {
      ...
      self->_nextStateOnPathToDesiredState = stateCopy;
    }
...
- (NSNumber)remainingMinDisplayTimeInterval
{
  if (self->_timeAppeared)
  {
    ...
    valueForFlagSecureUIMinDisplayTime = [mEMORY[0x277D24068] valueForFlagSecureUIMinDisplayTime];
    [valueForFlagSecureUIMinDisplayTime doubleValue];
    v6 = v5;
...
- (void)_setupView
{
  ...
  widthAnchor = [(LAUISecureFaceIDView *)self widthAnchor];
  v8 = [widthAnchor constraintEqualToConstant:v3];
  v12[0] = v8;
  heightAnchor = [(LAUISecureFaceIDView *)self heightAnchor];
...
- (void)layoutSubviews
{
  ...
  layer = [(LAUISecureFaceIDView *)self layer];
  sublayers = [layer sublayers];
  v5 = [sublayers containsObject:self->_secureFlipBookLayer];

  if ((v5 & 1) == 0)
  {
    layer2 = [(LAUISecureFaceIDView *)self layer];
    [layer2 addSublayer:self->_secureFlipBookLayer];
  }
}
...
- (id)_flipbookFromLAUISecureFaceIDViewType:(int64_t)type
{
  *&v15[5] = *MEMORY[0x277D85DE8];
  ...
  v4 = CASecureFlipBookLayerBaseNameFromLAUISecureFaceIDViewType(type);
  _flipbookFromLAUISecureFaceIDViewType__scale = [MEMORY[0x277CCACA8] stringWithFormat:@"%@-%dx", v4, _flipbookFromLAUISecureFaceIDViewType__scale];
  v6 = [MEMORY[0x277CD9F88] secureFlipBookWithType:_flipbookFromLAUISecureFaceIDViewType__scale];
  if (v6)
  {
    v7 = v6;
    v8 = LACLogFaceIDUI();
    ...
    *buf = 67109378;
    ...
    LOWORD(v15[1]) = 2112;
    *(&v15[1] + 2) = _flipbookFromLAUISecureFaceIDViewType__scale;
...
- (id)_caStateNameForState:(int64_t)state
{
  _transitionDict = [(LAUISecureFaceIDView *)self _transitionDict];
  v5 = [MEMORY[0x277CCABB0] numberWithInteger:state];
  v6 = [_transitionDict objectForKey:v5];
  ...
  else
  {
    ...
    {
      [(LAUISecureFaceIDView *)state _caStateNameForState:v8];
    }
  }
...
- (id)_shortNameForState:(int64_t)state
{
  v3 = [(LAUISecureFaceIDView *)self _caStateNameForState:state];
...
- (void)_tickWithInterval:(double)interval frequency:
{
  if (self)
  {
    ...
    if (*(self + 456) == 1)
    {
      [(LAUISecureFaceIDView *)self _assertVisibility];
    }

    [*(self + 464) tick:interval];
    if (*(self + 416) != *(self + 424))
    {
      if (*(self + 432) != 0x7FFFFFFFFFFFFFFFLL)
      {
        if ([self _canTransitionViewToState:?])
        {
          v8 = LACLogFaceIDUI();
      ...
      if (*(self + 424) != 0x7FFFFFFFFFFFFFFFLL)
      {
        if ([self _canTransitionViewToState:?])
        {
          v15 = LACLogFaceIDUI();
...
- (void)_assertVisibility
{
  ...
  isFully = [(LAUISecureFaceIDView *)self _isFullyVisible];
  v4 = *(self + 448);
  if ((isFully & 1) == 0)
  {
    if (!v4)
    ...
    if (OUTLINED_FUNCTION_4(v15))
    {
      _msSinceDisplayTime = [self _msSinceDisplayTime];
      OUTLINED_FUNCTION_0_1();
      OUTLINED_FUNCTION_1_0();
      _os_log_impl(v17, v18, v19, v20, v21, 0x16u);
    }
...
LABEL_15:
  ...
  remainingMinDisplayTimeInterval = [self remainingMinDisplayTimeInterval];
  ...
  if (v7 <= 0.0)
  {
    ...
    if (OUTLINED_FUNCTION_4(v8))
    {
      _msSinceDisplayTime2 = [self _msSinceDisplayTime];
      OUTLINED_FUNCTION_0_1();
      OUTLINED_FUNCTION_1_0();
      _os_log_impl(v10, v11, v12, v13, v14, 0x16u);
    }
