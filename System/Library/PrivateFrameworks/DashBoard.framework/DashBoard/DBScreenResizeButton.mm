...
- (DBScreenResizeButton)initWithFrame:(CGRect)frame;
...
- (DBScreenResizeButton)initWithFrame:(CGRect)frame
{
  ...
  v3 = [(DBScreenResizeButton *)&v11 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  v4 = v3;
  if (v3)
  {
    layer = [(DBScreenResizeButton *)v3 layer];
    [layer setCornerCurve:*MEMORY[0x277CDA138]];

    layer2 = [(DBScreenResizeButton *)v4 layer];
...
- (void)traitCollectionDidChange:(id)change
{
  ...
  [(DBScreenResizeButton *)&v4 traitCollectionDidChange:change];
...
- (void)_updateImage
{
  transitionControlType = [(DBScreenResizeButton *)self transitionControlType];
  v4 = @"arrow.down.right.and.arrow.up.left";
  if (transitionControlType != 2)
  {
    v4 = 0;
  }

  if (transitionControlType == 1)
  {
  ...
  traitCollection = [(DBScreenResizeButton *)self traitCollection];
  v8 = [v6 systemImageNamed:v5 compatibleWithTraitCollection:traitCollection];
...
- (void)didUpdateFocusInContext:(id)context withAnimationCoordinator:(id)coordinator
{
  ...
  [(DBScreenResizeButton *)&v5 didUpdateFocusInContext:context withAnimationCoordinator:coordinator];
  [(DBScreenResizeButton *)self _updateBackgroundColor];
  [(DBScreenResizeButton *)self _updateImage];
}
