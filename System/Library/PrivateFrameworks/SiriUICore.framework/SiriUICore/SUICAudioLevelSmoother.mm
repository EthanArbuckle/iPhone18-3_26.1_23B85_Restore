...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = SUICAudioLevelSmoother;
...
- (float)_updateMedianWithNewValue:(float)a3
{
  ...
  v9 = malloc_type_calloc(4uLL, historyLength, 0x46F70DE1uLL);
  if (historyLength >= 1)
  {
    v10 = 0;
...
LABEL_15:
  ...
  v17 = *(v9 + ((((historyLength + (historyLength >> 63)) << 31) >> 30) & 0xFFFFFFFFFFFFFFFCLL));
  free(v9);
  return v17;
}
...
- (float)smoothedLevelForMicPower:(float)minimumPower
{
  ...
  [(SUICAudioLevelSmoother *)self _updateMedianWithNewValue:*&minimumPower, v4];
  ...
  else
  {
    ...
    if (self->_usesAttackAndDecaySpeed)
    {
      ...
      v8 = self->_attackVelocity + ((fmaxf(v6 - previousLevel, 0.0) - self->_attackVelocity) * self->_attackSpeed);
      v6 = previousLevel + v8;
