...
- (id)description
{
  ...
  v9 = [MEMORY[0x277CCABB0] numberWithBool:{-[ASKResourceLoader isInBackground](self, "isInBackground")}];
  v10 = [MEMORY[0x277CCACA8] stringWithFormat:@"{onScreen: %ld, onScreenInitialViewport: %ld, offScreen: %ld, offScreenInitialViewport: %ld, prefetch: %ld, none: %ld}", self->_requestCountMap[4], self->_requestCountMap[5], self->_requestCountMap[2], self->_requestCountMap[3], self->_requestCountMap[1], self->_requestCountMap[0]];
...
void __61__ASKResourceLoader_postDidBeginLoadingForReason_requestKey___block_invoke(uint64_t a1)
{
  ...
  if (WeakRetained)
  {
    ...
    v8[0] = v4;
    v5 = [MEMORY[0x277CCABB0] numberWithUnsignedInteger:{objc_msgSend(*(a1 + 32), "hash")}];
    v8[1] = v5;
