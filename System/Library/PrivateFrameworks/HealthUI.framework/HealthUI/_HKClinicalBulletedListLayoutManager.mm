...
- (void)setBullet:(id)bullet
{
  ...
  textStorage = [(_HKClinicalBulletedListLayoutManager *)self textStorage];
  -[_HKClinicalBulletedListLayoutManager invalidateDisplayForCharacterRange:](self, "invalidateDisplayForCharacterRange:", 0, [textStorage length]);
}
...
- (void)drawGlyphsForGlyphRange:(_NSRange)range atPoint:(CGPoint)point
{
  ...
  [(_HKClinicalBulletedListLayoutManager *)&v18 drawGlyphsForGlyphRange:point.x atPoint:point.y];
  bullet = [(_HKClinicalBulletedListLayoutManager *)self bullet];
  if (bullet)
  {
    textStorage = [(_HKClinicalBulletedListLayoutManager *)self textStorage];
