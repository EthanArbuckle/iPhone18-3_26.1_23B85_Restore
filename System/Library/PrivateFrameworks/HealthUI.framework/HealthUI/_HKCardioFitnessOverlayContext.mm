@interface _HKCardioFitnessOverlayContext
- (_HKCardioFitnessOverlayContext)initWithMode:(int64_t)mode applicationItems:(id)items overlayChartController:(id)controller;
- (id)_cardioFitnessContextItemWithLevel:(int64_t)level chartPointCount:(id)count forTimeScope:(int64_t)scope;
- (id)_cardioFitnessContextItemWithLevel:(int64_t)level count:(id)count unitString:(id)string;
- (id)_cardioFitnessContextItemWithLevel:(int64_t)level sampleCount:(id)count;
- (id)contextItemForLastUpdate;
- (void)updateContextItemForDateInterval:(id)interval overlayController:(id)controller timeScope:(int64_t)scope resolution:(int64_t)resolution completion:(id)completion;
@end

@implementation _HKCardioFitnessOverlayContext

- (_HKCardioFitnessOverlayContext)initWithMode:(int64_t)mode applicationItems:(id)items overlayChartController:(id)controller
{
  controllerCopy = controller;
  v14.receiver = self;
  v14.super_class = _HKCardioFitnessOverlayContext;
  v9 = [(_HKCardioFitnessOverlayContext *)&v14 init];
  v10 = v9;
  if (v9)
  {
    v9->_mode = mode;
    objc_storeStrong(&v9->_overlayChartController, controller);
    v11 = [MEMORY[0x1E696C370] _quantityTypeWithCode:124];
    monitoringSampleType = v10->_monitoringSampleType;
    v10->_monitoringSampleType = v11;
  }

  return v10;
}

- (id)contextItemForLastUpdate
{
  lastUpdatedItem = [(_HKCardioFitnessOverlayContext *)self lastUpdatedItem];

  if (!lastUpdatedItem)
  {
    v4 = [(_HKCardioFitnessOverlayContext *)self _cardioFitnessContextItemWithLevel:-1 sampleCount:0];
    [(_HKCardioFitnessOverlayContext *)self setLastUpdatedItem:v4];
  }

  return [(_HKCardioFitnessOverlayContext *)self lastUpdatedItem];
}

- (void)updateContextItemForDateInterval:(id)interval overlayController:(id)controller timeScope:(int64_t)scope resolution:(int64_t)resolution completion:(id)completion
{
  currentHandler = [MEMORY[0x1E696AAA8] currentHandler];
  [currentHandler handleFailureInMethod:a2 object:self file:@"HKOverlayRoomCardioFitnessViewController.m" lineNumber:540 description:@"subclass must implement this"];
}

- (id)_cardioFitnessContextItemWithLevel:(int64_t)level chartPointCount:(id)count forTimeScope:(int64_t)scope
{
  countCopy = count;
  v9 = +[HKGraphZoomLevelConfiguration seriesPointIntervalUnitForTimeScope:pointCount:](HKGraphZoomLevelConfiguration, "seriesPointIntervalUnitForTimeScope:pointCount:", scope, [countCopy integerValue]);
  v10 = [(_HKCardioFitnessOverlayContext *)self _cardioFitnessContextItemWithLevel:level count:countCopy unitString:v9];

  return v10;
}

- (id)_cardioFitnessContextItemWithLevel:(int64_t)level sampleCount:(id)count
{
  v6 = MEMORY[0x1E696AEC0];
  v7 = MEMORY[0x1E696AAE8];
  countCopy = count;
  v9 = [v7 bundleWithIdentifier:@"com.apple.HealthUI"];
  v10 = [v9 localizedStringForKey:@"ENTRIES_NUMBERLESS_VALUE" value:&stru_1F42FFBE0 table:@"HealthUI-Localizable"];
  v11 = [v6 localizedStringWithFormat:v10, objc_msgSend(countCopy, "integerValue")];
  v12 = [(_HKCardioFitnessOverlayContext *)self _cardioFitnessContextItemWithLevel:level count:countCopy unitString:v11];

  return v12;
}

- (id)_cardioFitnessContextItemWithLevel:(int64_t)level count:(id)count unitString:(id)string
{
  countCopy = count;
  stringCopy = string;
  v10 = objc_alloc_init(HKDisplayTypeContextItem);
  v11 = [HKCardioFitnessUtilities localizedStringForCardioFitnessTitle:level];
  [(HKDisplayTypeContextItem *)v10 setTitle:v11];

  v12 = MEMORY[0x1E696AEC0];
  v13 = [MEMORY[0x1E696BF78] analyticsStringForLevel:level];
  v14 = [v12 hk_chartOverlayCardioFitnessAccessibilityIdentifier:v13];
  [(HKDisplayTypeContextItem *)v10 setAccessibilityIdentifier:v14];

  [(HKDisplayTypeContextItem *)v10 setInfoHidden:1];
  if ([countCopy integerValue])
  {
    [(HKDisplayTypeContextItem *)v10 setUnit:stringCopy];
    [countCopy stringValue];
  }

  else
  {
    [(HKDisplayTypeContextItem *)v10 setUnit:&stru_1F42FFBE0];
    HKLocalizedNoDataString();
  }
  v15 = ;
  [(HKDisplayTypeContextItem *)v10 setValue:v15];

  v16 = [MEMORY[0x1E696BF78] analyticsStringForLevel:level];
  [(HKDisplayTypeContextItem *)v10 setAnalyticsIdentifier:v16];

  v17 = [HKOverlayContextUtilities defaultMetricColorsForOverlayMode:[(_HKCardioFitnessOverlayContext *)self mode]];
  [(HKDisplayTypeContextItem *)v10 setMetricColors:v17];

  v18 = +[HKUIMetricColors vitalsColors];
  [(HKDisplayTypeContextItem *)v10 setSelectedMetricColors:v18];

  return v10;
}

@end