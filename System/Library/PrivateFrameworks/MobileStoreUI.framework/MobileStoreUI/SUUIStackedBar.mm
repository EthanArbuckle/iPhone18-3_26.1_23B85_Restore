...
- (SUUIStackedBar)initWithFrame:(CGRect)frame;
...
{
  v15.receiver = self;
  v15.super_class = SUUIStackedBar;
  v3 = [(SUUIStackedBar *)&v15 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  if (v3)
  {
...
- (void)setOffset:(double)offset
{
  ...
  {
    self->_offset = offset;
    [(SUUIStackedBar *)self sizeThatFits:*MEMORY[0x277CBF3A8], *(MEMORY[0x277CBF3A8] + 8)];
    offset = self->_offset;
...
{
  v22.receiver = self;
  v22.super_class = SUUIStackedBar;
...
- (void)sizeToFit
{
  ...
  v4 = v3;
  v6 = v5;
  [(SUUIStackedBar *)self sizeThatFits:v7, v8];

  [(SUUIStackedBar *)self setFrame:v4, v6, v9, v10];
}
...
- (void)_reload
{
  ...
  if ([(NSArray *)self->_items count])
  {
    ...
    do
    {
      ...
      title = [v5 title];
      [(SUUIStackedBarCell *)v6 setTitle:title forState:0];

      v8 = !self->_alwaysShowsBackButton && [(NSArray *)self->_items count]< 2 || [(SUUIStackedBar *)self splitViewStyle];
      backButton = [(SUUIStackedBarCell *)v6 backButton];
      [backButton setHidden:v8];

      backButton2 = [(SUUIStackedBarCell *)v6 backButton];
...
- (void)_selectItemAtIndex:(int64_t)index
{
  delegate = [(SUUIStackedBar *)self delegate];
