...
{
  v6 = *budget;
  event = [(USBudgetedApplication *)self event];
  LOBYTE(error) = [(USBudgetedApplication *)self _validateBudget:v6 andEvent:event error:error];
...
- (BOOL)validateEvent:(id *)event error:(id *)error
{
  budget = [(USBudgetedApplication *)self budget];
  LOBYTE(error) = [(USBudgetedApplication *)self _validateBudget:budget andEvent:*event error:error];
