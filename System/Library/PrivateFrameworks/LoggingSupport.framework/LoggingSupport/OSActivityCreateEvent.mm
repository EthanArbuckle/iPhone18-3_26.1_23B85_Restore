...
- (OSActivityCreateEvent)initWithEntry:(os_activity_stream_entry_s *)entry;
...
- (void)_addProperties:(id)properties
{
  ...
  [(OSActivityEvent *)&v7 _addProperties:propertiesCopy];
  [propertiesCopy setObject:@"OSActivityCreateEvent" forKey:@"eventType"];
  ...
  if ([(OSActivityCreateEvent *)self creatorActivityID])
  {
    ...
    [propertiesCopy setObject:v6 forKey:@"creatorActivityID"];
  }
}
...
- (OSActivityCreateEvent)initWithEntry:(os_activity_stream_entry_s *)entry
{
  if (*(&entry->var8.var12.var9 + 1))
  {
    ...
    if (v4)
    {
      [(OSActivityEvent *)v4 fillFromStreamEntry:entry eventMessage:*(&entry->var8.var12.var9 + 1) persisted:0];
      v5->_creatorActivityID = *(&entry->var8.var12.var10 + 4);
      v5->_creatorProcessUniqueID = *(&entry->var8.var12.var11 + 4);
