...
- (SBHViewDarkeningHelper)initWithView:(id)view;
- (UIView)view;
...
- (void)setBrightness:(double)brightness
{
  ...
  view = [(SBHViewDarkeningHelper *)self view];
  layer = [view layer];
  ...
  if (brightness < 1.0)
  {
    if (!multiplyFilter)
    {
      ...
      [layer sbh_addFilter:self->_multiplyFilter];
      multiplyFilter = self->_multiplyFilter;
      brightness = self->_brightness;
    }
  ...
  if (multiplyFilter)
  {
    [layer sbh_removeFilterWithName:@"SBHViewDarkening"];
...
- (void)appendDescriptionToFormatter:(id)formatter
{
  ...
  view = [(SBHViewDarkeningHelper *)self view];
  v5 = [formatterCopy appendObject:view withName:@"view"];

  [(SBHViewDarkeningHelper *)self brightness];
  v6 = [formatterCopy appendFloat:@"brightness" withName:?];
}
