@interface NSArray
- (BOOL)hasObjectPassingTest:(id)test;
- (id)firstObjectPassingTest:(id)test;
- (id)max:(id)max;
- (id)nph_map:(id)nph_map;
- (id)objectsPassingTest:(id)test;
@end

@implementation NSArray

- (id)max:(id)max
{
  maxCopy = max;
  if ([(NSArray *)self count])
  {
    v5 = [(NSArray *)self objectAtIndexedSubscript:0];
    v14 = 0u;
    v15 = 0u;
    v16 = 0u;
    v17 = 0u;
    selfCopy = self;
    v7 = [(NSArray *)selfCopy countByEnumeratingWithState:&v14 objects:v18 count:16];
    if (v7)
    {
      v8 = v7;
      v9 = *v15;
      do
      {
        for (i = 0; i != v8; i = i + 1)
        {
          if (*v15 != v9)
          {
            objc_enumerationMutation(selfCopy);
          }

          v11 = *(*(&v14 + 1) + 8 * i);
          if (maxCopy[2](maxCopy, v11, v5))
          {
            v12 = v11;

            v5 = v12;
          }
        }

        v8 = [(NSArray *)selfCopy countByEnumeratingWithState:&v14 objects:v18 count:16];
      }

      while (v8);
    }
  }

  else
  {
    v5 = 0;
  }

  return v5;
}

- (BOOL)hasObjectPassingTest:(id)test
{
  v3 = [(NSArray *)self firstObjectPassingTest:test];
  v4 = v3 != 0;

  return v4;
}

- (id)firstObjectPassingTest:(id)test
{
  v3 = [(NSArray *)self objectsPassingTest:test];
  firstObject = [v3 firstObject];

  return firstObject;
}

- (id)objectsPassingTest:(id)test
{
  v8[0] = _NSConcreteStackBlock;
  v8[1] = 3221225472;
  v8[2] = sub_593C;
  v8[3] = &unk_1C840;
  testCopy = test;
  v4 = testCopy;
  v5 = [NSPredicate predicateWithBlock:v8];
  v6 = [(NSArray *)self filteredArrayUsingPredicate:v5];

  return v6;
}

- (id)nph_map:(id)nph_map
{
  nph_mapCopy = nph_map;
  v5 = objc_opt_new();
  v14 = 0u;
  v15 = 0u;
  v16 = 0u;
  v17 = 0u;
  selfCopy = self;
  v7 = [(NSArray *)selfCopy countByEnumeratingWithState:&v14 objects:v18 count:16];
  if (v7)
  {
    v8 = v7;
    v9 = *v15;
    do
    {
      for (i = 0; i != v8; i = i + 1)
      {
        if (*v15 != v9)
        {
          objc_enumerationMutation(selfCopy);
        }

        v11 = nph_mapCopy[2](nph_mapCopy, *(*(&v14 + 1) + 8 * i));
        if (v11)
        {
          [v5 addObject:{v11, v14}];
        }
      }

      v8 = [(NSArray *)selfCopy countByEnumeratingWithState:&v14 objects:v18 count:16];
    }

    while (v8);
  }

  v12 = [v5 copy];

  return v12;
}

@end