...
+ (id)readMetaDataFromDictionary:(id)dictionary andValidWidth:(unint64_t)width andValidHeight:(unint64_t)height andLightSource:(opaqueCMSampleBuffer *)source andKeyPoints:(opaqueCMSampleBuffer *)points
{
  ...
  if (dictionaryCopy)
  {
    v34 = 0;
    v35 = 0;
    if ([RawMetaDataReader _getExposureInfo:dictionaryCopy info:v36])
    {
      LODWORD(v10) = v36[0];
    ...
    if (points)
    {
      v16 = CMGetAttachment(points, kFigCaptureSampleBufferAttachmentKey_MetadataDictionary, 0);
      if (v16)
      {
        Value = CFDictionaryGetValue(v16, kFigCaptureStreamMetadata_ImageRegistrationInfo);
        if (Value)
        {
          v18 = [RawMetaDataReader _getRegistrationInfo:v31 validWidth:width validHeight:height keypointDictionary:Value ispInfo:v33];
          if ((v18 & [RawMetaDataReader _getKeypointsMappingInfoFrom:v31 validWidth:width validHeight:height KeyPointDictionary:Value IspInfo:v33]) == 1)
          {
            LODWORD(v35) = 1;
          }
        }
    ...
    v23 = CMGetAttachment(source, kFigCaptureSampleBufferAttachmentKey_MetadataDictionary, 0);
    if (v23 && [RawMetaDataReader _getLightSourceMappingInfoFrom:v31 imageValidWidth:width imageValidHeight:height lightSourceDictionary:v23 lightSourceValidWidth:Width lightSourceValidHeight:Height ispInfo:v33])
    {
      HIDWORD(v35) = 1;
    }
