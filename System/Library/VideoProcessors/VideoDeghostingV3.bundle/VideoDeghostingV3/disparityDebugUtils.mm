...
- (void)saveF32FPBuffer:(__CVBuffer *)buffer AsPPMFile:(const char *)file scale:(float)scale
{
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile(file);
  if (PathIfNeededAndOpenFile)
  {
    v8 = PathIfNeededAndOpenFile;
    BaseAddress = CVPixelBufferGetBaseAddress(buffer);
    Width = CVPixelBufferGetWidth(buffer);
    Height = CVPixelBufferGetHeight(buffer);
...
- (void)saveF32Buffer:(__CVBuffer *)buffer AsPPMFile:(const char *)file
{
  ...
  v16 = file;
  writePPMFromRGBA(Width, Height, 4 * Width, v10, [file UTF8String]);
  free(v10);
}
...
- (void)saveU8Buffer:(__CVBuffer *)buffer AsPPMFile:(const char *)file
{
  ...
  [disparityDebugUtils createPathIfNeeded:file];
  v19 = file;
  writePPMFromRGBA(Width, Height, 4 * Width, v11, [file UTF8String]);
  free(v11);
}
...
- (void)saveVUYA4444Texture:(id)texture AsPPMFile:(const char *)file
{
  ...
  writePPMFromRGBA(width, height, 4 * width, v9, file);
  free(v9);
}
...
- (void)saveVUYA4444PixelBuffer:(__CVBuffer *)buffer out_width:(unsigned int)out_width out_height:(unsigned int)out_height AsPPMFile:(const char *)file
{
  ...
  writePPMFromRGBA(out_widthCopy, out_heightCopy, 4 * Width, v14, file);
...
- (void)saveRGB10A2PixelBuffer:(__CVBuffer *)buffer out_width:(unsigned int)out_width out_height:(unsigned int)out_height AsPPMFile:(const char *)file
{
  ...
  CVPixelBufferUnlockBaseAddress(buffer, 1uLL);
  writePPMFromRGBA(v8, v7, 4 * v8, v14, file);
...
- (void)saveRGB10A2PixelBuffer:(__CVBuffer *)buffer out_width:(unsigned int)out_width out_height:(unsigned int)out_height AsPPMFileWithAlpha:(const char *)alpha
{
  ...
  [(disparityDebugUtils *)self convertRGB10A2ToRGBA8:BaseAddress rbs:BytesPerRow ToRGBA:v14 outWidth:v8 outHeight:v7];
  CVPixelBufferUnlockBaseAddress(buffer, 1uLL);
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile(alpha);
  if (PathIfNeededAndOpenFile)
  {
    v17 = PathIfNeededAndOpenFile;
...
- (void)save420CVPixelBuffer:(__CVBuffer *)buffer to:(const char *)to
{
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile(to);
  if (PathIfNeededAndOpenFile)
  {
    v6 = PathIfNeededAndOpenFile;
    Width = CVPixelBufferGetWidth(buffer);
    Height = CVPixelBufferGetHeight(buffer);
    CVPixelBufferLockBaseAddress(buffer, 1uLL);
...
- (void)saveRGBATexture:(id)texture AsPPMFile:(const char *)file
{
  ...
  file = [NSString stringWithFormat:@"%s", file];
  [disparityDebugUtils createPathIfNeeded:file];
  writePPMFromRGBA(width, height, 4 * width, v9, [file UTF8String]);
  free(v9);
}
...
- (void)saveF16DisparityBufferAsTurbo:(__CVBuffer *)turbo AsPPMFile:(const char *)file WithMin:(float)min WithMax:(float)max
{
  ...
  if (v14)
  {
      ...
      for (i = 0; i != Height; ++i)
      {
        if (Width)
        {
          ...
          do
          {
            ...
            v25 = &turbo_srgb_bytes_1024 + 3 * ((((fmaxf(min, fminf(max, _S3)) - min) / (max - min)) * 1024.0) + -0.49);
            *v18 = *v25;
            v18[2] = v25[2];
            v18[3] = -1;
            v18 += 4;
    ...
    writePPMFromRGBA(Width, Height, 4 * Width, v15, file);
...
- (void)saveF16DisparityBuffer:(__CVBuffer *)buffer AsGrayScalePPMFile:(const char *)file range:
{
  ...
  writePPMFromRGBA(Width, Height, v30, v28, file);
  free(v29);
...
- (void)saveF16Texture:(id)texture AsGrayScalePPMFile:(const char *)file range:
{
  ...
  writePPMFromRGBA(width, height, v32, v30, file);
  free(v31);
  free(v10);
  free(v12);
}
...
- (void)ApplyColorWheelAndWrite:(unsigned int)write height:(unsigned int)height data:(float *)data fileFullPath:(const char *)path
{
  v15 = ColorsWheelContext_create(80, 30, 20, 20, 30, 0, 0, 1, 0.0);
  v10 = malloc_type_malloc(4 * write * height, 0x7C78171DuLL);
  ColorsWheelContext_drawMatrix_f32(v15, write, height, data, 4 * write, 4, v10, 4 * write, v11, v12, v13, v14, 4);
  writePPMFromRGBA(write, height, 4 * write, v10, path);
  free(v10);
  ColorsWheelContext_destroy(&v15);
}
...
- (void)saveF16DisparityBuffer:(__CVBuffer *)buffer AsBinaryFile:(const char *)file
{
  ...
  else
  {
    ...
    {
      [disparityDebugUtils saveF16DisparityBuffer:AsBinaryFile:];
    }

    v13 = getCameraCaptureLegacyLog_cameraCaptureLegacyLog;
...
- (void)saveU8Texture:(id)texture AsPGMFile:(const char *)file
{
  ...
  if (PathIfNeededAndOpenFile)
  {
    v12 = PathIfNeededAndOpenFile;
    fprintf(PathIfNeededAndOpenFile, "P5 %u %u 255\n", width, height);
    if (height)
    {
      v13 = v10;
      do
...
- (void)saveU16Texture:(id)texture AsPGMFile:(const char *)file
{
  ...
  if (PathIfNeededAndOpenFile)
  {
    v11 = PathIfNeededAndOpenFile;
    fprintf(PathIfNeededAndOpenFile, "P5 %u %u 65535\n", width, height);
    if (height)
    {
      v12 = 0;
      v13 = 2 * width;
...
- (void)saveRGF16Texture:(id)texture AsF32BinaryFile0:(id)file0 AsF32BinaryFile1:(id)file1
{
  ...
  file0Copy = file0;
  file1Copy = file1;
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile([file0Copy UTF8String]);
  if (PathIfNeededAndOpenFile)
  {
    v12 = PathIfNeededAndOpenFile;
    __stream = createPathIfNeededAndOpenFile([file1Copy UTF8String]);
    if (!__stream)
    {
      v36 = v12;
LABEL_17:
    ...
    if (!v16)
    {
      [disparityDebugUtils saveRGF16Texture:AsF32BinaryFile0:AsF32BinaryFile1:];
      v19 = 0;
      v18 = 0;
      goto LABEL_16;
    }
...
- (void)saveRGBA8Texture:(id)texture AsPPMFile:(const char *)file
{
  ...
  if (v9)
  {
    ...
    writePPMFromRGBA(width, height, 4 * width, v10, file);
    free(v10);
  }
  ...
  else
  {
    ...
    v11 = getCameraCaptureLegacyLog_cameraCaptureLegacyLog;
    if (os_log_type_enabled(getCameraCaptureLegacyLog_cameraCaptureLegacyLog, OS_LOG_TYPE_DEFAULT))
    {
      LOWORD(v12[0]) = 0;
...
- (void)saveRGBA8Texture:(id)texture AsBinaryFile:(const char *)file
{
  ...
  if (v9)
  {
    ...
    {
      [disparityDebugUtils saveRGBA8Texture:AsBinaryFile:];
    }
  }
...
- (void)saveRgbaF32PixelBuffer:(__CVBuffer *)buffer AsPPMFile:(const char *)file
{
  ...
  else
  {
    ...
    v17 = getCameraCaptureLegacyLog_cameraCaptureLegacyLog;
    if (os_log_type_enabled(getCameraCaptureLegacyLog_cameraCaptureLegacyLog, OS_LOG_TYPE_DEFAULT))
    {
      *buf = 136315138;
      fileCopy = file;
...
- (void)saveRGBAF16PixelBuffer:(__CVBuffer *)buffer out_width:(unsigned int)out_width out_height:(unsigned int)out_height AsPPMFile:(const char *)file
{
  ...
  else
  {
    ...
    v27 = getCameraCaptureLegacyLog_cameraCaptureLegacyLog;
    if (os_log_type_enabled(getCameraCaptureLegacyLog_cameraCaptureLegacyLog, OS_LOG_TYPE_DEFAULT))
    {
      *buf = 136315138;
      fileCopy = file;
...
- (void)saveNCCOutputFrom:(id)from asBinaryFiles:(const char *)files
{
  ...
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile([v15 UTF8String]);
  v20 = createPathIfNeededAndOpenFile([v16 UTF8String]);
  v44 = v17;
  v56 = createPathIfNeededAndOpenFile([v17 UTF8String]);
  v21 = createPathIfNeededAndOpenFile([v18 UTF8String]);
  v22 = 0;
  v54 = v21;
  __stream = PathIfNeededAndOpenFile;
  if (!PathIfNeededAndOpenFile)
  {
    v46 = 0;
    v23 = v20;
...
LABEL_23:
  if ((v12 & 1) == 0)
  {
    writePPMFromRGBA(v53, v51, 4 * v53, v22, [v46 UTF8String]);
  }
...
- (void)WriteBinDataToFile:(void *)file length:(unsigned int)length fileFullPath:(const char *)path
{
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile(path);
  if (PathIfNeededAndOpenFile)
  {
    v8 = PathIfNeededAndOpenFile;
    fwrite(file, 1uLL, length, PathIfNeededAndOpenFile);
...
+ (void)createPathIfNeeded:(id)needed
{
  ...
  if ((v5 & 1) == 0)
  {
    ...
    v7 = getCameraCaptureLegacyLog_cameraCaptureLegacyLog;
    if (os_log_type_enabled(getCameraCaptureLegacyLog_cameraCaptureLegacyLog, OS_LOG_TYPE_DEFAULT))
    {
      v8 = 138412546;
...
- (void)saveAccumulationFrom:(id)from asBinaryFiles:(const char *)files forSize:(CGSize)size costLineSize:(unsigned __int16)lineSize
{
  ...
  v34 = files;
  PathIfNeededAndOpenFile = createPathIfNeededAndOpenFile([files UTF8String]);
  v21 = createPathIfNeededAndOpenFile([files2 UTF8String]);
  v42 = createPathIfNeededAndOpenFile([files3 UTF8String]);
  v22 = createPathIfNeededAndOpenFile([files4 UTF8String]);
  v23 = PathIfNeededAndOpenFile;
  v24 = 0;
  v40 = v22;
  v41 = v21;
  if (!v23 || !v21)
...
LABEL_12:
  ...
  if (v42 && v22)
  {
    ...
    writePPMFromRGBA(v27, v38, 4 * v27, v24, [files5 UTF8String]);
    files2 = v33;
    v23 = __stream;
    goto LABEL_12;
  }
...
- (void)saveF32Texture:(id)texture AsF32BinaryFile:(const char *)file
{
  ...
  if (PathIfNeededAndOpenFile)
  {
    ...
    if (v12)
    {
      ...
      if (v13)
      {
        ...
        if (height)
        {
          ...
          do
          {
            ...
            fwrite(v12, 4uLL, v10, PathIfNeededAndOpenFile);
            ++v14;
            v15 += v11 & 0xFFFFFFFC;
          }
      ...
      {
        [disparityDebugUtils saveF32Texture:AsF32BinaryFile:];
      }
    }
  ...
  free(v12);
  free(v13);
  fclose(PathIfNeededAndOpenFile);
}
