...
- (id)_updatesForSpecifiers:(id)specifiers withGlobalSummarizationSetting:(int64_t)setting animated:(BOOL)animated;
- (id)allowSummarization:(id)summarization;
- (id)specifiers;
- (id)summarizationSetting:(id)setting;
- (void)_setGlobalSummarizationSettingEnabled:(BOOL)enabled;
- (void)setSummarization:(id)summarization specifier:(id)specifier;
...
- (NCSummarizePreviewsDetailControllerDelegate)delegate
{
  specifier = [(NCSummarizePreviewsDetailController *)self specifier];
...
- (void)viewDidAppear:(BOOL)appear
{
  v5.receiver = self;
  v5.super_class = NCSummarizePreviewsDetailController;
  [(NCSummarizePreviewsDetailController *)&v5 viewDidAppear:appear];
  specifier = [(NCSummarizePreviewsDetailController *)self specifier];
...
- (id)specifiers
{
  ...
  if (!v4)
  {
    ...
    effectiveGlobalSummarizationSetting = [v11 effectiveGlobalSummarizationSetting];

    v13 = [(NCSummarizePreviewsDetailController *)self _updatesForSpecifiers:v5 withGlobalSummarizationSetting:effectiveGlobalSummarizationSetting animated:0];
    currentSpecifiers = [v13 currentSpecifiers];
...
- (void)setSummarization:(id)summarization specifier:(id)specifier
{
  ...
  bOOLValue = [summarization BOOLValue];
...
LABEL_9:
    [(NCSummarizePreviewsDetailController *)self _setGlobalSummarizationSettingEnabled:bOOLValue];
    goto LABEL_10;
  }
...
- (id)_updatesForSpecifiers:(id)specifiers withGlobalSummarizationSetting:(int64_t)setting animated:(BOOL)animated
{
  ...
  if (setting == 2)
  {
    ...
    v10 = [specifiersCopy specifierForID:@"SUMMARIZATION_APP_LIST_GROUP_ID"];
    if (!v10)
    {
      v40 = v8;
      v41 = specifiersCopy;
      ...
      if (v16)
      {
        ...
        do
        {
          for (i = 0; i != v17; i = i + 1)
          {
            ...
            v25 = NCIsSummarizationFeatureEnabledForBundleIdentifier(sectionID);
  ...
  {
    v34 = [specifiersCopy indexOfSpecifierWithID:@"SUMMARIZATION_APP_LIST_GROUP_ID"];
...
- (void)_setGlobalSummarizationSettingEnabled:(BOOL)enabled
{
  ...
  delegate = [(NCSummarizePreviewsDetailController *)self delegate];
  [delegate summarizePreviewsDetailController:self didChangeGlobalSummarizePreviewsSetting:v4];
}
