uint64_t sub_100001118()
{
  v0 = sub_100001790(&qword_10000C038, &qword_1000053F0);
  v1 = *(v0 - 8);
  v2 = (*(v1 + 64) + 15) & 0xFFFFFFFFFFFFFFF0;
  __chkstk_darwin();
  v4 = &v13 - v3;
  sub_10000173C();
  sub_100004E9C();
  v5 = sub_100004E2C();
  v6 = *(v1 + 8);
  v6(v4, v0);
  sub_100004E9C();
  v7 = sub_100004E2C();
  v6(v4, v0);
  sub_100004E9C();
  v8 = sub_100004E2C();
  v6(v4, v0);
  sub_100004E9C();
  v9 = sub_100004E2C();
  v6(v4, v0);
  sub_100001790(&qword_10000C040, qword_1000053F8);
  v10 = swift_allocObject();
  *(v10 + 16) = xmmword_100005310;
  *(v10 + 32) = v5;
  *(v10 + 40) = v7;
  *(v10 + 48) = v8;
  *(v10 + 56) = v9;
  v11 = sub_100004E1C();

  return v11;
}

uint64_t sub_10000137C()
{
  v0 = sub_100004E8C();
  v1 = *(v0 - 8);
  v2 = *(v1 + 64);
  __chkstk_darwin();
  v4 = &v9 - ((v3 + 15) & 0xFFFFFFFFFFFFFFF0);
  swift_getKeyPath();
  sub_1000017D8();
  sub_100004E0C();

  sub_10000173C();
  sub_100004E7C();
  v5 = sub_100004E6C();
  (*(v1 + 8))(v4, v0);
  sub_100001790(&qword_10000C028, &qword_1000053E0);
  v6 = swift_allocObject();
  *(v6 + 16) = xmmword_100005320;
  *(v6 + 32) = v5;
  v7 = sub_100004E5C();

  return v7;
}

uint64_t sub_100001534()
{
  v0 = sub_100004E4C();
  sub_100001790(&qword_10000C030, &qword_1000053E8);
  v1 = swift_allocObject();
  *(v1 + 16) = xmmword_100005320;
  *(v1 + 32) = v0;
  v2 = sub_100004E3C();

  return v2;
}

uint64_t sub_1000015B8()
{
  v0 = sub_100004ECC();
  v1 = *(v0 - 8);
  v2 = *(v1 + 64);
  __chkstk_darwin(v0);
  v4 = v9 - ((v3 + 15) & 0xFFFFFFFFFFFFFFF0);
  v9[1] = sub_10000418C();
  sub_10000173C();
  sub_100004EDC();
  v5 = sub_100004EBC();
  (*(v1 + 8))(v4, v0);
  sub_100001790(&qword_10000C018, "6\r");
  v6 = swift_allocObject();
  *(v6 + 16) = xmmword_100005320;
  *(v6 + 32) = v5;
  v7 = sub_100004EAC();

  return v7;
}

unint64_t sub_10000173C()
{
  result = qword_10000C010;
  if (!qword_10000C010)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C010);
  }

  return result;
}

uint64_t sub_100001790(uint64_t *a1, uint64_t *a2)
{
  result = *a1;
  if (!result)
  {
    v4 = *a2;
    result = swift_getTypeByMangledNameInContext2();
    *a1 = result;
  }

  return result;
}

unint64_t sub_1000017D8()
{
  result = qword_10000C020;
  if (!qword_10000C020)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C020);
  }

  return result;
}

uint64_t sub_10000182C(uint64_t a1)
{
  v2 = sub_100001980();

  return AppIntentsExtension.configuration.getter(a1, v2);
}

int main(int argc, const char **argv, const char **envp)
{
  sub_1000018D8();
  sub_10000502C();
  return 0;
}

unint64_t sub_1000018D8()
{
  result = qword_10000C048;
  if (!qword_10000C048)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C048);
  }

  return result;
}

unint64_t sub_100001980()
{
  result = qword_10000C050;
  if (!qword_10000C050)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C050);
  }

  return result;
}

unint64_t sub_100001A18()
{
  result = qword_10000C068;
  if (!qword_10000C068)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C068);
  }

  return result;
}

Swift::Int sub_100001A6C()
{
  sub_1000050EC();
  sub_1000050CC();
  return sub_1000050FC();
}

Swift::Int sub_100001AD0()
{
  sub_1000050EC();
  sub_1000050CC();
  return sub_1000050FC();
}

uint64_t sub_100001B1C@<X0>(uint64_t *a1@<X0>, BOOL *a2@<X8>)
{
  v7._countAndFlagsBits = *a1;
  v3 = a1[1];
  v4._rawValue = &off_100008880;
  v7._object = v3;
  v5 = sub_1000050DC(v4, v7);

  *a2 = v5 != 0;
  return result;
}

uint64_t sub_100001BCC(uint64_t *a1, uint64_t *a2)
{
  result = *a1;
  if (!result)
  {
    v4 = *a2;
    result = swift_getTypeByMangledNameInContextInMetadataState2();
    *a1 = result;
  }

  return result;
}

Swift::Int sub_100001C38@<X0>(uint64_t a1@<X0>, void *a2@<X1>, BOOL *a3@<X8>)
{
  v6._object = a2;
  v6._countAndFlagsBits = a1;
  v4._rawValue = &off_1000088E0;
  result = sub_1000050DC(v4, v6);
  *a3 = result != 0;
  return result;
}

unint64_t sub_100001C78()
{
  result = qword_10000C080;
  if (!qword_10000C080)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C080);
  }

  return result;
}

uint64_t sub_100001CCC()
{
  v0 = sub_100001790(&qword_10000C140, &qword_100005CF0);
  v1 = *(*(v0 - 8) + 64);
  __chkstk_darwin(v0 - 8);
  v3 = v17 - v2;
  v17[0] = sub_100004FDC();
  v4 = *(v17[0] - 8);
  v5 = *(v4 + 64);
  __chkstk_darwin(v17[0]);
  v7 = v17 - ((v6 + 15) & 0xFFFFFFFFFFFFFFF0);
  v8 = sub_10000501C();
  v9 = *(*(v8 - 8) + 64);
  __chkstk_darwin(v8 - 8);
  v10 = sub_10000509C();
  v11 = *(*(v10 - 8) + 64);
  __chkstk_darwin(v10 - 8);
  v12 = sub_100004FEC();
  v13 = *(v12 - 8);
  v14 = *(v13 + 64);
  __chkstk_darwin(v12);
  v15 = sub_100004FAC();
  sub_1000047B4(v15, qword_10000D1A0);
  sub_10000477C(v15, qword_10000D1A0);
  sub_10000503C();
  sub_10000500C();
  (*(v4 + 104))(v7, enum case for LocalizedStringResource.BundleDescription.main(_:), v17[0]);
  sub_100004FFC();
  (*(v13 + 56))(v3, 1, 1, v12);
  return sub_100004F9C();
}

void sub_100001FB4(uint64_t a1@<X8>)
{
  v41 = a1;
  v1 = sub_100001790(&qword_10000C168, &qword_100005D00);
  v2 = *(*(v1 - 8) + 64);
  __chkstk_darwin(v1 - 8);
  v40 = v35 - v3;
  v4 = sub_100001790(&qword_10000C140, &qword_100005CF0);
  v5 = *(*(v4 - 8) + 64);
  __chkstk_darwin(v4 - 8);
  v39 = v35 - v6;
  v7 = sub_100004FDC();
  v8 = *(v7 - 8);
  v9 = *(v8 + 64);
  __chkstk_darwin(v7);
  v11 = v35 - ((v10 + 15) & 0xFFFFFFFFFFFFFFF0);
  v12 = sub_10000501C();
  v13 = *(*(v12 - 8) + 64);
  __chkstk_darwin(v12 - 8);
  v14 = sub_10000509C();
  v15 = *(*(v14 - 8) + 64);
  __chkstk_darwin(v14 - 8);
  v16 = sub_10000507C();
  v17 = *(*(v16 - 8) + 64);
  __chkstk_darwin(v16 - 8);
  v38 = sub_100004FEC();
  v18 = *(v38 - 8);
  v19 = *(v18 + 64);
  v20 = __chkstk_darwin(v38);
  v37 = v35 - ((v21 + 15) & 0xFFFFFFFFFFFFFFF0);
  __chkstk_darwin(v20);
  v42 = v35 - v22;
  v23 = sub_1000050AC();
  v24 = MGGetStringAnswer();

  if (v24)
  {
    v25 = sub_1000050BC();
    v27 = v26;

    sub_10000506C();
    v43._countAndFlagsBits = 0;
    v43._object = 0xE000000000000000;
    sub_10000505C(v43);
    v44._countAndFlagsBits = v25;
    v44._object = v27;
    sub_10000504C(v44);

    v45._countAndFlagsBits = 0xD00000000000001ELL;
    v45._object = 0x8000000100005EC0;
    sub_10000505C(v45);
    sub_10000508C();
    sub_10000500C();
    v28 = enum case for LocalizedStringResource.BundleDescription.main(_:);
    v29 = *(v8 + 104);
    v29(v11, enum case for LocalizedStringResource.BundleDescription.main(_:), v7);
    sub_100004FFC();
    sub_100001790(&qword_10000C170, &qword_100005D08);
    v35[1] = *(v18 + 72);
    v30 = (*(v18 + 80) + 32) & ~*(v18 + 80);
    v36 = swift_allocObject();
    *(v36 + 16) = xmmword_1000054D0;
    sub_10000503C();
    sub_10000500C();
    v29(v11, v28, v7);
    sub_100004FFC();
    sub_10000503C();
    sub_10000500C();
    v29(v11, v28, v7);
    sub_100004FFC();
    v31 = v38;
    (*(v18 + 16))(v37, v42, v38);
    sub_10000503C();
    sub_10000500C();
    v29(v11, v28, v7);
    v32 = v39;
    sub_100004FFC();
    (*(v18 + 56))(v32, 0, 1, v31);
    v33 = v40;
    sub_100004EEC();
    v34 = sub_100004EFC();
    (*(*(v34 - 8) + 56))(v33, 0, 1, v34);
    sub_100004F0C();
    (*(v18 + 8))(v42, v31);
  }

  else
  {
    __break(1u);
  }
}

uint64_t sub_10000261C(uint64_t a1)
{
  *(v1 + 16) = a1;
  v2 = swift_task_alloc();
  *(v1 + 24) = v2;
  *v2 = v1;
  v2[1] = sub_1000026C0;

  return sub_100004BF0();
}

uint64_t sub_1000026C0(uint64_t a1)
{
  v4 = *v2;
  v5 = *(*v2 + 24);
  v6 = *v2;

  if (!v1)
  {
    **(v4 + 16) = a1;
  }

  v7 = *(v6 + 8);

  return v7();
}

unint64_t sub_1000027D8()
{
  result = qword_10000C088;
  if (!qword_10000C088)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C088);
  }

  return result;
}

unint64_t sub_100002830()
{
  result = qword_10000C090;
  if (!qword_10000C090)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C090);
  }

  return result;
}

unint64_t sub_100002884()
{
  result = qword_10000C098;
  if (!qword_10000C098)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C098);
  }

  return result;
}

uint64_t sub_1000028D8(uint64_t a1)
{
  v3 = swift_task_alloc();
  *(v1 + 16) = v3;
  *v3 = v1;
  v3[1] = sub_100004CE8;

  return sub_10000496C(a1);
}

uint64_t sub_100002980(uint64_t a1)
{
  *(v1 + 16) = a1;
  v2 = swift_task_alloc();
  *(v1 + 24) = v2;
  *v2 = v1;
  v2[1] = sub_100004CC8;

  return sub_100004818();
}

uint64_t sub_100002A24(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4)
{
  v9 = async function pointer to EntityQuery.displayRepresentations(for:requestedComponents:)[1];
  v10 = swift_task_alloc();
  *(v4 + 16) = v10;
  *v10 = v4;
  v10[1] = sub_100002AE4;

  return EntityQuery.displayRepresentations(for:requestedComponents:)(a1, a2, a3, a4);
}

uint64_t sub_100002AE4(uint64_t a1)
{
  v4 = *(*v2 + 16);
  v8 = *v2;

  v6 = *(v8 + 8);
  if (!v1)
  {
    v5 = a1;
  }

  return v6(v5);
}

unint64_t sub_100002BE8()
{
  result = qword_10000C0A0;
  if (!qword_10000C0A0)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0A0);
  }

  return result;
}

unint64_t sub_100002C40()
{
  result = qword_10000C0A8;
  if (!qword_10000C0A8)
  {
    sub_100001BCC(&qword_10000C0B0, qword_1000056D0);
    sub_1000017D8();
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0A8);
  }

  return result;
}

uint64_t sub_100002CC4(uint64_t a1, uint64_t a2)
{
  v5 = async function pointer to EntityQuery.results()[1];
  v6 = swift_task_alloc();
  *(v2 + 16) = v6;
  v7 = sub_100001C78();
  *v6 = v2;
  v6[1] = sub_100002D78;

  return EntityQuery.results()(a1, a2, v7);
}

uint64_t sub_100002D78()
{
  v1 = *(*v0 + 16);
  v4 = *v0;

  v2 = *(v4 + 8);

  return v2();
}

uint64_t sub_100002E6C(uint64_t a1)
{
  *(v1 + 16) = a1;
  v2 = swift_task_alloc();
  *(v1 + 24) = v2;
  *v2 = v1;
  v2[1] = sub_100002F10;

  return sub_100004B10();
}

uint64_t sub_100002F10(char a1)
{
  v3 = *(*v1 + 24);
  v4 = *(*v1 + 16);
  v7 = *v1;

  *v4 = a1 & 1;
  v5 = *(v7 + 8);

  return v5();
}

unint64_t sub_100003030()
{
  result = qword_10000C0B8;
  if (!qword_10000C0B8)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0B8);
  }

  return result;
}

uint64_t sub_100003084(uint64_t a1)
{
  v2 = sub_100002884();

  return EntityURLRepresentation.init(stringLiteral:)(0xD00000000000003DLL, 0x8000000100005FB0, a1, v2);
}

unint64_t sub_1000030E8()
{
  result = qword_10000C0C0;
  if (!qword_10000C0C0)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0C0);
  }

  return result;
}

unint64_t sub_100003140()
{
  result = qword_10000C0C8;
  if (!qword_10000C0C8)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0C8);
  }

  return result;
}

unint64_t sub_100003198()
{
  result = qword_10000C0D0;
  if (!qword_10000C0D0)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0D0);
  }

  return result;
}

unint64_t sub_1000031F4()
{
  result = qword_10000C0D8;
  if (!qword_10000C0D8)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0D8);
  }

  return result;
}

uint64_t sub_100003248()
{
  sub_100004728();
  v1 = sub_100004F3C();
  v2 = *(v0 + 8);

  return v2(v1);
}

unint64_t sub_1000032B8()
{
  result = qword_10000C0E0;
  if (!qword_10000C0E0)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0E0);
  }

  return result;
}

unint64_t sub_100003310()
{
  result = qword_10000C0E8;
  if (!qword_10000C0E8)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0E8);
  }

  return result;
}

unint64_t sub_10000336C()
{
  result = qword_10000C0F0;
  if (!qword_10000C0F0)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0F0);
  }

  return result;
}

unint64_t sub_1000033C4()
{
  result = qword_10000C0F8;
  if (!qword_10000C0F8)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C0F8);
  }

  return result;
}

uint64_t sub_100003498(unint64_t *a1, uint64_t *a2, uint64_t *a3)
{
  result = *a1;
  if (!result)
  {
    sub_100001BCC(a2, a3);
    result = swift_getWitnessTable();
    atomic_store(result, a1);
  }

  return result;
}

uint64_t sub_1000034E0(uint64_t a1)
{
  v2 = sub_100002884();

  return static AppEntity.defaultResolverSpecification.getter(a1, v2);
}

unint64_t sub_100003530()
{
  result = qword_10000C110;
  if (!qword_10000C110)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C110);
  }

  return result;
}

uint64_t sub_100003588(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4)
{
  v9 = async function pointer to InstanceDisplayRepresentable.displayRepresentation(with:)[1];
  v10 = swift_task_alloc();
  *(v4 + 16) = v10;
  *v10 = v4;
  v10[1] = sub_100002D78;

  return InstanceDisplayRepresentable.displayRepresentation(with:)(a1, a2, a3, a4);
}

uint64_t sub_10000364C(uint64_t a1)
{
  v2 = sub_10000336C();

  return InstanceDisplayRepresentable.localizedStringResource.getter(a1, v2);
}

unint64_t sub_10000369C()
{
  result = qword_10000C118;
  if (!qword_10000C118)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C118);
  }

  return result;
}

uint64_t sub_1000036F4()
{
  v0 = sub_100004FDC();
  v1 = *(v0 - 8);
  v2 = *(v1 + 64);
  __chkstk_darwin(v0);
  v4 = &v11 - ((v3 + 15) & 0xFFFFFFFFFFFFFFF0);
  v5 = sub_10000501C();
  v6 = *(*(v5 - 8) + 64);
  __chkstk_darwin(v5 - 8);
  v7 = sub_10000509C();
  v8 = *(*(v7 - 8) + 64);
  __chkstk_darwin(v7 - 8);
  v9 = sub_100004FEC();
  sub_1000047B4(v9, qword_10000D1B8);
  sub_10000477C(v9, qword_10000D1B8);
  sub_10000503C();
  sub_10000500C();
  (*(v1 + 104))(v4, enum case for LocalizedStringResource.BundleDescription.main(_:), v0);
  return sub_100004FFC();
}

void (*sub_100003928(uint64_t *a1))(void *a1)
{
  if (&_swift_coroFrameAlloc)
  {
    v3 = swift_coroFrameAlloc();
  }

  else
  {
    v3 = malloc(0x28uLL);
  }

  *a1 = v3;
  v4 = *v1;
  *(v3 + 32) = sub_100004DCC();
  return sub_10000399C;
}

void sub_10000399C(void *a1)
{
  v1 = *a1;
  (*(*a1 + 32))(*a1, 0);

  free(v1);
}

uint64_t sub_1000039E8(uint64_t a1, uint64_t a2)
{
  v4 = sub_100004680();
  v5 = sub_100004728();

  return static URLRepresentableIntent<>.urlRepresentation.getter(a1, v4, a2, v5);
}

uint64_t sub_100003A58()
{
  v0 = qword_10000C058;

  return v0;
}

unint64_t sub_100003A98()
{
  result = qword_10000C120;
  if (!qword_10000C120)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C120);
  }

  return result;
}

unint64_t sub_100003AF0()
{
  result = qword_10000C128;
  if (!qword_10000C128)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C128);
  }

  return result;
}

unint64_t sub_100003B48()
{
  result = qword_10000C130;
  if (!qword_10000C130)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C130);
  }

  return result;
}

uint64_t sub_100003C24@<X0>(void *a1@<X2>, uint64_t (*a2)(void)@<X3>, uint64_t a3@<X4>, uint64_t a4@<X8>)
{
  if (*a1 == -1)
  {
    v6 = a2(0);
  }

  else
  {
    swift_once();
    v6 = a2(0);
  }

  v7 = v6;
  v8 = sub_10000477C(v6, a3);
  v9 = *(*(v7 - 8) + 16);

  return v9(a4, v8, v7);
}

uint64_t sub_100003CE4(uint64_t a1, uint64_t a2)
{
  v4 = async function pointer to URLRepresentableIntent<>.perform()[1];
  v5 = swift_task_alloc();
  *(v2 + 16) = v5;
  v6 = sub_100004680();
  v7 = sub_1000046D4();
  v8 = sub_100004728();
  *v5 = v2;
  v5[1] = sub_100003DB0;

  return URLRepresentableIntent<>.perform()(a2, v6, v7, v8);
}

uint64_t sub_100003DB0()
{
  v2 = *(*v1 + 16);
  v3 = *v1;

  if (v0)
  {
    v5 = *(v3 + 8);

    return v5();
  }

  return result;
}

uint64_t sub_100003EB8@<X0>(uint64_t *a1@<X8>)
{
  result = sub_10000418C();
  *a1 = result;
  return result;
}

uint64_t sub_100003EE0(uint64_t a1)
{
  v2 = sub_10000173C();

  return static AppIntent.persistentIdentifier.getter(a1, v2);
}

uint64_t getEnumTagSinglePayload for StorageSettingsDynamicDestination(unsigned int *a1, int a2)
{
  if (!a2)
  {
    return 0;
  }

  if ((a2 + 1) >= 0x10000)
  {
    v2 = 4;
  }

  else
  {
    v2 = 2;
  }

  if ((a2 + 1) < 0x100)
  {
    v3 = 1;
  }

  else
  {
    v3 = v2;
  }

  if (v3 == 4)
  {
    return *a1;
  }

  if (v3 == 2)
  {
    return *a1;
  }

  return *a1;
}

_WORD *storeEnumTagSinglePayload for StorageSettingsDynamicDestination(_WORD *result, int a2, int a3)
{
  if ((a3 + 1) >= 0x10000)
  {
    v3 = 4;
  }

  else
  {
    v3 = 2;
  }

  if ((a3 + 1) < 0x100)
  {
    v4 = 1;
  }

  else
  {
    v4 = v3;
  }

  if (!a3)
  {
    v4 = 0;
  }

  if (a2)
  {
    if (v4 > 1)
    {
      if (v4 == 2)
      {
        *result = a2;
      }

      else
      {
        *result = a2;
      }
    }

    else if (v4)
    {
      *result = a2;
    }
  }

  else if (v4 > 1)
  {
    if (v4 == 2)
    {
      *result = 0;
    }

    else
    {
      *result = 0;
    }
  }

  else if (v4)
  {
    *result = 0;
  }

  return result;
}

char *sub_100004040(char *a1, int64_t a2, char a3)
{
  result = sub_1000040A0(a1, a2, a3, *v3, &unk_10000C180, &qword_100005D28);
  *v3 = result;
  return result;
}

char *sub_100004070(char *a1, int64_t a2, char a3)
{
  result = sub_1000040A0(a1, a2, a3, *v3, &qword_10000C178, &qword_100005D18);
  *v3 = result;
  return result;
}

char *sub_1000040A0(char *result, int64_t a2, char a3, char *a4, uint64_t *a5, uint64_t *a6)
{
  v7 = result;
  if (a3)
  {
    v8 = *(a4 + 3);
    v9 = v8 >> 1;
    if ((v8 >> 1) < a2)
    {
      if (v9 + 0x4000000000000000 < 0)
      {
        __break(1u);
        return result;
      }

      v9 = v8 & 0xFFFFFFFFFFFFFFFELL;
      if ((v8 & 0xFFFFFFFFFFFFFFFELL) <= a2)
      {
        v9 = a2;
      }
    }
  }

  else
  {
    v9 = a2;
  }

  v10 = *(a4 + 2);
  if (v9 <= v10)
  {
    v11 = *(a4 + 2);
  }

  else
  {
    v11 = v9;
  }

  if (v11)
  {
    sub_100001790(a5, a6);
    v12 = swift_allocObject();
    v13 = j__malloc_size(v12);
    *(v12 + 2) = v10;
    *(v12 + 3) = 2 * v13 - 64;
  }

  else
  {
    v12 = &_swiftEmptyArrayStorage;
  }

  v14 = v12 + 32;
  v15 = a4 + 32;
  if (v7)
  {
    if (v12 != a4 || v14 >= &v15[v10])
    {
      memmove(v14, v15, v10);
    }

    *(a4 + 2) = 0;
  }

  else
  {
    memcpy(v14, v15, v10);
  }

  return v12;
}

uint64_t sub_10000418C()
{
  v0 = sub_100004F7C();
  v34 = *(v0 - 8);
  v35 = v0;
  v1 = *(v34 + 64);
  (__chkstk_darwin)();
  v33 = &v27 - ((v2 + 15) & 0xFFFFFFFFFFFFFFF0);
  v3 = *(*(sub_100001790(&qword_10000C138, &qword_100005CE8) - 8) + 64);
  v4 = (__chkstk_darwin)();
  v32 = &v27 - ((v5 + 15) & 0xFFFFFFFFFFFFFFF0);
  __chkstk_darwin(v4);
  v31 = &v27 - v6;
  v7 = sub_100001790(&qword_10000C140, &qword_100005CF0);
  v8 = (*(*(v7 - 8) + 64) + 15) & 0xFFFFFFFFFFFFFFF0;
  __chkstk_darwin(v7 - 8);
  v30 = &v27 - v9;
  v10 = sub_100004FDC();
  v11 = *(v10 - 8);
  v12 = *(v11 + 64);
  __chkstk_darwin(v10);
  v14 = &v27 - ((v13 + 15) & 0xFFFFFFFFFFFFFFF0);
  v15 = sub_10000501C();
  v16 = *(*(v15 - 8) + 64);
  __chkstk_darwin(v15 - 8);
  v17 = sub_10000509C();
  v18 = *(*(v17 - 8) + 64);
  __chkstk_darwin(v17 - 8);
  v28 = sub_100004FEC();
  v19 = *(v28 - 8);
  v20 = *(v19 + 64);
  __chkstk_darwin(v28);
  v29 = sub_100001790(&qword_10000C148, &qword_100005CF8);
  sub_10000503C();
  sub_10000500C();
  v21 = enum case for LocalizedStringResource.BundleDescription.main(_:);
  v22 = *(v11 + 104);
  v22(v14, enum case for LocalizedStringResource.BundleDescription.main(_:), v10);
  sub_100004FFC();
  sub_10000503C();
  sub_10000500C();
  v22(v14, v21, v10);
  v23 = v30;
  sub_100004FFC();
  (*(v19 + 56))(v23, 0, 1, v28);
  v36 = 1;
  v24 = sub_100004DBC();
  v25 = *(*(v24 - 8) + 56);
  v25(v31, 1, 1, v24);
  v25(v32, 1, 1, v24);
  (*(v34 + 104))(v33, enum case for InputConnectionBehavior.default(_:), v35);
  sub_100002884();
  return sub_100004DFC();
}

unint64_t sub_100004680()
{
  result = qword_10000C150;
  if (!qword_10000C150)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C150);
  }

  return result;
}

unint64_t sub_1000046D4()
{
  result = qword_10000C158;
  if (!qword_10000C158)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C158);
  }

  return result;
}

unint64_t sub_100004728()
{
  result = qword_10000C160;
  if (!qword_10000C160)
  {
    result = swift_getWitnessTable();
    atomic_store(result, &qword_10000C160);
  }

  return result;
}

uint64_t sub_10000477C(uint64_t a1, uint64_t a2)
{
  if ((*(*(a1 - 8) + 82) & 2) != 0)
  {
    return *a2;
  }

  return a2;
}

uint64_t *sub_1000047B4(uint64_t a1, uint64_t *a2)
{
  v3 = *(a1 - 8);
  if ((*(v3 + 80) & 0x20000) != 0)
  {
    v4 = *(v3 + 64);
    v5 = *(v3 + 80);
    v6 = swift_slowAlloc();
    *a2 = v6;
    return v6;
  }

  return a2;
}

uint64_t sub_100004834()
{
  sub_100004070(0, 1, 1);
  v2 = _swiftEmptyArrayStorage[2];
  v1 = _swiftEmptyArrayStorage[3];
  v3 = v2 + 1;
  if (v2 >= v1 >> 1)
  {
    sub_100004070((v1 > 1), v2 + 1, 1);
  }

  _swiftEmptyArrayStorage[2] = v3;
  sub_100004040(0, v3, 0);
  v4 = _swiftEmptyArrayStorage[2];
  do
  {
    v5 = _swiftEmptyArrayStorage[3];
    v6 = v4 + 1;
    if (v4 >= v5 >> 1)
    {
      sub_100004040((v5 > 1), v4 + 1, 1);
    }

    _swiftEmptyArrayStorage[2] = v6;
    v4 = v6;
    --v3;
  }

  while (v3);

  v7 = *(v0 + 8);

  return v7(_swiftEmptyArrayStorage);
}

uint64_t sub_10000498C()
{
  v1 = *(*(v0 + 16) + 16);
  if (v1)
  {
    do
    {
      if ((swift_isUniquelyReferenced_nonNull_native() & 1) == 0)
      {
        sub_100004070(0, _swiftEmptyArrayStorage[2] + 1, 1);
      }

      v3 = _swiftEmptyArrayStorage[2];
      v2 = _swiftEmptyArrayStorage[3];
      v4 = v3 + 1;
      if (v3 >= v2 >> 1)
      {
        sub_100004070((v2 > 1), v3 + 1, 1);
      }

      _swiftEmptyArrayStorage[2] = v4;
      --v1;
    }

    while (v1);
  }

  else
  {
    v4 = _swiftEmptyArrayStorage[2];
    if (!v4)
    {
      goto LABEL_13;
    }
  }

  sub_100004040(0, v4, 0);
  v5 = _swiftEmptyArrayStorage[2];
  do
  {
    v6 = _swiftEmptyArrayStorage[3];
    v7 = v5 + 1;
    if (v5 >= v6 >> 1)
    {
      sub_100004040((v6 > 1), v5 + 1, 1);
    }

    _swiftEmptyArrayStorage[2] = v7;
    v5 = v7;
    --v4;
  }

  while (v4);
LABEL_13:

  v8 = *(v0 + 8);

  return v8(_swiftEmptyArrayStorage);
}

uint64_t sub_100004B2C()
{
  sub_100004070(0, 1, 1);
  v2 = _swiftEmptyArrayStorage[2];
  v1 = _swiftEmptyArrayStorage[3];
  v3 = v2 + 1;
  if (v2 >= v1 >> 1)
  {
    sub_100004070((v1 > 1), v2 + 1, 1);
  }

  _swiftEmptyArrayStorage[2] = v3;

  v4 = *(v0 + 8);

  return v4(0);
}

uint64_t sub_100004C0C()
{
  sub_100004040(0, 1, 0);
  v2 = *(&_swiftEmptyArrayStorage + 2);
  v1 = *(&_swiftEmptyArrayStorage + 3);
  v3 = v2 + 1;
  if (v2 >= v1 >> 1)
  {
    sub_100004040((v1 > 1), v2 + 1, 1);
  }

  *(&_swiftEmptyArrayStorage + 2) = v3;
  v4 = *(v0 + 8);

  return v4();
}