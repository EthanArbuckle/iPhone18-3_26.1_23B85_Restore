@interface OctaneListener
+ (void)start;
- (BOOL)listener:(id)listener shouldAcceptNewConnection:(id)connection;
@end

@implementation OctaneListener

+ (void)start
{
  if (qword_1003D4AD0 != -1)
  {
    sub_1002D19EC();
  }
}

- (BOOL)listener:(id)listener shouldAcceptNewConnection:(id)connection
{
  listenerCopy = listener;
  connectionCopy = connection;
  processIdentifier = [connectionCopy processIdentifier];
  sk_clientID = [connectionCopy sk_clientID];
  userInfo = [connectionCopy userInfo];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) == 0)
    {
      v7 = [userInfo mutableCopy];

      userInfo = v7;
    }
  }

  if (!userInfo)
  {
    userInfo = objc_opt_new();
  }

  v8 = +[NSUUID UUID];
  uUIDString = [v8 UUIDString];

  [userInfo setObject:uUIDString forKeyedSubscript:@"octaneClientUUID"];
  [connectionCopy setUserInfo:userInfo];
  if (qword_1003D4B20 != -1)
  {
    sub_1002D1A00();
  }

  v9 = qword_1003D4B10;
  if (os_log_type_enabled(qword_1003D4B10, OS_LOG_TYPE_INFO))
  {
    v10 = v9;
    *buf = 138543874;
    v39 = objc_opt_class();
    v40 = 1024;
    v41 = processIdentifier;
    v42 = 2114;
    v43 = uUIDString;
    _os_log_impl(&_mh_execute_header, v10, OS_LOG_TYPE_INFO, "[%{public}@]: Got connection from pid: %i (%{public}@)", buf, 0x1Cu);
  }

  v11 = [NSXPCInterface interfaceWithProtocol:&OBJC_PROTOCOL___StoreKitTestingServiceProtocol];
  v12 = objc_opt_class();
  v13 = objc_opt_class();
  v14 = objc_opt_class();
  v15 = objc_opt_class();
  v16 = [NSSet setWithObjects:v12, v13, v14, v15, objc_opt_class(), 0];
  [v11 setClasses:v16 forSelector:"sendTestPingbackForBundleID:completion:" argumentIndex:0 ofReply:1];

  v17 = objc_opt_class();
  v18 = objc_opt_class();
  v19 = objc_opt_class();
  v20 = [NSSet setWithObjects:v17, v18, v19, objc_opt_class(), 0];
  [v11 setClasses:v20 forSelector:"showMessage:forBundleID:" argumentIndex:0 ofReply:0];

  [connectionCopy setExportedInterface:v11];
  v21 = +[StoreKitTestingService defaultService];
  [connectionCopy setExportedObject:v21];

  objc_initWeak(buf, self);
  v35[0] = _NSConcreteStackBlock;
  v35[1] = 3221225472;
  v35[2] = sub_100077C38;
  v35[3] = &unk_1003830E0;
  objc_copyWeak(&v36, buf);
  v37 = processIdentifier;
  [connectionCopy setInterruptionHandler:v35];
  v30[0] = _NSConcreteStackBlock;
  v30[1] = 3221225472;
  v30[2] = sub_100077CB4;
  v30[3] = &unk_100383108;
  objc_copyWeak(&v33, buf);
  v34 = processIdentifier;
  v22 = uUIDString;
  v31 = v22;
  v23 = sk_clientID;
  v32 = v23;
  [connectionCopy setInvalidationHandler:v30];
  [connectionCopy resume];

  objc_destroyWeak(&v33);
  objc_destroyWeak(&v36);
  objc_destroyWeak(buf);

  return 1;
}

@end