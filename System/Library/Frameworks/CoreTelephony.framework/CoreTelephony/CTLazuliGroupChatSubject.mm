@interface CTLazuliGroupChatSubject
- (BOOL)isEqual:(id)a3;
- (BOOL)isEqualToCTLazuliGroupChatSubject:(id)a3;
- (CTLazuliGroupChatSubject)initWithCoder:(id)a3;
- (CTLazuliGroupChatSubject)initWithReflection:(const void *)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
@end

@implementation CTLazuliGroupChatSubject

- (id)description
{
  v3 = [MEMORY[0x1E696AD60] stringWithFormat:@"<%@", objc_opt_class()];
  v4 = [(CTLazuliGroupChatSubject *)self displayString];
  [v3 appendFormat:@", displayString = %@", v4];

  [v3 appendString:@">"];

  return v3;
}

- (BOOL)isEqualToCTLazuliGroupChatSubject:(id)a3
{
  v4 = a3;
  v5 = [(CTLazuliGroupChatSubject *)self displayString];
  v6 = [v4 displayString];
  if (v5 == v6)
  {
    v9 = 1;
  }

  else
  {
    v7 = [(CTLazuliGroupChatSubject *)self displayString];
    v8 = [v4 displayString];
    v9 = [v7 isEqualToString:v8];
  }

  return v9;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  v5 = v4;
  if (v4 == self)
  {
    v6 = 1;
  }

  else
  {
    v6 = v4 && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0) && [(CTLazuliGroupChatSubject *)self isEqualToCTLazuliGroupChatSubject:v5];
  }

  return v6;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [CTLazuliGroupChatSubject allocWithZone:?];
  v6 = [(NSString *)self->_displayString copyWithZone:a3];
  [(CTLazuliGroupChatSubject *)v5 setDisplayString:v6];

  return v5;
}

- (CTLazuliGroupChatSubject)initWithCoder:(id)a3
{
  v4 = a3;
  v9.receiver = self;
  v9.super_class = CTLazuliGroupChatSubject;
  v5 = [(CTLazuliGroupChatSubject *)&v9 init];
  if (v5)
  {
    v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"kDisplayStringKey"];
    displayString = v5->_displayString;
    v5->_displayString = v6;
  }

  return v5;
}

- (CTLazuliGroupChatSubject)initWithReflection:(const void *)a3
{
  v9.receiver = self;
  v9.super_class = CTLazuliGroupChatSubject;
  v4 = [(CTLazuliGroupChatSubject *)&v9 init];
  if (v4)
  {
    if (*(a3 + 23) >= 0)
    {
      v5 = a3;
    }

    else
    {
      v5 = *a3;
    }

    v6 = [MEMORY[0x1E696AEC0] stringWithUTF8String:v5];
    displayString = v4->_displayString;
    v4->_displayString = v6;
  }

  return v4;
}

@end