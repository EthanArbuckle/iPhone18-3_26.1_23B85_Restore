...
- (id)_contentForAttachmentPassingTest:(id)test;
...
- (id)_scaleImages:(id)images toFit:(unint64_t)fit resultingSize:(unint64_t *)size;
...
- (id)_scaleImages:(id)images toFit:(unint64_t)fit resultingSize:(unint64_t *)size
{
  ...
  imagesCopy = images;
  attachmentContext = [(MFOutgoingMessageDelivery *)self attachmentContext];
  ...
  if (v11)
  {
    ...
    do
    {
      for (i = 0; i != v11; ++i)
      {
        ...
        v15 = *(*(&v23 + 1) + 8 * i);
        attachmentContext2 = [(MFOutgoingMessageDelivery *)self attachmentContext];
        contextID = [attachmentContext2 contextID];
        v18 = [v15 scaledImageToFit:fit saveScaledImage:1 attachmentContextID:contextID];
...
{
  imagesCopy = images;
  currentMessageLimit = [MEMORY[0x1E69B15B8] currentMessageLimit];
  v11 = 0;
  v6 = [(MFMailDropMailDeliveryUI *)self _scaleImages:imagesCopy toFit:2 resultingSize:&v11];
  v7 = v11;
  if (v11 > currentMessageLimit)
  {
  ...
  if (v7 > currentMessageLimit)
  {
    v9 = EMLogMailDrop();
    if (os_log_type_enabled(v9, OS_LOG_TYPE_ERROR))
    {
      [(MFMailDropMailDeliveryUI *)&v11 scaledImages:currentMessageLimit, v9];
    }
  }
...
- (id)deliverSynchronouslyWithCompletion:(id)completion
{
  ...
  v6 = [(MFMailDropMailDelivery *)&v20 deliverSynchronouslyWithCompletion:completionCopy];
  if ([v6 status])
  {
...
- (id)_headersByAddingMailDropHeaders:(id)headers
{
  ...
  headersCopy = headers;
  v29 = [headersCopy mutableCopy];
  v4 = objc_alloc(MEMORY[0x1E695DF70]);
  attachments = [(MFMailDropMailDelivery *)self attachments];
  v30 = [v4 initWithCapacity:{objc_msgSend(attachments, "count")}];

  imageArchiveMetadata = [(MFMailDropMailDelivery *)self imageArchiveMetadata];

  if (imageArchiveMetadata)
  {
    imageArchiveMetadata2 = [(MFMailDropMailDelivery *)self imageArchiveMetadata];
    directUrlString = [imageArchiveMetadata2 directUrlString];
    [v29 setHeader:directUrlString forKey:*MEMORY[0x1E699B100]];
  }
  ...
  if (v9)
  {
    ...
    do
    {
      for (i = 0; i != v9; ++i)
      {
        ...
        v11 = *(*(&v34 + 1) + 8 * i);
        mailDropMetadata = [v11 mailDropMetadata];
        directUrl = [mailDropMetadata directUrl];
        if (directUrl)
        {
          mailDropMetadata2 = [v11 mailDropMetadata];
          isPhotoArchive = [mailDropMetadata2 isPhotoArchive];

          if ((isPhotoArchive & 1) == 0)
          {
            v16 = MEMORY[0x1E696AEC0];
            mailDropMetadata3 = [v11 mailDropMetadata];
            directUrlString2 = [mailDropMetadata3 directUrlString];
            fileName = [v11 fileName];
            mimeType = [v11 mimeType];
            decodedFileSize = [v11 decodedFileSize];
            mailDropMetadata4 = [v11 mailDropMetadata];
            expiration = [mailDropMetadata4 expiration];
            [expiration timeIntervalSince1970];
            v25 = [v16 stringWithFormat:@"%@ filename=%@; mimeType=%@; filesize=%lu; expiration-date=%f", directUrlString2, fileName, mimeType, decodedFileSize, v24];;
...
- (id)_mailDropAttachmentHTMLStringForAttachment:(id)attachment
{
  ...
  decodedFileSize = [attachmentCopy decodedFileSize];
  v5 = objc_alloc(MEMORY[0x1E696AEC0]);
  directUrlString = [mailDropMetadata directUrlString];
  expiration = [mailDropMetadata expiration];
  [expiration timeIntervalSince1970];
  v8 = v7;
  wrappedUrlString = [mailDropMetadata wrappedUrlString];
  v10 = [MEMORY[0x1E696AAE8] bundleForClass:objc_opt_class()];
  v11 = [v10 localizedStringForKey:@"MAIL_DROP_DOWNLOADABLE" value:&stru_1F3CF3758 table:@"Main"];
  v12 = [MEMORY[0x1E696AEC0] mf_stringRepresentationForBytes:decodedFileSize];
  v13 = [v5 initWithFormat:*MEMORY[0x1E699A780], directUrlString, fileName, decodedFileSize, v8 * 1000.0, wrappedUrlString, v11, fileName, v12];
...
- (id)_contentForAttachmentPassingTest:(id)test
{
  ...
  v6 = testCopy;
  v14 = v6;
  v7 = [attachments indexOfObjectPassingTest:v13];
  ...
  {
    attachments2 = [(MFMailDropMailDelivery *)self attachments];
    v10 = [attachments2 objectAtIndex:v7];
...
- (BOOL)updateMessageWithAttachmentsSynchronously
{
  ...
  else
  {
    ...
    if (*(&self->super.super.super.isa + v9))
    {
      ...
      string = [*(&self->super.super.super.isa + v9) string];
      [v8 setHtmlBody:string];
...
LABEL_15:
  ...
  *(&self->super.super.super.isa + v19) = v23;
  ...
  rewrittenMessageContent = [(MFPlaceholderMessageRewriter *)self->_rewriter rewrittenMessageContent];
  _mailDropBannerHTMLString = [(MFMailDropMailDeliveryUI *)self _mailDropBannerHTMLString];
  richtextContent = [rewrittenMessageContent richtextContent];
  plaintextAlternative = [richtextContent plaintextAlternative];

  if (plaintextAlternative)
  {
    plaintextAlternative2 = [richtextContent plaintextAlternative];
    v30 = *MEMORY[0x1E69B16E8];
    v31 = *(&self->super.super.super.isa + v30);
    *(&self->super.super.super.isa + v30) = plaintextAlternative2;
  }

  mixedContent = [richtextContent mixedContent];
  multipartContent = [richtextContent multipartContent];
  ...
  if (multipartContent)
  {
    ...
    v37 = [_mailDropBannerHTMLString stringByAppendingString:htmlBody];
  ...
  {
    multipartContent2 = [MEMORY[0x1E699B288] mf_utf8HTMLStringWithString:_mailDropBannerHTMLString];
    v54 = multipartContent2;
    htmlBody = [MEMORY[0x1E695DEC8] arrayWithObjects:&v54 count:1];
    [htmlBody arrayByAddingObjectsFromArray:mixedContent];
    mixedContent = v37 = mixedContent;
  }

  v41 = *MEMORY[0x1E69B16E0];
  v42 = *(&self->super.super.super.isa + v41);
  *(&self->super.super.super.isa + v41) = mixedContent;
...
- (id)_mailDropBannerHTMLString
{
  ...
  if (!imageArchiveMetadata)
  {
    ...
    v35 = 0u;
    attachments = [(MFMailDropMailDelivery *)self attachments];
    v6 = [attachments countByEnumeratingWithState:&v34 objects:v38 count:16];
    if (v6)
    {
      v7 = *v35;
LABEL_4:
      ...
      imageArchiveMetadata = mailDropMetadata;
      if (!mailDropMetadata)
      {
        goto LABEL_11;
      }
...
LABEL_11:
      ...
      [currentHandler handleFailureInMethod:a2 object:self file:@"MFMailDropMailDeliveryUI.m" lineNumber:276 description:@"attempt to send a mail drop email without any mail drop attachments"];

      imageArchiveMetadata = 0;
    }
  }
  ...
  if ([imageArchiveMetadata isPhotoArchive])
  {
  ...
  expiration = [imageArchiveMetadata expiration];
  v18 = [v33 stringFromDate:expiration];
  ...
  wrappedUrlString = [imageArchiveMetadata wrappedUrlString];
  v23 = MEMORY[0x1E696AEC0];
  expiration2 = [imageArchiveMetadata expiration];
  [expiration2 timeIntervalSince1970];
  v26 = [v23 stringWithFormat:@"%f", v25 * 1000.0];
  directUrlString = [imageArchiveMetadata directUrlString];
  v28 = [MEMORY[0x1E696AEC0] stringWithFormat:@"%lu", objc_msgSend(imageArchiveMetadata, "fileSize")];
  v29 = [v21 stringWithFormat:@"<a href=%@ class=%@ data-expiration=%@ data-url=%@ data-size=%@>%@</a>", wrappedUrlString, *MEMORY[0x1E699A788], v26, directUrlString, v28, v20];
