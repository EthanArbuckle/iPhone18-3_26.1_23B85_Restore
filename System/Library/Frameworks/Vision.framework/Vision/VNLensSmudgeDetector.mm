...
- (id)observationsFromE5RTExecutionOutputs:(id)outputs forFunctionDescriptor:(id)descriptor originatingRequestSpecifier:(id)specifier options:(id)options error:(id *)error
{
  ...
  optionsCopy = options;
  v15 = [VNValidationUtilities requiredSessionInOptions:optionsCopy error:error];
  if (v15)
  {
    allOutputs = [descriptorCopy allOutputs];
    firstObject = [allOutputs firstObject];
    ...
    if (firstObject)
    {
      ...
      v19 = _Block_copy(&v27);
      name = [v18 name];
      v21 = [outputsCopy accessReadOnlyDataForName:name usingBlock:v19 error:error];

      if (v21)
      {
        v22 = [VNRequestSpecifier specifierForRequestClass:objc_opt_class() revision:1 error:error];
        LODWORD(v23) = *(v34 + 6);
        v24 = [VNLensSmudgeObservation observationForOriginatingRequestSpecifier:v22 confidence:error error:v23];
        v37[0] = v24;
...
+ (id)createE5RTFunctionDescriptorForConfigurationOptions:(id)options error:(id *)error
{
  [self E5RTVisionCoreComputeDeviceForConfigurationOptions:options error:?];
...
+ (id)modelURLForConfigurationOptions:(id)options error:(id *)error
{
  v4 = [MEMORY[0x1E69DF998] smudgeDetectionNetworkForModelVersion:1 error:error];
  v5 = [v4 URL];
...
+ (id)supportedComputeStageDevicesForOptions:(id)options error:(id *)error
{
  ...
  if ([VNLensSmudgeDetector supportsExecution:options])
  {
