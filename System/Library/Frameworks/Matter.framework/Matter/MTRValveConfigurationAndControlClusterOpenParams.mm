...
- (id)copyWithZone:(_NSZone *)zone
{
  ...
  openDuration = [(MTRValveConfigurationAndControlClusterOpenParams *)self openDuration];
  [(MTRValveConfigurationAndControlClusterOpenParams *)v4 setOpenDuration:openDuration];

  targetLevel = [(MTRValveConfigurationAndControlClusterOpenParams *)self targetLevel];
  [(MTRValveConfigurationAndControlClusterOpenParams *)v4 setTargetLevel:targetLevel];

  timedInvokeTimeoutMs = [(MTRValveConfigurationAndControlClusterOpenParams *)self timedInvokeTimeoutMs];
  [(MTRValveConfigurationAndControlClusterOpenParams *)v4 setTimedInvokeTimeoutMs:timedInvokeTimeoutMs];

  serverSideProcessingTimeout = [(MTRValveConfigurationAndControlClusterOpenParams *)self serverSideProcessingTimeout];
  [(MTRValveConfigurationAndControlClusterOpenParams *)v4 setServerSideProcessingTimeout:serverSideProcessingTimeout];
...
- (ChipError)_encodeToTLVReader:(PacketBufferTLVReader *)reader
{
  ...
  if (openDuration)
  {
    v26[0] = 1;
    v27 = 0;
    openDuration2 = [(MTRValveConfigurationAndControlClusterOpenParams *)self openDuration];

    if (openDuration2)
    {
      LODWORD(v27) = 0;
      BYTE4(v27) = 1;
      openDuration3 = [(MTRValveConfigurationAndControlClusterOpenParams *)self openDuration];
      LODWORD(v27) = [openDuration3 unsignedIntValue];
    }
  }
  ...
  {
    v28 = 1;
    targetLevel2 = [(MTRValveConfigurationAndControlClusterOpenParams *)self targetLevel];
    HIBYTE(v28) = [targetLevel2 unsignedCharValue];
  }
