...
+ (MPModelSongKind)kindWithVariants:(unint64_t)a3 options:(unint64_t)a4;
+ (id)identityKind;
- (BOOL)isEqual:(id)a3;
...
- (unint64_t)hash
{
  ...
  v4 = [(MPModelSongKind *)self variants];
  return v4 ^ [(MPModelSongKind *)self options]^ v3;
}

+ (MPModelSongKind)kindWithVariants:(unint64_t)a3 options:(unint64_t)a4
{
  v7 = [MEMORY[0x1E696AEC0] stringWithFormat:@"Song:v%lu:o%lu", a3, a4];
  v8 = objc_opt_class();
...
- (shared_ptr<mlcore::Predicate>)predicateWithBaseProperty:(void *)a3
{
  ...
  v5 = [(MPModelSongKind *)self variants];
  if ((v5 & 2) != 0)
  {
    LODWORD(v7) = 1032;
    std::vector<int>::push_back[abi:ne200100](v8, &v7);
...
- (void)applyToView:(shared_ptr<mlcore:(id)a4 :LibraryView>)a3 withContext:
{
  ...
  {
    v8 = [(__shared_weak_count *)v6 mediaLibrary];
    v9 = [MPMediaLibrarySystemFilters filtersForLibrary:v8];
    v10 = [v9 shouldExcludePurchaseHistoryContent];
