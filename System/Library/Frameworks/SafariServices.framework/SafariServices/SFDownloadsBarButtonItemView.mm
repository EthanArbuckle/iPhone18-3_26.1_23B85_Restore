...
- (SFDownloadsBarButtonItemView)initWithFrame:(CGRect)frame;
...
- (SFDownloadsBarButtonItemView)initWithFrame:(CGRect)frame
{
  ...
  v3 = [(SFDownloadsBarButtonItemView *)&v8 initWithFrame:frame.origin.x, frame.origin.y, frame.size.width, frame.size.height];
  if (v3)
  {
    safari_traitsAffectingTextAppearance = [MEMORY[0x1E69DD1B8] safari_traitsAffectingTextAppearance];
    v5 = [(SFDownloadsBarButtonItemView *)v3 registerForTraitChanges:safari_traitsAffectingTextAppearance withTarget:v3 action:sel__updateIcon];
...
- (void)_installSubviews
{
  ...
  imageView = [(SFDownloadsBarButtonItemView *)self imageView];
  ...
  [imageView addSubview:self->_iconView];
  ...
  heightAnchor = [(SFDownloadsBarButtonItemView *)self heightAnchor];
  ...
  [imageView addSubview:self->_progressView];
}
...
- (void)_updateIcon
{
  ...
  traitCollection = [(SFDownloadsBarButtonItemView *)self traitCollection];
  v14 = [v4 _sf_staticConfigurationWithTextStyle:v5 scale:3 weight:v3 compatibleWithTraitCollection:traitCollection];

  v7 = MEMORY[0x1E69DCAB8];
  sf_mobileSafariFrameworkBundle = [MEMORY[0x1E696AAE8] sf_mobileSafariFrameworkBundle];
  v9 = [v7 imageNamed:@"DownloadsIconCircle" inBundle:sf_mobileSafariFrameworkBundle withConfiguration:v14];

  v10 = MEMORY[0x1E69DCAB8];
  sf_mobileSafariFrameworkBundle2 = [MEMORY[0x1E696AAE8] sf_mobileSafariFrameworkBundle];
  v12 = [v10 imageNamed:@"DownloadsIconArrow" inBundle:sf_mobileSafariFrameworkBundle2 withConfiguration:v14];
...
- (CGRect)_barButtonHitRect
{
  ...
  items = [v3 items];
  WeakRetained = objc_loadWeakRetained(&self->_barButtonItem);
  v6 = [items indexOfObject:WeakRetained];

  v7 = v6 >= [items count];
  v8 = v6 < 1;
  _sf_usesLeftToRightLayout = [(SFDownloadsBarButtonItemView *)self _sf_usesLeftToRightLayout];
...
- (id)pointerInteraction:(id)interaction regionForRequest:(id)request defaultRegion:(id)region
{
  ...
  [(SFDownloadsBarButtonItemView *)self _barButtonHitRect:interaction];
...
- (void)_buttonPressed
{
  ...
  action = [WeakRetained action];
  target = [WeakRetained target];
  [mEMORY[0x1E69DC668] sendAction:action to:target from:WeakRetained forEvent:0];
}
...
- (void)layoutSubviews
{
  ...
  v4 = v3;
  v6 = v5;
  selfCopy = [(SFDownloadsBarButtonItemView *)self _sf_firstAncestorViewOfClass:objc_opt_class()];
  v8 = selfCopy;
  if (!selfCopy)
  {
    selfCopy = self;
  ...
  circleImage = [(_SFDownloadsIconView *)self->_iconView circleImage];
  [circleImage baselineOffsetFromBottom];
  v12 = v9 + v11;

  imageView = [(SFDownloadsBarButtonItemView *)self imageView];
  [imageView setFrame:{0.0, v12, v4, v6}];

  [(_SFDownloadsIconView *)self->_iconView setFrame:0.0, 0.0, v4, v6];
  arrowImage = [(_SFDownloadsIconView *)self->_iconView arrowImage];
  [arrowImage size];
  v16 = v15;
