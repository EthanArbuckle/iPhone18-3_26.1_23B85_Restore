@interface EvaluationRequestProcessor
- (EvaluationRequestProcessor)initWithContext:(id)context uiDelegate:(id)delegate;
- (void)_collectBiomeAnalyticsForRequest:(id)request dtoState:(int64_t)state ratchetState:(int64_t)ratchetState result:(id)result;
- (void)_collectBiomeAnalyticsForRequest:(id)request result:(id)result;
- (void)postProcessRequest:(id)request result:(id)result completion:(id)completion;
- (void)processRequest:(id)request configuration:(id)configuration completion:(id)completion;
@end

@implementation EvaluationRequestProcessor

- (EvaluationRequestProcessor)initWithContext:(id)context uiDelegate:(id)delegate
{
  contextCopy = context;
  delegateCopy = delegate;
  v11.receiver = self;
  v11.super_class = EvaluationRequestProcessor;
  v8 = [(EvaluationRequestProcessor *)&v11 init];
  v9 = v8;
  if (v8)
  {
    objc_storeWeak(&v8->_context, contextCopy);
    objc_storeStrong(&v9->_uiDelegate, delegate);
  }

  return v9;
}

- (void)processRequest:(id)request configuration:(id)configuration completion:(id)completion
{
  requestCopy = request;
  completionCopy = completion;
  v9 = requestCopy;
  client = [v9 client];
  v11 = client;
  if (!v9)
  {
    v14 = @"Unexpected request type";
LABEL_6:
    v15 = [LAErrorHelper errorWithCode:-1000 message:v14];
    v16 = [LACEvaluationResult resultWithFailure:v15];
    completionCopy[2](completionCopy, v16);

    goto LABEL_7;
  }

  if (!client)
  {
    v14 = @"Missing originator";
    goto LABEL_6;
  }

  WeakRetained = objc_loadWeakRetained(&self->_context);
  uiDelegate = self->_uiDelegate;
  v17[0] = _NSConcreteStackBlock;
  v17[1] = 3221225472;
  v17[2] = sub_10000D648;
  v17[3] = &unk_100055138;
  v18 = v9;
  v19 = completionCopy;
  [WeakRetained evaluateRequest:v18 uiDelegate:uiDelegate originator:v11 reply:v17];

LABEL_7:
}

- (void)postProcessRequest:(id)request result:(id)result completion:(id)completion
{
  requestCopy = request;
  resultCopy = result;
  completionCopy = completion;
  v11 = requestCopy;
  if ([v11 isInteractive] && (objc_msgSend(v11, "isImmediateSuccess") & 1) == 0)
  {
    [(EvaluationRequestProcessor *)self _collectBiomeAnalyticsForRequest:v11 result:resultCopy];
  }

  completionCopy[2](completionCopy, resultCopy);
}

- (void)_collectBiomeAnalyticsForRequest:(id)request result:(id)result
{
  requestCopy = request;
  resultCopy = result;
  serviceLocator = [requestCopy serviceLocator];
  v9 = NSStringFromProtocol(&OBJC_PROTOCOL___LACDTOService);
  v10 = [serviceLocator serviceWithIdentifier:v9];

  if (v10 && ([v10 conformsToProtocol:&OBJC_PROTOCOL___LACDTOService] & 1) != 0)
  {
    environmentProvider = [v10 environmentProvider];
    policy = [requestCopy policy];
    options = [requestCopy options];
    v15[0] = _NSConcreteStackBlock;
    v15[1] = 3221225472;
    v15[2] = sub_10000D998;
    v15[3] = &unk_100055278;
    v16 = requestCopy;
    selfCopy = self;
    v18 = resultCopy;
    [environmentProvider fetchEnvironmentForPolicy:policy options:options completion:v15];

    v14 = v16;
  }

  else
  {
    v14 = LACLogDTO();
    if (os_log_type_enabled(v14, OS_LOG_TYPE_ERROR))
    {
      sub_100022BEC(v14);
    }
  }
}

- (void)_collectBiomeAnalyticsForRequest:(id)request dtoState:(int64_t)state ratchetState:(int64_t)ratchetState result:(id)result
{
  resultCopy = result;
  requestCopy = request;
  v9 = +[NSNumber numberWithUnsignedInt:](NSNumber, "numberWithUnsignedInt:", [requestCopy evaluationUserId]);
  analytics = [requestCopy analytics];
  v33[0] = _NSConcreteStackBlock;
  v33[1] = 3221225472;
  v33[2] = sub_10000DD0C;
  v33[3] = &unk_1000552A0;
  v11 = v9;
  v34 = v11;
  v12 = analytics;
  v35 = v12;
  v13 = sub_10000DD0C(v33);
  v29[0] = _NSConcreteStackBlock;
  v29[1] = 3221225472;
  v29[2] = sub_10000DE40;
  v29[3] = &unk_1000552C8;
  v30 = v12;
  v31 = v11;
  v32 = resultCopy;
  v14 = resultCopy;
  v26 = v11;
  v25 = v12;
  v15 = sub_10000DE40(v29);
  v16 = [LACBiomeEvaluationEvent alloc];
  policy = [requestCopy policy];
  error = [v14 error];
  code = [error code];
  payload = [requestCopy payload];

  v21 = [payload objectForKeyedSubscript:LACEvaluationRequestPayloadKeyInternalInfo];
  v22 = [v21 objectForKeyedSubscript:@"CallerId"];
  v23 = [v16 initWithPolicy:policy errorCode:code biometry:v13 passcode:v15 dtoState:state ratchetState:ratchetState callerBundleID:v22];

  v24 = +[LACBiomeEvaluationDonationHelper sharedInstance];
  [v24 donateEvent:v23];
}

@end