...
+ (MTLAttributeDescriptor)allocWithZone:(_NSZone *)zone;
...
+ (MTLAttributeDescriptor)allocWithZone:(_NSZone *)zone
{
  if (objc_opt_class() == self)
  {
    ...
    return [MTLAttributeDescriptorInternal allocWithZone:zone];
  }
...
- (BOOL)isEqual:(id)equal
{
  ...
  format = [(MTLAttributeDescriptor *)self format];
  if (format != [equal format])
  {
    return 0;
  }

  offset = [(MTLAttributeDescriptor *)self offset];
  if (offset != [equal offset])
  {
    return 0;
  }

  bufferIndex = [(MTLAttributeDescriptor *)self bufferIndex];
