...
+ (id)confirmationRequiredWithSettingActionToConfirm:(int64_t)confirm;
+ (id)successWithResolvedSettingAction:(int64_t)action;
...
+ (id)confirmationRequiredWithSettingActionToConfirm:(int64_t)confirm
{
  if (!confirm)
  {
    v7 = [MEMORY[0x1E695DF30] exceptionWithName:*MEMORY[0x1E695D940] reason:@"Confirmation of INSettingActionUnknown is not supported." userInfo:0];
...
+ (id)successWithResolvedSettingAction:(int64_t)action
{
  ...
  {
    NSLog(@"Success resolution with INSettingActionUnknown will be reformed to notRequired.", a2);
...
- (id)_vocabularyValueForObject:(id)object slotDescription:(id)description
{
  ...
  if (valueType == 236)
  {
    ...
    if (objc_opt_isKindOfClass())
    {
      ...
      {
        v10 = *(&off_1E7287CF8 + integerValue - 1);
      }
...
- (id)_intentSlotValueForObject:(id)object slotDescription:(id)description
{
  ...
  if ([descriptionCopy valueType] == 236)
  {
