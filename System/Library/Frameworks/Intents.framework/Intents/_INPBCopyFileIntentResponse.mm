...
- (_INPBCopyFileIntentResponse)initWithCoder:(id)coder;
...
- (id)dictionaryRepresentation
{
  ...
  destinationName = [(_INPBCopyFileIntentResponse *)self destinationName];
  ...
  entityName = [(_INPBCopyFileIntentResponse *)self entityName];
  ...
  {
    entityType = [(_INPBCopyFileIntentResponse *)self entityType];
    if (entityType >= 4)
    {
...
- (BOOL)isEqual:(id)equal
{
  ...
  destinationName3 = [(_INPBCopyFileIntentResponse *)self destinationName];
  if (destinationName3)
  {
    v8 = destinationName3;
    destinationName4 = [(_INPBCopyFileIntentResponse *)self destinationName];
  ...
  destinationName = [(_INPBCopyFileIntentResponse *)self entityName];
...
LABEL_11:
  ...
  {
    v13 = entityName;
    entityName2 = [(_INPBCopyFileIntentResponse *)self entityName];
  ...
  hasEntityType = [(_INPBCopyFileIntentResponse *)self hasEntityType];
  if (hasEntityType == [equalCopy hasEntityType])
  {
    if (!-[_INPBCopyFileIntentResponse hasEntityType](self, "hasEntityType") || ![equalCopy hasEntityType] || (entityType = self->_entityType, entityType == objc_msgSend(equalCopy, "entityType")))
    {
      hasOverwrite = [(_INPBCopyFileIntentResponse *)self hasOverwrite];
      if (hasOverwrite == [equalCopy hasOverwrite])
      {
        if (!-[_INPBCopyFileIntentResponse hasOverwrite](self, "hasOverwrite") || ![equalCopy hasOverwrite] || (overwrite = self->_overwrite, overwrite == objc_msgSend(equalCopy, "overwrite")))
        {
          hasSuccess = [(_INPBCopyFileIntentResponse *)self hasSuccess];
          if (hasSuccess == [equalCopy hasSuccess])
          {
            if (!-[_INPBCopyFileIntentResponse hasSuccess](self, "hasSuccess") || ![equalCopy hasSuccess] || (success = self->_success, success == objc_msgSend(equalCopy, "success")))
            {
              v17 = 1;
              goto LABEL_13;
            }
...
{
  coderCopy = coder;
  data = [(_INPBCopyFileIntentResponse *)self data];
...
- (_INPBCopyFileIntentResponse)initWithCoder:(id)coder
{
  ...
  {
    self = [(_INPBCopyFileIntentResponse *)self initWithData:selfCopy];
...
- (void)writeTo:(id)to
{
  ...
  if (destinationName)
  {
    destinationName2 = [(_INPBCopyFileIntentResponse *)self destinationName];
