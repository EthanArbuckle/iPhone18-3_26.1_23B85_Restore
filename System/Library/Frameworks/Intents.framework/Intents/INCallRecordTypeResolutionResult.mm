...
+ (INCallRecordTypeResolutionResult)confirmationRequiredWithCallRecordTypeToConfirm:(INCallRecordType)callRecordTypeToConfirm
{
  ...
  {
    v7 = [MEMORY[0x1E695DF30] exceptionWithName:*MEMORY[0x1E695D940] reason:@"Confirmation of INCallRecordTypeUnknown is not supported." userInfo:0];
...
+ (INCallRecordTypeResolutionResult)successWithResolvedCallRecordType:(INCallRecordType)resolvedCallRecordType
{
  ...
  {
    NSLog(@"Success resolution with INCallRecordTypeUnknown will be reformed to notRequired.", a2);
...
- (id)_vocabularyValueForObject:(id)object slotDescription:(id)description
{
  ...
  if (valueType == 71)
  {
    ...
    if (objc_opt_isKindOfClass())
    {
      ...
      {
        v10 = off_1E7280748[integerValue - 1];
      }
...
- (id)_intentSlotValueForObject:(id)object slotDescription:(id)description
{
  ...
  if ([descriptionCopy valueType] == 71 || objc_msgSend(descriptionCopy, "valueType") == 72)
  {
    [(_INPBIntentSlotValue *)v7 setType:1000];
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0 && ([objectCopy integerValue] - 1) <= 7)
    {
