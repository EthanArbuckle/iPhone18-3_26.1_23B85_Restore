...
- (GCControllerButtonInputView)initWithCoder:(id)coder;
- (GCControllerButtonInputView)initWithFrame:(CGRect)frame;
- (GCControllerButtonInputView)initWithText:(id)text;
...
- (void)setupActions;
- (void)triggerButtonWithValue:(float)value;
@end
...
- (GCControllerButtonInputView)initWithCoder:(id)coder
{
  ...
  v5 = [(GCControllerButtonInputView *)&v15 initWithCoder:coderCopy];
  if (v5)
  {
    ...
    v8 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"saturationVibrancyEffectView"];
    saturationVibrancyEffectView = v5->_saturationVibrancyEffectView;
    v5->_saturationVibrancyEffectView = v8;

    v10 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"additiveVibrancyEffectView"];
    additiveVibrancyEffectView = v5->_additiveVibrancyEffectView;
    v5->_additiveVibrancyEffectView = v10;

    v12 = [coderCopy decodeObjectOfClass:objc_opt_class() forKey:@"button"];
    button = v5->_button;
    v5->_button = v12;
...
- (void)encodeWithCoder:(id)coder
{
  ...
  coderCopy = coder;
  [(GCControllerButtonInputView *)&v5 encodeWithCoder:coderCopy];
  [coderCopy encodeObject:self->_background forKey:{@"background", v5.receiver, v5.super_class}];
  [coderCopy encodeObject:self->_saturationVibrancyEffectView forKey:@"saturationVibrancyEffectView"];
  [coderCopy encodeObject:self->_additiveVibrancyEffectView forKey:@"additiveVibrancyEffectView"];
...
- (void)initUI
{
  ...
  layer3 = [(UIVisualEffectView *)self->_saturationVibrancyEffectView layer];
  [layer3 setMasksToBounds:1];

  [(UIVisualEffectView *)self->_saturationVibrancyEffectView frame];
  v20 = v19 * 0.5;
  contentView = [(UIVisualEffectView *)self->_saturationVibrancyEffectView contentView];
  layer4 = [contentView layer];
  [layer4 setCornerRadius:v20];

  LODWORD(v20) = GCVirtualControllerVibrancyOverlayOpacity();
  contentView2 = [(UIVisualEffectView *)self->_saturationVibrancyEffectView contentView];
  layer5 = [contentView2 layer];
  LODWORD(v25) = LODWORD(v20);
  [layer5 setOpacity:v25];

  contentView3 = [(UIVisualEffectView *)self->_saturationVibrancyEffectView contentView];
  ...
  layer7 = [(UIVisualEffectView *)self->_additiveVibrancyEffectView layer];
  ...
  v32 = v31 * 0.5;
  contentView4 = [(UIVisualEffectView *)self->_additiveVibrancyEffectView contentView];
  layer8 = [contentView4 layer];
  [layer8 setCornerRadius:v32];

  LODWORD(v32) = GCVirtualControllerVibrancyOverlayOpacity();
  contentView5 = [(UIVisualEffectView *)self->_additiveVibrancyEffectView contentView];
  layer9 = [contentView5 layer];
  LODWORD(v37) = LODWORD(v32);
  [layer9 setOpacity:v37];

  contentView6 = [(UIVisualEffectView *)self->_additiveVibrancyEffectView contentView];
  layer10 = [contentView6 layer];
  [layer10 setMasksToBounds:1];

  contentView7 = [(UIVisualEffectView *)self->_background contentView];
  [contentView7 addSubview:self->_saturationVibrancyEffectView];

  contentView8 = [(UIVisualEffectView *)self->_background contentView];
  [contentView8 addSubview:self->_additiveVibrancyEffectView];
}
...
- (void)setText
{
  ...
  [layer setContents:AlphaMaskImage];

  contentView2 = [(UIVisualEffectView *)self->_additiveVibrancyEffectView contentView];
  layer2 = [contentView2 layer];
  [layer2 setContents:AlphaMaskImage];
...
- (void)setCustomImage:(id)image
{
  ...
  if (imageCopy)
  {
    customImage = [(GCControllerButtonInputView *)self customImage];
    v7 = [customImage isEqual:imageCopy];
    ...
    if ((v7 & 1) == 0)
    {
      ...
      t1 = v54;
      [imageCopy applyTransform:&t1];
      cGPath = [imageCopy CGPath];
      shapeLayer = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer setPath:cGPath];

      usesEvenOddFillRule = [imageCopy usesEvenOddFillRule];
      ...
      v19 = *v18;
      shapeLayer2 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer2 setFillRule:v19];

      [imageCopy lineWidth];
      v22 = v21;
      shapeLayer3 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer3 setLineWidth:v22];

      if ([imageCopy lineCapStyle])
      {
        lineCapStyle = [imageCopy lineCapStyle];
        v25 = &kCALineCapRound;
        if (lineCapStyle != 1)
        {
      ...
      v27 = *v25;
      shapeLayer4 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer4 setLineCap:v27];

      if ([imageCopy lineJoinStyle])
      {
        lineJoinStyle = [imageCopy lineJoinStyle];
      ...
      v31 = *v30;
      shapeLayer5 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer5 setLineJoin:v31];

      [imageCopy miterLimit];
      v34 = v33;
      shapeLayer6 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer6 setMiterLimit:v34];

      t1.a = 0.0;
      v52.a = 0.0;
      [imageCopy getLineDash:0 count:&t1 phase:&v52];
      if (*&t1.a)
      {
        v36 = malloc_type_malloc(8 * *&t1.a, 0x100004000313F17uLL);
        [imageCopy getLineDash:v36 count:0 phase:0];
        ...
        free(v36);
        a = v52.a;
        shapeLayer7 = [(_GCControllerShapeView *)v9 shapeLayer];
        [shapeLayer7 setLineDashPhase:a];

        v43 = [v38 copy];
        shapeLayer8 = [(_GCControllerShapeView *)v9 shapeLayer];
        [shapeLayer8 setLineDashPattern:v43];
      }

      UIGraphicsBeginImageContextWithOptions(self->_innerFrame.size, 0, 0.0);
      shapeLayer9 = [(_GCControllerShapeView *)v9 shapeLayer];
      [shapeLayer9 renderInContext:UIGraphicsGetCurrentContext()];
...
- (void)setValueChangedHandler:(void *)handler
{
  if (handler)
  {
    objc_setProperty_nonatomic_copy(handler, newValue, newValue, 88);
  }
}
