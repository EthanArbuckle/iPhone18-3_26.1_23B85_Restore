...
- (id)_nonNilOrderedSetValueWithSelector:(SEL)selector
{
  ...
  if (!v5)
  {
    [(NSKeyValueSlowMutableOrderedSet *)self _raiseNilValueExceptionWithSelector:selector];
  }
...
- (void)insertObject:(id)object atIndex:(unint64_t)index
{
  ...
  {
    if (index || !self->_treatNilValuesLikeEmptyOrderedSets)
    {
      [(NSKeyValueSlowMutableOrderedSet *)self _raiseNilValueExceptionWithSelector:a2, objectCopy, v11];
    }

    v9 = [objc_alloc(MEMORY[0x1E695DFA0]) initWithObjects:&objectCopy count:1];
  }
