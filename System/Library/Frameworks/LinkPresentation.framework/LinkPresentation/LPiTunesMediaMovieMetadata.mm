...
- (LPiTunesMediaMovieMetadata)initWithCoder:(id)coder;
...
- (id)copyWithZone:(_NSZone *)zone
{
  v4 = [LPiTunesMediaMovieMetadata allocWithZone:zone];
  if (v4)
  {
    storeFrontIdentifier = [(LPiTunesMediaMovieMetadata *)self storeFrontIdentifier];
    [(LPiTunesMediaMovieMetadata *)v4 setStoreFrontIdentifier:storeFrontIdentifier];

    storeIdentifier = [(LPiTunesMediaMovieMetadata *)self storeIdentifier];
    [(LPiTunesMediaMovieMetadata *)v4 setStoreIdentifier:storeIdentifier];

    name = [(LPiTunesMediaMovieMetadata *)self name];
    [(LPiTunesMediaMovieMetadata *)v4 setName:name];

    genre = [(LPiTunesMediaMovieMetadata *)self genre];
    [(LPiTunesMediaMovieMetadata *)v4 setGenre:genre];

    artwork = [(LPiTunesMediaMovieMetadata *)self artwork];
    [(LPiTunesMediaMovieMetadata *)v4 setArtwork:artwork];

    artworkMetadata = [(LPiTunesMediaMovieMetadata *)self artworkMetadata];
    [(LPiTunesMediaMovieMetadata *)v4 setArtworkMetadata:artworkMetadata];
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ([(LPiTunesMediaMovieMetadata *)&v8 isEqual:equalCopy])
  {
    v5 = 1;
  }
