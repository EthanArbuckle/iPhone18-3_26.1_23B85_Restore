...
- (BOOL)configureWithEvent:(id)event calendar:(id)calendar preview:(BOOL)preview
{
  ...
  v9 = [(EKEventCalendarDetailItem *)self _calendarCanBeChanged]&& (([(EKEvent *)self->super._event isSelfOrganized]& 1) != 0 || ![(EKEventDetailItem *)self isPrivateEvent]&& ![(EKEventDetailItem *)self isReadOnlyDelegateCalendar]) || [(EKEventCalendarDetailItem *)self _shouldAllowViewingDetailsForCalendar:calendarCopy];
  [(EKEventDetailItem *)self setAllowsEditing:v9];
  if ([(EKEventCalendarDetailItem *)self minimalMode])
  {
    if ([calendarCopy isSubscribed])
    {
      v10 = [calendarCopy isSubscribedHolidayCalendar] ^ 1;
    }
...
- (BOOL)minimalMode
{
  ...
  {
    minimalMode = [delegate minimalMode];
  }
...
- (id)cellForSubitemAtIndex:(unint64_t)index withTraitCollection:(id)collection
{
  ...
  if (![(EKEventDetailItem *)self allowsEditing]|| ([(EKEvent *)self->super._event calendar], v6 = objc_claimAutoreleasedReturnValue(), v7 = [(EKEventCalendarDetailItem *)self _shouldAllowViewingDetailsForCalendar:v6], v6, v7))
  {
    ...
    [MEMORY[0x1E69DD1B8] _setCurrentTraitCollection:collectionCopy];
    v11 = objc_alloc(MEMORY[0x1E696AD40]);
    calendar = [(EKEvent *)self->super._event calendar];
    systemBackgroundColor = [MEMORY[0x1E69DC888] systemBackgroundColor];
    v14 = CalendarTitleWithDotAttributedString(calendar, systemBackgroundColor);
    ...
    [v15 addAttribute:v18 value:secondaryLabelColor range:{0, objc_msgSend(v15, "length")}];

    detailTextLabel = [(EKUITableViewCell *)self->_displayCell detailTextLabel];
    [detailTextLabel setAttributedText:v15];
  ...
  _currentTraitCollection = [MEMORY[0x1E69DC888] systemBackgroundColor];
  if (!EKUICatalyst())
  {
    calendar2 = [(EKEvent *)self->super._event calendar];
    v15 = [EKUICalendarMenu placeholderMenuForCalendar:calendar2 backgroundColor:_currentTraitCollection];
...
LABEL_6:
  ...
  v31 = [(EKEventCalendarDetailItem *)self popupMenu:_currentTraitCollection];
...
LABEL_7:
  ...
  textLabel = [(EKUITableViewCell *)v21 textLabel];
  [textLabel setText:v23];

  labelColor = [MEMORY[0x1E69DC888] labelColor];
  textLabel2 = [(EKUITableViewCell *)v21 textLabel];
...
- (id)popupMenu:(id)menu
{
  ...
  if (!popupMenu)
  {
    eligibleCalendars = [(EKEventCalendarDetailItem *)self eligibleCalendars];
    objc_initWeak(&location, self);
    ...
    v8 = [EKUICalendarMenu calendarMenuWithCalendars:eligibleCalendars eventStore:eventStore backgroundColor:menuCopy setupActionHandler:v14 selectionHandler:v12];
...
- (id)detailViewControllerWithFrame:(CGRect)frame forSubitemAtIndex:(unint64_t)index
{
  ...
  {
    calendar = [(EKEvent *)self->super._event calendar];
    if ([(EKEventCalendarDetailItem *)self _shouldAllowViewingDetailsForCalendar:calendar])
    {
      height = [[EKSubscribedCalendarEditItemViewController alloc] initWithFrame:calendar calendar:x, y, width, height];
    }
...
- (BOOL)_calendarCanBeChanged
{
  ...
  hasAttendees = [(EKEvent *)self->super._event hasAttendees];
  event = self->super._event;
  if (!hasAttendees)
  {
    eventStore = [(EKEvent *)event eventStore];
    source2 = [eventStore readWriteCalendarsForEntityType:0];
    v12 = [source2 count];
LABEL_7:
    v9 = v12 > 1;
  ...
  if ((requiresOutgoingInvitationsInDefaultCalendar & 1) == 0)
  {
    eventStore = [(EKEvent *)self->super._event calendar];
    source2 = [eventStore source];
    v13 = [source2 readWriteCalendarsForEntityType:0];
    v12 = [v13 count];
...
{
  calendarCopy = calendar;
  if ([calendarCopy isSubscribed] && (objc_msgSend(calendarCopy, "isSubscribedHolidayCalendar") & 1) == 0 && objc_msgSend(MEMORY[0x1E6992FA0], "currentProcessIsFirstPartyCalendarApp"))
  {
