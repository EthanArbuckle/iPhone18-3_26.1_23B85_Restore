...
- (void)labelButtonClicked:(id)clicked
{
  ...
  if (v7 >= 2)
  {
    ...
    if ([v14 count])
    {
      ...
      profile = [(CNPropertySocialProfileEditingCell *)self profile];
    ...
    if (!selectedItem)
    {
      profile2 = [(CNPropertySocialProfileEditingCell *)self profile];
      service = [profile2 service];
...
- (CNSocialProfile)profile
{
  ...
  if ((isKindOfClass & 1) == 0)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"CNPropertySocialProfileCell.m" lineNumber:38 description:@"Unexpected social profile value"];
  }
