...
+ (BOOL)_useSideButton;
+ (id)menuButtonConsumerWithHandler:(id)handler;
- (BuddyMenuController)initWithHandler:(id)handler;
- (void)dealloc;
- (void)invalidate;
@end
...
- (void)invalidate
{
  ...
  if (v3)
  {
    ...
    do
    {
      for (i = 0; i < v3; ++i)
      {
        ...
        v8 = *(v7[1] + 8 * i);
        buttonConsumer = [v8 buttonConsumer];
        [buttonConsumer invalidate];
      }
  ...
  objc_storeStrong(&selfCopy->_consumers, 0);
}
