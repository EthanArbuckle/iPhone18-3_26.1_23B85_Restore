@interface ICDrawingRegressionTestDataCollectionPreviewViewController
- (ICDrawingRegressionTestDataCollectionPreviewViewController)initWithDrawingImage:(id)image;
- (void)centerImageView;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
@end

@implementation ICDrawingRegressionTestDataCollectionPreviewViewController

- (ICDrawingRegressionTestDataCollectionPreviewViewController)initWithDrawingImage:(id)image
{
  imageCopy = image;
  v8.receiver = self;
  v8.super_class = ICDrawingRegressionTestDataCollectionPreviewViewController;
  v5 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)&v8 init];
  drawingImage = v5->_drawingImage;
  v5->_drawingImage = imageCopy;

  return v5;
}

- (void)viewDidLoad
{
  v20.receiver = self;
  v20.super_class = ICDrawingRegressionTestDataCollectionPreviewViewController;
  [(ICDrawingRegressionTestDataCollectionPreviewViewController *)&v20 viewDidLoad];
  v3 = +[NSBundle mainBundle];
  v4 = [v3 localizedStringForKey:@"Sketch Preview" value:&stru_100661CF0 table:0];
  [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self setTitle:v4];

  v5 = +[UIColor lightGrayColor];
  view = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self view];
  [view setBackgroundColor:v5];

  v7 = [[UIScrollView alloc] initWithFrame:{CGRectZero.origin.x, CGRectZero.origin.y, CGRectZero.size.width, CGRectZero.size.height}];
  [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self setScrollView:v7];

  scrollView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView setMinimumZoomScale:0.25];

  scrollView2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView2 setMaximumZoomScale:3.0];

  scrollView3 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView3 setDelegate:self];

  view2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self view];
  scrollView4 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [view2 addSubview:scrollView4];

  v13 = [UIImageView alloc];
  drawingImage = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self drawingImage];
  v15 = [v13 initWithImage:drawingImage];
  [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self setImageView:v15];

  v16 = +[UIColor whiteColor];
  imageView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self imageView];
  [imageView setBackgroundColor:v16];

  scrollView5 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  imageView2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self imageView];
  [scrollView5 addSubview:imageView2];
}

- (void)viewDidLayoutSubviews
{
  view = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self view];
  [view bounds];
  v5 = v4;
  v7 = v6;
  v9 = v8;
  v11 = v10;
  view2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self view];
  [view2 safeAreaInsets];
  v14 = v5 + v13;
  v16 = v7 + v15;
  v18 = v9 - (v13 + v17);
  v20 = v11 - (v15 + v19);

  scrollView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView frame];
  v42.origin.x = v22;
  v42.origin.y = v23;
  v42.size.width = v24;
  v42.size.height = v25;
  v39.origin.x = v14;
  v39.origin.y = v16;
  v39.size.width = v18;
  v39.size.height = v20;
  LOBYTE(view2) = CGRectEqualToRect(v39, v42);

  if ((view2 & 1) == 0)
  {
    scrollView2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
    [scrollView2 setFrame:{v14, v16, v18, v20}];

    imageView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self imageView];
    [imageView bounds];
    v29 = v28;
    v31 = v30;
    scrollView3 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
    [scrollView3 setContentSize:{v29, v31}];

    scrollView4 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
    [scrollView4 bounds];
    Width = CGRectGetWidth(v40);
    imageView2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self imageView];
    [imageView2 bounds];
    v36 = Width / CGRectGetWidth(v41);
    scrollView5 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
    [scrollView5 setZoomScale:v36];

    [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self centerImageView];
  }
}

- (void)centerImageView
{
  scrollView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView bounds];
  v5 = v4;
  scrollView2 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView2 contentSize];
  v8 = (v5 - v7) * 0.5;

  v9 = 0.0;
  if (v8 >= 0.0)
  {
    v10 = v8;
  }

  else
  {
    v10 = 0.0;
  }

  scrollView3 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView3 bounds];
  v13 = v12;
  scrollView4 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView4 contentSize];
  v16 = (v13 - v15) * 0.5;

  if (v16 >= 0.0)
  {
    v9 = v16;
  }

  scrollView5 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView5 contentSize];
  v18 = v10 + v17 * 0.5;
  scrollView6 = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self scrollView];
  [scrollView6 contentSize];
  v21 = v9 + v20 * 0.5;
  imageView = [(ICDrawingRegressionTestDataCollectionPreviewViewController *)self imageView];
  [imageView setCenter:{v18, v21}];
}

@end