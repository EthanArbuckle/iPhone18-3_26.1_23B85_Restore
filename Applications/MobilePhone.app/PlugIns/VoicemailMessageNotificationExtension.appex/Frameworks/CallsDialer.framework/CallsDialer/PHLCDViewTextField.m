    ...
    if (v6)
    {
      ...
      if (([v3 isEqualToString:v9] & 1) == 0)
      {
        ...
        if ([(PHLCDViewTextField *)self isEditing])
        {
          ...
          [(PHLCDViewTextField *)self setSelectionRange:&v14[Length], 0];
        }
      }
...
- (void)deleteBackward
{
  ...
  else
  {
    ...
    [(PHLCDViewTextField *)self replaceRange:v9 withText:&stru_50D80];
  }
...
- (void)buildMenuWithBuilder:(id)a3
{
  ...
  [v3 removeMenuForIdentifier:{UIMenuAutoFill, v4.receiver, v4.super_class}];
}
