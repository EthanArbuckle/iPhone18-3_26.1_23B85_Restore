...
- (_TtC11MobilePhone9FTMessage)init;
- (double)duration;
...
{
  selfCopy = self;
  v3 = FTMessage.messageType.getter();
...
- (void)setFolder:(int64_t)folder
{
  v4 = (self + OBJC_IVAR____TtC11MobilePhone9FTMessage_dirtyFolder);
  selfCopy = self;
...
- (BOOL)isRead
{
  v2 = (self + OBJC_IVAR____TtC11MobilePhone9FTMessage_dirtyRead);
  selfCopy = self;
...
- (void)setIsRead:(BOOL)read
{
  v4 = (self + OBJC_IVAR____TtC11MobilePhone9FTMessage_dirtyRead);
  selfCopy = self;
  os_unfair_lock_lock(v4);
...
{
  selfCopy = self;
  v3 = FTMessage.isDataAvailable.getter();
...
{
  selfCopy = self;
  v3 = specialized FTMessage.Lazy.wrappedValue.getter();
...
{
  selfCopy = self;
  v5 = FTMessage.shouldBeMarkedAsRead(for:)(time);
...
- (id)contactUsingContactStore:(id)store withKeysToFetch:(id)fetch
{
  ...
  storeCopy = store;
  selfCopy = self;
  v9 = FTMessage.getContact(using:with:)(storeCopy, v6);
...
- (id)displayNameUsingContactStore:(id)store
{
  storeCopy = store;
  selfCopy = self;
  object = FTMessage.getDisplayName(using:)(storeCopy).value._object;
