...
- (PHRecentsPersonFaceTimeHeaderSummaryView)initWithFrame:(CGRect)frame recentCall:(id)call occurrenceDate:(id)date status:(unsigned int)status duration:(double)duration type:(unsigned int)type dataUsage:(int64_t)usage;
@end
...
- (PHRecentsPersonFaceTimeHeaderSummaryView)initWithFrame:(CGRect)frame recentCall:(id)call occurrenceDate:(id)date status:(unsigned int)status duration:(double)duration type:(unsigned int)type dataUsage:(int64_t)usage
{
  ...
  height = [(PHRecentsPersonFaceTimeHeaderSummaryView *)&v103 initWithFrame:x, y, width, height];
  if (!height)
  {
    goto LABEL_52;
  }
  ...
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addSubview:v25];
...
LABEL_23:
  ...
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addSubview:v40];
  ...
  if (v100)
  {
    ...
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addSubview:v46];
    ...
    v52 = sub_100036F4C(duration);
    v53 = [NSByteCountFormatter stringFromByteCount:usage countStyle:0];
  ...
  screenSharingType = [callCopy screenSharingType];
  if (screenSharingType)
  {
    ...
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addSubview:v57];
    ...
    screenSharingType2 = [callCopy screenSharingType];
    if ((CHCallScreenSharingTypeScreenShareOrRemoteControl & ~screenSharingType2) != 0)
    {
      if (([callCopy screenSharingType] & 2) != 0)
      {
        v71 = +[NSBundle mainBundle];
        v72 = [v71 localizedStringForKey:@"REMOTE_CONTROL" value:&stru_10010E930 table:@"Recents"];
        v73 = callCopy;
        v74 = v72;
        callerNameForDisplay = [v73 callerNameForDisplay];
        v76 = [NSString localizedStringWithFormat:v74, callerNameForDisplay];
        [v57 setText:v76];
      }

      else if ([callCopy screenSharingType])
      {
    ...
    else
    {
      ...
      callerNameForDisplay2 = [v64 callerNameForDisplay];
      v68 = [NSString localizedStringWithFormat:v66, callerNameForDisplay2];
      [v57 setText:v68];
...
LABEL_41:
  ...
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraints:v81];
  ...
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraints:v82];

  v83 = [NSLayoutConstraint constraintWithItem:v25 attribute:3 relatedBy:0 toItem:v40 attribute:3 multiplier:1.0 constant:0.0];
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraint:v83];
  ...
  if (screenSharingType)
  {
    ...
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraints:v84];

    v85 = [NSLayoutConstraint constraintWithItem:v57 attribute:3 relatedBy:0 toItem:v25 attribute:4 multiplier:1.0 constant:0.0];
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraint:v85];
  }
  ...
  if (v100)
  {
    ...
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraints:v86];
    ...
    [(PHRecentsPersonFaceTimeHeaderSummaryView *)height addConstraint:v89];
  ...
  callCopy = v98;
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height setTopmostLabel:v40];
  if (v46)
  {
    v94 = v46;
  }
  ...
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height setBottommostLabel:v94];
  [(PHRecentsPersonFaceTimeHeaderSummaryView *)height setIntrinsicContentHeight:v39];
