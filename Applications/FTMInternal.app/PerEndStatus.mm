...
{
  v7.receiver = self;
  v7.super_class = PerEndStatus;
  v3 = [(PerEndStatus *)&v7 description];
  dictionaryRepresentation = [(PerEndStatus *)self dictionaryRepresentation];
...
- (BOOL)isEqual:(id)equal
{
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 16) & 1) == 0 || self->_endStatus != *(equalCopy + 2))
    {
      goto LABEL_11;
    }
  }
...
LABEL_11:
  ...
  if ((*&self->_has & 2) != 0)
  {
    if ((*(equalCopy + 16) & 2) == 0 || self->_numCalls != *(equalCopy + 3))
    {
      goto LABEL_11;
    }
