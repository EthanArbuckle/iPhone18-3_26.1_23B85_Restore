...
{
  v7.receiver = self;
  v7.super_class = EgwErrcNbrUtraMeas;
  v3 = [(EgwErrcNbrUtraMeas *)&v7 description];
  dictionaryRepresentation = [(EgwErrcNbrUtraMeas *)self dictionaryRepresentation];
...
- (BOOL)isEqual:(id)equal
{
  ...
  if ((*&self->_has & 8) != 0)
  {
    if ((*(equalCopy + 24) & 8) == 0 || self->_uarfcn != *(equalCopy + 5))
    {
      goto LABEL_21;
    }
  }
...
LABEL_21:
  ...
  if ((*&self->_has & 2) != 0)
  {
    if ((*(equalCopy + 24) & 2) == 0 || self->_psc != *(equalCopy + 3))
    {
      goto LABEL_21;
    }
  }
  ...
  if ((*&self->_has & 4) != 0)
  {
    if ((*(equalCopy + 24) & 4) == 0 || self->_rscp != *(equalCopy + 4))
    {
      goto LABEL_21;
    }
  }
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 24) & 1) == 0 || self->_ecno != *(equalCopy + 2))
    {
      goto LABEL_21;
    }
...
LABEL_11:
  self->_rscp = *(fromCopy + 4);
  *&self->_has |= 4u;
  if (*(fromCopy + 24))
  {
LABEL_5:
    self->_ecno = *(fromCopy + 2);
    *&self->_has |= 1u;
  }
