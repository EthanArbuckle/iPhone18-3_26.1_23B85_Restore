...
- (_TUIIdentifier)initWithOther:(id)other;
- (_TUIIdentifier)initWithUUID:(id)d indexPath:(id)path identifier:(id)identifier;
- (_TUIIdentifier)initWithUUID:(id)d indexes:(void *)indexes identifiers:(id)identifiers;
- (id).cxx_construct;
...
- (void)_appendIdentifier:(id)identifier;
- (void)_computeHash;
@end
...
- (_TUIIdentifier)initWithUUID:(id)d indexes:(void *)indexes identifiers:(id)identifiers
{
  ...
  if (v11)
  {
    ...
    v12->_indexes.__begin_ = *indexes;
    *indexes = begin;
    end = v12->_indexes.__end_;
    v12->_indexes.__end_ = *(indexes + 1);
    *(indexes + 1) = end;
    cap = v12->_indexes.__cap_;
    v12->_indexes.__cap_ = *(indexes + 2);
    *(indexes + 2) = cap;
...
- (_TUIIdentifier)initWithOther:(id)other
{
  ...
  if (v5)
  {
    objc_storeStrong(&v5->_UUID, otherCopy->_UUID);
    objc_storeStrong(&v6->_identifiers, otherCopy->_identifiers);
    if (v6 != otherCopy)
    {
      sub_10739C(&v6->_indexes.__begin_, otherCopy->_indexes.__begin_, otherCopy->_indexes.__end_, otherCopy->_indexes.__end_ - otherCopy->_indexes.__begin_);
    }

    v6->_hash = otherCopy->_hash;
  }
...
- (_TUIIdentifier)initWithUUID:(id)d indexPath:(id)path identifier:(id)identifier
{
  ...
  if (v11)
  {
    ...
    if (v16)
    {
      for (i = 0; i != v16; i = i + 1)
      {
        v18 = [pathCopy indexAtPosition:i];
...
- (BOOL)isEqual:(id)equal
{
  ...
  if (objc_opt_isKindOfClass())
  {
    ...
    v7 = equalCopy->_indexes.__begin_;
    if (v6 == (equalCopy->_indexes.__end_ - v7) && !memcmp(begin, v7, v6))
    {
      identifiers = self->_identifiers;
      if (identifiers == equalCopy->_identifiers || [(NSArray *)identifiers isEqual:?])
      {
        UUID = self->_UUID;
        if (UUID != equalCopy->_UUID)
        {
...
{
  stringCopy = string;
  if (stringCopy && !self->_UUID)
  {
    v5 = [[_TUIIdentifier alloc] initWithOther:self];
    [(_TUIIdentifier *)v5 _appendIdentifier:stringCopy];
...
- (id)tui_identifierByAppendingIndex:(unint64_t)index
{
  ...
  else
  {
    ...
    [(_TUIIdentifier *)v3 _appendIndex:index];
...
- (id)tui_identifierDescribeForDebuggingWithPackage:(id)package
{
  ...
  for (i = self->_indexes.__begin_; i != self->_indexes.__end_; ++i)
  {
    ...
    if (v9)
    {
      v10 = [TUIIdentifierMap describeForDebuggingIdentifierIndex:v9 >> 1 package:packageCopy];
      [v5 addObject:v10];
    }
...
+ (id)fromString:(id)string
{
  ...
  v25 = 0;
  v26 = 0;
  v5 = [[NSScanner alloc] initWithString:stringCopy];
  ...
  else
  {
    ...
    while (2)
    {
      ...
      while (1)
      {
        ...
        string = [v5 string];
        v14 = [string characterAtIndex:{objc_msgSend(v5, "scanLocation")}];
