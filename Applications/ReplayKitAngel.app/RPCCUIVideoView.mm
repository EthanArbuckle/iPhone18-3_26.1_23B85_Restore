...
- (RPCCUIVideoView)init
{
  ...
  if (v2)
  {
    ...
    v2->_currentRotation = &off_10005F0F0;

    v3->_mirrorType = 0;
    v3->_orientation = 0;
    portType = v3->_portType;
    v3->_portType = &stru_10005EBC0;

    deviceType = v3->_deviceType;
    v3->_deviceType = &off_10005F0F0;

    v3->_intrinsic = xmmword_1000479C0;
...
- (void)updateIntrinsicContentSize:(CGSize)size
{
  ...
  if (__RPLogLevel <= 1u && os_log_type_enabled(&_os_log_default, OS_LOG_TYPE_DEFAULT))
  {
    ...
    v11 = 2112;
    v12 = v6;
    _os_log_impl(&_mh_execute_header, &_os_log_default, OS_LOG_TYPE_DEFAULT, " [INFO] %{public}s:%d size=%@", &v7, 0x1Cu);
  }
...
LABEL_23:
  if (__RPLogLevel <= 1u && os_log_type_enabled(&_os_log_default, OS_LOG_TYPE_DEFAULT))
  {
    ...
    *&buf[46] = v21;
    ...
    v42 = 2112;
    v43 = v19;
    _os_log_impl(&_mh_execute_header, &_os_log_default, OS_LOG_TYPE_DEFAULT, " [INFO] %{public}s:%d currentRotation=%@ newRotation=%@ mirrorType=%d _orientation=%@ orientation=%@ portType=%@ deviceType=%@", buf, 0x54u);
  ...
  if ([(NSNumber *)v13 intValue]== 15 && [(__CFString *)v12 isEqualToString:@"PortTypeFront"])
  {
    if (__RPLogLevel <= 1u && os_log_type_enabled(&_os_log_default, OS_LOG_TYPE_DEFAULT))
    {
      ...
      *&buf[12] = 1024;
      *&buf[14] = 123;
      _os_log_impl(&_mh_execute_header, &_os_log_default, OS_LOG_TYPE_DEFAULT, " [INFO] %{public}s:%d external camera", buf, 0x12u);
    }
  ...
  else if ([(__CFString *)v12 containsString:@"PortTypeBack"])
  {
    if (__RPLogLevel <= 1u && os_log_type_enabled(&_os_log_default, OS_LOG_TYPE_DEFAULT))
    {
      ...
      *&buf[12] = 1024;
      *&buf[14] = 127;
      _os_log_impl(&_mh_execute_header, &_os_log_default, OS_LOG_TYPE_DEFAULT, " [INFO] %{public}s:%d back camera", buf, 0x12u);
    }

    [(RPCCUIVideoView *)self applyTransformWithOrientation];
    if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F108]|| [(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F120])
    {
      layer = [(RPCCUIVideoView *)self layer];
      v24 = layer;
      if (layer)
  ...
  else
  {
    if (__RPLogLevel <= 1u && os_log_type_enabled(&_os_log_default, OS_LOG_TYPE_DEFAULT))
    {
      ...
      *&buf[12] = 1024;
      *&buf[14] = 135;
      _os_log_impl(&_mh_execute_header, &_os_log_default, OS_LOG_TYPE_DEFAULT, " [INFO] %{public}s:%d front camera", buf, 0x12u);
    }
...
- (void)applyTransformForExternalCamera
{
  ...
  if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F108])
  {
  ...
  else if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F138])
  {
  ...
  {
    v10 = [(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F120];
...
- (void)applyTransformForPortrait
{
  ...
  else if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_10005F120])
  {
    memset(&v25, 0, sizeof(v25));
