...
- (void)updatePredicate
{
  ...
  isObserving = [queryObserver isObserving];
...
- (void)processResults
{
  ...
  v6[3] = &unk_1004DB750;
  v6[4] = &v7;
  [(MTBaseProcessor *)selfCopy results:v6];
  if ([(MTUPPSyncProcessor *)selfCopy shouldSync:v8[5]])
  {
...
- (BOOL)shouldSync:(id)sync
{
  ...
  if (![syncCopy count])
  {
    firstObject = _MTLogCategoryUPPSync();
    if (os_log_type_enabled(firstObject, OS_LOG_TYPE_ERROR))
    {
      *buf = 0;
      _os_log_impl(&_mh_execute_header, firstObject, OS_LOG_TYPE_ERROR, "MTUPPSyncProcessor - skipping sync because no items to sync.", buf, 2u);
    }
  ...
  if ([syncCopy count] <= 1)
  {
    firstObject = [syncCopy firstObject];
    controller = [(MTUPPSyncProcessor *)self controller];
    [controller reloadIfNeeded];

    controller2 = [(MTUPPSyncProcessor *)self controller];
    response = [controller2 response];

    if ([response state] == 2 && (objc_msgSend(response, "tracklist"), v10 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v10, "playingItem"), v11 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v11, "metadataObject"), v12 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v12, "identifiers"), v13 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v13, "library"), v14 = objc_claimAutoreleasedReturnValue(), objc_msgSend(v14, "databaseID"), v15 = objc_claimAutoreleasedReturnValue(), v16 = objc_msgSend(v15, "isEqualToString:", firstObject), v15, v14, v13, v12, v11, v10, v16))
    {
